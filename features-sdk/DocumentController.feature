Feature: Document Controller
  ...
  Scenario: Rejected document because of strict mapping
    ...
    When I call the action "document":"create" with args:
      | index      | "nyc-open-data"       |
  ...
  Scenario: Update document with and without returning updated document
    ...
    And I "create" the document "document-1" with content:
      | name | "document-1" |
      | age  | 42           |
    When I successfully call the action "document":"update" with args:
      | index      | "nyc-open-data"        |
  ...
  Scenario: Delete document and retrieve its source
    ...
    And I "create" the following documents:
      | _id          | body                    |
      | "document-1" | { "name": "document1" } |
    When I successfully call the action "document":"delete" with args:
      | index      | "nyc-open-data"        |
      | collection | "yellow-taxi"          |
      | _id        | "document-1"           |
      | source     | true                   |
  ...
  Scenario: deleteByQuery
    ...
    And I "create" the following documents:
      ...
      | -   | { "name": "document2", "age": 21 } |
    And I refresh the collection
    When I successfully call the action "document":"deleteByQuery" with args:
      | index      | "nyc-open-data"                                   |
      | collection | "yellow-taxi"                                     |
  ...
  Scenario: updateByQuery
    ...
    And I refresh the collection
    When I successfully call the action "document":"updateByQuery" with args:
      | index      | "nyc-open-data"                                                                                   |
      | collection | "yellow-taxi"                                                                                     |
