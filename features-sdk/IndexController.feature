Feature: Index Controller
  ...
  Scenario: Create a new index
    When I successfully call the action "index":"create" with args:
      | index | "nyc-open-data" |
    Then I should receive an empty result
    And I call the action "index":"list"
  ...
  Scenario: Re-create an existing index
    When I successfully call the action "index":"create" with args:
      | index | "nyc-open-data" |
    And I call the action "index":"create" with args:
      | index | "nyc-open-data" |
    Then I should receive an error matching:
      | status | 412 |

  Scenario: Create a index with illegal character
    When I call the action "index":"create" with args:
      | index | "%kuzzle" |
    Then I should receive an error matching:
      | status | 400 |
    When I call the action "index":"create" with args:
  ...
  Scenario: Test index existence
    ...
    When I successfully call the action "index":"exists" with args:
      | index | "nyc-open-data" |
  ...
  Scenario: List indexes
    ...
    When I successfully call the action "index":"list"
    Then I should receive a result matching:
  ...
  Scenario: List indexes with collection count
    ...
    When I successfully call the action "index":"list" with args:
