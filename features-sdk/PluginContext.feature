Feature: Plugin context
  ...
  Scenario: Instantiate a new embedded ES Client and use it
    ...
    When I successfully call the action "functional-test-plugin/constructors":"ESClient" with args:
      | _id   | "es-document"                    |
  ...
  Scenario: Access provided secrets
    When I successfully call the action "functional-test-plugin/secrets":"test" with args:
  ...
  Scenario: Trigger returns the pipe chain result
    When I successfully call the action "functional-test-plugin/pipes":"testReturn" with args:
      | name | "Mr Freeman" |
    Then I should receive a result matching:
  ...
  Scenario: Subscribe and unsubscribe to realtime notifications
    ...
    When I successfully call the action "realtime":"publish" with args:
      | index      | "test"     |
      | collection | "question" |
      | body       | {}         |
