Feature: Auth Controller
  ...
  Scenario: Create an API key
    When I successfully call the action "auth":"createApiKey" with args:
      | expiresIn | -1                                  |
      | refresh   | "wait_for"                          |
  ...
  Scenario: Search for API keys
    ...
    And I successfully call the action "auth":"createApiKey" with args:
      | expiresIn | -1                                  |
      | body      | { "description": "Sigfox API key" } |
    And I successfully call the action "auth":"createApiKey" with args:
      | expiresIn | -1                                |
    ...
    When I successfully call the action "auth":"searchApiKeys" with args:
  ...
  Scenario: Delete an API key
    ...
    When I successfully call the action "auth":"deleteApiKey" with args:
      | _id     | "SGN-HCM"  |
      | refresh | "wait_for" |
    And I successfully call the action "auth":"searchApiKeys"
