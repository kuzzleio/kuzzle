...
describe('Test missing method', function () {
  ...
  before(function () {
    ...
    return methods.missing(roomId, collection, filter, false);
  });
  ...
  it('should construct the filterTree with correct curried function name', function () {
    should(methods.dsl.filtersTree[collection].fields.lastName.missinglastName).not.be.empty();
  });
  ...
  it('should construct the filterTree with correct room list', function () {
    ...
    rooms = methods.dsl.filtersTree[collection].fields.lastName.missinglastName.rooms;
  ...
  it('should construct the filterTree with correct functions missing', function () {
    ...
    result = methods.dsl.filtersTree[collection].fields.lastName.missinglastName.fn(documentAda);
    should(result).be.exactly(true);
    result = methods.dsl.filtersTree[collection].fields.lastName.missinglastName.fn(documentGrace);
    should(result).be.exactly(false);
  });

  it('should return a rejected promise if the filter argument is empty', function () {
    return should(methods.missing('foo', 'bar', {}, false)).be.rejectedWith(BadRequestError, { message: 'A filter can\'t be empty' });
  });

  it('should return a rejected promise if the filter argument is invalid', function () {
    return should(methods.missing('foo', 'bar', { foo: 'bar' }, false)).be.rejectedWith(BadRequestError, { message: 'Filter \'missing\' must contains \'field\' attribute' });
  });
  ...
  it('should return a rejected promise if buildCurriedFunction fails', function () {
    return methods.__with__({
      ...
      return should(methods.missing('foo', 'bar', { field: 'foo' }, false));
    });
  });
  ...
  it('should register the filter in the lcao area in case of a "missing" filter', function () {
    return methods.__with__({
      ...
      return should(methods.missing('foo', 'bar', { field: 'foo' }, false)).be.fulfilled();
    });
  });
  ...
  it('should register the filter in the global area in case of a "not missing" filter', function () {
    return methods.__with__({
      ...
      return should(methods.missing('foo', 'bar', { field: 'foo' }, true)).be.fulfilled();
    });
  });
});