...
describe('OpenApiManager', () => {
  ...
  describe('#constructor', () => {
    it('should generate open API paths from kuzzle routes', () => {
      should(manager.kuzzleDefinition.paths).have.ownProperty('/nativeController/nativeAction');
    });

    it('should generate open API paths from kuzzle routes', () => {
      should(manager.applicationDefinition.paths).have.ownProperty('/logistic-objects/patch');
    });

    it('should make sure that route verbs are lowercase only', () => {
      should(manager.kuzzleDefinition.paths).have.ownProperty('/nativeController/nativeAction');
      should(manager.kuzzleDefinition.paths['/nativeController/nativeAction'])
        .have.ownProperty('get');
    });

    it('should transform our :param path notation to {param}', () => {
      should(manager.kuzzleDefinition.paths).have.ownProperty('/{index}/{collection}/_exists');
    });

    it('should extract parameters when required', () => {
      should(manager.kuzzleDefinition.paths).have.ownProperty('/{index}/{collection}/_exists');
      should(manager.kuzzleDefinition.paths['/{index}/{collection}/_exists'].get.parameters).match([
        { name: 'index', in: 'path' },
    ...
    it('should inject controller tags on already defined open api spec', () => {
      const routeDefinition = manager.applicationDefinition.paths['/logistic-objects/patch'].patch;
