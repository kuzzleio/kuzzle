...
describe.only('core/network/protocols/websocket', () => {
  ...
  describe('websocket configuration & initialization', () => {
    ...
    it('should set a minimum value to "idleTimeout" if set too low', async () => {
      for (const tooLow of [0, 999]) {
        ...
        should(kuzzle.log.warn).calledWith('[websocket] The "idleTimeout" parameter can neither be deactivated or be set with a value lower than 1000. Defaulted to 60000.');
      }
    });
    ...
    it('should warn if the deprecated "heartbeat" argument is set', async () => {
      ...
      should(kuzzle.log.warn).calledWith('[websocket] The "heartbeat" parameter has been deprecated and is now ignored. The "idleTimeout" parameter should now be configured instead.');
    });
  ...
  describe('ending a connection', () => {
    ...
    it('should clean up the memory associated to the closed connection', () => {
      ...
      should(httpWs.connectionBySocket)
        .not.have.value(closedSocket, closedClientConnection);

      should(httpWs.socketByConnectionId)
        .have.value(openedClientConnection.id, openedSocket);
      should(httpWs.socketByConnectionId)
        .not.have.value(closedClientConnection.id, closedSocket);
  ...
  describe('sending messages', () => {
    ...
    it('should send the message directly if backpressure allows it', () => {
      ...
      should(httpWs.backpressureBuffer.get(socket)).be.an.Array().and.be.empty();
    });
    ...
    it('should end the socket if the backpressure buffer is full', () => {
      ...
      should(socket.end)
        .calledOnce()
        .calledWithMatch(1011, Buffer.from('too much backpressure: client is too slow'));
    });
