...
describe("core/network/protocols/http", () => {
  ...
  beforeEach(() => {
    ...
    kuzzle.config.http.accessControlAllowOrigin = ["foo"];
  ...
  describe("#httpSendStream", () => {
    ...
    it("should end the connection if the stream is closed or errored", () => {
      ...
      should(httpWs.httpSendError).be.calledWithMatch(message, response, {
  ...
  describe("message reception", () => {
    ...
    it("should forward a well-formed request to the router", async () => {
      ...
      should(response.writeStatus)
        .calledOnce()
        .calledWithMatch(Buffer.from("200"));
