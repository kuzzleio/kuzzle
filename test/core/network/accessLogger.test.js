...
const { InternalError: KuzzleInternalError } = require('../../../lib/kerror/errors/internalError');
...
describe('#AccessLogger', () => {
  ...
  describe('#AccessLogger', () => {
    ...
    beforeEach(() => {
      ({ AccessLogger } = mockRequire.reRequire('../../../lib/core/network/accessLogger'));
    ...
    it('should create a new worker thread when logs are active', async () => {
      ...
      should(workerArgs).be.Array().and.match([
    ...
    it('should print an error log if an unusable transport is found', async () => {
      ...
      should(kuzzle.log.error).calledWith('Failed to initialize logger transport "ohnoes": unsupported transport. Skipped.');
    });
  ...
  describe('#AccessLoggerWorker', () => {
    ...
    before(() => {
      ...
      ({ AccessLoggerWorker } = mockRequire.reRequire('../../../lib/core/network/accessLogger'));
    });
    ...
    describe('#initTransport', () => {
      ...
      it('should ignore badly configured transports', () => {
        ...
        accessLoggerWorker.config.logs.transports.push(Object.assign({}, config));
    ...
    describe('#logAccess', () => {
      ...
      it('should output combined logs from an http request', () => {
        ...
        should(accessLoggerWorker.logger.info)
          .be.calledOnce()
          .be.calledWithMatch(/^1\.1\.1\.1 - admin \[\d\d\/[A-Z][a-z]{2}\/\d{4}:\d\d:\d\d:\d\d [+-]\d{4}] "METHOD url HTTP\/1\.1" 444 327 "http:\/\/referer.com" "user agent"$/);
      });
      ...
      it('should use the already verified user from the request, if available', () => {
        ...
        should(accessLoggerWorker.logger.info)
          .be.calledOnce()
          .be.calledWithMatch(/^ip - foobar \[\d{2}\/\w{3}\/\d{4}:\d{2}:\d{2}:\d{2} [+-]\d{4}] "DO \/controller\/action\/index\/collection WEBSOCKET" 102 339 - -$/);
      });
    });
