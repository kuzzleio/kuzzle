...
describe("Test: security/tokenRepository", () => {
  ...
  describe("#hydrate", () => {
    it("should return the given token if the given data is not a valid object", () => {
      ...
      return Bluebird.all([
        ...
        results.forEach((token) => should(token).be.exactly(t))
      );
    });
  ...
  describe("#verify", () => {
    ...
    it("should reject the promise if the jwt is invalid", () => {
      return should(
        tokenRepository.verifyToken("invalidToken")
    ...
    it("should reject the token if the uuid is not known", () => {
      const token = jwt.sign({ _id: -99999 }, kuzzle.secret, {
        algorithm: kuzzle.config.security.jwt.algorithm,
      });

      return should(tokenRepository.verifyToken(token)).be.rejectedWith(
        UnauthorizedError,
        {
          id: "security.token.invalid",
        }
    ...
    it("should reject if the jwt is expired", () => {
      const token = jwt.sign({ _id: -1 }, kuzzle.secret, {
        algorithm: kuzzle.config.security.jwt.algorithm,
        expiresIn: 0,
      });
    ...
    it("should reject if an error occurred while fetching the user from the cache", () => {
      const token = jwt.sign({ _id: "auser" }, kuzzle.secret, {
        algorithm: kuzzle.config.security.jwt.algorithm,
      });

      sinon
        .stub(tokenRepository, "loadFromCache")
    ...
    it("should reject the token if it does not contain the user id", () => {
      const token = jwt.sign({ forged: "token" }, kuzzle.secret, {
        algorithm: kuzzle.config.security.jwt.algorithm,
      });
    ...
    it("should return the token loaded from cache", async () => {
      const _id = "auser";
      const token = jwt.sign({ _id }, kuzzle.secret, {
        algorithm: kuzzle.config.security.jwt.algorithm,
      });
  ...
  describe("#create", () => {
    ...
    it("should reject the promise if the username is null", () => {
      return should(tokenRepository.generateToken(null)).be.rejectedWith(
        KuzzleInternalError,
        {
    ...
    it("should resolve to a token signed with the provided username", async () => {
      const user = new User();
      const rawToken = jwt.sign({ _id: "userInCache" }, kuzzle.secret, {
        algorithm: kuzzle.config.security.jwt.algorithm,
      ...
      should(tokenRepository.persistForUser).be.calledWith(
        checkToken,
        user._id,
        3600000
      );
    ...
    it("should return an internal error if an error occurs when generating token", async () => {
      ...
      const MockedTokenRepository = mockrequire.reRequire(
        "../../../lib/core/security/tokenRepository"
      );
      ...
      try {
        await should(tokenRepository.generateToken(user)).be.rejectedWith(
          KuzzleInternalError,
          {
    ...
    it("should reject if the ttl exceeds the maxTTL for jwt", () => {
      ...
      return should(
        tokenRepository.generateToken(user, { expiresIn: "1m" })
    ...
    it("should reject if the ttl exceeds the maxTTL for apiKey", () => {
      ...
      return should(
        tokenRepository.generateToken(user, { expiresIn: "1m", type: "apiKey" })
      ).be.rejectedWith(BadRequestError, { id: "security.token.ttl_exceeded" });
    });
    ...
    it("should reject if the ttl is infinite and the maxTTL is finite for jwt", () => {
      ...
      return should(
        tokenRepository.generateToken(user, { expiresIn: -1 })
      ).be.rejectedWith(BadRequestError, { id: "security.token.ttl_exceeded" });
    });
    ...
    it("should reject if the ttl is infinite and the maxTTL is finite for apiKey", () => {
      ...
      return should(
        tokenRepository.generateToken(user, { expiresIn: -1, type: "apiKey" })
      ).be.rejectedWith(BadRequestError, { id: "security.token.ttl_exceeded" });
    });
    ...
    it("should reject if the ttl is not ms-compatible or not a number", () => {
      ...
      return should(
        tokenRepository.generateToken(user, { expiresIn: "ehh" })
  ...
  describe("#delete", () => {
    ...
    it("should be able to expires a token", async () => {
      ...
      try {
        ...
        should(kuzzle.tokenManager.expire).be.calledOnce().be.calledWith(token);
      } finally {
    ...
    it("should delete the tokens associated to a user identifier", async () => {
      ...
      kuzzle.ask
        ...
        .resolves(
          JSON.stringify({
            userId: "foo",
            _id: `${Token.APIKEY_PREFIX}baz`,
            expiresAt: 3,
            jwt: `${Token.APIKEY_PREFIX}baz`,
          })
        );
      ...
      should(kuzzle.ask)
        ...
        .calledWith(
          ...
          `repos/kuzzle/token/${Token.APIKEY_PREFIX}baz`,
          -1
        );
  ...
  describe("#refresh", () => {
    ...
    it("should leave the current token intact if a new one cannot be created", async () => {
      ...
      await should(
        tokenRepository.refresh("user", oldToken, "10m")
      ).rejectedWith("foo");
