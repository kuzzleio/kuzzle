...
describe("Test: validation initialization", () => {
  let Validation, validation, kuzzle;
  ...
  it("should add the type provided in defaultTypesFiles", () => {
    const validationStub = sinon.spy(() => {}),
        ...
        hits: [
          ...
          },
          {
            ...
            _source: {
              ...
              validation: { another: "specification" },
            },
          },
          {
            _id: "anIndex#anotherCollection",
  ...
  describe("#validateFormat", () => {
    it("should resolve true if the specification is correct", () => {
      const curateCollectionSpecificationStub = sinon.stub().resolves({});

      validation.curateCollectionSpecification =
        curateCollectionSpecificationStub;

      return validation
        .validateFormat("anIndex", "aCollection", { a: "specification" })
        .then((result) => {
    ...
    it("should resolve false if the specification is not correct", () => {
      const curateCollectionSpecificationStub = sinon
        .stub(validation, "curateCollectionSpecification")
        .rejects(new Error("Mocked Error"));

      return validation
        .validateFormat("anIndex", "aCollection", { a: "bad specification" })
        .then((result) => {
          should(curateCollectionSpecificationStub.callCount).be.eql(1);
  ...
  describe("#curateCollectionSpecification", () => {
    ...
    it("should return structured fields when a collection specification is provided", () => {
      ...
      validation.structureCollectionValidation =
        structureCollectionValidationStub;
    ...
    it("should reject an error if the field specification throws an error", () => {
      const indexName = "anIndex",
        structureCollectionValidationStub = sinon
          .stub()
    ...
    it("should reject an error if the field specification returns an error", () => {
      ...
      checkAllowedPropertiesStub.returns(true);
      sinon
        .stub(validation, "structureCollectionValidation")
        .returns({ isValid: false, errors: ["an error"] });
    ...
    it("should reject an error if validators are not valid", () => {
      ...
      checkAllowedPropertiesStub.returns(true);
      sinon
        .stub(validation, "curateValidatorFilter")
  ...
  describe("#structureCollectionValidation", () => {
    it("should return a structured collection specification if configuration is correct", () => {
      const curateFieldSpecificationStub = sinon.spy(function (...args) {
    ...
    it("should throw an error if one of the field curation throws an error", () => {
      const curateFieldSpecificationStub = sinon
          .stub()
          .throws(new Error("an error")),
        collectionSpec = {
          fields: {
    ...
    it("should return an error array if one of the field curation returns an error in verbose mode", () => {
      const curateFieldSpecificationStub = sinon.stub(),
        indexName = "anIndex",
        collectionName = "aCollection",
        verboseErrors = true,
        collectionSpec = {
      ...
      curateFieldSpecificationStub
        .onCall(0)
        .returns({ isValid: false, errors: ["error one"] });
      curateFieldSpecificationStub
        .onCall(1)
        .returns({ isValid: false, errors: ["error two"] });
      curateFieldSpecificationStub
        .onCall(2)
        .returns({ isValid: false, errors: ["error three"] });
  ...
  describe("#curateFieldSpecification", () => {
    beforeEach(() => {
      validation.curateFieldSpecificationFormat = sinon
        .stub()
        .returns({ isValid: true });
    });

    it("should validate and curate field specifications with default configuration", () => {
      const typeValidateSpecValidation = sinon.stub().returns({}),
        fieldSpec = {
          type: "string",
        },
        expectedReturn = {
    ...
    it("should throw an error if type validation throws", () => {
      const typeValidateSpecValidation = sinon
          .stub()
          .throws(new PreconditionError("foobar")),
        fieldSpec = {
          type: "string",
        };
    ...
    it("should return an error if type validation returns false with verbose mode", () => {
      const typeValidateSpecValidation = sinon.stub().returns(true),
        fieldSpec = {
          type: "string",
          typeOptions: "foobar",
        };
    ...
    it("should throw an error if an option of typeOptions is invalid", () => {
      ...
      should(() => {
        ...
        message:
          /^The object "undefined.undefined.undefined" contains unexpected properties/,
      });
    });
    ...
    it("should return an error if a field specification format is invalid in verbose mode", () => {
      const anError = { isValid: false, errors: ["an error"] },
        fieldSpec = {
          type: "string",
          typeOptions: {
            some: "options",
