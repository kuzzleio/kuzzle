...
describe("Test: validation.validate", () => {
  ...
  describe("#validate", () => {
    it("should return a validation if the specification is empty", () => {
      ...
      return validation.validate(request, verbose).then((result) => {
    ...
    it("should trigger all validation if specification enables them", () => {
      ...
      sinon.stub(validation, "recurseFieldValidation").returns(true);
      sinon
        .stub(validation.koncorde, "test")
        .returns([filterId, "anotherFilter"]);
    ...
    it("should intercept a strictness error and set the message accordingly", () => {
      const error = new Error("strictness"),
        recurseFieldValidationStub = sinon
          .stub(validation, "recurseFieldValidation")
          .throws(error),
  ...
  describe("#recurseApplyDefault", () => {
    it("should update document with defaults if fields are missing or null", () => {
      const isUpdate = false,
        documentSubset = {
          aField: { anotherSubField: "some value" },
          aDefaultField: null,
        },
    ...
    it("should update document with defaults only if fields are null in write/update mode", () => {
      const isUpdate = true,
        documentSubset = {
          aField: { anotherSubField: "some value", aSubField: null },
        },
        collectionSpecSubset = {
          aField: {
            type: "typeChildren",
  ...
  describe("#isValidField", () => {
    ...
    it("should returns false in verbose mode", () => {
      ...
      should(errorMessages).be.deepEqual({
        fieldScope: {
          children: {
            aField: { messages: ["An error has occurred during validation."] },
          },
        },
      });
    });
    ...
    it("should return false if the field has not enough elements", () => {
      ...
      should(errorMessages).be.deepEqual({
        fieldScope: {
          children: {
            aField: {
              messages: ["Not enough elements. Minimum count is set to 2."],
            },
          },
        },
      });
    ...
    it("should return false if the field has too much elements", () => {
      ...
      should(errorMessages).be.deepEqual({
        fieldScope: {
          children: {
            aField: {
              messages: ["Too many elements. Maximum count is set to 2."],
            },
          },
        },
      });
    ...
    it("should return false if the field is mandatory but not provided", () => {
      ...
      should(errorMessages).be.deepEqual({
        fieldScope: {
          children: { anotherField: { messages: ["The field is mandatory."] } },
        },
      });
    });
        ...
        fieldScope: {
          children: {
            aField: {
              children: {
                aSubField: {
                  messages: ["An error has occurred during validation."],
                },
              },
            },
          },
    ...
    it("should return false if one of the subfields throws an strictness error in verbose mode", () => {
      ...
      typeValidateStub
        ...
        .returns(true)
        .onSecondCall()
        .throws({ message: "strictness", details: { field: "field" } });
