...
describe("ApiKey", () => {
  ...
  describe("ApiKey.create", () => {
    ...
    beforeEach(() => {
      ...
      createTokenStub = kuzzle.ask.withArgs(createTokenEvent).resolves(token);
    ...
    it("should create a new API key and generate a token", async () => {
      ...
      should(createTokenStub).be.calledWith(createTokenEvent, user, {
        expiresIn: "expiresIn",
        bypassMaxTTL: true,
        type: "apiKey",
      });

      should(saveStub).be.calledWith({ userId: "aschen", refresh: "wait_for" });
