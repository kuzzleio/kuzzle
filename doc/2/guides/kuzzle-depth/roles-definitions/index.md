...
In the [Getting Started Guide](/core/2/guides/essentials/security#user-permissions), we discussed how to assign basic permissions to users through roles and profiles. We are now going to look at more complex and dynamic permissions.
...
This can be address either with a [**Pipe Plugin**](/core/2/plugins/guides/pipes), or **Permission Closures**.
...
Permission closures are deprecated in favor of [Pipe Plugin](/core/2/plugins/guides/pipes), which are more flexible and simpler to use.
...
- `test` is the body of [the permission function](/core/2/guides/kuzzle-depth/roles-definitions#the-permission-function)
- `args` is the parameter given to [the fetch definition function](/core/2/guides/kuzzle-depth/roles-definitions#the-fetch-definition)
...
- `$currentUserId`: The current user [`<kuid>`](/core/2/guides/essentials/user-authentication#kuzzle-user-identifier-kuid) (equivalent to `request.context.token.userId`).
...
Documents fetched from the storage layer are stored in the `args` object as defined by the [fetch definition](/core/2/guides/kuzzle-depth/roles-definitions#the-fetch-definition).
...
The `search` action type performs a search on the persistence layer and returns the resulting documents. It is a typical [document search](/core/2/guides/essentials/store-access-data#document-search). For example:
...
Please refer to our [Elasticsearch Cookbook](/core/2/guides/cookbooks/elasticsearch) for more help on how to build your search query.
