...
class InternalIndexHandler extends Store {
  ...
  async init () {
    ...
    const mutex = new Mutex(this.kuzzle, 'InternalIndexBootstrap', 30000);
    ...
    try {
      ...
      await this.create('config', this._BOOTSTRAP_DONE_ID, {
        timestamp: Date.now(),
      });
    }
    catch (error) {
  ...
  async _persistSecret () {
    ...
    try {
      await this.create('config', this._JWT_SECRET_ID, { seed });
    }
    catch (e) {
