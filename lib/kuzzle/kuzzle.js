...
class Kuzzle extends KuzzleEventEmitter {
  ...
  async start (application, options = {}) {
    ...
    try {
      ...
      const executedCount = await this.install(options.installations);

      this.log.info(`[✔] ${executedCount} new installations executed`);
  ...
  /**
   * Execute multiple handlers only once during the lifetime of the app
   * 
   * @param {Array<{ id: string, handler: () => void }>} installations - Array of unique methods to execute
   * 
   * @returns {Promise<number>} Executed handler count
   */
  async install (installations) {
    ...
    try{
      for (const installation of installations) {
        const alreadyInstalled = await this.ask(
        ...
        if (!alreadyInstalled) {
          ...
          await this.ask(
            ...
            'kuzzle',
            'installations',
            { handler: installation.handler.toString(), installedAt: Date.now() },
            { id: installation.id });
      ...
      throw kerror.get( 'plugin', 'runtime', 'unexpected_error', error);
    }
    finally {
      await mutex.unlock();
    }
