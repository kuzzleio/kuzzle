...
class Kuzzle extends KuzzleEventEmitter {
  ...
  async start (application, options = {}) {
    ...
    try {
      ...
      if ( options.installations && options.installations.length) {
        const promises = [];
        for (const installation of options.installations) {
          promises.push(this.install(installation.id, installation.handler));
        }

        const results = await Promise.all(promises);

        const executedCount = results.reduce((count, executed) => {
          if (executed) {
            count += 1;
          }
          return count;
   ...
   * 
   * @returns {Promise<boolean>} true when successful, false if already installed
   */
  async install (id, handler) {
    const mutex = new Mutex(`installation-${id}`);
    await mutex.lock();
