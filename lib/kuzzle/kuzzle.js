...
class Kuzzle extends KuzzleEventEmitter {
  ...
  async import (toImport = {}, toSupport = {}) {
    const types = ['userMappings', 'mappings', 'fixtures', 'securities'];
    ...
    try {
      ...
      if (await mutexes.securities.lock()) {
        lockedMutexes.push(mutexes.securities);
        const isSecuritiesToImport = ! (
          _.isEmpty(toImport.profiles)
          && _.isEmpty(toImport.roles)
          && _.isEmpty(toImport.users)
        );
        const isSecuritiesToSupport = toSupport.securities
          && ! (
            _.isEmpty(toSupport.securities.profiles)
            && _.isEmpty(toSupport.securities.roles)
            && _.isEmpty(toSupport.securities.users)
          );
        if (isSecuritiesToSupport && isSecuritiesToImport) {
          throw kerror.get(
            'plugin',
            'runtime',
            'incompatible',
        ...
        else if (isSecuritiesToSupport) {
          await this.ask('core:security:load', toSupport.securities, { force: true });
          this.log.info('[✔] Securities import successful');
        }
        else if (isSecuritiesToImport) {
