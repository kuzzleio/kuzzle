...
class KuzzleEventEmitter extends EventEmitter {
   ...
   */
  onCall (event, fn) {
    assert(typeof fn === 'function', `Cannot register callback for "${event}": "${fn}" is not a function`);
    assert(! this.coreSyncedAnswerers.has(event), `Cannot register callback to "${event}": callback has already an answerer`);
  ...
  async call (event, ...payload) {
    ...
    if (! fn) {
      throw kerror.get('core', 'fatal', 'assertion_failed', `the requested callback '${event}' doesn't have an answerer`);
    }
