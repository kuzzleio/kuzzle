  ...
  */
  ...
  async _getIndice (index, collection) {
    ...
    if (body.length < 1) {
      ...
      throw kerror.get('multiple_indice_alias', '"alias" starting with "@"', '"indices"');
    }
   ...
   *
   * @returns {String} Alias name (eg: '@&nepali.liia')
   * @throws If there is not exactly one alias associated that is prefied with @
   */
  async _getAliasFromIndice (indice) {
    ...
    if (aliases.length < 1) {
      ...
      throw kerror.get('multiple_indice_alias', '"indice"', '"aliases" starting with an "@"');
    }
