  ...
  */
    ...
    { refresh, retryOnConflict = 0, timeout, userId=null } = {})
  {
    ...
    for (let i = 0; i < extractedDocuments.length; i++) {
      esRequest.body.push(
        ...
        {
          doc: extractedDocuments[i]._source.doc,
          upsert: extractedDocuments[i]._source.upsert
        }
      );
      // _source: true
    ...
    response.items = response.items.map(item => ({
      ...
      _source: item.get._source,
      _version: item._version,
      created: item.created === 'created', // Needed by the notifier
      status: item.status
    }));
  ...
  _extractMDocuments (documents, metadata, { prepareMGet=false, requireId=false, prepareMUpsert=false } = {}) {
    ...
    for (let i = 0; i < documents.length; i++) {
      ...
      if (!isPlainObject(document.body)) {
      ...
      else if (prepareMUpsert && !isPlainObject(document.body.changes)) {
        rejected.push({
          document,
          reason: 'document body.changes must be an object',
          status: 400
        });
      }
      else if (prepareMUpsert && document.body.default && !isPlainObject(document.body.default)) {
        rejected.push({
          document,
          reason: 'document body.default must be an object',
          status: 400
        });
      }
        ...
        if (prepareMUpsert) {
          extractedDocument = {
            _source: {
              doc: Object.assign({}, metadata.doc, document.body.changes),
              upsert: Object.assign({}, metadata.upsert, document.body.changes, document.body.default)
            }
          };
        }
        else {
