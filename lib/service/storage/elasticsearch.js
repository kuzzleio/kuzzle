  ...
  */
  ...
  _extractMDocuments (documents, metadata, { prepareMGet=false, requireId=false, prepareMUpsert=false } = {}) {
    ...
    for (let i = 0; i < documents.length; i++) {
      ...
      if (!isPlainObject(document.body) && !prepareMUpsert) {
      ...
      else if (prepareMUpsert && !isPlainObject(document.changes)) {
        rejected.push({
          document,
        ...
        if (prepareMUpsert) {
          extractedDocument = {
            _source: {
              doc: Object.assign({}, metadata.doc, document.changes),
              upsert: Object.assign({}, metadata.upsert, document.changes, document.default)
            }
          };
        }
        else {
