...
class ElasticSearch extends Service {
  ...
  constructor(kuzzle, config, scope = scopeEnum.PUBLIC) {
  ...
  async scroll (scrollId, { scrollTTL } = {}) {
    ...
    const cacheKey = SCROLL_CACHE_PREFIX + this._kuzzle.hash(esRequest.scrollId);
    ...
    let fetched = await this._kuzzle.ask('core:cache:internal:get', cacheKey);
    ...
    try {
      ...
      if (fetched >= body.hits.total.value) {
        ...
        await this._kuzzle.ask('core:cache:internal:del', cacheKey);
    ...
    { from, size, scroll } = {})
  {
    ...
    try {
      ...
      if (body._scroll_id) {
        ...
        await this._kuzzle.ask(
          'core:cache:internal:store',
          SCROLL_CACHE_PREFIX + this._kuzzle.hash(body._scroll_id),
        ...
        return this._client.get(esRequest)
          .then(({ body }) => {
            for (let field of fields) {
              _.unset(body._source, field);
            }

            _.set(body, '_source._kuzzle_info',
              _.merge(
                _.get(body, '_source._kuzzle_info', { createdAt: Date.now() }),
                _kuzzle_info
              )
            );
            ...
            return this._client.index(newEsRequest)
              .then(() => ({
                _id: id,
                _source: body._source,
                _version: body._version
  ...
  */
  ...
  async _mExecute (esRequest, documents, partialErrors, { limits=true } = {}) {
    ...
    if (limits && documents.length > this._kuzzle.config.limits.documentsWriteCount) {
  ...
  async waitForElasticsearch () {
    ...
    this._kuzzle.log.info('[ℹ] Trying to connect to Elasticsearch...');
    ...
    while (esState !== esStateEnum.OK) {
      try {
        ...
        if (health.body.number_of_pending_tasks === 0) {
          ...
          this._kuzzle.log.info(`[ℹ] Still waiting for Elasticsearch: ${health.body.number_of_pending_tasks} cluster tasks remaining`);
          await Bluebird.delay(1000);
        }
      }
      catch (e) {
