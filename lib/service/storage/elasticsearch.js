  ...
  async stats () {
    ...
    for (const [esIndex, esIndexInfo] of Object.entries(body.indices)) {
      ...
      indexes[indexName].collections.push({
        ...
        name: collectionName,
        sizeInBytes: esIndexInfo.total.store.size_in_bytes,
      });
      indexes[indexName].sizeInBytes += esIndexInfo.total.store.size_in_bytes;
      sizeInBytes += esIndexInfo.total.store.size_in_bytes;
    }

    return {
      indexes: Array.from(Object.values(indexes)),
      sizeInBytes,
    };
  }
