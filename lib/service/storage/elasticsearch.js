...
class ElasticSearch extends Service {
  ...
  async stats () {
    ...
    for (const [esIndex, indexInfo] of Object.entries(body.indices)) {
      ...
      if (!response.indexes[indexName]) {
        response.indexes[indexName] = {
          collections: [],
          name: indexName,
          size_in_bytes: 0,
        };
      }
      response.indexes[indexName].collections.push({
        documentCount: indexInfo.total.docs.count,
        name: collectionName,
        size_in_bytes: indexInfo.total.store.size_in_bytes,
      });
      response.indexes[indexName].size_in_bytes += indexInfo.total.store.size_in_bytes;
      response.size_in_bytes += indexInfo.total.store.size_in_bytes;
    }

    return {
      indexes: Array.from(Object.values(response.indexes)),
