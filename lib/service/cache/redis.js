...
class Redis extends Service {
  ...
  _initSequence() {
    ...
    if (this._config.database) {
      this._config.options = { db: this._config.database, ...this._config.options };
    }

    if (this._config.nodes) {
      if (this._config.overrideDnsLookup) {
        this._config.clusterOptions.dnsLookup = (address, callback) => callback(null, address);
      }
