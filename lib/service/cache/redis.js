...
class Redis extends Service {
  ...
  setCommands () {
    ...
    IORedis.Command.setReplyTransformer('setnx', booleanTransformer);
  }
  ...
  store (id, value, timeout = 0) {
    if (timeout > 0) {
      return this.commands.setex(id, timeout, value);
    }

    return this.commands.set(id, value);
  }
...
/**
 * Transforms commands resulting in "0" or "1" into a boolean response
 * @param  {string} result
 * @return {boolean}
 */
function booleanTransformer (result) {
  return result !== '0';
}
