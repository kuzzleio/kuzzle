 ...
 *    - direct callback calls for listeners registered from inside the server instance
...
function serverStart () {
  ...
  this.server = new wsServer({port: this.port, perMessageDeflate: false}, function () {
    deferred.resolve('IPC Server started');
  }.bind(this));
...
function clientConnect() {
  ...
  this.client = new wsClient('ws://' + this.host + ':' + this.port, {perMessageDeflate: false});

  this.client.on('open', function () {
    this.connected.resolve(this.client);
  }.bind(this));

  this.client.on('message', function (payload) {
    handleSocketMessage.call(this, this.client, this.uuid, JSON.parse(payload));
  }.bind(this));
