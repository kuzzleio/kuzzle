...
function ElasticSearch (kuzzle, options, config) {
  ...
  this.getInfos = function () {
    ...
    return this.client.info()
      ...
      .catch(error => Promise.reject(formatESError.call(this, error)));
  };
        ...
        return new Promise((resolve, reject) => {
          async.each(ids, (id, callback) => {
            ...
            this.client.bulk({body: bodyBulk})
              ...
              .catch(error => reject(formatESError.call(this, error)));
          });
        });
      });
  };
  ...
  this.import = function (requestObject) {
    ...
    return this.client.bulk(data)
      ...
      .catch(err => Promise.reject(formatESError.call(this, err)));
  };
...
function formatESError(error) {
  ...
  formattedMessages = {
    ...
    '^\\[illegal_argument_exception\\] object mapping \\[([^\\]]+)\\] can\'t be changed from nested to non-nested$': 'Can not change mapping for field "$1" from nested to another type',

    // [illegal_argument_exception] object mapping [baz] can\'t be changed from non-nested to nested
    '^\\[illegal_argument_exception\\] object mapping \\[([^\\]]+)\\] can\'t be changed from non-nested to nested$': 'Can not change mapping for field "$1" from object to another type',
    ...
    '^\\[illegal_argument_exception\\] \\[([^\\]]+)\\] is defined as an object in mapping \\[([^\\]]+)\\] but this name is already used for a field in other types$': 'Can not set mapping for field "$1" on collection "$2" because the field name is already used in another collection with a different type',

    // [illegal_argument_exception] mapper [source.flags] of different type, current_type [string], merged_type [long]
    '^\\[illegal_argument_exception\\] mapper \\[([^\\]]+)\\] of different type, current_type \\[([^\\]]+)\\], merged_type \\[([^\\]]+)\\]$': 'Can not change type of field "$1" from "$2" to "$1"',

    // [mapper_parsing_exception] Mapping definition for [flags] has unsupported parameters:  [index : not_analyzed]
    '^\\[mapper_parsing_exception\\] Mapping definition for \\[([^\\]]+)\\] has unsupported parameters:  \\[([^\\]]+)\\]$': 'Parameter "$2" is not supported for field "$1"',

    // [mapper_parsing_exception] No handler for type [booleasn] declared on field [not]
    '^\\[mapper_parsing_exception\\] No handler for type \\[([^\\]]+)\\] declared on field \\[([^\\]]+)\\]$': 'Can not set mapping for field "$1" because type "$2" does not exist',

    // [mapper_parsing_exception] failed to parse [conditions.host.flags]
    '^\\[mapper_parsing_exception\\] failed to parse \\[([^\\]]+)\\]$': 'Failed to validate value of field "$1". Are you trying to insert nested value in a non-nested field ?',

    // [index_not_found_exception] no such index, with { resource.type=index_or_alias resource.id=foso index=foso }
    '^\\[index_not_found_exception\\] no such index, with { resource\\.type=index_or_alias resource\\.id=([^\s]+) index=([^\s]+) }$': 'Index "$1" does not exist, please create it first',

    // [mapper_parsing_exception] Expected map for property [fields] on field [foo] but got a class java.lang.String
    '^\\[mapper_parsing_exception\\] Expected map for property \\[fields\\] on field \\[([^\\]]+)\\] but got a class java\\.lang\\.String$': 'Mapping for field "$1" must be an object with a property "type"'
  };

  Object.keys(formattedMessages).find(pattern => {
    var
      matches,
      re = new RegExp(pattern, 'i');

    matches = re.exec(error.message);

    if (matches) {
      message = formattedMessages[pattern];
      ...
      matches.forEach((v, index) => {
        ...
        message = message.replace('$' + index, matches[index]);
      });
  ...
  if (error.displayName === 'BadRequest') {
  ...
  else if (error.displayName === 'NotFound') {
    if (!message) {
      message = error.body.error.reason + ': ' + error.body.error['resource.id'];
    }
    ...
    this.kuzzle.pluginsManager.trigger('log:warn', new InternalError('[warning] unhandled elasticsearch error:\n' + error.message));
  }
