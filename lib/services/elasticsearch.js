...
class ElasticSearch extends Service {
   ...
   */
  delete (index, collection, id, { refresh=false } = {}) {
    const esRequest = {
  ...
  /**
   * Retrieve the complete list of existing collections in the current index
   *
   * @param {string} index - Index name
   *
  ...
  /**
   * Retrive the complete list of indexes
   *
  ...
  mCreate (
    ...
    { refresh=null, timeout=null, userId=null } = {})
  {
    const extracted = extractMDocuments(request, {
        _kuzzle_info: {
          active: true,
   ...
   */
  _mergeDefaultMappings (index, oldMapping, newMapping) {
  ...
  /**
   * Throw an error if the provided mapping is invalid
   *
   ...
   * @returns {Promise.<object>} resolve to an array of documents IDs
   */
  _fetchIds(esRequest) {
    const ids = [];
   ...
   * @return {object}
   */
  _extractMDocuments(index, collection, documents, metadata) {
    const result = {
      rejected: [],
      documents: [],
      mGetRequest: {
