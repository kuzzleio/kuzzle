...
class ElasticSearch extends Service {
  ...
  scroll (
    ...
    collection,
    scrollId,
    { scroll=this._config.defaults.scrollTTL } = {})
  {
    const esRequest = {
    ...
    return this._kuzzle.cacheEngine.internal.exists(cacheKey)
      .then(exists => {
        ...
        const ttl = ms(esRequest.scroll) || ms(this._config.defaults.scrollTTL);
   ...
   *
   * @returns {Promise.<Object>} { ids, total, deleted, failures: [ _shardId, reason ] }
   */
  deleteByQuery (
    index,
    collection,
    ...
    { refresh, from, size } = {})
  {
    ...
    return this._getAllIdsFromQuery(esRequest)
      .then(ids => {
        matchingIds = ids;
  ...
  createCollection (index, collection, mappings = {}) {
    const esRequest = {
      index: this._getESIndex(index, collection),
      body: { mappings: _.merge(mappings, this._config.commonMapping) }
    };
  ...
  truncateCollection (index, collection) {
    ...
    return this.getMapping(index, collection)
      ...
      .then(() => this._client.indices.create({ ...esRequest, mappings }))
      .then(() => {
        // return nothing
      })
      .catch(error => this._esWrapper.reject(error));
   ...
   *
   * @returns {Promise.<Array>} resolve to an array of documents IDs
   */
  _getAllIdsFromQuery (esRequest) {
    const ids = [];

    return new Bluebird((resolve, reject) => {
      this._client.search({ index: esIndex, searchBody}, function getMoreUntilDone(error, response) {
        if (error) {
          return reject(error);
        }
