...
module.exports = function (kuzzle, options) {
  ...
  this.create = function (requestObject) {
    ...
    this.client.create(data)
      .then(function (result) {
        ...
        if (kuzzle.indexes[data.index].indexOf(data.collection) === -1) {
          kuzzle.indexes[data.index].push(data.collection);
        }
  ...
  this.deleteCollection = function (requestObject) {
    ...
    this.client.indices.deleteMapping(data)
      .then(function () {
        kuzzle.indexes[data.index].splice(kuzzle.indexes[data.index].indexOf(data.collection));
    ...
    if (data.body) {
      ...
      async.eachLimit(data.body, 20, item => {
        ...
        if (nameActions.indexOf(action) !== -1) {
          ...
          // @TODO: implement multi index : endpoint
          item[action]._index = requestObject.index;
        }
      });
  ...
  this.reset = function () {
    ...
    this.client.indices.delete({index: '_all'})
      .then(() => {
        this.client.indices.create({index: '@kuzzle'})
          .then(function () {
  ...
  this.listIndexes = function (requestObject) {
    ...
    this.client.cat.indices({h: 'index'})
      .then(result => {
        ...
        indexes = indexes.filter(function(indexName) {return indexName !== ''});
