...
class ElasticSearch extends Service {
  ...
  init() {
    ...
    if (process.env.NODE_ENV === 'production' && this.config.dynamic === 'true') {
      this.kuzzle.log.warn('Your dynamic mapping policy is set to \'true\' ' +
      'for new fields.\nElasticsearch will try to automatically infer mapping ' +
      'for new fields, and those cannot be changed afterward.\nSee the ' +
      '"services.db.dynamic" option in the kuzzlerc configuration file to ' +
      'change this value.'
      );
    }
        ...
        for (i = 0; i < extracted.documents.length; i++) {
          ...
          if (typeof document._id === 'string') {
            ...
            if (mgetResult.docs[idx].found) {
              ...
              extracted.rejected.push(
                {document,
                  reason: 'cannot replace a non-existing document ' +
                  '(use mCreateOrReplace if you need to create ' +
