...
module.exports = function (kuzzle, options) {
  ...
  /**
   * Reset all indexes that the users is allowed to
   *
   * @return {Promise}
   */
  this.deleteIndexes = function (requestObject) {
    var
      allowedIndexes = requestObject.data.body.indexes,
      deletedIndexes = [];
    ...
    return this.listIndexes(requestObject)
      .then(result => {
        deletedIndexes = result.data.body.indexes.filter(indexName => {
          return (allowedIndexes === undefined || _.indexOf(allowedIndexes, indexName) !== -1);
        });
