...
class ElasticSearch extends Service {
            ...
            return this.client.putScript({
              ...
              body: {
                script: `if (ctx._source._kuzzle_info == null) {
                  ...
                  ctx._source._kuzzle_info.deleteAt = params.now;
                }`
              }
            })
              .then(() => this);
  ...
  deleteByQuery(request) {
    ...
    let esMethod = req => this.client.updateByQuery(req);
    let responseMember = 'updated';

    esRequest.body = request.input.body;

    if (request.input.args.includeTrash || request.input.bypassTrash) {
      // real deletion from es
    ...
    const deleteBatch = (req, attempts = 0) => esMethod(req)
      .then(response => {
        ...
        const conflicts = response.version_conflicts;
