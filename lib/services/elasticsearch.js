...
function addActiveFilter(data) {
  ...
  if (data.body) {
    if (data.body.query) {
      dataBody.query.bool.must.push(data.body.query);
    }

    if (data.body.sort) {
      dataBody.sort = data.body.sort;
    }

    if (data.body.aggs) {
      dataBody.aggs = data.body.aggs;
    }

    if (data.body.aggregations) {
      dataBody.aggregations = data.body.aggregations;
    }

    if (data.body.from) {
      dataBody.from = data.body.from;
    }

    if (data.body.size) {
      dataBody.size = data.body.size;
    }
  }
