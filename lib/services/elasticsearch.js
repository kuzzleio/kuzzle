...
class ElasticSearch extends Service {
    ...
    { scroll=this.config.defaults.scrollTTL } = {})
  {
    ...
    return this.kuzzle.services.internalCache.exists(cacheKey)
      .then(exists => {
        if (exists === 0) {
        ...
        return this.kuzzle.services.internalCache.pexpire(cacheKey, ttl);
      })
    ...
    return this.client.search(esRequest)
      .then(({ body }) => {
        if (body.scrollId) {
          ...
          return this.kuzzle.services.internalCache.psetex(key, ttl, 0)
            .then(() => body);
        }
  ...
  update (
    ...
    id,
    content,
    { refresh, userId=null, retryOnConflict=this.config.defaults.onUpdateConflictRetries } = {})
  {
    const esRequest = {
      index: this._getESIndex(index, collection),
      body: { doc: content },
   ...
   *
   * @returns {Promise.<Object>} { aliases: { name, index, collection } }
   */
  listAliases () {
