...
class ElasticSearch extends Service {
  ...
  get(request) {
    ...
    if (esRequest.id === '_search') {
      return Bluebird.reject(errorsManager.get('wrong_get_action'));
    }
    ...
    return this.client.get(esRequest)
      .then(result => {
        if (
          ...
          return Bluebird.reject(errorsManager.get('document_not_found'));
        }
        ...
        if (esRequest.id) {
          ...
          return this.client.get(
            ...
            .then(result => {
              ...
              return Bluebird.reject(errorsManager.get('document_already_exists'));
            })
    ...
    return this.client
      ...
      .then(result => {
        if (result._source._kuzzle_info && !result._source._kuzzle_info.active) {
          errorsManager.throw('document_id_not_found', esRequest.id);
        }
        // injecting retryOnConflict default configuration
        if (
  ...
  deleteByQuery(request) {
    ...
    if (!esRequestSearch.body.query ||
      ...
      return Bluebird.reject(errorsManager.get('empty_query'));
    }
  ...
  deleteByQueryFromTrash(request) {
    ...
    if (esRequestSearch.body.query === null) {
      return Bluebird.reject(errorsManager.get('document_id_cannot_be_null'));
    }
  ...
  createCollection(request) {
    ...
    return this.kuzzle.indexCache.exists(esRequest.index)
      .then(indexExists => {
        if (! indexExists) {
          errorsManager.throw('index_does_not_exist');
        }
  ...
  import(request) {
    ...
    if (!request.input.body || !(request.input.body.bulkData instanceof Object)) {
      return Bluebird.reject(
        errorsManager.get('missing_or_invalid_import_attribute')
      );
    }
        ...
        for (let i = 0; i < esRequest.body.length; i++) {
          ...
          if (actionNames.indexOf(action) !== -1) {
            ...
            if (item[action]._index === this.kuzzle.internalEngine.index) {
              return Bluebird.reject(
                errorsManager.get('index_protected',
