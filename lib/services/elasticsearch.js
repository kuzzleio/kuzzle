var
  ...
  q = require('q'),
  async = require('async'),
  RequestObject = require('../api/core/models/requestObject'),
...
module.exports = function (kuzzle, options) {
  ...
  this.create = function (requestObject) {
    ...
    this.client.create(data)
      .then(function (result) {
        if (kuzzle.indexes[data.index] === undefined) {
          kuzzle.indexes[data.index] = [];
        }

        if (kuzzle.indexes[data.index].indexOf(data.type) === -1) {
          kuzzle.indexes[data.index].push(data.type);
        }
   ...
   * @TODO: implement multi index : check index deletion
   *
   * @param {RequestObject} requestObject
  ...
  this.deleteCollection = function (requestObject) {
    ...
    this.client.indices.deleteMapping(data)
      .then(function () {
        kuzzle.indexes[data.index].splice(kuzzle.indexes[data.index].indexOf(data.type));
  ...
  this.truncateCollection = function (requestObject) {
    ...
    this.client.indices.getMapping(data)
      .then(mapping => {
        if (!mapping[requestObject.index]) {
          return Promise.reject(new NotFoundError('The collection "' + data.type + '" in index "' + requestObject.index + '" does not exist'));
        }

        mappings = mapping[requestObject.index].mappings[data.type];
    ...
    if (data.body) {
      ...
      async.eachLimit(data.body, 20, item => {
        ...
        if (nameActions.indexOf(action) !== -1) {
          if (data.type !== undefined && data.type && data.type.length > 0) {
          ...
          if (data.index !== undefined && data.index && data.index.length > 0) {
        ...
        this.client.bulk(data)
          .then(function (result) {
            ...
            async.each(result.items, function(resultItem, resultCallback) {
              async.each(Object.keys(resultItem), function(action, callback) {
                var item = resultItem[action];
                if (action === 'create' && !item.error) {

                  if (kuzzle.indexes[item._index] === undefined) {
                    kuzzle.indexes[item._index] = [];
                  }
                  if (kuzzle.indexes[item._index].indexOf(item._type) === -1) {
                    kuzzle.indexes[item._index].push(item._type);
                  }
                }
                callback();
              });
  ...
  this.deleteIndexes = function () {
    ...
    this.client.indices.delete(new RequestObject({index: '_all'}))
      .then(() => {
        kuzzle.indexes = {};
  ...
  this.listIndexes = function (requestObject) {
    ...
    this.client.cat.indices({h: 'index'})
      .then(result => {
        indexes = result.split(' ').join('').split('\n');
        indexes = indexes.filter(indexName => {return indexName !== '';});
  ...
  this.deleteIndex = function (requestObject) {
    ...
    this.client.indices.delete({index: data.index})
      .then(result => {
        delete kuzzle.indexes[data.index];
        deferred.resolve(new ResponseObject(requestObject, result));
      })
      .catch(error => deferred.reject(error));
