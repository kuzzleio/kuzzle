...
function ElasticSearch(kuzzle, options, config) {
  ...
  this.errorMessagesMapping = [
    ...
    },
    {
      ...
      regex: /^\[index_not_found_exception\] no such index, with { resource\.type=index_or_alias resource\.id=([^\s]+) index=([^\s]+) }$/,
      replacement: 'Index "$1" does not exist, please create it first'
    },
    {
    ...
    return this.client.get(data)
      .then(result => {
        if (result._source._kuzzle_info && !result._source._kuzzle_info.active) {
          ...
          return Promise.reject(new NotFoundError('Document has been deleted'));
        }
      ...
      .then(ids => {
        return new Promise((resolve, reject) => {
          async.each(ids, (id, callback) => {
            bodyBulk.push({update: {_index: data.index, _type: data.type, _id: id}});
            bodyBulk.push({doc: {_kuzzle_info: { active: false, deletedAt: new Date()}}});
            callback();
          }, () => {
  ...
  /**
   * Delete inactive documents (having _kuzzle_info.active to false)
   *
   * @param maxDocument
   * @returns {Promise.<TResult>}
   */
  this.deleteInactiveDocuments = function (maxDocument) {
    var
      promises = [],
      filter = {
        scroll: '30s',
          ...
          res.indexes.forEach(index => {
            if (index !== '%kuzzle') {
              this.client.indices.getMapping({index})
                .then(res => {
                  filter.collection = Object.keys(res[index].mappings);
                  filter.index = index;
                  getAllIdsFromQuery.call(this, filter)
                    .then(ids => {
                      var
                        bodyBulk = [];
                      ...
                      promises.push(new Promise((resolve, reject) => {
                        async.each(ids, (id, callback) => {
                          bodyBulk.push({delete: {_index: index, _id: id}});
                          callback();
                        }, () => {
                          if (bodyBulk.length === 0) {
                            return resolve({ids: []});
                          ...
                          this.client.bulk({body: bodyBulk})
                            .then(res => refreshIndexIfNeeded.call(this, data, {ids: ids}))
                            .then(result => resolve(result))
                            .catch(error => reject(this.formatESError(error)));
                        });
                      }));
...
function addActiveFilter(data) {
  var inputFilter = (data.body ? data.body.query : null);
