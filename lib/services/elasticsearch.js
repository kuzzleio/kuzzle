...
class ElasticSearch extends Service {
   ...
   * @returns {Promise} { _id, _version, _source }
   */
  create (index, collection, content, { id=null, refresh=false, userId=null }) {
   ...
   * @returns {Promise} { _id, _version, _source }
   */
  replace (index, collection, id, content,  { refresh=false, userId=null }) {
    const
  ...
  getMapping (index, collection, { includeKuzzleMeta=false } ) {
    ...
    // TODO return collection es metadata
