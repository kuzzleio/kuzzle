...
class ElasticSearch extends Service {
  ...
  getMapping(request) {
    ...
    if (request.context.connection.protocol !== 'http'
      && !_.isNil(request.input.args.includeKuzzleMeta)
      && typeof request.input.args.includeKuzzleMeta !== 'boolean') {

      throw new BadRequestError(`Invalid includeKuzzleMeta value ${request.input.args.includeKuzzleMeta}: boolean expected`);
    } else if (request.context.connection.protocol !== 'http' && request.input.args.includeKuzzleMeta !== undefined) {
      includeKuzzleMeta = request.input.args.includeKuzzleMeta;
    } else {
      includeKuzzleMeta = request.input.args.includeKuzzleMeta !== undefined ? true : false;
    }
