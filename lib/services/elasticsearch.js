...
function formatESError(error) {
  var kuzzleError = new Error(error.message);

  if (error.displayName === 'BadRequest') {
    kuzzleError = new BadRequestError(error.body.error.reason);
  }
  else if (error.displayName === 'NotFound') {
    kuzzleError = new NotFoundError(error.body.error.reason + ': ' + error.body.error['resource.id']);
  }
  else {
