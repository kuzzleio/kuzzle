...
function ElasticSearch (kuzzle, options, config) {
  ...
  this.count = function elasticsearchCount (request) {
     ...
     */
    // TODO see if still necessary with new Request
...
function getElasticsearchRequest(request, kuzzle) {
  ...
  if (typeof request.input.args.from !== 'undefined') {
    data.from = request.input.args.from;
  }
  if (typeof request.input.args.size !== 'undefined') {
    data.size = request.input.args.size;
  }
  if (typeof request.input.args.scroll !== 'undefined') {
    data.scroll = request.input.args.scroll;
  }
  if (typeof request.input.args.scrollId !== 'undefined') {
    data.scrollId = request.input.args.scrollId;
  }
  if (typeof request.input.args.refresh !== 'undefined') {
    data.refresh = request.input.args.refresh;
  }
 ...
 *
 * @this ElasticSearch
 * @param {Object} esRequest The data computed from the requestObject
...
function refreshIndexIfNeeded(esRequest, response) {
  if (esRequest && esRequest.index && this.settings.autoRefresh[esRequest.index]) {
    return this.refreshIndex(new Request({resource: {index: esRequest.index}}))
      .then(() => response)
      .catch(error => {
