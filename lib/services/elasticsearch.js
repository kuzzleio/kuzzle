...
class ElasticSearch extends Service {
  ...
  createCollection(request) {
    ...
    esRequest.body = {
      [esRequest.type]: {
        properties: this.config.commonMapping
      }
    };
  ...
  import(request) {
    const
      nameAction = ['index', 'create', 'update', 'delete'],
  ...
  mcreate(request) {
    ...
    if (extracted.mgetRequest.body.ids.length > 0) {
      mgetRequest = this.client.mget(extractMDocuments.mgetRequest);
    } else {
      mgetRequest = new Bluebird.resolve({docs: []});
    }
      ...
      .then(mgetResult => {
        ...
        for (const document of extracted.documents) {
          if (typeof document._id === 'string') {
            ...
            if (document._id === mgetResult.docs[idx]._id && (!mgetResult.docs[idx].found || !mgetResult.docs[idx]._source._kuzzle_info || !mgetResult.docs[idx]._source._kuzzle_info.active)) {
              esRequest.body.push({index: {_index: index, _type: type, _id: document._id}});
              esRequest.body.push(document._source);
            } else {
              extracted.rejected.push({document, reason: 'document already exists'});
            }
  ...
  mupdate(request) {
    ...
    for (const document of extracted.documents) {
      ...
      esRequest.body.push({doc: document._source});
    } 
  ...
  mdelete(request) {
    ...
    for (const element of request.input.body.ids) {
      if (typeof element === 'string') {
        esRequest.body.push({update: {_index: index, _type: collection, _id: element}});
        esRequest.body.push({doc: metadata});
      } else {
        partialErrors.push({bad_format: element});
      }
    }

    return this._mexecute(esRequest, [], partialErrors);
  }
  ...
  _mexecute(esRequest, documents, partialErrors) {
    ...
    if (documents.length > this.kuzzle.config.limits.documentsWriteCount) {
      return Bluebird.reject(new SizeLimitError(`Number of documents exceeds the server configured value (${this.kuzzle.config.limits.documentsWriteCount})`));
    }
    ...
    return this.client.bulk(esRequest)
      ...
      .then(result => {
        ...
        for (i = 0; i < result.items.length; i++) {
          ...
          if (item.error) {
            partialErrors.push(item);
          } else {
            successes.push(Object.assign(item, documents[i]));
          }
        }
  ...
  /**
   * Perform multiple actions on documents with one request
   * Similar to bulk import with a more restrictive syntax
   * (used by m* API routes)
   * 
   * @param  {Request} request - API request
   * @param  {String} action - document action: create, replace, createOrRepalce, update or delete
   * @return {Promise}
   * @throws 
   */
  mWrite(request, action) {
    const 
      esRequest = initESRequest(request, ['consistency', 'refresh', 'timeout', 'fields']),
      ts = Date.now(),
      fromUser = request.context.user._id ? String(request.context.user._id) : null,
      index = request.input.resource.index,
      collection = request.input.resource.collection,
      sources = [],
      partialErrors = [];
    ...
    let arrayName, metadata;

    switch(action) {
      case 'delete':
        arrayName = 'ids';
        metadata = {_kuzzle_info: {active: false, deletedAt: ts, updater: fromUser}};
        break;
      case 'update':
      case 'replace':
        arrayName = 'documents';
        metadata = {_kuzzle_info: {active: true, updatedAt: ts, updater: fromUser, deletedAt: null}};
        break;
      case 'create':
      case 'createOrReplace':
        arrayName = 'documents';
        metadata = {_kuzzle_info: {active: true, author: fromUser, updater: null, updatedAt: null, deletedAt: null}};
        break;
    }

    if (request.input.body[arrayName].length > this.kuzzle.config.limits.documentsWriteCount) {
      return Bluebird.reject(new SizeLimitError(`Number of documents to update exceeds the server configured value (${this.kuzzle.config.limits.documentsWriteCount})`));
    }

    for (const element of request.input.body[arrayName]) {
      if (action === 'delete') {
        if (typeof element === 'string') {
          esRequest.body.push({update: {_index: index, _type: collection, _id: element}});
          esRequest.body.push({doc: metadata});
        } else {
          rejected.push({bad_format: element});
        }
      } else if (typeof element.body === 'object' && element.body !== null && !Array.isArray(element.body)) {
          sources.push(element.body);

          if (action === 'create' && !element._id) {
            esRequest.body.push({index: {_index: index, _type: collection, _id: element._id}});
          } else {
            esRequest.body.push({[bulkAction]: {_index: index, _type: collection, _id: element._id}});
          }

          const document = Object.assign({}, metadata, element.body);

          if (action === 'update') {
            esRequest.body.push({doc: document});
          } else {
            esRequest.body.push(document);
          }
      } else {
        rejected.push({bad_format: element});
      }
      ...
      .then(result => {
        ...
        for (i = 0; i < result.items.length; i++) {
          ...
          if (item.error) {
            ...
            successes.push({_id: item._id, _source: sources[i]});
          }
        }
...
function extractMDocuments(request, metadata) {
  ...
  for (const document of request.input.body.documents) {
    if (typeof document.body === 'object' && document.body !== null && !Array.isArray(document.body)) {
      ...
      if (typeof document._id === 'string') {
        result.mgetRequest.body.ids.push({_id: document._id, _source: '_kuzzle_info.active'});
      }
    } else {
