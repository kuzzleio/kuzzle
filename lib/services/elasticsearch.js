...
class ElasticSearch extends Service {
   ...
   * @param {String} id - Document id
   * @param {Object} content - Document content
   * @param {Object} options - refresh (false), userId (null), injectKuzzleMeta (false)
   *
  ...
  createOrReplace (
    ...
    id,
    content,
    { refresh=false, userId=null, injectKuzzleMeta=false } = {})
  {
    const esRequest = {
      index: this._getESIndex(index, collection),
      id,
  ...
  update (
    ...
    id,
    content,
    { refresh=false, userId=null, retryOnConflict=-1 } = {})
  {
    const esRequest = {
      index: this._getESIndex(index, collection),
      id,
   ...
   */
  delete (index, collection, id, { refresh=false, retryOnConflict=-1 } = {}) {
    const esRequest = {
      index: this._getESIndex(index, collection),
      id,
      refresh,
  ...
  createCollection (index, collection, mappings = {}) {
    const esRequest = {
      index: this._getESIndex(index, collection),
      body: _.merge(mappings, this.config.commonMapping)
    };
  ...
  updateMapping (index, collection, mappings) {
    ...
    return this.getMapping(index, collection, true)
      .then(collectionMappings => {
        this._checkMappings(collectionMappings);

        esRequest.body = _.merge(mappings, collectionMappings);
  ...
  getMapping (index, collection, { includeKuzzleMeta=false } = {}) {
    ...
    return this.kuzzle.indexCache.exists(index, collection)
      .then(exists => {
        if (! exists) {
          this.throw('index_or_collection_does_not_exist', index, collection);
        }
    ...
    { refresh=null, timeout=null, userId=null } = {})
  {
    ...
    // @todo use indexCache.indexes instead
    const esCache = {};

    return this.client.indices.getMapping()
      .then(raw => {
        for (const esIndex of Object.keys(raw)) {
          esCache[esIndex] = Object.keys(raw[esIndex].mappings);
        }

        return this.client.cat.aliases({ format: 'json' });
      })
      .then(aliases => {
  ...
  collectionExists (index, collection) {
    return this.listCollections(index, collection)
  ...
  mCreateOrReplace(
    ...
    collection,
    documents,
    { refresh=null, timeout=null, userId=null, injectKuzzleMeta=false } = {})
  {
    let kuzzleMeta = {};
    ...
    return promise
      .then(({ body }) => {
        ...
        for (let i = 0; i < body.items.length; i++) {
          ...
          if (item.status >= 400) {
            partialErrors.push(Object.assign({}, documents[i], item));
          } else {
            successes.push(Object.assign({}, documents[i], item));
          }
        }
  ...
  /**
   * Removes depth in object (replaces hits.hits<array>, with hits<array>)
   *
   * @param result
   *
   * @returns {Object}
   ...
   *
   * @return {Object} { rejectedn documents, documentsToGet }
   */
