...
class ElasticSearch extends Service {
  ...
  constructor(kuzzle, config, scope = 'public') {
    ...
    assert(
      typeof this._config.maxScrollDuration === 'string',
      'services.storageEngine.maxScrollDuration must be a string.');

    this.maxScrollDuration = ms(this._config.maxScrollDuration);

    assert(
      typeof this.maxScrollDuration === 'number',
      `Invalid parsed value from ms for services.storageEngine.maxScrollDuration ("${typeof this.maxScrollDuration}").`);;
  }
    ...
    { scrollTTL } = {})
  {
    ...
    if (_scrollTTL) {
      ...
      if (scrollDuration > this.maxScrollDuration) {
        throw errorsManager.get('scroll_duration_too_great', _scrollTTL);
      }
    }
