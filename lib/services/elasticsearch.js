...
class ElasticSearch extends Service {
    ...
    { refresh, timeout } = {})
  {
    ...
    // @todo should be done in controllers ?
    ...
    // @todo should be done in the clientAdapter ?
  ...
  _getAllDocumentsFromQuery ({ index, body, scroll, from, size }) {
    ...
    return new Bluebird((resolve, reject) => {
      client.search(
        ...
        function getMoreUntilDone(error, { body }) {
          if (error) {
            return reject(error);
          }
          ...
          if (body.hits.total.value !== documents.length) {
            client.scroll({
              scrollId: body._scroll_id,
              scroll
            }, getMoreUntilDone);
