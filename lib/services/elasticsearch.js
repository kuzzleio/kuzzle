...
class ElasticSearch extends Service {
  ...
  scroll(request) {
    ...
    const cacheKey = scrollCachePrefix + highwayhash.asHexString(this.kuzzle.hashKey, Buffer.from(esRequest.scrollId));
      ...
      .then(result => {
        ...
        if (flattened._scroll_id !== undefined) {
          const
            ...
            key = scrollCachePrefix + highwayhash.asHexString(this.kuzzle.hashKey, Buffer.from(flattened._scroll_id));
  ...
  create(request) {
    ...
    if (esRequest.body._routing) {
      return Bluebird.reject(new BadRequestError('Kuzzle does not support "_routing" in create action.'));
    }

    if (esRequest.hasOwnProperty('refresh')) {
      if (compareVersions(this.config.apiVersion, '5.0') < 0) {
        return Bluebird.reject(new BadRequestError(`Refresh parameter is not supported by the version "${this.config.apiVersion}" of Elasticsearch API`));
      }
      if (esRequest.refresh !== 'wait_for' && esRequest.refresh !== 'false' && esRequest.refresh !== false) {
        return Bluebird.reject(new BadRequestError('Refresh parameter only supports the value "wait_for" or false'));
      }
    }
  ...
  createOrReplace(request) {
    ...
    if (esRequest.body._routing) {
      return Bluebird.reject(new BadRequestError('Kuzzle does not support "_routing" in createOrReplace action.'));
    }
  ...
  update(request) {
    ...
    if (esRequest.body._routing) {
      return Bluebird.reject(new BadRequestError('Kuzzle does not support "_routing" in update action.'));
    }
  ...
  replace(request) {
    ...
    if (esRequest.body._routing) {
      return Bluebird.reject(new BadRequestError('Kuzzle does not support "_routing" in replace action.'));
    }
