...
class ElasticSearch extends Service {
    ...
    { from, size, scroll } = {})
  {
    ...
    if (scroll) {
      const { values: ongoingScrolls } = await this._kuzzle.cacheEngine.internal.scan(
        0,
        { match: `${SCROLL_CACHE_PREFIX}*` });

      if (ongoingScrolls.length >= this._config.maxScroll) {
        throw errorsManager.get('too_many_scroll');
      }

      const scrollDuration = ms(scroll);

      if (scrollDuration > ms(this._config.maxScrollDuration)) {
        throw errorsManager.get('scroll_duration_too_great', scroll);
      }
    }
