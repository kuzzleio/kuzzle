...
class ElasticSearch extends Service {
  ...
  import(request) {
    ...
    return this.client.indices.getMapping().then(mappings => {
      // Keep a local cache of existents index/collections on elasticsearch
      const esCache = Object.keys(mappings).reduce((indexes, index) => {
        return Object.assign(indexes, { [index]: Object.keys(mappings[index].mappings) });
      }, {});
