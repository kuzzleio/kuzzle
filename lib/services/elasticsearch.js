...
class ElasticSearch extends Service {
    ...
    { refresh, timeout } = {})
  {
    ...
    let id;
    while ((id = ids.shift()) !== undefined) {
      if (typeof id === 'string') {
        validIds.push(id);
      }
      else {
        partialErrors.push({ id, reason: 'the document ID must be a string', status: 400 });
      }
    }
      ...
      .then(({ items }) => {
        ...
        for (let i = 0; i < validIds.length; i++) {
          ...
          if (item && item._id === validId) {
            ...
            partialErrors.push({ id: validId, reason: 'cannot find document', status: 404 });
          }
        }
