...
module.exports = function (kuzzle, options) {
  ...
  this.create = function (requestObject) {
    ...
    this.client.create(data)
      .then(function (result) {
        indexCache.add(kuzzle, data.index, data.type);
  ...
  this.deleteCollection = function (requestObject) {
    ...
    this.client.indices.deleteMapping(data)
      .then(function () {
        indexCache.remove(kuzzle, data.index, data.type);
            ...
            async.each(result.items, function(resultItem, resultCallback) {
              async.each(Object.keys(resultItem), function(action, callback) {
                ...
                if (action === 'index' && !item.error) {
                  indexCache.add(kuzzle, item._index, item._type);
                }
                callback();
              });
              resultCallback();
  ...
  this.createIndex = function (requestObject) {
    ...
    this.client.indices.create({index: data.index})
      .then(result => {
        indexCache.add(kuzzle, data.index);
        deferred.resolve(new ResponseObject(requestObject, result));
      })
      .catch(error => deferred.reject(error));
  ...
  this.deleteIndex = function (requestObject) {
    ...
    this.client.indices.delete({index: data.index})
      .then(result => {
        indexCache.remove(kuzzle, data.index);
        deferred.resolve(new ResponseObject(requestObject, result));
      })
      .catch(error => deferred.reject(error));
...
indexCache = {
  add: function(kuzzle, index, collection) {
    if (index !== undefined) {
      if (kuzzle.indexes[index] === undefined) {
        kuzzle.indexes[index] = [];
      }

      if (collection !== undefined) {
        if (kuzzle.indexes[index].indexOf(collection) === -1) {
          kuzzle.indexes[index].push(collection);
        }
      }
    }
  },
  remove: function(kuzzle, index, collection) {
    if (index !== undefined) {
      if (collection !== undefined) {
        kuzzle.indexes[index].splice(kuzzle.indexes[index].indexOf(collection));
      }
      else {
        delete kuzzle.indexes[index];
      }
    }
  },
  reset: function(kuzzle, index) {
    if (index !== undefined) {
      kuzzle.indexes[index] = [];
    }
    else {
