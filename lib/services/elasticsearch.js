...
const ESWrapper = require('../util/esWrapper');
...
class ElasticSearch extends Service {
    ...
    { scrollTTL=this._config.defaults.scrollTTL } = {})
  {
    ...
    if (scrollTTL) {
      const scrollDuration = ms(scrollTTL);

      if (scrollDuration > ms(this._config.maxScrollDuration)) {
        throw errorsManager.get('scroll_duration_too_great', scrollTTL, this._config.maxScrollDuration);
      }
    }
    ...
    { from, size, scroll } = {})
  {
    ...
    if (scroll) {
      const ongoingScrolls = await this._kuzzle.cacheEngine.internal.keys(
        `${SCROLL_CACHE_PREFIX}*`);

      if (ongoingScrolls.length >= this._config.maxScroll) {
        throw errorsManager.get('too_many_scroll', this._config.maxScroll);
      }

      const scrollDuration = ms(scroll);

      if (scrollDuration > ms(this._config.maxScrollDuration)) {
        throw errorsManager.get('scroll_duration_too_great', scroll, this._config.maxScrollDuration);
      }
    }
