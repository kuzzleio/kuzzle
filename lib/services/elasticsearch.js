...
class ElasticSearch extends Service {
  ...
  init() {
    ...
    if (process.env.NODE_ENV === 'production' && this.config.dynamic === 'true') {
      this.kuzzle.pluginsManager.trigger('log:warn',
        ...
        Elasticsearch will try to automatically infer mapping for new fields.'
      );
    }
  ...
  createCollection(request) {
    ...
    esRequest.body = {
      [esRequest.type]: {
        dynamic: requestBody.dynamic || this.config.dynamic,
  ...
  updateMapping(request) {
    ...
    if (requestBody.dynamic) {
      esRequest.body.dynamic = requestBody.dynamic;
    }

    esRequest.body.properties = Object.assign({}, requestBody.properties, defaultMapping);
  ...
  getMapping(request) {
    ...
    const includeMeta = request.input.args.includeMeta || false;

    return this.esWrapper.getMapping(esRequest, includeMeta);
  }
