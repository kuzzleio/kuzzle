        ...
        for(i = 0; i < esRequest.body.length; i++) {
          ...
          if (actionNames.indexOf(action) !== -1) {
            ...
            if (! (esCache[item[action]._index] && esCache[item[action]._index].includes(item[action]._type))) {
              return Bluebird.reject(new PreconditionError(`Index "${item[action]._index}" and/or collection "${item[action]._type}" don't exist`));
            }
    ...
    if (esRequest && esRequest.index && this.settings.autoRefresh[esRequest.index]) {
      return this.refreshIndex(new Request({index: esRequest.index}))
        ...
        .catch(error => {
          ...
          this.kuzzle.log.error(new KuzzleInternalError(`Error refreshing index ${esRequest.index}:\n${error.message}`));
    ...
    for (const property of properties) {
      if (check && mappingProperties.indexOf(property) === -1) {
        ...
        throw new BadRequestError(
          `Incorrect mapping mapping property "mapping.${currentPath}".` + didYouMean(property, mappingProperties));
      }
