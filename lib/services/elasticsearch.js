...
class ElasticSearch extends Service {
  ...
  async createIndex (index) {
    ...
    try {
      const res = await this._client.cat.indices({ format: 'json' });
      body = res.body;
    }
    catch (e) {
  ...
  _assertValidIndexAndCollection (index, collection = null) {
    if (!this.isIndexNameValid(index)) {
      errorsManager.throw('invalid_index_name', index);
    }

    if (collection !== null && !this.isCollectionNameValid(collection)) {
      errorsManager.throw('invalid_collection_name', collection);
    }
  }
...
function _isObjectNameValid (name) {
  ...
  for (let i = 0; valid && i < forbiddenChars.length; i++) {
    valid = !name.includes(forbiddenChars[i]);
  }
