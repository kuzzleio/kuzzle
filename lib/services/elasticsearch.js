...
class ElasticSearch extends Service {
  constructor(kuzzle, options, config) {
    ...
    this.dynamicMappingPolicies = ['true', 'false', 'strict'];
  }
  ...
  init() {
    ...
    this.config.dynamicMapping = this.config.dynamicMapping.toString();
    if (this.dynamicMappingPolicies.indexOf(this.config.dynamicMapping) === -1) {
      throw new KuzzleInternalError(`Wrong configuration value '${this.config.dynamicMapping}' for server.db.dynamicMapping. Allowed values are the following string '${this.dynamicMappingPolicies.join('\', \'')}'.`);
    }

    if (process.env.NODE_ENV === 'production' && this.config.dynamicMapping !== 'strict') {
      this.kuzzle.pluginsManager.trigger('log:warn', 'You are not using the \'strict\' policy for collection mappings.\nYou should strictly define your mapping before sending data to prevent Elasticsearch default mapping types.');
    }
  ...
  createCollection(request) {
    ...
    const dynamicMapping = this._getMappingPolicy(requestBody);
    const collectionMapping = {
      dynamic: dynamicMapping,
      properties: Object.assign({}, requestBody.properties, defaultMapping)
    };

    esRequest.body = {
      [esRequest.type]: collectionMapping
    };
  ...
  updateMapping(request) {
    ...
    if (requestBody.dynamic) {
      collectionMapping.dynamic = this._getMappingPolicy(requestBody);
    }

    esRequest.body = collectionMapping;
  ...
  _getMappingPolicy (requestBody) {
    if (!requestBody.dynamic) {
      return this.config.dynamicMapping;
    }

    if (this.dynamicMappingPolicies.indexOf(requestBody.dynamic) === -1) {
      throw new BadRequestError(`Wrong value '${requestBody.dynamic}' for mapping 'dynamic' property. Allowed values are the following string ${this.dynamicMappingPolicies.join('\', \'')}'.`);
    }

    return requestBody.dynamic;
  }
}
