...
module.exports = function (kuzzle, options) {
      ...
      .then(ids => {
        async.each(ids, (id, callback) => {
          ...
          this.client.bulk({body: bodyBulk})
            .then(result => deferred.resolve({ids: ids}))
  ...
  this.deleteIndex = function (requestObject) {
    ...
    delete this.autoRefresh[data.index];
  ...
  this.getAutoRefresh = function (requestObject) {
    return q(this.autoRefresh[requestObject.index] === true);
  };
  ...
  this.setAutoRefresh = function (requestObject) {
    ...
    this.autoRefresh[index] = (requestObject.data.body.autoRefresh === true);
    return this.getAutoRefresh(requestObject);
  };
...
function refreshIndexIfNeeded(data, response) {
  ...
  if (data && data.index && this.autoRefresh[data.index]) {
    deferred = q.defer();
    ...
    this.refreshIndex(new RequestObject({ index: data.index }))
      ...
      .catch(error => {
        ...
        _kuzzle.pluginsManager.trigger('log:error', new InternalError('Error refreshing index ' + data.index + ':\n' + error.message));
