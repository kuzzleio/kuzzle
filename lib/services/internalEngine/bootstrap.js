...
class InternalEngineBootstrap {
  ...
  * _allGen () {
  ...
  createCollections () {
    return this.createPluginsCollection()
      ...
      .then(() => this.createValidationCollection(
        this.kuzzle.config.validation)
      );
  }
    ...
    return this.kuzzle.indexCache.exists(this.db.index, 'roles', false)
      .then(exists => {
        ...
        return this.db.updateMapping('roles', mapping)
          .then(() => this.createDefaultRoles());
      });
  }

  createDefaultRoles () {
    const promises = ['anonymous', 'default', 'admin']
      .map(roleId => this.db.createOrReplace(
        'roles',
        roleId,
        this.kuzzle.config.security.default.role)
      );
    ...
    return this.kuzzle.indexCache.exists(this.db.index, 'profiles', false)
      .then(exists => {
        ...
        return this.db.updateMapping('profiles', mapping)
          .then(() => this.createDefaultProfiles());
      });
  }
  ...
  createValidationCollection (validationConfiguration) {
    return this.kuzzle.indexCache.exists(this.db.index, 'validations', false)
      .then(exists => {
        ...
        this.kuzzle.log.info('== Creating validation collection...');
      ...
      .then(() => {
        if (validationConfiguration) {
          ...
          Object.keys(validationConfiguration).forEach(indexName => {
            Object.keys(
              validationConfiguration[indexName]).forEach(collectionName => {
              // createOrReplace instead of create to avoid mess on concurency
              // TODO fix with a lock instead
              promises.push(
                this.kuzzle.internalEngine.createOrReplace(
                  'validations',
  ...
  * _lockGen () {
    ...
    try {
      ...
      // Lock just removed: re-attempt to acquire a lock later
      return true;
    }

    if (lock._source.timestamp < Date.now() - 30000) {
      try {
        yield this.db.createOrReplace(
          'config',
          lockId,
          {timestamp: Date.now()});
        return false;
  ...
  _waitTillUnlocked (attempts = 0) {
    return this.db.exists('config', lockId)
      .then(isLocked => {
        ...
        if (attempts > 10) {
          errorsManager.throw('bootstrap_timeout', 'Internal engine');
        }

        return Bluebird.delay(Math.round(
...
InternalEngineBootstrap.prototype.all =
  Bluebird.coroutine(InternalEngineBootstrap.prototype._allGen);

InternalEngineBootstrap.prototype.getJWTSecret =
  Bluebird.coroutine(InternalEngineBootstrap.prototype._getJWTSecretGen);

InternalEngineBootstrap.prototype.lock =
  Bluebird.coroutine(InternalEngineBootstrap.prototype._lockGen);
