...
class InternalEngineBootstrap {
  constructor (kuzzle) {
    ...
    this.attemptDelay = Math.round(this.config.bootstrapLockTimeout / 10);
  }
  ...
  async _bootstrap () {
    const internalMappings = {
      [this.engine.index]: this.kuzzle.config.services.db.internalMappings
    };
    ...
    await this.engine.create('config', DATAMODEL_VERSION_ID, '2.0.0');
