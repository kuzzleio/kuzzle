...
const errorsManager = require('../../config/error-codes/throw');
...
class PluginInternalEngineBootstrap {
  constructor(pluginName, kuzzle, engine) {
    ...
    this._LOCK_ID =
      `internalEngine.pluginBootstrap.${kuzzle.constructor.hash(this.pluginName)}.lock`;
  }
  ...
  _unlock () {
    return this.kuzzle.internalEngine.delete('config', this._LOCK_ID);
  }
