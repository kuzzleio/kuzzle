...
class PluginInternalEngineBootstrap {
  ...
  createCollections (collections) {
    return Bluebird.all(Object.keys(collections).map(
  ...
  * _lockGen () {
    try {
      yield this.kuzzle.internalEngine.create(
        'config',
        this._lockId,
        {timestamp: Date.now()}
      );
    ...
    if ( lock._source.timestamp < now - this.outdatedDelay
      || lock._source.timestamp > now
    ) {
      yield this.kuzzle.internalEngine.createOrReplace(
        'config',
        this._lockId,
        {timestamp: Date.now()}
      );
  ...
  _waitTillUnlocked (attempts = 0) {
    return this.kuzzle.internalEngine.exists('config', this._lockId)
      .then(isLocked => {
        ...
        if (attempts > 10) {
          errorsManager.throw('bootstrap_timeout', `Plugin ${this.pluginName}`);
        }
...
PluginInternalEngineBootstrap.prototype.lock =
  Bluebird.coroutine(PluginInternalEngineBootstrap.prototype._lockGen);
