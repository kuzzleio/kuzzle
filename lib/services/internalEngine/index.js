...
class InternalEngine {
  ...
  listIndexes () {
    return this.client.indices.getMapping()
      .then(response => extractIndexes(Object.keys(response), { internal: true }))
      .catch(error => Bluebird.reject(this.esWrapper.formatESError(error)));
  }
  ...
  listCollections (index) {
    return this.client.indices.getMapping()
      .then(response => extractCollections(
  ...
  async adminExists () {
    ...
    const result = await this.search('users', { query }, { from: 0, size: 0});
  ...
  createCollection (collection, mappings = {}) {
    ...
    esRequest.body.mappings = mappings;
