...
class InternalEngine {
  ...
  search (collection, body, { from, size, scroll }) {
    const
      request = {
        index: indexName(this.index, collection),
        from,
        size,
        scroll
      };
  ...
  createOrReplace (collection, id, body, options = {}) {
    ...
    // When we write in ES, the response from it doesn't contain the initial document body
    return this.client.index(request)
      .then(result => _.extend(result, { _source: body }))
      .catch(error => Bluebird.reject(this.esWrapper.formatESError(error)));
  }
  ...
  getMapping (index, collection, includeKuzzleMeta = false) {
    return this.esWrapper.getMapping(
      { index: indexName(index, collection) },
      includeKuzzleMeta);
  }
  ...
  createInternalIndex () {
    // TODO
    return this.client.indices.exists({ index: indexName(this.index)})
      .then(exists => {
        if (!exists) {
...
function indexName (index, collection, { internal=true } = {}) {
