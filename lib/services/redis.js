...
class Redis extends Service {
  ...
  _initSequence() {
    ...
    this.client.on('ready', () => {
      ...
      if (this.config.node) {
        this.kuzzle.log.info(`Redis service: Connected to ${this.config.node.host}:${this.config.node.port}`);
      }
      else if (this.config.nodes) {
        this.kuzzle.log.info('Redis service: Connected to Redis cluster');
      }
    });

    this.client.on('error', error => {
      if (this.connected) {
        this.kuzzle.log.error(`Redis service seem to be down, see original error for more info:\n${error.message}`);
      }
      this.connected = false;
    });
      ...
      this.client.once('ready', () => {
        if (this.config.database > 0) {
          return this.client.select(
            this.config.database, error => error ? reject(error) : resolve());
        }
        resolve();
      });
  ...
  _buildClient () {
    return new IORedis(this.config.node);
  }

  _buildClusterClient () {
    return new IORedis.Cluster(this.config.nodes, { enableReadyCheck: true });
  }
}
