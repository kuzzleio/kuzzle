...
const
  ...
  errorsManager = require('../config/error-codes/throw').wrap('external', 'redis'),
...
class Redis extends Service {
  ...
  init() {
    ...
    this._client.on('ready', () => {
      ...
      this._kuzzle.log.info('Redis service: Connected to ' + this.settings.service);
    });
    ...
    this._commands.forEach(command => {
      ...
      this[command] = (...args) => {
        if (!this._connected) {
          return Bluebird.reject(errorsManager.get('redis_service_not_connected'));
        }
      ...
      this._client.once('ready', () => {
        if (this._config.database > 0) {
          return this._client.select(
            this._config.database, error => error ? reject(error) : resolve(this));
        }
        resolve(this);
      });
