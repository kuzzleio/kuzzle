...
class Redis extends Service {
  ...
  init() {
    ...
    this.client.on('ready', () => {
      this.connected = true;
      this.kuzzle.pluginsManager.trigger('log:info', 'Redis service: Connected to ' + this.settings.service);
    });

    this.client.on('error', error => {
      if (this.connected) {
        this.kuzzle.pluginsManager.trigger('log:error', 'Redis service seem to be down, see original error for more info:\n' + error.message);
      }
      this.connected = false;
    });
    ...
    return new Bluebird((resolve, reject) => {
      this.client.once('ready', () => {
        if (this.config.database > 0) {
          return this.client.select(this.config.database, error => error ? reject(error) : resolve(this));
        }
        resolve(this);
      });
