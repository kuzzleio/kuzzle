...
class Redis extends Service {
  ...
  _initSequence() {
    ...
    this.client.on('ready', () => {
      this.connected = true;
      this.kuzzle.log.info(`Redis service: Connected to ${this.settings.service}`);
    });
    ...
    this.commands.forEach(command => {
      ...
      this[command] = (...args) => {
        if (!this.connected) {
          return Bluebird.reject(this.getError('redis_service_notconnected'));
        }
      ...
      this.client.once('ready', () => {
        if (this.config.database > 0) {
          return this.client.select(
            this.config.database, error => error ? reject(error) : resolve(this));
        }
        resolve();
      });
