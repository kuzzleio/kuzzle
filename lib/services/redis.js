...
module.exports = function (kuzzle, options) {
  ...
  this.init = function () {
    ...
    if (this.kuzzleConfig.cache.nodes) {
      this.client = new Redis.Cluster(this.kuzzleConfig.cache.nodes, {enableReadyCheck: true});
    }
    else {
      this.client = new Redis(this.kuzzleConfig.cache.node);
    }
