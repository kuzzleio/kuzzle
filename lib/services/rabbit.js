...
module.exports = function (kuzzle) {
    ...
    if (toggle) {
      ...
      amqpConnection
        .then(function (connection) {
          ...
          self.log.info('RabbitMQ Service started');
          deferred.resolve('RabbitMQ Service started');
        })
        .catch(function (error) {
          self.log.error('Unable to contact the RabbitMQ server: ', error.message);
          self.log.error('Check that it is running and the connection URL');
          deferred.reject(new Error('Unable to contact the RabbitMQ server: ' + error.message));
        });
    }
    else {
      self.close();
      self.log.info('RabbitMQ Service stopped');
  ...
  this.add = function (room, data) {
    if (!this.enabled || !room) {
      this.log.warn('AMQP service disabled => discarded message to room ', room);
      return false;
    }
    ...
    waitForMainChannel()
      ...
      .catch(function (error) {
        this.log.error(new Error(error));
      }.bind(this));
  };
    ...
    amqpConnection.then(function (conn) {
      return conn.createChannel().then(function (channel) {
        return channel.assertQueue(room, {durable: true})
          ...
          .then(function () {
            channel.consume(room, function (msg) {
              ...
              try {
                ...
                self.log.error('Parse error: ', e.message, '\nIncriminated message: ', msg);
                return false;
              }
          ...
          .catch(function (error) {
            self.log.error(new Error(error));
          });
      });
    });
  };
  ...
  this.onErrorRestart = function () {
    this.log.error('Catched an error from the AMQP broker service. Restarting connection...');
