...
module.exports = rabbit =  {
  ...
  listen: function (room, onListenCB) {
    if (!rabbit.listeners[room]) {
      rabbit.listeners[room] = { type: 'listen', callback: onListenCB, active: false };
    }

    if (!rabbit.enabled || rabbit.listeners[room].active) {
      return false;
    }

    rabbit.listeners[room].active = true;
  ...
  listenOnce: function (room, onListenCB) {
    if (!rabbit.listeners[room]) {
      rabbit.listeners[room] = { type: 'listenOnce', callback: onListenCB, active: false };
    }
  ...
  listenExchange: function (routingKey, onListenCB) {
    if (!rabbit.listeners[routingKey]) {
      rabbit.listeners[routingKey] = { type: 'listenExchange', callback: onListenCB, active: false };
    }

    if (!rabbit.enabled || rabbit.listeners[routingKey].active) {
      return false;
    }
