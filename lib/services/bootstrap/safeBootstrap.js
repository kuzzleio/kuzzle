...
class SafeBootstrap {
  ...
  async startOrWait (...args) {
    const bootstrapDone =
      await this.storage.exists(this.index, 'config', this._BOOTSTRAP_DONE_ID);
  ...
  async _waitTillUnlocked (attempts = 0) {
    const exists = await this.storage.exists(this.index, 'config', this._LOCK_ID);
  ...
  /**
   * Returns true if the bootstrap is already locked, otherwise lock
   * the bootstrap otherwise and return false
  ...
  async _isLocked () {
    ...
    try {
      lock = await this.storage.get(this.index, 'config', this._LOCK_ID);
    } catch (error) {
      if (error.status !== 404) {
        throw error;
      }
   ...
   */
  _unlock () {
    return this.storage.delete(this.index, 'config', this._LOCK_ID);
  }
}
