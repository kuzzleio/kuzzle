...
class WSBrokerClient {
   ...
   */
  constructor(brokerType, options, kuzzle, notifyOnListen) {
    this.kuzzle = kuzzle;
    ...
    if (options.host) {
      ...
      throw new KuzzleInternalError('No endpoint configuration given to connect. Please provide "host".');
    }
  ...
  _connect() {
    ...
    this.client.socket.on('open', () => {
      ...
      this.kuzzle.emit(`${this.eventName}:connected`, this.server.address);
      ...
      Object.keys(this.handlers).forEach(room => {
        ...
        this.kuzzle.emit(`${this.eventName}:reregistering`, room);
      ...
      this.kuzzle.log.warn(`[${this.eventName}] A node opened an already open connection`);
    });
    ...
    this.client.socket.on('close', code => {
      ...
      this.kuzzle.emit(`${this.eventName}:socketClosed`, code);
      this.kuzzle.log.warn(`[${this.eventName}] Socket closed with code ${code}`);
    ...
    this.client.socket.on('error', err => {
      ...
      this.kuzzle.log.warn(`[${this.eventName}] Error while trying to connect to ${this.server.address} [${err.message}]`);
      this.kuzzle.emit(
  ...
  retryConnection() {
    ...
    this.kuzzle.log.info(`[${this.eventName}] Reconnecting socket in ${this.client.retryInterval}ms`);
  }
  ...
  send(room, data) {
    debug(
      '[%s] broker client sending data to room "%s": %a',
      this.eventName,
      room,
      data);
...
function emit (broker, mode, room, data) {
  if (!broker.client.socket) {
    broker.kuzzle.log.error(`No socket for broker ${broker.eventName}`);
    return false;
  }

  debug(
    '[%s] broker client %s a message in room "%s": %a',
    broker.eventName,
    mode,
    room,
    data);
