...
class WSBrokerClient {
  ...
  listen(room, cb) {
    ...
    if (this.notifyOnListen !== false) {
      if (!this.client.socket || broker.client.socket.readyState !== WS.OPEN) {
   ...
   */
  broadcast(room, data) {
    debug('[%s] broker client broadcasting data to room %s: %O', this.eventName, room, data);
   ...
   */
  send(room, data) {
    debug('[%s] broker client sending data to room %s: %O', this.eventName, room, data);
