...
class WSBrokerClient {
  ...
  listen(room, cb) {
    ...
    if (this.notifyOnListen !== false) {
      this.client.connected.promise.then(() => {
        if (!this.client.socket) {
          this.pluginsManager.trigger('log:error', `[${this.eventName}] Can not notify listening to room ${room}: socket not available`);
          return false;
        }
  ...
  unsubscribe(room) {
    ...
    if (this.notifyOnListen !== false) {
      this.client.connected.promise.then(() => {
        if (!this.client.socket) {
          this.pluginsManager.trigger('log:error', `[${this.eventName}] Can not notify unsubscribing to room ${room}: socket not available`);
          return false;
        }
  ...
  close() {
    ...
    if (this.client.socket.readyState === WS.OPEN) {
  ...
  _connect() {
    ...
    this.client.socket.on('message', raw => {
      ...
      if (msg.room && this.handlers[msg.room]) {
        debug('[%s] broker client received a message in room %s:\n%O', this.eventName, msg.room, msg.data);

        this.handlers[msg.room].forEach(cb => cb(msg.data));
      }
      else {
        debug('[%s] broker client received a message with unknown room:\n%O', this.eventName, msg);
      }
    });
...
function pingRequest(broker) {
  ...
  broker._pingRequestTimeoutId = setTimeout(() => {
    if (broker.client.socket && broker.client.socket.readyState === WS.OPEN) {
      ...
      // Socket was not ready yet, retrying
