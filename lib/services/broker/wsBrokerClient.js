...
/**
 * @param {WSBrokerClient} client
 * @param {string} mode
 * @param {string} room
 * @param {object} data
 * @returns {*}
 */
function emit (client, mode, room, data) {
  if (!client.socket) {
    client.pluginsManager.trigger('log:error', `No socket for broker ${client.eventName}`);
    return false;
  }

  debug('[%s] broker client %s a message in room "%s":\n%O', client.eventName, mode, room, data);
 ...
 * @param {WSBrokerClient} client
 */
function resetPing(client) {
  debug('[%s] broker client reset ping requests status', client.eventName);

  if (client.client && client.client.socket) {
    client.client.socket.removeAllListeners('pong');
  }

  if (client._pingRequestIntervalId) {
    clearTimeout(client._pingRequestIntervalId);
    client._pingRequestIntervalId = null;
  }

  if (client._pingRequestTimeoutId) {
    clearTimeout(client._pingRequestTimeoutId);
    client._pingRequestTimeoutId = null;
  }
}
 ...
 * @param {WSBrokerClient} client
 */
function pingRequest(client) {
  debug('[%s] broker client sending ping request to server', client.eventName);

  client.client.socket.once('pong', () => handlePongResponse(client));
  client.client.socket.ping();

  client._pingRequestTimeoutId = setTimeout(() => {
    debug('[%s] ping timed out, disconnecting from server', client.eventName);

    if (client.client.socket) {
      client.client.socket.emit('error', new Error('Connection to server timed out'));
    }
 ...
 * @param {WSBrokerClient} client
 */
function handlePongResponse(client) {
  debug('[%s] broker client received pong from server', client.eventName);
  ...
  client._pingRequestIntervalId = setTimeout(() => {
    pingRequest(client);
  }, client._pingInterval);
}