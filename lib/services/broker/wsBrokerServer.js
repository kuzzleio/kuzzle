...
class WSBrokerServer {
  ...
  init() {
    if (this.isDisabled) {
      this.kuzzle.log.warn('Internal broker disabled by configuration');
      ...
      this.ws(() => {
        this.wss.on('connection', (clientSocket, req) => {
          ...
          clientSocket.on('close', (code, message) => {
            ...
            this.kuzzle.log.info(`client disconnected [${code}] ${message}`);
        ...
        this.wss.on('error', error => {
          ...
          this.kuzzle.log.error(`Connection error: [${error.code}] ${error.message}`);
