...
function WSBrokerServer (brokerType, options, pluginsManager) {
  ...
  this.ws = cb => {
    ...
    if (options.socket) {
      let
        socketPath = path.resolve(__dirname, '../../../', options.socket),
        socketDir = path.dirname(socketPath);

      if (!fs.existsSync(socketDir)) {
        throw new InternalError(`Invalid configuration provided for ${this.eventName}. Could not find ${socketDir} directory.`);
      }
