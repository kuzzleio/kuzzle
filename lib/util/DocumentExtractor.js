  ...
  },
  {
    ...
    methods: {
      ...
      extractFromResult: request => {
        ...
        for (let it = 0; it < request.result.deleted.length; it++) {
          documents.push({ _id: request.result.deleted[it] });
        }
      ...
      insertInResult: (documents, request) => {
        ...
        for (let it = 0; it < documents.length; it++) {
          result.deleted.push(documents[it]._id);
        }
    ...
    methods: {
      extractFromRequest: request => {
        ...
        for (let it = 0; it < request.input.body.documents.length; it++) {
          ...
          documents.push({ _id: document._id, _source: document.body._source });
        }
