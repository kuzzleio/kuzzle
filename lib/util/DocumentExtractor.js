    ...
    methods: {
      extractFromRequest: request => {
        ...
        if ( request.input.body
          ...
          if (Array.isArray(request.input.args.ids)) {
            documents.push({ _id: request.input.args.ids });
            return documents;
          }
          if (typeof request.input.args.ids === 'string') {
            const ids = request.input.args.ids.split(',');
      ...
      insertInRequest: (documents, request) => {
        if (request.input.body && Object.keys(request.input.body).length) {
          ...
          for (let it = 0; it < documents.length; it++) {
            request.input.args.ids.push(documents[it]._id);
          }
        }
        return request;
      },
