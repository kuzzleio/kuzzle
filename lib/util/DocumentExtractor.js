...
const extractors = ([
  ...
  },
  {
    ...
    methods: {
      extractFromRequest: request => request.input.body.ids.map(id => ({ _id: id })),
      extractFromResult: request => {
        if (request.input.action === 'mGet') {
          return request.result.hits;
        }
        return request.result.map(id => ({ _id: id }));
      },
      insertInRequest: (documents, request) => {
      ...
      insertInResult: (documents, request) => {
        ...
        request.setResult(documents.map(({ _id }) => _id));
        return request;
      }
    }
  },
  {
    ...
    methods: {
      extractFromRequest: request => request.input.body.documents.map(({ _id, body: _source }) => ({ _id, _source })),
