...
class ESWrapper {
  ...
  formatESError(error) {
    ...
    const messageReplaced = errorMessagesMapping.some(mapping => {
    ...
    switch (error.displayName) {
      case 'BadRequest':
        ...
        kuzzleError = new BadRequestError(error);
        break;
      case 'NotFound':
        if (!messageReplaced) {
          if (error.body && error.body.error) {
        ...
        kuzzleError = new NotFoundError(error);
        break;
      case 'Conflict':
        if (!messageReplaced) {
        ...
        kuzzleError = new ExternalServiceError(error);
        break;
      default:
        debug(
    ...
    return this.client.indices.getMapping(data)
      .then(result => {
        ...
        if (data && data.index && Object.keys(picked).length === 0) {
          throw new NotFoundError(`No mapping found for index "${data.index}"`);
        }
