...
function runShutdown (kuzzle, statusUpdate = console.log) {
  ...
  return new Bluebird(resolve => {
    ...
    setTimeout(() => waitRemainingRequests(kuzzle, resolve, statusUpdate), 1000);
  });
}

function waitRemainingRequests(kuzzle, resolve, statusUpdate) {
  const remaining = kuzzle.funnel.remainingRequests;

  if (remaining !== 0) {
    statusUpdate(`Waiting: ${remaining} remaining requests`);
    return setTimeout(() => waitRemainingRequests(kuzzle, resolve, statusUpdate), 1000);
  }
