...
const { http } = require('../config/default.config');
...
class Funnel {
  ...
  execute (request, callback) {
    ...
    if (request.input.headers
      ...
      && !this._isDomainAuthorized(request.input.headers.origin)
    ) {
  ...
  /**
   * Verifies if requests sent from a specific domain are allowed
   * @param {string} domain 
   * @returns 
   */
  _isDomainAuthorized(domain) {
    const httpConfig = global.kuzzle.config.http;
    ...
    return httpConfig.accessControlAllowOrigin.includes(domain);
  }
}
