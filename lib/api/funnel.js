...
class Funnel {
  ...
  async checkRights (request) {
    ...
    if (targets.length > 0
      ...
      throw kerror.get('api', 'assert', 'mutually_exclusive', 'index, collection', 'targets');
    }
    ...
    if (targets.length > 0) {
      // await _targetsWildCardExpansion(request);
    }
  ...
  /**
   * Loop through each targets of the request and expand their wildcards
   * 
   * @param {Request} request 
   */
   _targetsWildCardExpansion(request) {
    const targets = request.getTargets();
    ...
    for (const target of targets) {
      // Skip target with no index, as they are not valid
      if (!target.index) {
        continue;
      }
      // No wild card
      if (target.index.indexOf('*') === -1) {
        expandedTargets.push(target);
      }
    }
  }

  _expandWildCardIndex(target) {
