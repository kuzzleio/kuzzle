...
class Funnel {
  ...
  async checkRights (request) {
    ...
    // When the Support of Cookie as Jwt is enabled we check if a jwt cookie is present
    if (global.kuzzle.config.security.supportBrowserCookieAsJwt) {
      
      const isCookieOnly = request.input.args.cookieOnly;
      // When a request id made with CookieOnly set to true
      // We try to use the jwt cookie if present as jwt token
      // otherwise check for jwt as input
      if (isCookieOnly) {

        if (request.input.headers && request.input.headers.cookie) {
          const cookie = Cookie.parse(request.input.headers.cookie);

          request.input.jwt = cookie.jwt || request.input.jwt;
        }
      }
    }
