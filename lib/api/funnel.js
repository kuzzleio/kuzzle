...
class Funnel {
  ...
  async processRequest (request) {
    const controller = await this.getController(request);
  ...
  async executePluginRequest (request) {
    try {
      ...
      return await doAction(controller, request);
    }
    catch (e) {
   ...
   */
  async getController (request) {
    const pluginControllers = await this.kuzzle.ask('core:plugin:controllers:get');

    for (const controllers of [this.controllers, pluginControllers]) {
