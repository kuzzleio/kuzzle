            ...
            .then(modifiedRequest => {
              ...
              return this.checkRights(modifiedRequest)
                ...
                .then(processResult => {
                  ...
                  global.kuzzle.emit('request:afterExecution', { request: _request, success: true });
                  ...
                  global.kuzzle.emit('request:afterExecution', { request: modifiedRequest, success: false });
                  return this._executeError(err, modifiedRequest, true, callback);
                });
            })
            .catch(err => {
              debug('Error processing request %s: %a', req.id, err);
              global.kuzzle.emit('request:afterExecution', { request: req, success: false });
