...
class Funnel {
  ...
  async checkRights (request) {
    ...
    // When the Support of Cookie as an auth token is enabled we check if an auth token cookie is present
    // When a request is made with CookieOnly set to true
    // We try to use the cookie if present as an auth token
