...
class Funnel {
        ...
        global.kuzzle.asyncStore.run(() => {
          ...
          global.kuzzle.pipe('request:beforeExecution', req)
            .then(modifiedRequest => {
              if (!modifiedRequest.input.controller || !modifiedRequest.input.controller.length) {
              ...
              if (!modifiedRequest.input.action || !modifiedRequest.input.action.length) {
                return this._executeError(
                  kerror.get('api', 'assert', 'missing_argument', 'action'),
                  modifiedRequest,
                  true,
              ...
              return this._preprocessRequest(modifiedRequest, callback);
            })
            .catch(err => {
  ...
  /**
   * 1/ checking if the requesting user has the right credentials
   * 2/ send the request itself to the corresponding controller+action
   * 
   * @param {Request} request
   * @param {Function} callback
   * @returns 
   */
  async _preprocessRequest(request, callback) {
    global.kuzzle.asyncStore.set('REQUEST', request);

    let _request;

    return this.checkRights(request)
      .then(modifiedRequest => {
