...
const debugAsync = require('../util/debug')('kuzzle:funnel:async');
...
class Funnel {
  ...
  init () {
    ...
    this.kuzzle.contextLog = (message, ...args) => {
      const store = asyncLocalStorage.getStore();

      if (store && store.has('REQUEST')) {
        const request = store.get('REQUEST');
        debugAsync(`[%s] ${message}`, request.id, ...args);
      }
      else {
  ...
  execute (request, callback) {
    ...
    asyncLocalStorage.run(new Map(), () => {
      ...
      this.kuzzle.contextLog('%s', request.input.action)
