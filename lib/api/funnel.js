...
class Funnel {
  ...
  async checkRights (request) {
      ...
      // When a request is made with CookieOnly set to true
      ...
      if (global.kuzzle.config.security.supportCookieAuthentication
        ...
        && request.input.headers.cookie
          ...
          request.input.jwt = cookie['__Host-authToken'] || null;
          
          skipTokenVerification = request.input.controller === 'auth'
            && (request.input.action === 'login' || request.input.action === 'checkToken');
      }
