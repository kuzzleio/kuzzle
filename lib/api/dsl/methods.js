...
module.exports = function Methods(filtersObject) {
  ...
  this.range = function (roomId, index, collection, filter, not) {
    ...
    async.each(Object.keys(filter[field]), (rangeOperator, callback) => {
      ...
      hashedFunctionName = md5(`${not ? 'not' : ''}range${field}${rangeOperator}${value}`);

      result = this.filters.add(index, collection, field, rangeOperator, value, hashedFunctionName, roomId, not);
  ...
  this.exists = function (roomId, index, collection, filter, not) {
    ...
    result = this.filters.add(index, collection, fieldName, 'exists', fieldName, hashedFunctionName, roomId, not, inGlobals);
    if (util.isError(result)) {
  ...
  this.ids = function (roomId, index, collection, filter, not) {
    ...
    result = this.filters.add(index, collection, '_id', 'terms', filter.values, hashedFunctionName, roomId, not, false);
  ...
  this.geoDistance = function (roomId, index, collection, filter, not) {
    ...
    hashedFunctionName = md5(`${not ? 'not': ''}${fieldName}geoDistance${geohash.encode(lat, lon)}${distance}`);
  ...
  this.geoDistanceRange = function (roomId, index, collection, filter, not) {
    ...
    hashedFunctionName = md5(`${not ? 'not': ''}${fieldName}geoDistanceRange${geohash.encode(lat, lon)}${from}${to}`);
  ...
  this.geoPolygon = function (roomId, index, collection, filter, not) {
    ...
    hashedFunctionName = md5(`${not ? 'not': ''}${fieldName}geoPolygon${geoHashPolygon.join('')}`);
  ...
  this.regexp = function (roomId, index, collection, filter, not) {
    ...
    result = this.filters.add(index, collection, field, 'regexp', regexp.toString(), hashedFunctionName, roomId, not, false);
    if (util.isError(result)) {
      return q.reject(result);
    }
  ...
  this.missing = function (roomId, index, collection, filter, not) {
    ...
    result = this.filters.add(index, collection, fieldName, 'missing', fieldName, hashedFunctionName, roomId, not, not);
    if (util.isError(result)) {
      return q.reject(result);
    }
...
function termFunction(termType, roomId, index, collection, filter, not) {
  ...
  result = this.filters.add(index, collection, field, termType, value, hashedFunctionName, roomId, not);
  if (util.isError(result)) {
    return q.reject(result);
  }
