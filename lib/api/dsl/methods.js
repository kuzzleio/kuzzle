...
var geoUtil = {
  ...
  constructPolygon: function(geoFilter) {
    ...
    for (i in geoFilter.points) {
      if (geoFilter.points.hasOwnProperty(i)) {
        entry = geoFilter.points[i];

        if (_.isArray(entry)) {
          if (entry.length >= 2) {
            point = {lat: entry[0], lon: entry[1]};
          } 
          else {
            throw new BadRequestError('Unable to parse some point coordinates into polygon: ' + JSON.stringify(entry));
          }
        }
        else if (_.isObject(entry)) {
          if (entry.lat !== undefined && entry.lon !== undefined) {
            point = {lat: entry.lat, lon: entry.lon};
          } 
          else {
            throw new BadRequestError('Unable to parse some point coordinates into polygon: ' + JSON.stringify(entry));
          }
          ...
          if (/^[-.0-9]+,\s*[-.0-9]+$/.test(entry)) {
            tmp = entry.match(/^([-.0-9]+),\s*([-.0-9]+)$/);
            point = {lat: tmp[1], lon: tmp[2]};
          }
          else if (/^[0-9a-z]{4,}$/.test(entry)) {
            tmp = geohash.decode(geoFilter.topLeft);
            point = {lat: tmp.latitude, lon: tmp.longitude};
          }
          else {
            throw new BadRequestError('Unable to parse some point coordinates into polygon: ' + JSON.stringify(entry));
          }
        ...
        polygon.push(geoUtil.constructPoint(point));
      }
    }
  ...
  constructPoint: function(geoFilter) {
    ...
    // { latLon: [ -74.1, 40.73 ] }
      ...
      lon = geoFilter.latLon[0]; // lon is first to comply with GeoJSON !
      lat = geoFilter.latLon[1];
    }
    // { latLon: "40.73, -74.1" }
    else if (geoFilter.latLon &&
...
module.exports = methods = {
  ...
  geoDistance: function (roomId, collection, filter, not) {
    ...
    try {
      for (i in Object.keys(filter)) {
        if (Object.keys(filter).hasOwnProperty(i)) {
          field = Object.keys(filter)[i];
          if (field !== 'distance') {
            fieldName = field;
            break;
          }
