...
var geoUtil = {
  ...
  constructPoint: function(geoFilter) {
    ...
    // { latLon: { lat: 40.73, lon: -74.1 } }
      ...
      geoFilter.latLon.lat &&
      geoFilter.latLon.lon
    ) {
  ...
  getDistance: function(distance) {
    ...
    if (_.isString(distance)) {
      // just clean enough the distance so that localized notations (ie "3 258,55 Ft" instead of "3258.55 ft")
      // could be accepted
...
module.exports = methods = {
  ...
  geoDistance: function (roomId, collection, filter, not) {
    ...
    if (fieldName === 'distance') {
      fieldName = Object.keys(filter)[1];
    }
  ...
  _geoUtil: geoUtil // in order to allow tests
};
