...
module.exports = function Methods(filtersTree) {
  ...
  this.range = function (roomId, index, collection, filter, not) {
    ...
    async.each(Object.keys(filter[field]), (rangeOperator, callback) => {
      ...
      result = this.filtersTree.add(index, collection, field, rangeOperator, value, encodedFunctionName, roomId, not);
  ...
  this.exists = function (roomId, index, collection, filter, not) {
    ...
    result = this.filtersTree.add(index, collection, fieldName, 'exists', fieldName, encodedFunctionName, roomId, not, inGlobals);
    if (util.isError(result)) {
  ...
  this.ids = function (roomId, index, collection, filter, not) {
    ...
    result = this.filtersTree.add(index, collection, '_id', 'terms', filter.values, encodedFunctionName, roomId, not, false);
  ...
  this.missing = function (roomId, index, collection, filter, not) {
    ...
    result = this.filtersTree.add(index, collection, fieldName, 'missing', fieldName, encodedFunctionName, roomId, not, inGlobals);
    if (util.isError(result)) {
      return q.reject(result);
    }
...
function termFunction(termType, roomId, index, collection, filter, not) {
  ...
  result = this.filtersTree.add(index, collection, field, termType, value, encodedFunctionName, roomId, not);
  if (util.isError(result)) {
    return q.reject(result);
  }
