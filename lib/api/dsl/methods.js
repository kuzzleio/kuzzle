...
module.exports = methods = {
  ...
  regexp: function (roomId, index, collection, filter, not) {
    ...
    encodedFunctionName = `${not ? 'not' : ''}regexp${field.replace(/\./g, '')}${regexp.toString()}`;

    result = addToFiltersTree(index, collection, field, 'regexp', regexp.toString(), encodedFunctionName, roomId, not, false);
    if (util.isError(result)) {
      return q.reject(result);
    }
...
function addToFiltersTree(index, collection, field, operator, value, encodedFunctionName, roomId, not, inGlobals) {
  ...
  if (!methods.dsl.filtersTree[index][collection].fields[field][hashedFunctionName]) {
    ...
    diff = { ftF: { i: index, c: collection, o: operator, n: not, f: field, v: value }};
  }
