...
module.exports = function Dsl () {
  ...
  this.test = function (index, collection, data, documentId) {
    ...
    async.parallel({
      ...
      onGlobals: callback => {
        this.filters.testGlobalsFilters(index, collection, flattenBody, cachedResults).nodeif(callback);
      }
    }, (error, filters) => {
      if (error) {
  ...
  /**
   * Removes all references to a given filter
   *
  ...
  this.exists = function (index, collection) {
    return this.filters.filtersTree[index] && this.filters.filtersTree[collection];
  };
};
