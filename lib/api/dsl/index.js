...
module.exports = function Dsl (kuzzle) {
  ...
  this.testFilters = function (modelObject) {
    var
      ...
      flattenBody = flattenObject(modelObject.data.body._source),
      that = this;
    ...
    if (!this.filtersTree[modelObject.index] || !this.filtersTree[modelObject.index][modelObject.collection]) {
      deferred.resolve([]);
      return deferred.promise;
    }

    // add the _id in flattenBody allowing to filter on it
    flattenBody._id = modelObject.data.body._id;
