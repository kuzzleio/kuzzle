...
function convertGeopoint (obj) {
  ...
  // { latLon: { lat: 40.73, lon: -74.1 } }
    ...
    typeof camelCased.latLon === 'object' &&
    !Array.isArray(camelCased.latLon) &&
    fieldsExist(camelCased.latLon, ['lat', 'lon'], 'number')) {
    point = new Coordinate(camelCased.latLon.lat, camelCased.latLon.lon);
  }
  // { latLon: [ -74.1, 40.73 ] }
  else if (camelCased.latLon && Array.isArray(camelCased.latLon) && camelCased.latLon.length === 2) {
    point = new Coordinate(camelCased.latLon[0], camelCased.latLon[1]);
  }
  else if (camelCased.latLon && typeof camelCased.latLon === 'string') {
    // { latLon: "40.73, -74.1" }
    if (regexLatLon.test(camelCased.latLon)) {
      tmp = camelCased.latLon.match(regex);
