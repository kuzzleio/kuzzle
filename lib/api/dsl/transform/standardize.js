    ...
    return mustBeNonEmptyObject(filter.regexp, 'regexp')
      ...
      .then(regexpValues => {
        if (regexpValues.findIndex(v => ['flags', 'value'].indexOf(v) === -1) !== -1) {
          return Bluebird.reject(new BadRequestError('Keyword "regexp" can only contain the following attributes: flags, value'));
        }
      ...
      .then(() => {
        try {
          ...
          new RegExp(filter.regexp[regexpField].value, filter.regexp[regexpField].flags); //NOSONAR
        }
        catch (err) {
          return Bluebird.reject(new BadRequestError(`Invalid regular expression "${filter.regexp[regexpField].value}": ${err.message}`));
        }
...
function mustBeNonEmptyObject(filter, keyword) {
  const err = new BadRequestError(`"${keyword}" must be a non-empty object`);
