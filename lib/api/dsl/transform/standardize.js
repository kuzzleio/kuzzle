...
function Standardizer () {
  ...
  this.dsl.exists = function exists (filter, name) {
    ...
    return mustBeNonEmptyObject(filter[name], name)
      .then(field => onlyOneFieldAttribute(field, 'exists'))
  ...
  this.dsl.geoDistance = function geoDistance (filter) {
    ...
    return mustBeNonEmptyObject(filter.geoDistance, 'geoDistance')
      ...
      .then(distance => {
        standardized.geospatial.geoDistance[docField].distance = distance;
        return standardized;
      });
  };
  ...
  this.dsl.geoDistanceRange = function geoDistanceRange (filter) {
    ...
    return mustBeNonEmptyObject(filter.geoDistanceRange, 'geoDistanceRange')
      ...
      .then(distance => {
        standardized.geospatial.geoDistanceRange[docField].from = distance;
        return convertDistance(filter.geoDistanceRange.to, 'geoDistanceRange');
      })
      .then(distance => {
        standardized.geospatial.geoDistanceRange[docField].to = distance;
        return standardized;
      });
  };
    ...
    return mustBeNonEmptyObject(filter.geoPolygon, 'geoPolygon')
      ...
      .then(() => {
        ...
        for (let i = 0; i < filter.geoPolygon[docField].points.length; ++i) {
          ...
          if (point === null) {
            return Promise.reject(new BadRequestError(`geoPolygon.${docField}: unrecognized point format (${filter.geoPolygon[docField].points[i]}`));
          }
