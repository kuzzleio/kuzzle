    ...
    filters.forEach(sf => {
      ...
      if (sfResult.created) {
        ...
        addedConditions.forEach(cond => {
          ...
          this.storeOperand[cond.keyword](this.foPairs[index][collection][cond.keyword], this.testTables[index][collection], subfilter, cond);
        });
      }
    });
  ...
  if (!testTables[index][collection].subfilters) {
    testTables[index][collection] = {
      subfilters: {
        [id]: {
          cidx: 0,
          fidx: new Uint32Array(subfilter.filters.length)
        }
      },
      filters: {
    ...
    subfilter.filters.forEach((f, i) => {
      testTables[index][collection].filters[f.id] = testTables[index][collection].filterIds.length;
      testTables[index][collection].subfilters[id].fidx[i] = testTables[index][collection].filterIds.length;
      testTables[index][collection].filterIds.push(f.id);
    });
  }
  else if (!testTables[index][collection].subfilters[id]) {
    ...
    subfilter.filters.forEach((f, i) => {
      ...
      if (idx === undefined) {
        ...
        testTables[index][collection].subfilters[id].fidx[i] = idx;
      }
    });
  }
}
