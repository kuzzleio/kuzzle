...
function Storage () {
   ...
   * and is used to keep track of (in)validated conditions
   *
   * @type {Object}
   *
   * @example
   ...
   *      collection: {
   *        subfilters: {
   *          subfilterId: <conditions count array index>,
   *          subfilterId: <conditions count array index>,
   *          ...
   *        },
   *        conditionsCount: [<x>, <y>, <z>, ...]
   *      }
   *    }
   *  }
   */
  ...
  this.remove = function (filterId) {
    ...
    this.filters[filterId].subfilters.forEach(subfilter => {
      if (subfilter.filters.length === 1) {
        ...
        removeFromTestTables(this.testTables, index, collection, subfilter.id);
...
function addTestTables(testTables, subfilter, index, collection, id) {
  ...
  if (!testTables[index][collection].subfilters) {
    ...
    testTables[index][collection].subfilters[id] = testTables[index][collection].conditionsCount.length;
...
/**
 * Removes a subfilter from test tables and rebuilds them if necessary
 *
 * @param {Object} testTables - test tables object to mutate
 * @param {String} index
 * @param {String} collection
 * @param {String} id - subfilter id
 */
function removeFromTestTables(testTables, index, collection, id) {
  var deletedIndex;

  if (testTables[index][collection].conditionsCount.length === 1) {
    ...
    deletedIndex = testTables[index][collection].subfilters[id];
    delete testTables[index][collection].subfilters[id];
    testTables[index][collection].conditionsCount.splice(deletedIndex, 1);

    // rebuilding array indexes references
    Object.keys(testTables[index][collection].subfilters).forEach(sfid => {
      if (testTables[index][collection].subfilters[sfid] > deletedIndex) {
        testTables[index][collection].subfilters[sfid]--;
      }
    });
  }
}
