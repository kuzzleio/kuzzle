...
Storage.prototype.store = function store (index, collection, filters) {
  ...
  if (!result.created) {
    return {diff: false, id: result.id, filter: filters};
  }
...
function reindexTestTable(filters, subfilters, testTables, index, collection) {
  ...
  for(let i = 0; i < keys.length; i++) {
    let
      ...
      idx = tt.removedConditions.array.findIndex(idx => idx > sf.cidx);

    sf.cidx -= idx > -1 ? idx : tt.removedConditions.array.length;
  }
  ...
  for (let i = 0; i < keys.length; i++) {
    let idx = tt.removedFilters.array.findIndex(idx => idx > filters[keys[i]].fidx);

    filters[keys[i]].fidx -= idx > -1 ? idx : tt.removedFilters.array.length;
  }
