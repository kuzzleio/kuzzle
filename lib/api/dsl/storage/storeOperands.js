...
OperandsStorage.prototype.regexp = function regexp (foPairs, subfilter, condition) {
  var
    fieldName = Object.keys(condition.value)[0],
    value = new RegexpCondition(condition.value[fieldName].value, condition.value[fieldName].flags, subfilter),
    idx;
...
OperandsStorage.prototype.geospatial = function exists (foPairs, subfilter, condition) {
  ...
  switch (geotype) {
    case 'geoBoundingBox':
      foPairs.custom.index.addBoundingBox(condition.id,
        value.bottom,
        value.left,
        value.top,
        value.right
      ...
      break;
    case 'geoDistance':
      foPairs.custom.index.addCircle(condition.id, value.lat, value.lon, value.distance);
      break;
    case 'geoDistanceRange':
      foPairs.custom.index.addAnnulus(condition.id, value.lat, value.lon, value.to, value.from);
      break;
    case 'geoPolygon':
      foPairs.custom.index.addPolygon(condition.id, value);
      break;
  }
};
