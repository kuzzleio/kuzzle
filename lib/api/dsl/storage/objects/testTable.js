...
class TestTable {
  constructor(subfilter) {
    ...
    let i; // NOSONAR
    for (i = 0; i < subfilter.filters.length; i++) {
      subfilter.filters[i].fidx = i;
    }
  }
  ...
  addSubfilter(subfilter) {
    if (subfilter.cidx === -1) {
      ...
      let i; // NOSONAR
      for(i = 0; i < subfilter.filters.length; i++) {
        if (subfilter.filters[i].fidx === -1) {
          subfilter.filters[i].fidx = this.filtersCount;
          this.filtersCount++;
        }
      }
    }
