...
function Matcher (store) {
  ...
  this.match = function (index, collection, data) {
    ...
    if (testTables.matched === 0) {
      return [];
    }

    matchedFilters = new Array(testTables.matched);

    for(i = 0; i < testTables.filters.length && inserted < testTables.matched; i++) {
      if (testTables.filters[i] === 1) {
        matchedFilters[inserted++] = testTables.reference.filterIds[i];
      }
    }
