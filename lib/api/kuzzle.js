...
function registerSignalHandlers(kuzzle) {
  ...
  process.on('unhandledRejection', (reason, promise) => {
    ...
    // dump+exit on an unhandled rejection is a good idea during development
    // as it helps // spotting code errors (even if automated dumps
    // are disabled).
    // But in production, these problems // should only come from plugins,
    // and it seems a really bad idea to stop Kuzzle because a plugin created
    // an unchained, uncatchable promise that got rejected
    if (process.env.NODE_ENV === 'development') {
      kuzzle.janitor.dump('unhandled-rejection').finally(() => process.exit(1));
    }
  });
