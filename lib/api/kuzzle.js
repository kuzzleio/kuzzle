...
class Kuzzle extends EventEmitter {
  ...
  throw(...args) {
    ...
    if (!errorCodes.hasOwnProperty(domain)
      || !errorCodes[domain].subdomains.hasOwnProperty(subdomain)
      || !errorCodes[domain].subdomains[subdomain].errors.hasOwnProperty(errorName)
    ) {
      const message = format('Internal Error : Cannot find error in config file. %s', ...placeholders);
      throw new errors.InternalError(message, name, 0);
    }
    const error = errorCodes[domain].subdomains[subdomain].errors[errorName];    
    const code =
