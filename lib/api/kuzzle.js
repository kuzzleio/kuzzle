...
const
  ...
  InternalIndexBootstrap = require('./core/bootstrap/internalIndexBootstrap'),
  RestrictedStorage = require('./core/restrictedStorage'),
...
// @todo must be removed in Node.js 12
...
class Kuzzle extends EventEmitter {
  constructor() {
    ...
    this.internalIndex = new RestrictedStorage(
      this,
      this.config.services.db.internalIndex.name,
      this.services.internalStorage);

    // Attach bootstraper class
    this.internalIndex.indexBootstrap = new InternalIndexBootstrap(
      this,
      this.internalIndex);
  ...
  start (params = {}) {
    ...
    return this.services.init()
      .then(() => this.internalIndex.bootstrap())
      .then(() => this.log.info('External services initialized'))
      .then(() => {
...
function dumpAndExit(kuzzle, suffix) {
  ...
  kuzzle.janitor.dump(suffix).finally(() => runShutdown(kuzzle));
}
