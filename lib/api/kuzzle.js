...
class Kuzzle extends EventEmitter {
  ...
  throw(...args) {
    ...
    if (!errorCodes[domain] 
      || !errorCodes[domain].subdomains[subdomain] 
      || !errorCodes[domain].subdomains[subdomain].errors[errorName]
      ) {
      const message = format(...args.slice(3)) ? `An error occured : ${format(...args.slice(3))}` : 'An error occured';
      throw new errors.KuzzleError(message, 0);
    }
    
    const placeholders = args.slice(3) || '';
