...
const EventEmitter = require("eventemitter2").EventEmitter2,
  ...
  Errors = require("../util/errors-codes");
...
class Kuzzle extends EventEmitter {
  ...
  throw(domain = '00', subdomain = '0000', error = '0000', response = '') {
    const message = Errors[domain][subdomain][error].message;
    console.log(message);
    if (domain === '01') {
      throw new ExternalServiceError(`${message}:${response}`, `${subdomain}-${error}`);
    } else {
      throw new KuzzleError('Undocumented error');
    }
  }
}
