...
function registerErrorHandlers() {
  ...
  process.on('unhandledRejection', (err) => {
    request.data.body.prefix = 'unhandled-rejection';
  ...
  process.on('uncaughtException', (err) => {
    request.data.body.prefix = 'uncaught-exception';
    this.remoteActionsController.actions['dump'](request)
      .finally(() => {
        process.exit(1);
      })
  ...
  process.on('SIGTRAP', () => {
    request.data.body.prefix = 'signal-sigtrap';
    this.remoteActionsController.actions['dump'](request);
  });
  ...
  Object.keys(coreDumpSigals).forEach((signal) => {
    ...
    process.on(signal, () => {
      request.data.body.prefix = 'signal-'.concat(signal.toLowerCase());
      this.remoteActionsController.actions['dump'](request)
        .finally(() => {
