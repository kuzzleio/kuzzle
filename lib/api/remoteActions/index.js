...
module.exports = function RemoteActions (kuzzle) {
  ...
  this.do = function (actionIdentifier, data, params) {
    ...
    return this.kuzzle.pluginsManager.init(this.kuzzle.isServer, true)
      .then(() => {
        this.kuzzle.pluginsManager.run();

        /** @type {Service[]} */
        this.kuzzle.services.list = {
          broker: new InternalBroker(this.kuzzle, {isServer: false})
        };

        return this.kuzzle.services.list.broker.init();
      })
      .then(() => {
        request = new ResquestObject({
          controller: 'actions', 
      ...
      .catch(error => {
        error.message = `Failed to init Kuzzle plugin manager ${error.message}`;
        console.log(error);
...
function initActions () {
  this.actions.adminExists = new Action({isPidMandatory: true});
  ...
  this.actions.swagger = new Action({isPidMandatory: false});
}
