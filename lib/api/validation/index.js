    ...
    if (collectionSpec.validators && Array.isArray(collectionSpec.validators)) {
      this
        ...
        .then(validationFilter => {
          treatedSpecification.validators = validationFilter;
...
Validation.prototype.curateValidatorFilter = function (indexName, collectionName, validatorFilter, dryRun) {
  var
    ...
    filterId = sandboxDsl.createFilterId(indexName, collectionName, query);

  promise = sandboxDsl.register(filterId, indexName, collectionName, query);

  if (!dryRun) {
    promise.then(() => {
      return this.dsl.register(filterId, indexName, collectionName, query);
    });
  }
