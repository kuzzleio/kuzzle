  ...
  return new Promise((resolve, reject) => {
    ...
    if (requestObject.controller === 'write' && requestObject.action === 'update') {
      if (!requestObject.data.hasOwnProperty('_id')) {
        return reject(new BadRequestError('Update request must provide an _id.'));
      }

      engine = this.kuzzle.services.list.storageEngine;
      engine.get({
        index,
        collection,
        controller: 'read',
        ...
        // We don't want to modify the requestObject with default values, they must have been set previously
        requestUpdateClone = JSON.parse(JSON.stringify(requestObject));
        _.defaultsDeep(requestUpdateClone.data.body, document._source);
        return resolve(requestUpdateClone.data.body);
      });
    }
    else {
    ...
    if (collectionSpec.validators) {
      return this.dsl.test(index, collection, documentBody, requestObject.data._id)
        .then(filters => {
          var result = false;
          ...
          if (!result) {
            throw new BadRequestError('The document does not match validation filters');
          }
  ...
  Object.keys(collectionSpecSubset).forEach(fieldName => {
    ...
    if (documentSubset.hasOwnProperty(fieldName) && typeof documentSubset[fieldName] !== 'undefined') {
      ...
      if (this.types[field.type].allowChildren) {
        nestedStrictness = this.types[field.type].getStrictness(field.type_options, strictness);
      }
...
Validation.prototype.curateFieldSpecification = function (fieldSpec) {
  ...
  if (!fieldSpec.hasOwnProperty('type_options')) {
    fieldSpec.type_options = {};
  }

  if (!checkAllowedProperties(fieldSpec.type_options, this.types[fieldSpec.type].allowedTypeOptions || [])) {
    throw new InternalError(`Field of type ${fieldSpec.type} is not specified properly`);
  }

  try {
    returnedFieldSpec = this.types[fieldSpec.type].validateFieldSpecification(fieldSpec.type_options);
  }
  catch (error) {
    throw error;
  }

  if (typeof returnedFieldSpec === 'boolean') {
    if (returnedFieldSpec) {
      return fieldSpec;
    }
...
/**
 * @param {IpAddressType} validationType
