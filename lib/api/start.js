...
module.exports = function start (params, feature) {
  ...
  this.isDummy = feature.dummy || false;
  ...
  if (!this.isDummy) {
    if (this.isServer) {
      ...
      this.services.init({server: false, blacklist: ['notificationCache', 'mqBroker', 'statsCache']});
    }
  }
  else {
  ...
  this.pluginsManager.init(this.isServer, this.isDummy)
