...
module.exports = function start (params, feature) {
  ...
  if (!this.isWorker) {
    if (!feature.dummy) {
      ...
      this.services.init({server: false, blacklist: ['mqBroker', 'logger', 'writeEngine', 'readEngine', 'notificationCache', 'monitoring', 'remoteActions', 'statsCache']});
    }
    ...
    if (!feature.dummy) {
      ...
      this.pluginsManager.injectControllers();
