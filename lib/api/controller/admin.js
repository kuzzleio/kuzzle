...
class AdminController extends NativeController {
   ...
   */
  async resetSecurity () {
    const mutex = new Mutex(this._kuzzle, 'resetSecurity', { timeout: 0 });
   ...
   */
  async resetDatabase () {
    const mutex = new Mutex(this._kuzzle, 'resetDatabase', { timeout: 0 });
  ...
  loadFixtures (request) {
    ...
    return this._waitForAction(
      ...
      this.kuzzle.ask('core:storage:public:document:import', fixtures));
  }
  ...
  loadMappings (request) {
    ...
    return this._waitForAction(
      this.getRefresh(request, 'true'),
      this.kuzzle.ask('core:storage:public:mappings:import', mappings));
  }
