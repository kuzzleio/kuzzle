...
class ServerController extends NativeController {
  ...
  async healthCheck (request) {
    ...
    if (!services || services.includes('internalCache')) {
      try {
        await this.kuzzle.ask('core:cache:internal:info:get');
    ...
    if (!services || services.includes('memoryStorage')) {
      try {
        await this.kuzzle.ask('core:cache:public:info:get');
    ...
    if (!services || services.includes('storageEngine')) {
      try {
        const response = await this.kuzzle.ask('core:storage:public:info:get');
  ...
  async info () {
    ...
    const pluginControllers = await this.kuzzle.ask('core:plugin:controllers:get');
    const pluginRoutes = await this.kuzzle.ask('core:plugin:routes:get');
    const pluginsApi = this._buildApiDefinition(pluginControllers, pluginRoutes);

    // @todo kuzzle.api should contain the apiDefinition directly
    response.kuzzle.api.routes = { ...kuzzleApi, ...pluginsApi };
    ...
    response.services = {
      internalCache: await this.kuzzle.ask('core:cache:internal:info:get'),
      internalStorage: await this.kuzzle.ask('core:storage:private:info:get'),
      memoryStorage: await this.kuzzle.ask('core:cache:public:info:get'),
      publicStorage: await this.kuzzle.ask('core:storage:public:info:get'),
    };
  ...
  async openapi (request) {
    ...
    const specifications = format === 'yaml'
      ? jsonToYaml.stringify(generateSwagger(this.kuzzle))
