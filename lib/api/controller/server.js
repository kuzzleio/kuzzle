...
class ServerController extends NativeController {
  ...
  async healthCheck (request) {
    ...
    if (!services || services.includes('storageEngine')) {
      try {
        const response = await this.kuzzle.ask('core:store:public:info:get');
  ...
  async info () {
    ...
    response.services = {
      ...
      internalStorage: await this.kuzzle.ask('core:store:private:info:get'),
      memoryStorage: await this.kuzzle.ask('core:cache:public:info:get'),
      publicStorage: await this.kuzzle.ask('core:store:public:info:get'),
    };
    ...
    for (const [ name, controller ] of controllers.entries()) {
      ...
      for (const action of controller._actions) {
        ...
        for (const route of routes) {
          ...
          const url = route.fullUrl || route.url;
