...
class ServerController extends NativeController {
  ...
  async info () {
    ...
    response.services.internalCache = await this.kuzzle
      .ask('core:cache:internal:info:get');

    response.services.memoryStorage = await this.kuzzle
      .ask('core:cache:public:info:get');
  ...
  async swagger (request) {
    const format = 
      // The YAML response format is only valid with an HTTP request
      request.context.connection.protocol === 'http'&& request.input.args.format
