...
const LARGE_COMMANDS = [ 'mset', 'mget', 'msetnx' ];
...
class MemoryStorageController extends NativeController {
  constructor(kuzzle) {
    ...
    Object.keys(mapping).forEach(command => {
      ...
      this[command] = async request => {
        const args = extractArgumentsFromRequest(command, request);

        if (command === 'mexecute') {
          return this._kuzzle.ask('core:cache:public:mExecute', args);
        }

        if (LARGE_COMMANDS.includes(command)) {
          return this._kuzzle.ask('core:cache:public:execute', command, args);
        }
