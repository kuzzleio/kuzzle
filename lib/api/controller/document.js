...
class DocumentController extends NativeController {
  ...
  async createOrReplace (request) {
    ...
    const modifiedRequest = await this.kuzzle.validation.validate(request, false);

    // response: { _id, _version, _source, created }
    const response = await this.publicStorage.createOrReplace(
      index,
      collection,
      id,
      content,
    ...
    if (response.created) {
      ...
      await this.kuzzle.ask('core:realtime:notify:replaced', modifiedRequest);
    }
  ...
  async update (request) {
    ...
    const updatedDocument = await this.publicStorage.update(
      index,
      collection,
      id,
      content,
  ...
  async replace (request) {
    ...
    const response = await this.publicStorage.replace(
      index,
      collection,
      id,
      content,
  ...
  async mDelete (request) {
    ...
    await this.kuzzle.ask('core:realtime:notify:mDeleted', request, documents);
