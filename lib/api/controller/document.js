...
class DocumentController extends NativeController {
  ...
  async search (request) {
    ...
    if (lang === 'koncorde') {
      ...
      // await this.kuzzle.koncorde.validate(searchBody.query);
      if (typeof searchBody.query !== 'object') {
        throw kerror.get(
          'api',
          'assert',
          'invalid_type',
      ...
      try {
        console.log(JSON.stringify(searchBody.query, null, 2))

        searchBody.query = await this.ask(
