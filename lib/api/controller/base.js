...
class NativeController extends BaseController {
  ...
  getSearchBody (request) {
    ...
    let searchBody = {};
    if (request.input.args.verb && request.input.args.verb === '"GET"') {
      searchBodyKeys.forEach(key => {
        if (request.input.args[key]) {
          searchBody[key] = JSON.parse(request.input.args[key]);
        }
      });
    }
    else {
