...
class NativeController extends BaseController {
  constructor(actions = []) {
    ...
    this._kuzzle = Kuzzle.getInstance();
    this.ask = this._kuzzle.ask.bind(this._kuzzle);
    this.pipe = this._kuzzle.pipe.bind(this._kuzzle);
  ...
  async translateKoncorde (koncordeFilters) {
    ...
    await this.kuzzle.koncorde.validate(koncordeFilters);
