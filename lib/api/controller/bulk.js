...
class BulkController extends NativeController {
  ...
  async import (request) {
    ...
    const { items, errors } = await this.publicStorage.import(
      index,
      collection,
      bulkData,
      options);
  ...
  async write (request) {
    ...
    const result = await this.publicStorage.createOrReplace(
      index,
      collection,
      id,
      content,
    ...
    if (notify && result.created) {
      ...
      await this.kuzzle.ask('core:realtime:notify:replaced', request);
    }
  ...
  async mWrite (request) {
    ...
    const { items, errors } = await this.publicStorage.mCreateOrReplace(
      index,
      collection,
      documents,
      { injectKuzzleMeta: false, refresh });
  ...
  async deleteByQuery (request) {
    ...
    const { deleted } = await this.publicStorage.deleteByQuery(
      index,
      collection,
      query,
      { fetch: false, refresh });
