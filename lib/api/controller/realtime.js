...
class RealtimeController extends NativeController {
  ...
  async unsubscribe (request) {
    ...
    await this.kuzzle.ask('core:realtime:unsubscribe', request.context, roomId);
  ...
  async count (request) {
    ...
    return {
      count: await this.kuzzle.ask('core:realtime:getRoomSize', roomId),
    };
  }
   ...
   */
  list (request) {
    return this.kuzzle.ask('core:realtime:list', request.context);
  }
