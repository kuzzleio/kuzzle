...
class AuthController extends NativeController {
  ...
  async login (request) {
    ...
    // Only if the support of Browser Cookie as Jwt is enabled
    ...
    if (global.kuzzle.config.security.supportBrowserCookieAsJwt) {
      const isCookieOnly = request.input.args.cookieOnly;

      if (isCookieOnly) {
        // Here we are not sending jwt when cookieOnly is set to true
        // This allow us to detect if kuzzle does support cookie as jwt directly from the SDK
        // if the response contains the jwt this means that the configuration [security.supportBrowserCookieAsJwt] is set to false
        // or that the version of kuzzle doesn't support the feature Browser Cookie as Jwt
        const resultContent = { 
          _id: token.userId,
        ...
        request.setResult(resultContent,
          {
            headers: {
  ...
  async checkToken (request) {
    ...
    const isCookieOnly = request.input.args.cookieOnly;
    if (!global.kuzzle.config.security.supportBrowserCookieAsJwt || !isCookieOnly) {
      token = this.getBodyString(request, 'token');
    } else {
      token = request.context.token.jwt;
    }
