...
class AuthController extends NativeController {
  ...
  async _sendToken(token, request) {
    ...
    if (global.kuzzle.config.security.supportCookieAuthentication) {
      ...
      if (request.input.args.cookieOnly) {
        ...
        // if the response contains the jwt this means that the configuration [security.supportCookieAuthentication] is set to false
  ...
  async checkToken (request) {
    ...
    if (!global.kuzzle.config.security.supportCookieAuthentication || !irequest.input.args.cookieOnly) {
