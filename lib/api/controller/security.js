...
const { assertIdStartsNotUnderscore } = require('../../util/requestAssertions');
...
class SecurityController extends NativeController {
  ...
  async getRole(request) {
    ...
    const role = await this.ask('core:security:roles:get', id);
  ...
  async validateCredentials(request) {
    ...
    return validateMethod(
      ...
      request.input.resource._id, // _id can be null
      strategy,
      false);
  }
  ...
  async revokeTokens(request) {
    ...
    await this.ask('core:security:tokens:deleteByUser', id);
