...
class SecurityController extends NativeController {
  ...
  async createApiKey (request) {
    ...
    const apiKeyId = this.getId(request, {ifMissing: 'ignore'});
  ...
  createUser(request) {
    ...
    if (!Array.isArray(profileIds)) {
      throw kerror.get('api', 'assert', 'invalid_type', 'content.profileIds', 'array');
    }
  ...
  updateUser(request) {
    ...
    const updated = this.ask('core:security:user:update', id, content, {
      refresh: this.getRefresh(request),
  ...
  async replaceUser (request) {
    ...
    // used as an assertion check
    this.getBodyArray(request, 'profileIds');

    const user = await this.ask('core:security:user:replace', id, content, {
      refresh: this.getRefresh(request),
      userId,
    });
  ...
  async createFirstAdmin (request) {
    ...
    if (reset) {
      for (const type of ['roles', 'profiles']) {
        await Bluebird.map(
          Object.entries(this.kuzzle.config.security.standard[type]),
          ([name, content]) => this.ask(
  ...
  mDeleteProfiles(request) {
    return this._mDelete('profiles', request);
  }
  ...
  mDeleteRoles(request) {
    return this._mDelete('roles', request);
  }
  ...
  mDeleteUsers(request) {
    return this._mDelete('users', request);
  }
  ...
  async revokeTokens(request) {
    ...
    await this.ask('core:security:token:deleteByUser', id);
  ...
  async _persistUser(request, profileIds, content) {
    const id = this.getId(request, {ifMissing: 'generate'});
    ...
    try {
      this.ask('core:security:user:delete', id);
    }
    catch (e) {
    ...
    if (deletionError) {
      ...
      throw kerror.get(
        ...
        'runtime',
        'unexpected_error',
        [creationFailure.error.message, deletionError.message].join('\n'));
    }
