  ...
  async getUserStrategies(request) {
    ...
    if (this.anonymousId === id) {
      ...
      for (const strategy of availableStrategies) {
        const existStrategy = this.getStrategyMethod(strategy, 'exists');

        promises.push(
          existStrategy(request, id, strategy)
    ...
    return Bluebird.all(promises)
      .then(strategies => {
        const userStrategies = strategies.filter(item => item !== null);
