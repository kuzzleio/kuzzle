...
module.exports = function (kuzzle, fixtures, mappings) {
  ...
  if (kuzzle.isServer) {
    ...
    if (process.env.FIXTURES !== '') {
      this.files.fixtures = process.env.FIXTURES;
    }
    if (process.env.DEFAULT_MAPPING !== '') {
      this.files.mappings = process.env.DEFAULT_MAPPING;
    }

    if (fixtures) {
      this.files.fixtures = fixtures;
    }
    if (mappings) {
      this.files.mappings = mappings;
    }
    ...
    deferred.resolve(new ResponseObject(new RequestObject({controller: 'admin', action: 'prepareDb'})));
