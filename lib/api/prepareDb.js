...
module.exports = function (kuzzle, fixtures, mappings) {
  var
    deferred = q.defer();
  ...
  if (process.env.FIXTURES !== '') {
    files.fixtures = process.env.FIXTURES;
  }
  if (process.env.DEFAULT_MAPPING != '') {
    files.mappings = process.env.DEFAULT_MAPPING;
  }
  ...
  if (kuzzle.isServer) {
    ...
    createInternalStructure.call(this)
      ...
      .then(() => {
        ...
        return deferred.resolve(new ResponseObject(new RequestObject({controller: 'admin', action: 'prepareDb'})));
      })
      .catch(error => {
        kuzzle.pluginsManager.trigger('log:error', '!! An error occured during the process.\nHere is the original error object:\n', error);
        return deferred.resolve(new ResponseObject(new RequestObject({controller: 'admin', action: 'prepareDb'}), new InternalError(error)));
      });
  }
...
function readFile(which) {
  ...
  if (!files[which] || files[which] === '') {
    this.kuzzle.pluginsManager.trigger('log:info', '== No default ' + which + ' file specified in env vars: continue.');
    this.data[which] = {};
    return deferred.resolve(new ResponseObject(new RequestObject({controller: 'admin', action: 'prepareDb'})));
  }

  this.kuzzle.pluginsManager.trigger('log:info', '== Reading default ' + which + ' file ' + files[which] + '...');

  try {
    this.data[which] = JSON.parse(fs.readFileSync(files[which], 'utf8'));
    this.kuzzle.pluginsManager.trigger('log:info', '== Reading default ' + which + ' file ' + files[which] + ' done.');
    return deferred.resolve(new ResponseObject(new RequestObject({controller: 'admin', action: 'prepareDb'})));
  }
  catch (e) {
    this.kuzzle.pluginsManager.trigger('log:info',
      'An error occured when reading the ' + which + ' file located at' + files[which] + '! \n' +
      'Remember to put the file into the docker scope...\n' +
      'Here is the original error: ' + e
    );

    this.kuzzle.pluginsManager.trigger('preparedb:error', 'Error while loading the file ' + files[which]);
    return deferred.reject(new InternalError('Error while loading the file ' + files[which]));
  }
  return deferred.promise;
}
