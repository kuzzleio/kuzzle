...
module.exports = function () {
  ...
  if (!this.isServer) {
    ...
    this.services.list.readEngine.listIndexes(new RequestObject({}))
      .then((_actualIndexes) => {
        actualIndexes = _actualIndexes.data.indexes;
      })
      .then(() => {
...
function createIndexes () {
  ...
  async.map(
    ...
    (error) => {
      ...
      this.pluginsManager.trigger('log:info','== Index creation terminated');
...
function importMapping () {
  ...
  async.each(Object.keys(data.mappings), (index, callbackIndex) => {
    ...
    this.pluginsManager.trigger('prepare:MappingEnd', 'All mapping imports launched.');
