var
  ...
  rc = require('rc'),
  files = {
    fixtures: process.env.FIXTURES,
    mappings: process.env.DEFAULT_MAPPING
  };

module.exports = function prepareDB (kuzzle, fixtures, mappings) {
  var
    deferred = q.defer();
...
function readFile(which) {
  ...
  try {
    ...
    return q.reject(new InternalError('Error while loading the file ' + files[which]));
  }
}
