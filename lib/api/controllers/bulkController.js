    ...
    return this.engine.import(request)
      .then(response => {
        if (response.partialErrors && response.partialErrors.length > 0) {
          request.setError(this.getError('document_creations_failed', response.partialErrors));
        }
    ...
    return this.engine.mcreateOrReplace(request, false)
      .then(response => {
        if (response.error.length > 0) {
          request.setError(this.getError('document_creations_failed', response.error));
        }
