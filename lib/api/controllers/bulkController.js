...
class BulkController extends NativeController {
  ...
  async import(request) {
    ...
    global.kuzzle.log.info(
      `[STORAGE] imported ${bulkData.length} documents into "${index}#${collection}"`,
    );
  ...
  async write(request) {
    ...
    global.kuzzle.log.info(
      `[STORAGE] bulk write operation completed in "${index}#${collection}" (${result.successes.length} success, ${result.errors.length} errors)`,
    );
  ...
  async mWrite(request) {
    ...
    global.kuzzle.log.info(
      `[STORAGE] bulk mWrite operation completed in "${index}#${collection}" (${items.length} success, ${errors.length} errors)`,
    );
