...
module.exports = function ReadController (kuzzle) {
  ...
  this.listCollections = function (requestObject) {
    ...
    if (type === 'realtime') {
      return q(new ResponseObject(requestObject, {type, collections: realtimeCollections}));
    }

    return kuzzle.services.list.readEngine.listCollections(requestObject)
      .then(response => {
        response.data.type = type;
        response.data.collections = _.union(response.data.collections, realtimeCollections);
        return q(response);
      });
  };
