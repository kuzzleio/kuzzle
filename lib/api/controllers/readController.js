...
function ReadController (kuzzle) {
  ...
  this.collectionExists = requestObject => {
    var modifieRequestObject;

    return kuzzle.pluginsManager.trigger('data:beforeCollectionExists', requestObject)
      .then(newRequestObject => {
        modifieRequestObject = newRequestObject;
        return engine.collectionExists(modifieRequestObject);
      })
      .then(response => kuzzle.pluginsManager.trigger('data:afterCollectionExists', new ResponseObject(modifieRequestObject, response)));
  };

  this.indexExists = requestObject => {
