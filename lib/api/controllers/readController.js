...
module.exports = function ReadController (kuzzle) {
      ...
      .then(newRequestObject => {
        ...
        if (type === 'stored') {
          return kuzzle.services.list.readEngine.listCollections(modifiedRequestObject)
            .then(response => {
              response.type = type;
              return q(new ResponseObject(modifiedRequestObject, response));
            })
        ...
        if (type === 'realtime') {
          return q(new ResponseObject(modifiedRequestObject, {type, collections: {realtime: realtimeCollections}}));
        }
  ...
  this.now = function (requestObject) {
    return kuzzle.pluginsManager.trigger('data:beforeNow', requestObject)
      .then(newRequestObject => kuzzle.pluginsManager.trigger('data:afterNow', new ResponseObject(requestObject, {now: Date.now()})));
  };
