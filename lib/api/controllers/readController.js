...
function ReadController (kuzzle) {
  ...
  this.search = function readSearch (request) {
    if (!request.input.resource.index) {
      return Promise.reject(new BadRequestError('Search must specify an index to search on.'));
    }
    if (!request.input.resource.collection) {
      return Promise.reject(new BadRequestError('Search must specify a collection to search on.'));
    }
    ...
    if (request.input.resource.collection.split(',').length > 1) {
      return Promise.reject(new BadRequestError('Search on multiple collections is not available.'));
    }
  ...
  this.scroll = function readScroll (request) {
    if (!request.input.resource.index) {
      return Promise.reject(new BadRequestError('Scroll must specify an index to search on.'));
    }
    if (!request.input.resource.collection) {
      return Promise.reject(new BadRequestError('Scroll must specify a collection to search on.'));
    }
    if (request.input.resource.index.split(',').length > 1) {
      return Promise.reject(new BadRequestError('Scroll on multiple indexes is not available.'));
    }
    if (request.input.resource.collection.split(',').length > 1) {
      return Promise.reject(new BadRequestError('Scroll on multiple collections is not available.'));
    }
   ...
   * @returns {Promise<Object>}
   */
  this.listIndexes = function readListIndexes (request) {
    return engine.listIndexes(request);
  };
  ...
  this.listCollections = function readListCollections (request) {
    var
      type = request.input.body.type || 'all',
      realtimeCollections = [];

    if (!request.input.resource.index) {
      return Promise.reject(new BadRequestError('listCollections must specify an index.'));
    }
...
function paginateCollections (request, response) {
  if (request.input.args.from || request.input.args.size) {
    response.from = Number.parseInt(request.input.args.from) || 0;
