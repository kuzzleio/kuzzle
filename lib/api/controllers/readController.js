...
var
  ...
  os = require('os'),
  {assertBody, assertId, assertIndex, assertIndexAndCollection} = require('./util/requestAssertions'),
...
function ReadController (kuzzle) {
  ...
  this.scroll = function readScroll (request) {
    assertIndexAndCollection(request, 'scroll');

    if (request.input.resource.index.split(',').length > 1) {
      throw new BadRequestError('scroll on multiple indexes is not available.');
    }
    if (request.input.resource.collection.split(',').length > 1) {
      throw new BadRequestError('scroll on multiple collections is not available.');
    }
  ...
  this.listCollections = function readListCollections (request) {
    var
      type = request.input.body && request.input.body.type ? request.input.body.type : 'all',
      realtimeCollections = [];
