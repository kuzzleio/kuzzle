...
const
  ...
  assertHasScrollId = require('../../util/requestAssertions').assertHasScrollId,
...
class CollectionController {
  ...
  searchSpecifications(request) {
    ...
    if (request.input.args.size) {
      ...
      if (size > this.kuzzle.config.limits.documentsFetchCount) {
        throw new SizeLimitError(`${getRequestRoute(request)} Search page size exceeds server configured documents limit (${this.kuzzle.config.limits.documentsFetchCount})`);
      }
    }
  ...
  list(request) {
    ...
    if (['all', 'stored', 'realtime'].indexOf(type) === -1) {
      throw new BadRequestError(`${getRequestRoute(request)} must specify a valid type argument; Expected: "all", "stored" or "realtime"; Received: "${type}".`);
    }
