...
console.log('###########################')
    ...
    return this.kuzzle.indexCache.exists(index)
      .then(exists => {
        if (!exists) {
          this.throw('index_not_exists', index);
        }
      ...
      .then(exists => {
        if (!exists) {
          this.throw('collection_not_exists', collection);
        }
  ...
  searchSpecifications(request) {
    ...
    if (request.input.args.size) {
      ...
      if (size > this.kuzzle.config.limits.documentsFetchCount) {
        this.throw('search_page_size', this.kuzzle.config.limits.documentsFetchCount);
      }
    }
      ...
      .then(response => {
        if (response.valid) {
          ...
          specifications.forEach(specification => {
            ...
            this.kuzzle.emit('log:info', `Validation specification for ${specLogName} is about to be stored.`);
          });
  ...
  deleteSpecifications(request) {
    ...
    return this.internalEngine.delete('validations', `${request.input.resource.index}#${request.input.resource.collection}`)
      .then(() => {
        this.kuzzle.emit('log:info', `Validation specification for ${request.input.resource.index}#${request.input.resource.collection} has been deleted.`);
  ...
  list(request) {
    ...
    if (['all', 'stored', 'realtime'].indexOf(type) === -1) {
      this.throw('invalid_type_argument', type);
    }
