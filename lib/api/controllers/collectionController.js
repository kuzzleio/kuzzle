...
class CollectionController {
  ...
  updateSpecifications(request) {
    ...
    return createSpecificationList(request)
      .then(list => {
        specifications = list;
        return validateSpecificationList(this.kuzzle.validation.isValidSpecification, list);
      })
      .then(response => {
        if (response.valid) {
          const promises = [];
  ...
  validateSpecifications(request) {
    ...
    return createSpecificationList(request)
      .then(list => validateSpecificationList(this.kuzzle.validation.isValidSpecification, list));
  }
...
function validateSpecificationList (validator, list) {
  ...
  list.forEach(specification => {
    promises.push(validator(specification._source.index, specification._source.collection, specification._source.validation, true));
  });
