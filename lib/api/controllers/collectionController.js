...
class CollectionController extends BaseController {
  ...
  scrollSpecifications (request) {
    const
      ttl = this.getNumberParam(request, 'args.scroll', this.defaultScrollTTL),
      id = this.getStringParam(request, 'args.scrollId');

    return this.kuzzle.internalIndex.scroll('validations', id, ttl)
  ...
  _createSpecificationList (index, collection, specifications) {
    ...
    _.forEach(specifications, (collections, _index) => {
      _.forEach(collections, (validation, _collection) => {
        formatedSpecs.push(formatSpecification(_index, _collection, validation));
      });
    });

    return Bluebird.resolve(formatedSpecs);
  }
