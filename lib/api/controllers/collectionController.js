...
function CollectionController (kuzzle) {
  ...
  this.searchSpecifications = function collectionSearchSpecifications (request) {
    ...
    if ((size - from) > kuzzle.config.limits.documentsFetchCount) {
      throw new SizeLimitError(`collection:searchSpecifications cannot fetch more documents than the server configured limit (${kuzzle.config.limits.documentsFetchCount})`);
    }
