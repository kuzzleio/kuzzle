...
class CollectionController extends BaseController {
  ...
  getSpecifications(request) {
    ...
    return this.kuzzle.indexCache.exists(index)
      .then(exists => {
        if (!exists) {
          this.throw('index_does_not_exist', index);
        }
      ...
      .then(exists => {
        if (!exists) {
          this.throw('collection_does_not_exist', collection);
        }
  ...
  deleteSpecifications(request) {
    ...
    return this.internalEngine.delete('validations', `${request.input.resource.index}#${request.input.resource.collection}`)
      .then(() => {
        this.kuzzle.log.info(`Validation specification for ${request.input.resource.index}#${request.input.resource.collection} has been deleted.`);
