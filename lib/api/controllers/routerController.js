...
module.exports = function RouterController (kuzzle) {
   ...
   * Returns an object containing wrapper functions allowing the plugin to declare a disconnection, and to send
   * requests on behalf of a connectionId
   *
  ...
  this.newConnection = function (protocol, connectionId) {
    if (!connectionId || !protocol || typeof connectionId !== 'string' || typeof protocol !== 'string') {
      return q.reject(new PluginImplementationError('Rejected new connection declaration: invalid arguments'));
    }
console.log(kuzzle.pluginsManager.protocols);
    if (!kuzzle.pluginsManager.protocols[protocol]) {
      return q.reject(new PluginImplementationError('Attempted to declare a new connection on an unknown protocol plugin'));
    }
...
console.log('===== EXECUTING REQUEST: ', requestObject, '\nfor context: ', context);;
    if (!requestObject || !context || !context.connection) {
      return q.reject(new PluginImplementationError('Unable to execute request: ' + requestObject +
  ...
  this.removeConnection = function (context) {
    this.protocols[context.connection.type]--;
  ...
  /**
   * Broadcast a response object to all listening plugins
   *
   * @param channel
   * @param responseObject
   */
  this.broadcast = function (channel, responseObject) {
    Object.keys(this.protocols).forEach(protocol => {
      if (this.protocols[protocol] > 0) {
        kuzzle.pluginsManager.protocols[protocol].broadcast(channel, responseObject);
      }
    });
  };

  /**
   * Notifies protocol plugins that a user ID has joined a Kuzzle channel
   *
   * @param channel
   * @param connectionId
   */
  this.joinChannel = function (channel, connectionId) {
    Object.keys(this.protocols).forEach(protocol => {
      if (this.protocols[protocol] > 0) {
        kuzzle.pluginsManager.protocols[protocol].joinChannel(channel, connectionId);
      }
    });
  };

  /**
   * Notifies protocol plugins that a user ID has left a Kuzzle channel
   *
   * @param channel
   * @param connectionId
   */
  this.leaveChannel = function (channel, connectionId) {
    Object.keys(this.protocols).forEach(protocol => {
      if (this.protocols[protocol] > 0) {
        kuzzle.pluginsManager.protocols[protocol].leaveChannel(channel, connectionId);
      }
    });
  };
