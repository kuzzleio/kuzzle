...
function RouterController (kuzzle) {
   ...
   */
  this.newConnection = function routerNewConnection (context) {
    if (!context.connectionId || !context.protocol) {
      kuzzle.pluginsManager.trigger('log:error', new PluginImplementationError('Rejected new connection - invalid arguments:' + JSON.stringify(context)));
    }
    else {
      this.connections[context.connectionId] = context;

      kuzzle.statistics.newConnection(this.connections[context.connectionId]);
    }
  };
  ...
  this.removeConnection = function routerRemoveConnection (context) {
    ...
    else if (!this.connections[context.connectionId]) {
      kuzzle.pluginsManager.trigger('log:error', new PluginImplementationError('Unable to remove connection - unknown connectionId:' + JSON.stringify(context.connectionId)));
    }
    else {
      let requestContext = this.connections[context.connectionId];

      delete this.connections[context.connectionId];
