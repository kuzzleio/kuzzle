  ...
  this.initRouterHttp = function () {
    ...
    toSwagger = function () {
      ...
      _routes.forEach(route => {
        ...
        if (swagger.paths['/' + kuzzle.config.apiVersion + route.url] === undefined) {
          swagger.paths['/' + kuzzle.config.apiVersion + route.url] = {};
        }

        if (swagger.paths['/' + kuzzle.config.apiVersion + route.url][route.verb] === undefined) {
          swagger.paths['/' + kuzzle.config.apiVersion + route.url][route.verb] = {
        ...
        while ((m = reg.exec(route.url)) !== null) {
          ...
          swagger.paths['/' + kuzzle.config.apiVersion + route.url][route.verb].parameters.push({
            description: 'TODO',
            in: 'path',
            name: m[1],
            required: true,
    ...
    apiBase.get('/swagger.json', (request, response) => {
      var swagger = toSwagger();

      response.writeHead('Access-Control-Allow-Origin', '*');
      response.writeHead('Access-Control-Allow-Headers', 'X-Requested-With');
      response.writeHead(200, {'Content-Type': 'application/json'});
      response.end(stringify(swagger));
    });

    apiBase.get('/swagger.yaml', (request, response) => {
      var swagger = toSwagger();
