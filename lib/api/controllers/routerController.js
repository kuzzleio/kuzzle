...
module.exports = function RouterController (kuzzle) {
  ...
  this.routeWebsocket = function (socket) {
    var
      routerCtrl = this,
      WebSocketConnection = require('../core/models/webSocketConnection'),
    ...
    async.each(routerCtrl.controllers, function recordSocketListener (controller) {
      socket.on(controller, function (data) {
        ...
        if (data.headers && data.headers.authorization) {
          r = /^Bearer (.+)$/.exec(data.headers.authorization);
          if (r && r[1] && r[1].trim() !== '') {
            userToken = r[1].trim();
          }
        }
        ...
        kuzzle.repositories.user.loadFromToken(userToken)
          .then(function (user) {
            context.user = user;
            requestObject._context = context;

            kuzzle.funnel.execute(requestObject)
              ...
              .catch(function (error) {
                webSocketError.call(kuzzle, requestObject, ':websocket:funnel:reject', error, connection);
              });
          })
          .catch(function onExecuteError(error) {
            webSocketError.call(kuzzle, requestObject, ':websocket:authorization:reject', error, connection);
          });
      });
    });
...
function executeFromRest(params, request, response) {
  var
    HttpConnection = require('../core/models/httpConnection'),
    requestObject,
    data,
    context,
    connection;

  connection = new HttpConnection(request, response);
  ...
  this.repositories.user.loadFromToken(connection.getUserToken())
    .then(function (user) {
      context.user = user;
      requestObject._context = context;
      ...
      this.funnel.execute(requestObject)
        ...
        .catch(function (error) {
          httpError.call(this, requestObject, response, error, ':rest:funnel:reject');
        }.bind(this));
    }.bind(this))
    .catch(function (error) {
      httpError.call(this, requestObject, response, error, ':rest:connection:reject');
    }.bind(this));
}

/**
 * Displays an error using the REST protocol
 * @param error
 * @param msg
 */
function httpError (requestObject, response, error, msg) {
  this.emit(requestObject.controller + msg, error);
...
/**
 * Displays an error using the WebSocket protocol
 * @param msg
 * @param error
 */
function webSocketError (requestObject, msg, error, connection) {
  this.emit(requestObject.controller + msg, error);
  this.pluginsManager.trigger('log:error', error);
  this.notifier.notify(requestObject.requestId, {error: error.message, result: null}, connection);
}
