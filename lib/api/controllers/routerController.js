      ...
      socket.on(controller, function (data) {
        ...
        kuzzle.funnel.execute(requestObject, connection)
          ...
          .catch(function onExecuteError(error) {
            ...
            kuzzle.notifier.notify(requestObject.requestId, errorObject.toJson(), connection);
          });
      });
    });
...
function executeFromRest(params, request, response) {
  ...
  if (!params.controller) {
    errorObject = new ResponseObject({}, new Error('The "controller" argument is missing'));
