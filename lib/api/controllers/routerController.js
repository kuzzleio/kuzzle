...
const
  ...
  PluginImplementationError = 
    require('kuzzle-common-objects').errors.PluginImplementationError,
...
class RouterController {
    ...
    if (!requestContext.connection.id || !requestContext.connection.protocol) {
      this.kuzzle.log.error(
        new PluginImplementationError(
          'Rejected new connection - invalid arguments:'
      ...
      this.kuzzle.statistics.newConnection(
        this.connections[requestContext.connection.id]
      );
    }
  }
  ...
  removeConnection(requestContext) {
    if (!requestContext.connection.id || !requestContext.connection.protocol) {
      return this.kuzzle.log.error(
        new PluginImplementationError(
          'Unable to remove connection - invalid arguments:'
    ...
    if (!this.connections[requestContext.connection.id]) {
      return this.kuzzle.log.error(
        new PluginImplementationError(
          'Unable to remove connection - unknown connection identifier:'
