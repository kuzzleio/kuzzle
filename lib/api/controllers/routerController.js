      ...
      socket.on(controller, function (data) {
        ...
        if (['write', 'admin', 'bulk'].indexOf(data.controller) !== -1) {
          ...
          kuzzle.services.list.broker.listen('write_response_' + data.internalId, function (writeResponse) {
            kuzzle.notifier.notify(requestId, cleanProperties(writeResponse), connection);
          }, true);
...
function executeFromRest(params, request, response) {
  ...
  if (['write', 'admin', 'bulk'].indexOf(data.controller) !== -1) {
    ...
    this.kuzzle.services.list.broker.listen('write_response_' + data.internalId , function (writeResponse) {
      response.end(stringify(cleanProperties(writeResponse)));
    }, true);
  }
  ...
  this.kuzzle.funnel.execute(data)
    .then(function (result) {
      ...
      // if something is returned we notify the user
      if (!_.isEmpty(result)) {
        response.end(stringify({error: null, result: cleanProperties(result.data)}));
      }
...
function wrapObject (requestBody, data) {
  // if the action or controller are directly in request sent by client (for websocket)
  if (requestBody.action || requestBody.controller || requestBody.query) {
    data = _.extend(data, requestBody);
  }

  // In search case, the user can pass an attribute query with additionnal parameters
  // see https://www.elastic.co/guide/en/elasticsearch/reference/current/search-request-from-size.html
  // In this case we don't want wrap the request in a body attribute
  if (requestBody.query === undefined) {
    ...
    else if (!data._id && requestBody._id) {
      data._id = requestBody._id;
    }
...
function cleanProperties (writeResponse) {
  if (writeResponse.result) {
    delete writeResponse.result.internalId;
  }
