...
const persistUser = Bluebird.coroutine(function* persistUserGenerator (kuzzle, request, pojoUser) {
  ...
  if (loadedUser !== null) {
    errorsManager.throw('api', 'security', 'user_already_exists', pojoUser._id);
  }
  ...
  for (const strategy of strategies) {
    ...
    if (exists === true) {
      errorsManager.throw('api', 'security', 'creds_on_non_existing_user', pojoUser._id);
    }
...
class SecurityController extends BaseController {
  ...
  getRole(request) {
    ...
    return this.kuzzle.repositories.role.load(request.input.resource._id)
      .then(role => {
        if (!role) {
          return Bluebird.reject(this.getError('role_not_found', request.input.resource._id));
        }
  ...
  getProfile(request) {
    ...
    return this.kuzzle.repositories.profile.load(request.input.resource._id)
      .then(profile => {
        if (!profile) {
          return Bluebird.reject(this.getError('profile_not_found', request.input.resource._id));
        }
  ...
  getUser(request) {
    ...
    return this.kuzzle.repositories.user.load(request.input.resource._id)
      .then(user => {
        if (!user) {
          return Bluebird.reject(this.getError('user_not_found', request.input.resource._id));
        }
  ...
  replaceUser(request) {
    ...
    return this.kuzzle.repositories.user.load(request.input.resource._id)
      .then(loadedUser => (!loadedUser) ? Bluebird.reject(this.getError('user_not_found', request.input.resource._id)) : Bluebird.resolve())
...
function mDelete (kuzzle, type, request) {
  ...
  if (request.input.body.ids.length > kuzzle.config.limits.documentsWriteCount) {
    errorsManager.throw('api', 'security', 'delete_limit_reached', kuzzle.config.limits.documentsWriteCount);
  }
  ...
  return Bluebird.all(request.input.body.ids.map(id => {
    ...
    .then(delRequests => {
      ...
      if (errors.length) {
        request.setError(errorsManager.getError('api', 'security', 'mdelete', action, type, errors));
      }
...
function checkSearchPageLimit(request, limit) {
  if (request.input.args.size) {
    ...
    if (size > limit) {
      errorsManager.throw('api', 'security', 'search_page_size_limit_reached', limit);
    }
  }
}
...
function updateMetadata(securityDocument, request) {
  if (!securityDocument) {
    return Bluebird.reject(
      errorsManager.throw('api', 'security', 'cant_update_non_existing', request.input.action.replace('update', '').toLowerCase(), request.input.resource._id)
    );
  }
