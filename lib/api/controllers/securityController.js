...
class SecurityController extends BaseController {
  ...
  async createApiKey (request) {
    ...
    // @todo what if Kuzzle reboot since the token are keep only in redis cache ?
    const apiKey = await this.kuzzle.apiKeyManager.create(
  ...
  async deleteUser(request) {
    const
      userId = this.getId(request),
      options = { refresh: request.input.args.refresh };
    ...
    // @todo test this
    await ApiKey.deleteByQuery({ query: { term: { userId } } });
