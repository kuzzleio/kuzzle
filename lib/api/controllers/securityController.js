...
const
  ...
  apiErrorsManager = require('../../config/error-codes/throw').wrap('api', 'security'),
  {
    Request,
  ...
  function* persistUserGenerator (kuzzle, request, pojoUser) {
    ...
    for (const strategy of strategies) {
      if (registeredStrategies.indexOf(strategy) === -1) {
        apiErrorsManager.throw('unknown_strategy', strategy);
      }
      ...
      try {
        ...
        if (! (error instanceof NotFoundError)) {
          apiErrorsManager.throw('validate_method', error.message);
        }
      }
    }
...
class SecurityController extends BaseController {
   ...
   */
  getRoleMapping() {
    return this.kuzzle.internalIndex
      .getMapping(this.kuzzle.internalIndex.index, 'roles')
      .then(response => (
        {
          mapping:
            response[this.kuzzle.internalIndex.index].mappings.roles.properties
        }
      ));
  }
  ...
  updateRoleMapping(request) {
    ...
    return this.kuzzle.internalIndex.updateMapping(
      'roles',
      request.input.body);
  }
  ...
  getProfileMapping() {
    return this.kuzzle.internalIndex
      .getMapping(this.kuzzle.internalIndex.index, 'profiles')
      .then(response => (
        {
          mapping:
            response[this.kuzzle.internalIndex.index].mappings.profiles.properties
        }
      ));
  }
  ...
  getUserMapping() {
    return this.kuzzle.internalIndex
      .getMapping(this.kuzzle.internalIndex.index, 'users')
      .then(response => (
        {
          mapping:
            response[this.kuzzle.internalIndex.index].mappings.users.properties
        }
      ));
  }
  ...
  getRole(request) {
    ...
    return this.kuzzle.repositories.role.load(request.input.resource._id)
      .then(role => {
        if (!role) {
          apiErrorsManager.throw('role_not_found', request.input.resource._id);
        }
  ...
  getProfile(request) {
    ...
    return this.kuzzle.repositories.profile.load(request.input.resource._id)
      .then(profile => {
        if (!profile) {
          return Bluebird.reject(apiErrorsManager.getError(
            'profile_not_found',
  ...
  getProfileRights(request) {
    ...
    return this.kuzzle.repositories.profile.load(request.input.resource._id)
      .then(profile => {
        if (!profile) {
          apiErrorsManager.throw('profile_not_found', request.input.resource._id);
        }
  ...
  getUserRights(request) {
    ...
    return this.kuzzle.repositories.user.load(request.input.resource._id)
      .then(user => {
        if (!user) {
          apiErrorsManager.throw('user_not_found', request.input.resource._id);
        }
  ...
  searchUsers(request) {
    ...
    return this.kuzzle.repositories.user
      .search(request.input.body ? request.input.body : {}, request.input.args)
      .then(response => {
  ...
  createCredentials(request) {
    ...
    return this.kuzzle.repositories.user.load(id)
      .then(user => {
        if (user === null) {
          apiErrorsManager.throw('cant_create_creds', id);
        }
  ...
  updateCredentials(request) {
    ...
    return this.kuzzle.repositories.user.load(id)
      .then(user => {
        if (user === null) {
          apiErrorsManager.throw('cant_update_creds', id);
        }
  ...
  revokeTokens(request) {
    ...
    return this.kuzzle.repositories.user.load(userId)
      .then(user => {
        if (!user) {
          apiErrorsManager.throw('user_not_found', userId);
        }
...
function mDelete (kuzzle, type, request) {
  ...
  return Bluebird.all(request.input.body.ids.map(id => {
    ...
    .then(delRequests => {
      ...
      if (errors.length) {
        request.setError(apiErrorsManager.getError(
          'mdelete',
          action,
          type,
          errors));
