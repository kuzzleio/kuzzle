...
const { NativeSecurityController } = require('./baseController');
...
class SecurityController extends NativeSecurityController {
  ...
  async searchRoles (request) {
    const { from, size } = request.getSearchParams({ defaultSize: -1, withoutBody: true });
  ...
  async searchProfiles (request) {
    const { from, size, scrollTTL: scroll } = request.getSearchParams({ defaultSize: -1, withoutBody: true });
