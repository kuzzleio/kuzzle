...
const { promiser } = require('../../util/promiser');
...
class SecurityController extends NativeController {
  ...
  /**
   * Applies a partial update to multiple existing user.
   * If the user doesn't already exist, a new user is created.
   *
   * @param {Request} request
   * @returns {Promise}
   */
  async mUpsertUsers (request) {
    const users = request.getBodyArray('users');
    ...
    for (const user of users) {
      if (! user._id) {
        throw new BadRequestError('Missing property "id" in users object');
      }
      if (! user.content) {
        throw new BadRequestError('Missing property "content" in users object');
      }
      if (! user.credentials) {
        throw new BadRequestError('Missing property "credentials" in users object');
      }
    }

    for (const user of users) {
      try {
        successes.push(await this._changeUser(
          request,
          user._id,
          user.userId,
          user.content.profileIds,
        ));
      }
      catch (err) {
        if (err.id && err.id === 'security.user.not_found') {
          const [resPersistUser, errPersistUser] = await promiser(
            this._persistUser(
              request,
              user.content.profileIds,
              { ...defaultValues, ...user.content },
              {},
              user.credentials,
            ),
          );
          ...
          if (errPersistUser) {
            errors.push(errPersistUser.message ? errPersistUser.message : errPersistUser);
            continue;
          }

          successes.push(resPersistUser);
          continue;
        }
  ...
  async _persistUser (
    ...
    credentials = request.getBodyObject('credentials', {})
  ) {
