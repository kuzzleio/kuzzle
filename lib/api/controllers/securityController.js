  ...
  this.searchUsers = function (requestObject) {
    ...
    return kuzzle.repositories.user.search(
      ...
      .then(users => {
        ...
        return q.all(promises).then(users => {
          return q(new ResponseObject(requestObject, {hits: users, total: users.length}));
        });
      });
  };
