...
class SecurityController {
  ...
  deleteUser(request) {
    ...
    return this.kuzzle.repositories.user.delete(request.input.resource._id, {refresh: request.input.args.refresh})
      ...
      .then(Bluebird.coroutine(function* securityDeleteUserGenerator () {
        ...
        if (errors.length > 0) {
          throw new KuzzleInternalError(`${getRequestRoute(request)} was not able to remove bad credentials of user "${request.input.resource._id}": ` + errors.map(error => error.message).join(';'));
        }
      }))
  ...
  createCredentials(request) {
    ...
    return validateMethod(request, request.input.body, request.input.resource._id, request.input.args.strategy)
...
function* persistUserAndStrategies (kuzzle, request, pojoUser) {
  ...
  if (request.input.body.credentials && Object.keys(request.input.body.credentials).length > 0) {
    ...
    for (let i = 0; i < strategies.length; i++) {
      ...
      try {
        yield validateMethod(request, request.input.body.credentials[strategy], pojoUser._id, request.input.args.strategy);
      }
      catch (error) {
  ...
  throw new KuzzleInternalError(`${getRequestRoute(request)} An error occured during the creation of user "${request.input.resource._id}": ` + errors.map(error => error.message).join(';'));
}
