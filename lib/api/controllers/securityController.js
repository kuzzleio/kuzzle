...
module.exports = function SecurityController (kuzzle) {
  ...
  /**
   * Given a profile id, returns the matching user's policies as an array.
   * @param {RequestObject} requestObject
   * @returns {Promise}
   */
  this.getProfilePolicies = function (requestObject) {
    var modifiedRequestObject = null;

    return kuzzle.pluginsManager.trigger('security:beforeGetProfilePolicies', requestObject)
      .then(newRequestObject => {
        modifiedRequestObject = newRequestObject;
      ...
      .then(policies => {
        return q(Object.keys(policies).reduce((array, item) => array.concat(policies[item]), []));
      })
      .then(policies => {
        var response = {
          hits: policies,
        ...
        return kuzzle.pluginsManager.trigger('security:afterGetProfilePolicies', new ResponseObject(modifiedRequestObject, response));
      });
  };

  /**
   * Given a user id, returns the matching user's policies as an array.
   * @param {RequestObject} requestObject
   * @returns {Promise}
   */
  this.getUserPolicies = function (requestObject) {
    var modifiedRequestObject = null;

    return kuzzle.pluginsManager.trigger('security:beforeGetUserPolicies', requestObject)
      .then(newRequestObject => {
        modifiedRequestObject = newRequestObject;
      ...
      .then(policies => Object.keys(policies).reduce((array, item) => array.concat(policies[item]), []))
      .then(policies => {
        var response = {
          hits: policies,
          total: policies.length
        };
        return kuzzle.pluginsManager.trigger('security:afterGetUserPolicies', new ResponseObject(modifiedRequestObject, response));
      });
  };
