...
function SecurityController (kuzzle) {
  ...
  this.mGetRoles = function securityMGetRoles (request) {
    if (!request.input.body || !request.input.body.ids || !Array.isArray(request.input.body.ids)) {
  ...
  this.deleteRole = function securityDeleteRole (request) {
    var role = kuzzle.repositories.role.getRoleFromRequest(request);
  ...
  this.createOrReplaceProfile = function securityCreateOrReplaceProfile (request) {
    return createOrReplaceProfile.call(kuzzle, request, {method: 'createOrReplace'})
  ...
  this.createProfile = function securityCreateProfile (request) {
    return createOrReplaceProfile.call(kuzzle, request, {method: 'create'})
      .then(profile => Promise.resolve(formatProcessing.formatProfileForSerialization(profile)));
  };
  ...
  this.searchUsers = function securitySearchUsers (request) {
    ...
    return kuzzle.repositories.user.search(request.input.body.query || {}, request.input.args.from, request.input.args.size)
      .then(response => {
  ...
  this.createUser = function securityCreateUser (request) {
    ...
    if (!request.input.body || !request.input.body.profileIds) {
      return Promise.reject(new BadRequestError('Invalid user object. No profileIds property found.'));
    }

    pojoUser = request.input.body;
    pojoUser._id = (typeof request.input.resource._id !== 'undefined') ? request.input.resource._id : uuid.v4();
 ...
 */
function createOrReplaceProfile (request, opts) {
  if (!_.isArray(request.input.body.policies)) {
    return Promise.reject(new BadRequestError('Policies property must be an array.'));
  }

  return this.repositories.profile.buildProfileFromRequest(request)
    .then(profile => this.repositories.profile.hydrate(profile, request.input.body))
    .then(hydratedProfile => this.repositories.profile.validateAndSaveProfile(hydratedProfile, opts));
}
