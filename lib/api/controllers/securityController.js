...
module.exports = function SecurityController (kuzzle) {
  ...
  this.getProfile = function (requestObject) {
    ...
    return kuzzle.pluginsManager.trigger('security:beforeGetProfile', requestObject)
      ...
      .then(profile => {
        ...
        return formatProcessing.formatProfileForSerialization(kuzzle, profile, true);
      })
      .then(formatedProfile => kuzzle.pluginsManager.trigger('security:afterGetProfile', new ResponseObject(modifiedRequestObject, formatedProfile)));
  };<
  ...
  this.mGetProfiles = function (requestObject) {
    ...
    return kuzzle.pluginsManager.trigger('security:beforeMGetProfiles', requestObject)
      ...
      .then(profiles => {
        profiles = profiles.map(profile => formatProcessing.formatProfileForSerialization(kuzzle, foreachprofile, modifiedRequestObject.data.body.hydrate));
        profiles = profiles.map(profile => {
          formatProcessing.formatProfileForSerialization(kuzzle, foreachprofile, modifiedRequestObject.data.body.hydrate));
...
console.dir(updatedUser, {depth: 20});

        return formatProcessing.formatUserForSerialization(kuzzle, updatedUser, true);}
      )
      .then(serialized => {
console.log('************ serialized');
console.dir(serialized, {depth: 20});
        return kuzzle.pluginsManager.trigger('security:afterUpdateUser', new ResponseObject(modifiedRequestObject, serialized));
      });
  };
  ...
  this.updateProfile = function (requestObject, context) {
    ...
    return kuzzle.pluginsManager.trigger('security:beforeUpdateProfile', {requestObject, context})
      ...
      .then(updatedProfile => formatProcessing.formatProfileForSerialization(kuzzle, updatedProfile, true))
      .then(formatedProfile => kuzzle.pluginsManager.trigger('security:afterUpdateProfile', new ResponseObject(modifiedRequestObject, formatedProfile)));
  };
