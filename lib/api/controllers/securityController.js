...
function createUser (kuzzle, request, restricted) {
  ...
  if (restricted) {
    if (request.input.body.content.profileIds) {
      throw new BadRequestError('The request must not specify the body attribute "content.profileIds".');
    }
  }
  else {
    if (!request.input.body.content.profileIds) {
      throw new BadRequestError('The request must specify a body attribute "content.profileIds".');
    }
    if (!Array.isArray(request.input.body.content.profileIds)) {
      throw new BadRequestError('The request must specify the body attribute "content.profileIds" of type "array".');
    }
  }
  ...
  return Bluebird.coroutine(persistUserAndCredentials)(kuzzle, request, pojoUser);
}
...
function* persistUserAndCredentials (kuzzle, request, pojoUser) {
  ...
  if (loadedUser !== null && loadedUser._id !== kuzzle.repositories.user.anonymous()._id) {
    throw new BadRequestError(`user "${pojoUser._id}" already exist.`);
  }
