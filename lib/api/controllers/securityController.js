...
const persistUser = Bluebird.coroutine(function* persistUserGenerator (kuzzle, request, pojoUser) {
  ...
  if (loadedUser !== null) {
    this.throw('user_already_exists', pojoUser._id);
  }
  ...
  for (const strategy of strategies) {
    if (registeredStrategies.indexOf(strategy) === -1) {
      this.throw('unknown_strategy', strategy);
    }
    ...
    if (exists === true) {
      this.throw('creds_on_non_existing_user', pojoUser._id);
    }
...
class SecurityController extends BaseController {
    ...
    return this.kuzzle.funnel.controllers.server.adminExists()
      .then(adminExists => {
        if (adminExists.exists) {
          return Bluebird.reject(this.getError('admin_exists'));
        }
  ...
  updateCredentials(request) {
    ...
    return this.kuzzle.repositories.user.load(id)
      .then(user => {
        if (user === null) {
          this.throw('cant_update_cred', id);
        }
...
function mDelete (kuzzle, type, request) {
  ...
  if (request.input.body.ids.length > kuzzle.config.limits.documentsWriteCount) {
    this.throw('delete_limit_reached', kuzzle.config.limits.documentsWriteCount);
  }
  ...
  return Bluebird.all(request.input.body.ids.map(id => {
    ...
    .then(delRequests => {
      ...
      if (errors.length) {
        request.setError(this.getError('security_mdelete', action, type, errors));
      }
...
function checkSearchPageLimit(request, limit) {
  if (request.input.args.size) {
    ...
    if (size > limit) {
      this.throw('search_page_size_reached', limit);
    }
  }
}
...
function updateMetadata(securityDocument, request) {
  if (!securityDocument) {
    return Bluebird.reject(
      this.getError('cant_update_non_existing', request.input.action.replace('update', '').toLowerCase(), request.input.resource._id
      )
    );
  }
