...
const persistUser = Bluebird.coroutine(
  function* persistUserGenerator (kuzzle, request, pojoUser) {
    ...
    const createdUser = yield kuzzle.repositories.user.fromDTO(pojoUser)
      .then(modifiedUser => kuzzle.repositories.user
        .persist(modifiedUser, options))
      .then(modifiedUser => formatProcessing
        .formatUserForSerialization(kuzzle, modifiedUser));
...
class SecurityController extends BaseController {
  ...
  getRoleMapping() {
    return this.kuzzle.internalEngine
      .getMapping(this.kuzzle.internalEngine.index, 'roles')
      .then(response => (
        {
          mapping:
  ...
  getProfileMapping() {
    return this.kuzzle.internalEngine
      .getMapping(this.kuzzle.internalEngine.index, 'profiles')
      .then(response => (
        {
          mapping:
  ...
  getUserMapping() {
    return this.kuzzle.internalEngine
      .getMapping(this.kuzzle.internalEngine.index, 'users')
      .then(response => (
        {
          mapping:
