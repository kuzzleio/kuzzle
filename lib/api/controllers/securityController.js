...
function SecurityController (kuzzle) {
  ...
  this.getUser = function (requestObject) {
    ...
    return kuzzle.pluginsManager.trigger('security:beforeGetUser', requestObject)
      .then(newRequestObject => {
        modifiedRequestObject = newRequestObject;
        if (!modifiedRequestObject.data._id && !modifiedRequestObject.data.body._id) {
          return q.reject(new BadRequestError('No user id given'));
        }
        // TODO find where the _id is passed directly into data and not data.body
        // quick fix
        if (modifiedRequestObject.data._id && !modifiedRequestObject.data.body._id) {
          modifiedRequestObject.data.body._id = modifiedRequestObject.data._id;
        }

        return kuzzle.repositories.user.load(modifiedRequestObject.data.body._id);
      })
      .then(user => {
        if (!user) {
   ...
   * @returns {Promise}
   */
  this.getProfileRights = function (requestObject) {
    var modifiedRequestObject = null;
