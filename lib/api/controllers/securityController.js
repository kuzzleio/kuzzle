...
class SecurityController extends NativeController {
  ...
  async createRestrictedUser(request) {
    ...
    if (request.input.action && requestPayload.action === undefined) {
      requestPayload.action = 'signup';
    }

    if (request.input.controller && requestPayload.controller === undefined) {
      requestPayload.controller = 'auth';
    }
    ...
    return await global.kuzzle.funnel.executePluginRequest(apiRequest);
  }
