...
const persistUser = Bluebird.coroutine(function* persistUserGenerator (kuzzle, request, pojoUser) {
  ...
  if (loadedUser !== null) {
    kuzzle.throw('api', 'security', 'user_alreay_exists', pojoUser._id);
  }
  ...
  for (const strategy of strategies) {
    if (registeredStrategies.indexOf(strategy) === -1) {
      kuzzle.throw('api', 'security', 'unknown_strategy', strategy);
    }
    ...
    if (exists === true) {
      kuzzle.throw('api', 'security', 'creds_on_non_existing_user', pojoUser._id);
    }
...
class SecurityController extends BaseController {
   ...
   */
  searchRoles(request) {
    checkSearchPageLimit(request, this.kuzzle.config.limits.documentsFetchCount, this.kuzzle);
...
function mDelete (kuzzle, type, request) {
  ...
  if (request.input.body.ids.length > kuzzle.config.limits.documentsWriteCount) {
    kuzzle.throw('api', 'security', 'delete_limit_reached', kuzzle.config.limits.documentsWriteCount);
  }
 ...
 * @throws
 */
function checkSearchPageLimit(request, limit, kuzzle) {
  if (request.input.args.size) {
    const size = request.input.args.size - (request.input.args.from || 0);

    if (size > limit) {
      kuzzle.throw('api', 'security', 'search_page_size_limit_reached', limit);
    }
  }
}
