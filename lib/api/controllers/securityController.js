...
module.exports = function SecurityController (kuzzle) {
  ...
  this.getRole = function (requestObject) {
    ...
    return kuzzle.repositories.role.loadRole(
      ...
      .then(role => {
        return Promise.resolve(new ResponseObject(requestObject, {body: role || {}}));
      });
  };
  ...
  this.searchRoles = function (requestObject) {
    ...
    return kuzzle.repositories.role.searchRole(requestObject)
      .then(roles => {
        return Promise.resolve(new ResponseObject(requestObject, {body: roles || {}}));
      });
  };
  ...
  this.getProfile = function (requestObject) {
    ...
    return kuzzle.repositories.profile.loadProfile(requestObject.data._id)
      .then(profile => {
        return Promise.resolve(new ResponseObject(requestObject, {body: profile || {}}));
      });
  };
    ...
    return kuzzle.repositories.profile.searchProfiles(requestObject)
      .then(profiles => {
        serializedProfiles = profiles.map((p) => {
          return kuzzle.repositories.profile.serializeToDatabase(p);
        });
        return Promise.resolve(new ResponseObject(requestObject, {body: serializedProfiles || []}));
      });
  };
};
