  ...
  createFirstAdmin(request) {
    ...
    return this.kuzzle.funnel.controllers.get('server').adminExists()
      .then(adminExists => {
        ...
        request.input.args.reset = undefined;
...
function mDelete (kuzzle, type, request) {
  ...
  return Bluebird.all(promises)
    .then(delRequests => {
      ...
      if (ids.length > 1000) {
        kuzzle.log.info(`[SECURITY] User "${request.context.user}" deleted the following ${type}s ${ids.join(', ')}... (${ids.length - 1000} more users deleted)."`);
      }
      else {
