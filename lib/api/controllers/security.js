...
class SecurityController extends NativeController {
  ...
  async refresh (request) {
    ...
    await this._kuzzle.storageEngine.internal.refreshCollection(
      this.internalIndex,
      collection);
  ...
  createOrReplaceRole(request) {
    ...
    return createOrReplaceRole(
      ...
      .then(role => {
        this.kuzzle.log.info(`[SECURITY] User "${request.context.user._id}" applied action "${request.input.action}" on role "${role._id}."`);
        return formatProcessing.serializeRole(role);
      });
  }
  ...
  createOrReplaceProfile(request) {
    ...
    return createOrReplaceProfile(
      ...
      .then(profile => {
        this.kuzzle.log.info(`[SECURITY] User "${request.context.user._id}" applied action "${request.input.action}" on profile "${profile._id}."`);
        return formatProcessing.serializeProfile(profile);}
      );
  }
  ...
  createUser(request) {
    ...
    this.kuzzle.log.info(`[SECURITY] User "${request.context.user._id}" applied action "${request.input.action}" on user "${pojoUser._id}."`);
  ...
  updateUser(request) {
    ...
    return this.kuzzle.repositories.user.load(request.input.resource._id)
      ...
      .then(updatedUser => {
        this.kuzzle.log.info(`[SECURITY] User "${request.context.user._id}" applied action "${request.input.action}" on user "${updatedUser._id}."`);
  ...
  updateProfile(request) {
    ...
    return this.kuzzle.repositories.profile.load(request.input.resource._id)
      ...
      .then(profile => this.kuzzle.repositories.profile.validateAndSaveProfile(
        ...
        options))
      .then(updatedProfile => {
        this.kuzzle.log.info(`[SECURITY] User "${request.context.user._id}" applied action "${request.input.action}" on profile "${updatedProfile._id}."`);
  ...
  updateRole(request) {
    ...
    return this.kuzzle.repositories.role.load(request.input.resource._id)
      ...
      .then(role => this.kuzzle.repositories.role.validateAndSaveRole(
        ...
        options))
      .then(updatedRole => {
        this.kuzzle.log.info(`[SECURITY] User "${request.context.user._id}" applied action "${request.input.action}" on role "${updatedRole._id}."`);
  ...
  createFirstAdmin(request) {
    ...
    return this.kuzzle.funnel.controllers.get('server').adminExists()
      ...
      .then(response => {
        ...
        this.kuzzle.log.info(`[SECURITY] User "${request.context.user._id}" applied action "${request.input.action}".`);
        return response;
      });
  }
  ...
  createCredentials(request) {
    ...
    return this.kuzzle.repositories.user.load(id)
      ...
      .then(() => {
        ...
        this.kuzzle.log.info(`[SECURITY] User "${request.context.user._id}" applied action "${request.input.action}" on user "${id}."`);
  ...
  deleteCredentials(request) {
    ...
    return deleteMethod(request, request.input.resource._id, request.input.args.strategy)
      .then(() => {
        this.kuzzle.log.info(`[SECURITY] User "${request.context.user._id}" applied action "${request.input.action}" on user "${request.input.resource._id}."`);
...
function mDelete (kuzzle, type, request) {
  ...
  return Bluebird.all(promises)
    .then(delRequests => {
      ...
      if (ids.length > 1000) {
        kuzzle.log.info(`[SECURITY] User "${request.context.user._id}" deleted the following ${type}s ${ids.slice(0, 1000).join(', ')}... (${ids.length - 1000} more users deleted)."`);
      }
      else {
        kuzzle.log.info(`[SECURITY] User "${request.context.user._id}" deleted the following ${type}s ${ids.join(', ')}."`);
      }
      return ids;
    });
}
