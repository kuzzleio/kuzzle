...
class SecurityController extends NativeController {
  ...
  async searchRoles(request) {
    ...
    const controllers = request.input.body && request.input.body.controllers;
    const from = request.input.args
      && request.input.args.from
      && Number(request.input.args.from);
    const size = request.input.args
  ...
  async scrollUsers(request) {
    ...
    const response = await this.ask('core:security:users:scroll', id, scroll);
