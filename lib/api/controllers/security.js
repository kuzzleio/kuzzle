...
class SecurityController extends NativeController {
  ...
  mDeleteProfiles(request) {
    return this.mDelete(this.kuzzle, 'profile', request);
  }
  ...
  mDeleteRoles(request) {
    return this.mDelete(this.kuzzle, 'role', request);
  }
  ...
  mDeleteUsers(request) {
    return this.mDelete(this.kuzzle, 'user', request);
  }
 ...
 * @returns {Promise<*>}
 */
  async mDelete (kuzzle, type, request) {
    ...
    if (ids.length > 1000) {
      kuzzle.log.info(`[SECURITY] User "${this.getUserId(request)}" deleted the following ${type}s ${ids.slice(0, 1000).join(', ')}... (${ids.length - 1000} more users deleted)."`);
    }
    else {
      kuzzle.log.info(`[SECURITY] User "${this.getUserId(request)}" deleted the following ${type}s ${ids.join(', ')}."`);
    }
    return ids;
  }
