...
class UserController extends NativeController {
  ...
  async _persistUser (request, profileIds, content, { humanReadableId = true } = {}) {
    ...
    do {
      let generated = false;
      id = request.getId(ifMissingEnum.GENERATE, () => {
        generated = true;
  ...
  async mDelete (request) {
    const securityController = global.kuzzle.funnel.controllers.get('security');

    return securityController._mDelete('user', request);
  }
