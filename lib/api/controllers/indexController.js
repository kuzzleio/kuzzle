...
class IndexController extends NativeController {
  ...
  async mDelete(request) {
    ...
    for (const indexName of request.getBodyArray("indexes")) {
      await this.ask("core:storage:public:index:delete", indexName);
      global.kuzzle.log.info(`[STORAGE] deleted index "${indexName}"`);
    }
  ...
  async create(request) {
    ...
    global.kuzzle.log.info(`[STORAGE] created index "${index}"`);
  }
  ...
  async delete(request) {
    ...
    global.kuzzle.log.info(`[STORAGE] deleted index "${index}"`);
