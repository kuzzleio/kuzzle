...
class IndexController extends NativeController {
  ...
  async create(request) {
    ...
    const options =
      request.input.body && request.input.body.options
        ? request.input.body.options
        : {};
    await this.ask("core:storage:public:index:create", index, options);
  }

  async createvirtual(request) {
    const index = request.getIndex();
    const options =
      request.input.body && request.input.body.options
        ? request.input.body.options
        : {};
    await this.ask("core:storage:public:index:createvirtual", index, options);
  }
  ...
  async list(request) {
    ...
    const onlyVirtual = request.getBoolean("onlyVirtual");
    const onlyPhysical = request.getBoolean("onlyPhysical");

    const indexes = await this.ask(
      "core:storage:public:index:list",
      onlyVirtual,
      onlyPhysical
    );
