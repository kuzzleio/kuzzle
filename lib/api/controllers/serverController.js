...
class ServerController extends NativeController {
  ...
  async adminExists() {
    ...
    if (exists) {
      global.kuzzle.log.info("[SERVER] admin user exists");
    }
   ...
   */
  async now() {
    const now = await global.kuzzle.ask("core:api:time:get");

    global.kuzzle.log.info(`[SERVER] current server time: ${now}`);
  ...
  async healthCheck(request) {
    ...
    const status = await global.kuzzle.ask("core:api:healthCheck");

    global.kuzzle.log.info(`[SERVER] health check status: ${status.status}`);
