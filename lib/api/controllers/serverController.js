...
class ServerController extends NativeController {
  ...
  async capabilities () {
    ...
    const servicesVersion = {
      elasticsearch: async () => {
        const res = await global.kuzzle.ask('core:storage:public:info:get');
        return res.version;
      },
      redis: async () => {
        const res = await global.kuzzle.ask('core:cache:public:info:get');
        return res.version;
      }, 
    };
    const services = {};
    ...
    for (const [key, value] of Object.entries(config.services)) {
      if (value.backend !== undefined) {
        services[key] = {
          ...
          version: await servicesVersion[value.backend]()
        };
      }
    }
