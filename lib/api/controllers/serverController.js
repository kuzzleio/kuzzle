...
const
  ...
  { ServiceUnavailableError } = require('kuzzle-common-objects').errors,
  { format } = require('util'),
  errorCodes = require('../error-codes/api');
...
class ServerController extends BaseController {
   ...
   */
  adminExists() {
    return this.kuzzle.internalEngine.bootstrap
      .adminExists()
  ...
  throw(...args) {
    ...
    const error = errorCodes[this.domain].subdomains[this.subdomain].errors[errorName];
    const name = `${this.domain}-${this.subdomain}-${errorName}`;
    const code =
      (errorCodes[this.domain].code >> 24) |
      (errorCodes[this.domain].subdomains[this.subdomain].code >> 16) |
      errorCodes[this.domain].subdomains[this.subdomain].errors[errorName].code;
    
    const message = format(error.message, ...placeholders);

    throw new errors[error.class](message, name, code);
  }
    ...
    ['controllers', 'pluginsControllers'].forEach(controllers => {
      ...
      Object.keys(this.kuzzle.funnel[controllers]).forEach(controller => {
        ...
        if (controllers === 'controllers') {
          properties.push(
            ...Object.getOwnPropertyNames(
              Object.getPrototypeOf(this.kuzzle.funnel[controllers][controller])
            ).filter(p => p !== 'constructor')
          );
        }

        properties.push(
          ...Object.keys(this.kuzzle.funnel[controllers][controller]).filter(
            p => this.kuzzle.funnel[controllers][controller].hasOwnProperty(p)
          )
        );
        ...
        properties.forEach(action => {
          ...
          if (
            action[0] !== '_' &&
            typeof this.kuzzle.funnel[controllers][controller][action] ===
              'function'
          ) {
