...
class ServerController extends BaseController {
  ...
  healthCheck(request) {
    ...
    return this.kuzzle.services.internalCache.info()
      .then(() => {
      ...
      .then(() => this.kuzzle.services.publicCache.info())
      .then(() => {
      ...
      .then(() => this.kuzzle.services.publicStorage.info())
      .then(response => {
    ...
    return Object.keys(this.kuzzle.services.list)
      .reduce((previous, current) => {
        return previous.then(serviceInfo => {
          ...
          if (! this.kuzzle.services.list[current].info) {
            return Bluebird.resolve();
          }

          return this.kuzzle.services.list[current].info();
        });
      }, Bluebird.resolve())
      .then(serviceInfo => {
        if (serviceInfo) {
