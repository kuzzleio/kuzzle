...
class ServerController {
   ...
   */
  healthCheck() {
    return this.kuzzle.services.list.memoryStorage.getInfos()
      .then(() => this.kuzzle.services.list.internalCache.getInfos())
      .then(() => this.kuzzle.services.list.storageEngine.getInfos())
      .catch(err => {
        throw new ExternalServiceError(err);
      })
      .then(response => {
        if (['yellow', 'green'].indexOf(response.status) < 0) {
          throw new ExternalServiceError('es is down: ' + JSON.stringify(response));
        }
        return Bluebird.resolve({status: 'ok'});
      });
  }
