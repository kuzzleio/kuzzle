...
function WriteController (kuzzle) {
  ...
  this.create = function writeCreate (requestObject, context) {
    ...
    return requestObject.isValid()
      .then(() => {
        if (requestObject.data.body._routing) {
  ...
  this.createOrReplace = function writeCreateOrReplace (requestObject) {
    ...
    return requestObject.isValid()
      .then(() => {
        if (requestObject.data.body._routing) {
          throw new BadRequestError('Kuzzle does not support "_routing" in createOrReplace action.');
        }
      })
      .then(() => {
  ...
  this.update = function writeUpdate (requestObject, context) {
    ...
    return requestObject.isValid()
      .then(() => {
        if (requestObject.data.body._routing) {
          throw new BadRequestError('Kuzzle does not support "_routing" in update action.');
        }
      })
      .then(() => {
  ...
  this.replace = function writeReplace (requestObject) {
    ...
    return requestObject.isValid()
      ...
      .then(() => {
        if (requestObject.data.body._routing) {
          throw new BadRequestError('Kuzzle does not support "_routing" in replace action.');
        }
      })
      .then(newRequestObject => {
        modifiedRequestObject = newRequestObject;
