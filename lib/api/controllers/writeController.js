...
function WriteController (kuzzle) {
  ...
  this.validateDocument = function (requestObject) {
    ...
    return kuzzle.pluginsManager.trigger('data:beforeValidateDocument', requestObject)
      .then(newRequestObject => {
        modifiedRequestObject = newRequestObject;
        console.log(modifiedRequestObject)
        return kuzzle.validation.validationPromise(modifiedRequestObject.index, modifiedRequestObject.collection, 'write', 'validateDocument', null,  modifiedRequestObject.data.body, error);
      })
      .then(response => {
      ...
      .then(response => {
        return kuzzle.pluginsManager.trigger('data:afterValidateDocument', new ResponseObject(modifiedRequestObject, response));
      });
  };
}
