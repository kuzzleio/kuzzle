...
const User = require('../../core/security/document/user');
...
class AuthController extends NativeController {
   ...
   */
  async init () {
    const anonymous = await this.kuzzle.ask('core:security:users:anonymous');
  ...
  async logout (request) {
    ...
    const promise = this.getBoolean(request, 'global')
      ? this.kuzzle.ask('core:security:tokens:delete', request.context.user._id)
      : this.kuzzle.ask('core:security:tokens:expire', request.context.token);
  ...
  async checkToken (request) {
    ...
    try {
      const {expiresAt} = await this.ask('core:security:tokens:verify', token);
  ...
  async updateSelf (request) {
    ...
    const user = await this.ask('core:security:users:update', userId, body, {
