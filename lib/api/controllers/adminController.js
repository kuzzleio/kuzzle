    ...
    return kuzzle.pluginsManager.trigger('data:beforeGetConfig', requestObject)
      .then(newRequestObject => {
        ...
        _.forEach(kuzzle.pluginsManager.plugins, (object, id) => {
          pluginsConfig[id] = JSON.parse(stringifySafe(object));
          ...
          if (pluginsConfig[id].object.kuzzle) {
            // remove the kuzzle configuration which can be included into plugin object
            delete pluginsConfig[id].object.kuzzle;
          }
          if (id === 'kuzzle-plugin-cluster') {
            // remove the kuzzle configuration which can be included into plugin object node
            delete pluginsConfig[id].object.node.kuzzle;
            delete pluginsConfig[id].object.node.context.accessors.kuzzle;
            delete pluginsConfig[id].object.node.context.config;

            // remove the dynamic internal objects which are not part of the configuration
            delete pluginsConfig[id].object.lbBroker.client.connected;
            delete pluginsConfig[id].object.lbBroker.client.socket;
            delete pluginsConfig[id].object.node.lbBroker.client.connected;
            delete pluginsConfig[id].object.node.lbBroker.client.socket;
            delete pluginsConfig[id].object.node.broker.rooms;
            delete pluginsConfig[id].object.node.broker.server;

            // strip the redundant pluginsManager objects
            pluginsConfig[id].object.lbBroker.pluginsManager = '[Stripped. Cause: redundant]';
            pluginsConfig[id].object.node.lbBroker.pluginsManager = '[Stripped. Cause: redundant]';
            pluginsConfig[id].object.node.broker.pluginsManager = '[Stripped. Cause: redundant]';
          }
        });
        ...
        return Promise.resolve(JSON.parse(stringifySafe(response)));
      })
