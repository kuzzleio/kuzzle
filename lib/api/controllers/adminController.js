    ...
    return kuzzle.pluginsManager.trigger('data:beforeUpdateSpecifications', requestObject)
      ...
      .then(response => {
        ...
        return Promise.all(promises)
          .then(() => {
            return modifiedRequestObject.data.body;
          });
      })
      .then(response => {
        return kuzzle.internalEngine.refreshIndex()
          .then(() =>  kuzzle.validation.curateSpecification())
          .then(() => kuzzle.pluginsManager.trigger('data:afterUpdateSpecifications', new ResponseObject(modifiedRequestObject, response)));
      });
  };
