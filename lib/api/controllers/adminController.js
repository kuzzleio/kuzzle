...
function AdminController(kuzzle) {
   ...
   * @returns {Promise}
   */
  this.searchSpecifications = function adminSearchSpecifications (requestObject) {
    var
      query = {},
      from = 0,
      size = 20,
    ...
    return kuzzle.pluginsManager.trigger('data:beforeSearchSpecifications', requestObject)
      .then(newRequestObject => {
        modifiedRequestObject = newRequestObject;
        return internalEngine.search('validations', query, from, size);
      })
      .then(response => kuzzle.pluginsManager.trigger('data:afterSearchSpecifications', new ResponseObject(modifiedRequestObject, response)));
  };
   ...
   * @returns {Promise}
   */
  this.refreshInternalIndex = function adminRefreshInternalIndex (requestObject) {
    var modifiedRequestObject = null;

    return kuzzle.pluginsManager.trigger('data:beforeRefreshInternalIndex', requestObject)
      .then(newRequestObject => {
        modifiedRequestObject = newRequestObject;
        return kuzzle.internalEngine.refresh();
      })
      .then(() => kuzzle.pluginsManager.trigger('data:afterRefreshInternalIndex', new ResponseObject(modifiedRequestObject, {acknowledged: true})));
  };
