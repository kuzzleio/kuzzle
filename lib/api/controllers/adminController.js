...
function AdminController(kuzzle) {
   ...
   * @returns {Promise}
   */
  this.getSpecifications = requestObject => {
    var modifiedRequestObject = null;
  ...
  this.updateSpecifications = requestObject => {
    ...
    return kuzzle.pluginsManager.trigger('data:beforeUpdateSpecifications', requestObject)
      ...
      .then(response => {
        var promises = [];
        if (response.error) {
          kuzzle.pluginsManager.trigger('log:error', `Validation specification has not been updated because of an error: ${requestObject.data.body}.`);
        ...
        return Promise.all(promises)
          ...
          .then(() => modifiedRequestObject.data.body);
      })
      .then(response => kuzzle.pluginsManager.trigger('data:afterUpdateSpecifications', new ResponseObject(modifiedRequestObject, response)));
  };
   ...
   * @returns {Promise}
   */
  this.deleteSpecifications = requestObject => {
    var modifiedRequestObject = null;
    ...
    return kuzzle.pluginsManager.trigger('data:beforeDeleteSpecifications', requestObject)
      .then(newRequestObject => {
        ...
        return internalEngine.delete('validations', `${modifiedRequestObject.index}#${modifiedRequestObject.collection}`)
          .catch(() => {
            // no need to respond a 404 error if there was no specification set.
          });
      })
      .then(() => {
