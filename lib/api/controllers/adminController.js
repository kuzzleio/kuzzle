...
function AdminController (kuzzle) {
  ...
  this.deleteIndexes = (requestObject, context) => {
    ...
    console.dir(requestObject, {depth: 30});
    return kuzzle.services.list.readEngine.listIndexes()
      .then(response => {
        console.log('listIndexes response', response)
        var
          deferred = q.defer(),
          allowedIndexes = [],
...
console.log('indexes', indexes)
        async.each(indexes, (index, callback) => {
          console.log('prout!!!!!!!!!!!!!!!!!!!!!!!!!', context.token.user)
          kuzzle.repositories.user.load(context.token.user)
            .then((uu) => {
              console.log('uuuuuuu', uu)
              if (_.isString(context.token.user)) {
                return kuzzle.repositories.user.load(context.token.user);
              }
              console.log('prout')
            ...
            .then(u => {
              ...
              u.getRights(kuzzle).then(r => console.log('!!!!!!!!!!!!!!!!',r));
              return q(u);
            })
            .then(u => u.isActionAllowed({controller: 'admin', action: 'deleteIndex', index: index}, context, kuzzle))
            .then((isAllowed) => {
        ...
        error => {
          ...
          if (error) {
            ...
            console.log('allowedIndexes',allowedIndexes)
