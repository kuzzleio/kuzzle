...
class AdminController {
  ...
  resetKuzzleData (request) {
    ...
    const promise = this.kuzzle.repositories.user.truncate({ refresh: 'wait_for' })
      ...
      .then(() => delete _locks.resetKuzzleData)
      .catch(() => delete _locks.resetKuzzleData);
  ...
  resetSecurity (request) {
    ...
    const promise = this.kuzzle.repositories.user.truncate(options)
      ...
      .then(() => delete _locks.resetSecurity)
      .catch(() => delete _locks.resetSecurity);

    return this._waitForAction(request, promise, result);
  }
  ...
  resetDatabase (request) {
    ...
    const promise = Bluebird.map(indexes, index => {
      ...
      .then(() => delete _locks.resetDatabase)
      .catch(() => delete _locks.resetDatabase);
