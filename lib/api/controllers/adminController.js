...
function AdminController(kuzzle) {
  ...
  this.getSpecifications = requestObject => {
    ...
    return kuzzle.pluginsManager.trigger('data:beforeGetSpecifications', requestObject)
      .then(newRequestObject => {
        modifiedRequestObject = newRequestObject;
        return internalEngine.get('validation', `${modifiedRequestObject.index}#${modifiedRequestObject.collection}`);
      })
...
function validateSpecificationList (kuzzle, list) {
  ...
  _.forEach(list, specification => {
    promises.push(kuzzle.validation.isValidSpecification(specification.indexName, specification.collectionName, specification.specifications, true));
  });
  ...
  return Promise.all(promises)
    .then(response => {
      ...
      _.forEach(response, value => {
        if (!value.isValid) {
          errors = _.concat(errors, value.errors);
        }
      });
