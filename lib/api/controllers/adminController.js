...
class AdminController extends BaseController {
  ...
  resetKuzzleData (request) {
    ...
    return this.kuzzle.repositories.user.truncate({ refresh: 'wait_for' })
      .then(() => this.kuzzle.internalIndex.deleteIndex())
      .then(() => this.kuzzle.services.internalCache.flushdb())
      .then(() => this.kuzzle.internalIndex.bootstrap.startOrWait())
      .then(() => {
  ...
  resetCache (request) {
    ...
    const database = request.input.args.database;
  ...
  resetDatabase (request) {
    ...
    return Bluebird.map(indexes, index => {
      ...
      return this.kuzzle.services.publicStorage.deleteIndex(req)
