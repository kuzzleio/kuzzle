...
class AdminController extends NativeController {
  ...
  async resetCache(request) {
    ...
    if (database === "internalCache") {
      ...
      global.kuzzle.log.info("[ADMIN] cache successfully reset");
  ...
  async resetSecurity() {
    ...
    try {
      ...
      global.kuzzle.log.info("[ADMIN] security settings successfully reset");
    } finally {
      await mutex.unlock();
    }
  ...
  async resetDatabase() {
    ...
    try {
      ...
      await this.ask("core:storage:public:deleteIndexes", "*");

      global.kuzzle.log.info("[ADMIN] database successfully reset");
  ...
  async shutdown() {
    ...
    global.kuzzle.log.info(
      "[ADMIN] shutdown request received. Starting shutdown...",
    );
