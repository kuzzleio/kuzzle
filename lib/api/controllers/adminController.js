...
class AdminController {
  ...
  resetKuzzleData () {
    ...
    this.kuzzle.repositories.user.truncate()
  ...
  resetSecurity () {
    ...
    this.kuzzle.repositories.user.truncate(options)
      .then(() => this.kuzzle.repositories.profile.truncate(options))
      .then(() => this.kuzzle.repositories.role.truncate(options))
  ...
  dump (request) {
    ...
    if (! this.kuzzle.config.dump.enabled) {
      throw new BadRequestError('Dump generation is disabled.');
    }
