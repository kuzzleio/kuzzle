...
module.exports = function AdminController (kuzzle) {
  ...
  this.putRole = function (requestObject) {
    kuzzle.pluginsManager.trigger('data:puRole', requestObject);

    return kuzzle.repositories.role
      .validateAndSaveRole(
        kuzzle.repositories.role.getRoleFromRequestObject(requestObject)
      ).
      then(result => {
        return Promise.resolve(new ResponseObject(requestObject, {result: result}));
      });
  };
};
