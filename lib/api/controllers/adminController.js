...
class AdminController {
  ...
  generateDump (request) {
    ...
    return dump(this.kuzzle, suffix);
  }
  ...
  shutdown () {
    if (_shutdown) {
      return Bluebird.resolve('Kuzzle is already shutting down.');
    }
    ...
    return Bluebird.resolve('Gracefully shutdown Kuzzle..');
  }
 ...
 * Create a dump
 *
<<<<<<< HEAD:lib/api/controllers/adminController.js
 * @param {Kuzzle} kuzzle
 * @param {string} suffix
=======
 * @param {Request} request
 * @param {Function} statusUpdate
>>>>>>> 1.x:lib/api/controllers/cli/dump.js
 * @returns {Promise}
 */
// eslint-disable-next-line no-console
function dump (kuzzle, suffix, statusUpdate = console.log) {
  if (_lock) {
  ...
  return new Bluebird((resolve, reject) => {
    ...
    try {
      ...
      if (e.message.startsWith('EEXIST')) {
        ...
        return resolve('Dump directory already exists. Skipping..');
      }
    ...
    fs.readdir(logDir, (err, files) => {
      if (err) {
        ...
        return resolve('... error reading log directory');
      }
    ...
    .then(() => new Bluebird((resolve, reject) => {
      ...
      glob(`${dumpPath}/core*`, (err, res) => {
        ...
        if (!res[0]) {
          ...
          return resolve('> warning: could not generate dump');
        }
