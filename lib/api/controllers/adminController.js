...
function deleteObjects(kuzzle, objectType, options, part = null) {
  ...
  if (part === null) {
    return kuzzle.repositories[objectType].search({}, {scroll: '10m', size: 100})
      .then(objects => {
        kuzzle.pluginsManager.trigger('log:info', `${objects.total} ${objectType}s found`);

        return deleteObjectsPart(kuzzle, objectType, objects, options)
          .then(() => {
            kuzzle.pluginsManager.trigger('log:info', `... ${objects.hits.length}/${objects.total} ${objectType}s deleted`);
    ...
    .then(objects => {
      return deleteObjectsPart(kuzzle, objectType, objects, options)
        .then(() => {
          ...
          kuzzle.pluginsManager.trigger('log:info', `... ${part.deleted}/${part.total} ${objectType}s deleted`);
...
function deleteObjectsPart (kuzzle, objectType, objects, options) {
  ...
  for (let i = 0; i < objects.hits.length; i++) {
    if (['admin', 'default', 'anonymous'].indexOf(objects.hits[i]._id) !== -1) {
      continue;
    }

    const request = new Request({ _id: objects.hits[i]._id });
    ...
    promises.push(securityController[action].apply(securityController, [request])); // eslint-disable-line no-useless-call
  }
