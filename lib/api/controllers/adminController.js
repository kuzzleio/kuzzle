...
class AdminController extends BaseController {
   ...
   */
  resetCache (request) {
    const database = this.stringParam(request, 'args.database');
    ...
    if (database === 'internalCache') {
      ...
      cacheEngine = this.kuzzle.services.publicCache;
    } else {
  ...
  resetDatabase (request) {
    ...
    return this.storageEngine.listIndexes()
      .then(indexes => this.storageEngine.deleteIndexes(indexes))
      .then(deleted => {
   ...
   */
  dump (request) {
    const suffix = this.stringParam(request, 'args.suffix', 'manual-api-action');
  ...
  loadFixtures (request) {
    const fixtures = this.objectParam(request, 'body');
  ...
  loadMappings (request) {
    const mappings = this.objectParam(request, 'body');
  ...
  loadSecurities (request) {
    const securities = this.objectParam(request, 'body');
