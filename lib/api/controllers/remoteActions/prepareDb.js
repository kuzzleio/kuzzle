...
function createInternalStructure() {
  return createInternalIndex()
    ...
    .then(() => createValidationCollection(_params.validation));
}
...
function createValidationCollection(initialValidationConfiguration) {
  if (_kuzzle.indexCache.indexes[_kuzzle.internalEngine.index].indexOf('validation') !== -1) {
    return Promise.resolve();
  }

  _kuzzle.pluginsManager.trigger('log:info', '== Creating validation collection...');

  return _kuzzle.internalEngine.updateMapping('validation', {
    properties: {
      index: {
        type: 'string'
      },
    ...
    var promises = [];
    if (initialValidationConfiguration) {
      _.each(initialValidationConfiguration, (indexSpec, indexName) => {
        _.each(indexSpec, (collectionSpec, collectionName) => {
          ...
          promises.push(_kuzzle.internalEngine.createOrReplace('validation', `${indexName}#${collectionName}`, {
            index: indexName,
