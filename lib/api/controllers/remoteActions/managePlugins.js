...
function pluginsManager(requestObject) {
  var
    ...
    console.log('pluginsManager', requestObject);
    ...
    .then(() => {
      ...
      if (options.activate) {
        return _kuzzle.internalEngine
          .update('plugins', plugin, {activated: true})
          .then(() => getPluginConfiguration(plugin));
      }

      if (options.deactivate) {
        return _kuzzle.internalEngine
          .update('plugins', plugin, {activated: false})
          .then(() => getPluginConfiguration(plugin));
      }
...
function importPluginConfiguration(plugin, filePath) {
  ...
  return _kuzzle.internalEngine.get('plugins', plugin)
    .then(res => {
      res._source.config = content;
      return _kuzzle.internalEngine.createOrReplace('plugins', plugin, res._source);
    })
    .catch(err => {
...
function setPluginConfiguration(plugin, property) {
  ...
  return _kuzzle.internalEngine
    ...
    .then(result => {
      ...
      return _kuzzle.internalEngine.update('plugins', plugin, newConfiguration);
    })
    .then(() => getPluginConfiguration(plugin));
}
...
function unsetPluginConfiguration(plugin, property) {
  return _kuzzle.internalEngine
    ...
    .then(result => {
      ...
      return _kuzzle.internalEngine.replace('plugins', plugin, config);
    })
    .then(() => getPluginConfiguration(plugin));
}
...
function replacePluginConfiguration(plugin, content) {
  ...
  return _kuzzle.internalEngine
    ...
    .then(result => {
      ...
      return _kuzzle.internalEngine.replace('plugins', plugin, result._source);
    })
    .then(() => getPluginConfiguration(plugin));
}
...
function removePlugin(plugin) {
  ...
  return _kuzzle.internalEngine
    ...
    .then(result => {
      ...
      return _kuzzle.internalEngine.delete('plugins', plugin);
    })
    .then(() => {
...
module.exports = function remoteManagePlugins (kuzzle) {
  _kuzzle = kuzzle;
