...
module.export = function PluginsManager(plugin, options) {
  var kuzzleConfiguration = require('../../lib/config')(defaultConfig),
    ...
    initializeInternalIndex(dbService, kuzzleConfiguration)
      .then(() => {
        ...
        if (options.importConfig) {
          return console.log('## importing config');
        }
        ...
        if (options.importConfig) {
          return kuzzle.remoteActions.do('importPluginConfigFile', {});
        }
      })
      .then(() => {
        release();
