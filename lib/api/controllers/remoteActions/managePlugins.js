          ...
          return getPluginsList(dbService, kuzzleConfiguration)
            .then(pluginsInfos => {
              ...
              Object.keys(pluginsInfos).forEach(name => {
                // todo return promises instead of console.log
...
function importPluginConfiguration(db, plugin, filePath, kuzzleConfiguration) {
  ...
  try {
    ...
    return q.reject(new Error('File not found: ' + filePath));
  }
  ...
  try {
    ...
    return q.reject(new Error('Unable to parse ' + content + '. Expected: JSON Object'));
  }
...
function npmInstall(plugin, name) {
  ...
  childProcess.exec('npm install ' + name + (plugin.npmVersion ? ('@' + plugin.npmVersion) : ''), (err, stdout, stderr) => {
    if (err) {
