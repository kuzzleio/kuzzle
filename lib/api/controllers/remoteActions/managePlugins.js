...
function setPluginConfiguration(plugin, property) {
  ...
  return _kuzzle.internalEngine
    ...
    .then(result => {
      ...
      return _kuzzle.internalEngine.update(_kuzle.config.pluginsManager.dataCollection, plugin, newConfiguration);
    })
    .then(() => getPluginConfiguration(plugin));
}
...
function unsetPluginConfiguration(plugin, property) {
  return _kuzzle.internalEngine
    .get(_kuzzle.pluginsManager.dataCollection, plugin)
    .then(result => {
...
function replacePluginConfiguration(plugin, content) {
  ...
  return _kuzzle.internalEngine
    ...
    .then(result => {
      ...
      return _kuzzle.internalEngine.replace(cfg.pluginsManager.dataCollection, plugin, result._source);
    })
    .then(() => getPluginConfiguration(plugin));
}
