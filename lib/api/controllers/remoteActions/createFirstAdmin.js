...
function resetRoles () {
  ...
  promises = ['admin', 'default', 'anonymous'].map(id => {
    return _kuzzle.internalEngine.createOrReplace('roles', id, _kuzzle.config.defaultUserRoles[id]);
  });
...
function resetProfiles () {
  return _kuzzle.internalEngine
    .createOrReplace('profiles', 'admin', {policies: {_id: 'admin', allowInternalIndex: true}})
    .then(() => _kuzzle.internalEngine.createOrReplace('profiles', 'anonymous', {policies: [{_id: 'anonymous'}]}))
    .then(() => _kuzzle.internalEngine.createOrReplace('profiles', 'default', {policies: [{_id: 'default'}]}));
}
