...
function dump (request) {
  var sufix = request.data.body.sufix ? '-' + request.data.body.sufix: '';
  ...
  fs.writeFileSync(path.join(dumpPath, 'plugins.json'), JSON.stringify(_kuzzle.pluginsManager.plugins, null, ' ').concat('\n'));
