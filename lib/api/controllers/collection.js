...
class CollectionController extends NativeController {
  constructor(kuzzle) {
    ...
    this.internalCollections = Object.keys(
      this.kuzzle.storageEngine.config.internalIndex.collections);
  ...
  /**
   * Refresh an internal collection
   *
   * @param {Request} request
   * @returns {Promise}
   */
  async refreshInternal (request) {
    const collection = this.getCollection(request);

    if (!this.internalCollections.includes(collection)) {
      throw errorsManager.get(
        'api',
        'assert',
        'unexpected_argument',
