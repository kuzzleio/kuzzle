...
function CliController (kuzzle) {
  ...
  this.init = () => {
    this.actions = {
      adminExists: request => kuzzle.funnel.controllers.admin.adminExists(request),
      createFirstAdmin: request => kuzzle.funnel.controllers.admin.createFirstAdmin(request),
  ...
  this.onListenCB = (payload) => {
    ...
    if (!this.actions[request.input.action]) {
