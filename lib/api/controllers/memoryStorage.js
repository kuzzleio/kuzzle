...
const scanMatchProperty = {
  ...
  map: val => {
    if (typeof val !== 'string') {
      throw errorsManager.get('invalid_type', 'match', '<string>');
    }
    ...
    geoadd: {
      ...
      points: {
        map: (val, request) => {
          ...
          if (val.length === 0) {
            throw errorsManager.get('empty_argument', 'points');
          }

          val.forEach(v => {
            if (typeof v !== 'object' || !v.lon || !v.lat || !v.name) {
              throw errorsManager.get('invalid_argument', 'points', '<array of geopoints>');
            }
        ...
        map: (val, request) => {
          ...
          val.forEach(v => {
            if (typeof v !== 'object' || !v.field || !v.value) {
              throw errorsManager.get('invalid_argument', 'entries', '<array of object>');
            }
        ...
        map: (val, request) => {
          ...
          val.forEach(entry => {
            if (typeof entry !== 'object' || !entry.key || !entry.value) {
              throw errorsManager.get('invalid_argument', 'entries', '<array of objects>');
            }
...
function extractArgumentsFromRequest (command, request) {
  ...
  Object.keys(mapping[command]).forEach(key => {
    ...
    if (value === undefined) {
      ...
      throw errorsManager.get('missing_argument', key);
    }
...
function extractArgumentsFromRequestForSet (request) {
  ...
  if (['undefined', 'boolean', 'object'].indexOf(typeof request.input.body.value) !== -1) {
    throw errorsManager.get('invalid_type', 'value', 'string, number');
  }

  if (request.input.body.nx && request.input.body.xx) {
    throw errorsManager.get('mutually_exclusive', 'nx', 'xx');
  }

  if (request.input.body.ex && request.input.body.px) {
    throw errorsManager.get('mutually_exclusive', 'ex', 'px');
  }
  ...
  if (request.input.body) {
    ...
    if (request.input.body.direction !== undefined) {
      ...
      if (['ASC', 'DESC'].indexOf(direction) === -1) {
        throw errorsManager.get('invalid_argument', 'direction', '"ASC", "DESC"');
      }
...
function extractArgumentsFromRequestForZAdd (request) {
  ...
  if (request.input.body.elements.length === 0) {
    throw errorsManager.get('empty_argument', 'elements');
  }

  if (request.input.body.incr && request.input.body.elements.length > 1) {
    throw errorsManager.get('too_many_arguments', 'elements');
  }

  request.input.body.elements.forEach((element, index) => {
    if (!isPlainObject(element)) {
      throw errorsManager.get('invalid_argument', 'elements', '<array of objects>');
    }

    if (!element.member) {
      throw errorsManager.get('missing_argument', `elements[${index}].member`);
    }
...
function extractArgumentsFromRequestForZInterstore (request) {
  ...
  if (request.input.body.keys.length === 0) {
    throw errorsManager.get('empty_argument', 'keys');
  }
  ...
  if (request.input.body.aggregate) {
    ...
    if (['SUM', 'MIN', 'MAX'].indexOf(aggregate) === -1) {
      throw errorsManager.get('invalid_argument', 'aggregate', '"SUM", "MIN" or "MAX"');
    }
...
function assertFloat(request, name, value) {
  ...
  if (Array.isArray(value) || Number.isNaN(Number.parseFloat(value))) {
    throw errorsManager.get('invalid_type', name, 'number');
  }
}
...
function assertInt(request, name, value) {
  ...
  if (Array.isArray(value) || Number.isNaN(Number.parseInt(value))) {
    throw errorsManager.get('invalid_type', name, 'integer');
  }
}
...
function processLimit (arg) {
  ...
  if (result.length !== 3) {
    throw errorsManager.get('invalid_argument', 'limit', '<offset, count>');
  }
