...
class DocumentController extends BaseController {
  ...
  scroll(request) {
    if (!request.input.args.scrollId) {
      errorsManager.throw('missing_scroll_id');
    }
    ...
    return this.engine.mdelete(request)
      .then(_response => {
        ...
        if (response.error.length > 0) {
          request.setError(errorsManager.getError('deletion_failed', response.error));
        }
  ...
  _mChanges(request, methodName, cached) {
    ...
    return this.engine[methodName](request)
      .then(_response => {
        ...
        if (response.error.length > 0) {
          request.setError(errorsManager.getError('creation_failed', response.error));
        }
