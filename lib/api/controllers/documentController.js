...
class DocumentController {
  ...
  mCreate(request) {
    ...
    return this.engine.mcreate(request)
      .then(response => {
        if (response.error.length > 0) {
          request.setError(new PartialError('Some document creations failed', response.error));
        }

        this.kuzzle.notifier.notifyDocumentMCreate(request, response.result);
  ...
  mCreateOrReplace(request) {
    ...
    return this.engine.mcreateOrReplace(request)
      .then(response => {
        if (response.error.length > 0) {
          request.setError(new PartialError('Some document creations failed', response.error));
        }

        this.kuzzle.notifier.notifyMDocumentCreate(request, response.result);
        return response.result;
      });
  }
  ...
  mReplace(request) {
    ...
    return this.engine.mreplace(request)
      .then(response => {
        if (response.error.length > 0) {
          request.setError(new PartialError('Some document creations failed', response.error));
        }

        this.kuzzle.notifier.notifyDocumentMReplace(request, response.result);
        return response.result;
      });
  }
...
function doMultipleWriteActions (kuzzle, request, action) {
  ...
  for (const document of request.input.body.documents) {
    ...
    promises.push(kuzzle.funnel.processRequest(modificationRequest));
  }
  ...
  return Bluebird.all(promises)
    .then(documentRequests => {
      ...
      for (const documentRequests of documentRequest) {
        if (documentRequest.error) {
          errors.push(documentRequest.error);
        }
        else {
