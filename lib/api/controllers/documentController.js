...
function DocumentController (kuzzle) {
  ...
  this.scroll = function documentScroll (request) {
    ...
    assertBodyHasAttribute(request, 'scroll');
    assertBodyAttributeType(request, 'scroll', 'string');
    assertBodyHasAttribute(request, 'scroll_id');
    assertBodyAttributeType(request, 'scroll_id', 'string');
