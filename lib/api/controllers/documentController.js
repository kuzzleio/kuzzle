...
function DocumentController (kuzzle) {
  ...
  this.mGet = function documentMGet (request) {
    ...
    if (request.input.body.ids.length > kuzzle.config.server.maxMultiActionsCount) {
      throw new BadRequestError(`Number of gets to perform exceeds the server configured value (${kuzzle.config.server.maxMultiActionsCount})`);
    }
  ...
  this.mDelete = function documentMDelete (request) {
    ...
    if (request.input.body.ids.length > kuzzle.config.server.maxMultiActionsCount) {
      throw new BadRequestError(`Number of delete to perform exceeds the server configured value (${kuzzle.config.server.maxMultiActionsCount})`);
    }
...
function doMultipleActions (kuzzle, request, action) {
  ...
  if (request.input.body.documents.length > kuzzle.config.server.maxMultiActionsCount) {
    throw new BadRequestError(`Number of actions to perform exceeds the server configured value (${kuzzle.config.server.maxMultiActionsCount})`);
  }
