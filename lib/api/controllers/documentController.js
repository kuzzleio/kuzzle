...
/**
 * @description actions available on the document Controller
 * @key actions
 * @value event type (empty for action which are not related to events)
 */
  ...
  */
  ...
  async updateByQuery (request) {
    ...
    if (strict && result.errors.length) {
      throw kerror.get('api', 'process', 'incomplete_multiple_request', event, result.errors);
    }
  ...
  async _mChanges (request, event, action) {
    ...
    const response = await this.ask(
      `core:storage:public:document:${event}`,
      index,
      collection,
      documents,
    ...
    if (strict && response.errors.length) {
      throw kerror.get('api', 'process', 'incomplete_multiple_request', event, response.errors);
    }
...
module.exports = { DocumentController, actions };
