...
class DocumentController extends NativeController {
  ...
  async update (request) {
    ...
    const _updatedFields = this._extractUpdatedFields(content);
  ...
  */
    ...
    if (! silent) {
      await this.ask(
        ...
        result.successes.map(doc => ({
          _id: doc._id,
          _source: doc._source,
          _updatedFields: this._extractUpdatedFields(changes),
        })));
    }
  ...
  async _mChanges (request, methodName, action) {
    ...
    if (! silent && (action === actionEnum.UPDATE || action === actionEnum.UPSERT)) {
      const documentsDictionnary = documents.reduce((result, document) => {
        result[document._id] = document.changes;
        return result;
      }, {});
      ...
      await this.ask(
        ...
        response.items.map(item => {
          if (item.created) {
            item._updatedFields =
              this._extractUpdatedFields(documentsDictionnary[item._id]);
          }
          return item;
        })
      );
  ...
  _extractUpdatedFields (document, path = null, paths = []) {
    for (const [key, value] of Object.entries(document)) {
      ...
      if (isPlainObject(value)) {
        this._extractUpdatedFields(value, currentPath, paths);
      }
      else {
