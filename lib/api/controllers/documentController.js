...
class DocumentController extends BaseController {
  ...
  search (request) {
    const
      { from, size, scroll, searchBody } = this.getSearchParams(request),
  ...
  scroll (request) {
    ...
    return this.publicStorage.scroll(index, collection, scrollId, { scroll })
  ...
  delete (request) {
    ...
    return this.kuzzle.notifier.publish(request, 'unknown', 'pending')
      .then(() => this.publicStorage.delete(index, collection, id))
      .then(() => this.kuzzle.notifier.notifyDocumentMDelete(request, [id]))
      .then(() => ({
        _id: id
      }));
  }
  ...
  mDelete (request) {
    ...
    return this.publicStorage.mDelete(index, collection, ids, { refresh })
      .then(({ result, errors }) => {
        ...
        return this.kuzzle.notifier.notifyDocumentMDelete(request, result);
      })
      .then(() => _result);
  }
  ...
  deleteByQuery (request) {
    ...
    return this.publicStorage.deleteByQuery(
      ...
      .then(({ ids }) => {
        ...
        return this.kuzzle.notifier.notifyDocumentMDelete(request, ids)
      })
      .then(() => deletedIds);
  }
  ...
  validate (request) {
    const
      document = this.getObjectParam(request, 'body'),
      { index, collection } = this.getIndexAndCollection(request);
