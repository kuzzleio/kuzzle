...
function DocumentController (kuzzle) {
  ...
  this.mReplace = function documentMReplace (request) {
    return doMultipleActions(kuzzle, request, 'replace', ['updated']);
  };
    ...
    .then(documentsRequests => {
      ...
      for (let i = 0; i < documentsRequests.length; i++) {
        if (documentsRequests[i].error || resultStates.indexOf(documentsRequests[i].result.result) === -1) {
          errorMessages.push(`${request.input.controller}:${request.input.action} was not able to treat the document ${documentsRequests[i].input.resource._id}`);
        }
        else {
      ...
      if (errorMessages.length > 0) {
        request.setError(new PartialError(`${request.input.controller}:${request.input.action} was not able to treat all documents`, errorMessages.join('; ')));
      }
