    ...
    if (this.concurrentRequests < kuzzle.config.server.maxConcurrentRequests) {
      ...
      return processRequest(kuzzle, this.controllers, requestObject, context)
        ...
        .catch(err => {
          ...
          if (kuzzle.config.dump.handledErrors.active) {
            setImmediate(() => {
              var
                lastSeen = Date.now(),
                request,
              ...
              if (kuzzle.config.dump.handledErrors.whitelist.indexOf(errorType) > -1) {
                ...
                errorMessage = errorMessage.toLowerCase().replace(/[:,'"\.\\/\s]/g, '-').replace(/[-]+/g, '-').split('-');
                ...
                if (!this.lastDumpedErrors[request.data.body.sufix] || this.lastDumpedErrors[request.data.body.sufix] < lastSeen - 100000) {
                  kuzzle.remoteActionsController.actions.dump(request);
