...
const
  ...
  errorsManager = require('../../config/error-codes/throw').wrap('api', 'funnel'),
  {
    errors: {
      KuzzleError,
      BadRequestError,
...
class FunnelController {
  ...
  getRequestSlot(executor, request, executeCallback) {
    ...
    if (this.requestsCacheQueue.length >= this.kuzzle.config.limits.requestsBufferSize) {
      const error = errorsManager.getError('server_overloaded');
  ...
  getController(request) {
    ...
    if (this.controllers[controller]) {
      ...
      errorsManager.throw('unknown_controller', controller);
    }

    if (!target) {
      errorsManager.throw('unknown_action', action, controller);
    }
