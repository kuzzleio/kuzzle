...
class FunnelController {
  ...
  getRequestSlot(executor, request, executeCallback) {
    if (this.overloaded) {
      ...
      if (
        ...
        this.kuzzle.config.limits.requestsBufferWarningThreshold
  ...
  processRequest(request) {
    ...
    return this.performDocumentAlias(request, 'before')
      ...
      .then(responseData => {
        ...
        if (
          !this.isNativeController(modifiedRequest)
  ...
  _playCachedRequests () {
    ...
    const quantityToInject = Math.min(
      this.requestsCacheQueue.length,
      this.kuzzle.config.limits.concurrentRequests
      - this.concurrentRequests
    );
    ...
    if (quantityToInject > 0) {
      ...
      for (i = 0; i < quantityToInject; i++) {
        ...
        if (
          this[cachedItem.executor](cachedItem.request, cachedItem.callback)
          === -1
        ) {
