var
  ...
  ForbiddenError = require('../core/errors/forbiddenError'),
...
module.exports = function FunnelController (kuzzle) {
  ...
  this.execute = function (requestObject, context, callback) {
    ...
    if (this.cachedRequests >= kuzzle.config.request.maxRetainedRequests) {
      return callback(
        new ResponseObject(requestObject, new ServiceUnavailableError('Request discarded: Kuzzle Server is temporarily overloaded'))
      );
    }
...
function processRequest(kuzzle, controllers, requestObject, context) {
  ...
  return kuzzle.repositories.token.verifyToken(getBearerTokenFromHeaders(requestObject.headers))
    ...
    .then(() => {
      ...
      return context.token.user.profile.isActionAllowed(requestObject, context, kuzzle);
    })
    .then(isAllowed => {
      if (!isAllowed) {
        // anonymous user => we throw a 401 (Unauthorised) error
        if (context.token.user._id === -1) {
        ...
        // logged-in user with insuficient permissions => we throw a 403 (Forbidden) error
