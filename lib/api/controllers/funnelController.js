...
function FunnelController (kuzzle) {
  ...
  this.execute = function funnelExecute (request, callback) {
    ...
    if (this.concurrentRequests < kuzzle.config.server.maxConcurrentRequests) {
      ...
      processRequest(kuzzle, this.controllers, request)
        .then(processResult => callback(null, processResult.response))
        .catch(err => {
 ...
 * @param {KuzzleRequest} request
 */
function processRequest(kuzzle, controllers, request) {
  kuzzle.statistics.startRequest(request);

  // TODO treat HTTP Header informations Bearer in HTTP Router instead
  // TODO we presume for now that it will be treated before it is provided to the funnel
  if (
  ...
  return kuzzle.repositories.user.load(request.input.token.userId)
    ...
    .then(isAllowed => {
      var
        modifiedRequest,
        input = request.input,
        beforeEvent = `${input.controller}:before${capitalizeFirstLetter(input.action)}`,
        afterEvent = `${input.controller}:after${capitalizeFirstLetter(request.input.action)}`;
      ...
      return kuzzle.pluginsManager.trigger(beforeEvent, request)
        ...
        .then(responseData => {
          ...
          return kuzzle.pluginsManager.trigger(afterEvent, request);
        });
    })
    .then(newRequest => {
