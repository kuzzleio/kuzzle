...
class FunnelController {
  ...
  processRequest(request) {
    ...
    return this.kuzzle.pluginsManager.trigger(this.getEventName(request, 'before'), request)
      ...
      .then(responseData => {
        if (responseData instanceof Request) {
          modifiedRequest = this._replyWithRequest(modifiedRequest, responseData);
        } else {
  ...
  /**
   * Use directly the Request object passed in response as response
   *
   * @param {Request} originalRequest
   * @param {Request} responseRequest
   * @returns {Request}
   */
  _replyWithRequest (originalRequest, responseRequest) {
    if (originalRequest.id !== responseRequest.id) {
      originalRequest.setError(new PluginImplementationError('You can only return the same request object'));
      return originalRequest;
    }

    if (responseRequest.status === 102) {
      originalRequest.setError(new PluginImplementationError('Response status code must be set'));
      return originalRequest;
    }
