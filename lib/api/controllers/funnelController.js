...
const
  ...
  apiErrorsManager = require('../../config/error-codes/throw').wrap('api', 'funnel'),
...
class FunnelController {
  ...
  getRequestSlot(executor, request, executeCallback) {
    ...
    if (this.requestsCacheQueue.length >= this.kuzzle.config.limits.requestsBufferSize) {
      const error = apiErrorsManager.getError('server_overloaded');
  ...
  execute(request, callback) {
    ...
    if (!this.kuzzle.router.isConnectionAlive(request.context)) {
      ...
      callback(apiErrorsManager.getError('client_connection_dropped'));
      return 0;
    }
      ...
      .then(isAllowed => {
        if (!isAllowed) {
          ...
          if (request.context.token.userId === -1) {
            error = apiErrorsManager.getError(
              'unauthorized_action',
              getActionContext(request));
          } else {
      ...
      .then(responseData => {
        ...
        if (!this.isNativeController(modifiedRequest)
          ...
          try {
            ...
            errorsManager.throw('plugins', 'runtime', 'unable_to_serialize_response');
          }
        }
  ...
  handleProcessRequestError(modifiedRequest, request, error) {
    ...
    if (!this.isNativeController(request) && !(error instanceof KuzzleError)) {
      _error = errorsManager.getError('plugins', 'runtime', 'plugin_error', error.message);
    }
    ...
    return this.kuzzle.pipe(eventError, modifiedRequest)
      ...
      .catch(customError => {
        ...
        if (!this.isNativeController(request)
          ...
          _error = errorsManager.getError('plugins', 'runtime', 'plugin_error', customError.message);
        }
        ...
        return this.kuzzle.pipe('request:onError', modifiedRequest)
          ...
          .catch(err => {
            ...
            errorsManager.throw('plugins', 'runtime', 'plugin_error', err.message);
          });
      });
  }
  ...
  getController(request) {
    ...
    if (this.controllers[controller]) {
      ...
      apiErrorsManager.throw('unknown_controller', controller);
    }

    if (!target) {
      apiErrorsManager.throw('unknown_action', action, controller);
    }
  ...
  throw(...args) {
    ...
    apiErrorsManager.throw(this.domain, this.subdomain, ...args);
  }
  ...
  getError(...args) {
    return apiErrorsManager.getError(this.domain, this.subdomain, ...args);
  }
}

/**
 *  Return a string with the action name and the index/collection
 *  concerned by the request
 *
 * @param {Request} request
 * @returns {String} "controller:action" on collection "index/collection"
 */
function getActionContext (request) {
  const {
    controller, action,
    resource: {
      index, collection
  ...
  let contextMessage = `"${controller}:${action}"`;

  if (index && collection) {
    contextMessage += ` on collection "${index}/${collection}"`;
  } else if (index) {
    contextMessage += ` on index "${index}"`;
  }
