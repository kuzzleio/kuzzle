...
function processRequest(kuzzle, controllers, requestObject, userContext) {
  ...
  return kuzzle.repositories.token.verifyToken(getBearerTokenFromHeaders(requestObject.headers))
    ...
    .then(isAllowed => {
      var
        beforeEvent = requestObject.controller + ':before' + capitalizeFirstLetter(requestObject.action),
        afterEvent = requestObject.controller + ':after' + capitalizeFirstLetter(requestObject.action);
      ...
      return kuzzle.pluginsManager.trigger(beforeEvent, {requestObject, userContext})
        .then(modifiedData => {
          return controllers[requestObject.controller][requestObject.action](modifiedData.requestObject, modifiedData.userContext);
        })
        .then(responseData => {
          return kuzzle.pluginsManager.trigger(afterEvent, {
            responseObject: responseData.responseObject,
