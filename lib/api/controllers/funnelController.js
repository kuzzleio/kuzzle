      ...
      .then(isAllowed => {
        if (!isAllowed) {
          ...
          if (request.context.token.userId === -1) {
            error = this.getError('anonymous_user', request.input.resource.index, request.input.resource.collection, request.input.controller, request.input.action);
          }
          else {
            // logged-in user with insufficient permissions => we throw a 403 (Forbidden) error
            error = this.getError('insufficient_permissions', request.input.resource.index, request.input.resource.collection, request.input.controller, request.input.action, request.context.user._id);
          }
