    ...
    requestObject.checkInformation()
      .then(() => {
        ...
        if (requestObject.index === undefined ||
          _.isEmpty(requestObject.index)) {
          return Promise.reject(new BadRequestError('No index selected'));
        }
        ...
        if (context.user.profile.isActionAllowed(requestObject, context) === false) {
          return Promise.reject(new UnauthorizedError('Unauthorized action [' + requestObject.index + '/' + requestObject.collection + '/' + requestObject.controller + '/' + requestObject.action + '] for user ' + context.user._id, 401));
        }
