...
class FunnelController {
  ...
  processRequest(request) {
    ...
    return Bluebird.resolve(request)
      .then(newRequest => this.performDocumentAlias(newRequest, 'before'))
   ...
   * @param {KuzzleRequest} request
   * @param {String} prefix
   * @return {KuzzleRequest}
   */
  performDocumentAlias(request, prefix = 'before') {
    const { controller: controllerName, action: actionName } = request.input;
    const mustTrigger = controllerName === 'document'
      && this.documentEventAliases.mirrorList[actionName]
      && (prefix !== 'before' || !this.documentEventAliases.notBefore.includes(actionName));
  ...
  extractDocumentsFromRequest(request) {
    switch (request.input.action) {
      ...
      case 'mReplace':
      case 'mUpdate':
        return request.input.body.documents.map(({ _id, body: _source }) => ({ _id, _source }));
      default:
        return [{ _id: request.input.resource._id, _source: request.input.body }];
    }
  }
   ...
   * @returns {Object}
   */
  insertDocumentsInRequest(documents, request) {
    switch (request.input.action) {
      case 'delete':
      case 'get':
      ...
      case 'mDelete':
      case 'mGet':
        request.input.body.ids = documents.map(({ _id }) => _id);
        return request;
      case 'mCreate':
      case 'mCreateOrReplace':
      case 'mReplace':
      case 'mUpdate':
        request.input.body.documents = documents.map(({ _id, _source: body }) => ({ _id, body}));
        return request;
      default:
        request.input.resource._id = documents[0] && documents[0]._id;
