    ...
    requestObject.checkInformation()
      .then(function () {
        ...
        if (context.user.profile.isActionAllowed(requestObject, context) === false) {
          deferred.reject(new ForbiddenError('Unauthorized action [' + requestObject.collection + '/' + requestObject.controller + '/' + requestObject.action + '] for user ' + context.user._id));
          return false;
        }
