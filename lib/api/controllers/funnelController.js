...
class FunnelController {
  ...
  getRequestSlot(executor, request, executeCallback) {
    if (this.overloaded) {
      ...
      if (this.requestsCacheQueue.length > this.kuzzle.config.limits.requestsBufferWarningThreshold
        ...
        this.kuzzle.emit('log:warn', `[!WARNING!] Kuzzle overloaded: ${overloadPercentage}%. Delaying requests...`);
    ...
    if (this.kuzzle.config.dump.enabled && this.kuzzle.config.dump.handledErrors.enabled) {
      setImmediate(() => {
        ...
        if (this.kuzzle.config.dump.handledErrors.whitelist.indexOf(errorType) > -1) {
          ...
          this.kuzzle.emit('log:error', err instanceof Error && !(err instanceof KuzzleError)
            ? err.message + '\n' + err.stack
            : err
          );
  ...
  processRequest(request) {
    ...
    const { controller: controllerName, action: actionName } = request.input;
    const triggerAfterAlias = controllerName !== 'document' || !this.documentEventAliases.mirrorList[actionName];
    const triggerBeforeAlias = triggerAfterAlias && !this.documentEventAliases.notBefore.includes(actionName)

    return Bluebird.resolve(request)
      .then(newRequest => {
        if (!triggerBeforeAlias) {
          return newRequest;
        }
        const alias = this.documentEventAliases.mirrorList[actionName];
        const event = this.documentEventAliases.namespace + ':before' + capitalizeFirstLetter(alias);
        return this.kuzzle.pipe(event, this.extractDocumentsFromRequest(newRequest));
      })
      .then(({ document, request: newRequest }) => newRequest)
      ...
      //   const alias = this.documentEventAliases.mirrorList[actionName];
      //   const event = this.documentEventAliases.namespace + ':before' + capitalizeFirstLetter(alias);
      //   return this.kuzzle.pipe(event, newRequest);
      // })
      .then(responseData => {
  ...
  _playCachedRequests () {
    ...
    if (this.requestsCacheQueue.length > 0) {
      ...
      if (this.overloadWarned && (this.lastOverloadTime === 0 || this.lastOverloadTime < now - 500)) {
        this.overloadWarned = false;
        this.kuzzle.emit('log:info', 'End of overloaded state. Resuming normal activity.');
   ...
   * @returns {Object}
   */
  extractDocumentsFromRequest(request) {
    switch (request.input.action) {
      case 'delete':
      case 'get':
        return [{ _id: request.input.resource._id }];
      case 'mDelete':
      case 'mGet':
        return request.input.body.ids.map(id => ({ _id: id }));
      case 'mCreate':
      case 'mCreateOrReplace':
      case 'mReplace':
      case 'mUpdate':
        return request.input.body.documents.map(({ _id, body: source }) => ({ _id, source }));
      default:
        return [{ _id: request.input._id, source: request.input.body }];
    }
  }
}
...
//  * @returns {Object}
//  */
// function joinDocumentAndRequest(document, request) {
