...
class FunnelController {
  ...
  getRequestSlot(request, executeCallback) {
    ...
    if (this.overloaded) {
      ...
      if (this.requestsCache.length > this.kuzzle.config.limits.requestsBufferWarningThreshold
        && (this.lastWarningTime === 0 || this.lastWarningTime < now - 500)
      ) {
        const overloadPercentage = Math.round(10000 * this.requestsCache.length / this.kuzzle.config.limits.requestsBufferSize) / 100;
    ...
    if (this.concurrentRequests < this.kuzzle.config.limits.concurrentRequests) {
      if (this.cachedIds[request.id]) {
        delete this.cachedIds[request.id];
      }
      return true;
    }
     ...
     */
    if (this.requestsCache.length >= this.kuzzle.config.limits.requestsBufferSize) {
    ...
    if (!this.cachedIds[request.id]) {
      this.cachedIds[request.id] = true;
      this.requestsCache.push(new CacheItem(request, executeCallback));
  ...
  _playCachedRequests () {
    ...
    const quantityToInject = Math.min(this.requestsCache.length, this.kuzzle.config.limits.concurrentRequests - this.concurrentRequests);

    if (quantityToInject > 0) {
      let i;
      for (i = 0; i < quantityToInject; i++) {
        const cachedItem = this.requestsCache.shift();
        this.execute(cachedItem.request, cachedItem.callback);
      }
    }
