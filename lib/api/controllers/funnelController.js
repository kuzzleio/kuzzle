...
class FunnelController {
  ...
  _checkSdkVersion (request) {
    ...
    if (sdkVersion) {
      return processError.throw('incompatible_sdk_version', sdkVersion, 'Kuzzle v2');
    }
    ...
    const semverRange = this.sdkCompatibility[name];
    if (! semverRange) {
      return;
    }

    if (! satisfiesMajor(version, semverRange)) {
      return processError.throw('incompatible_sdk_version', version, semverRange);
    }
  }
...
function satisfiesMajor (version, range) {
  if (range[1] === '=') {
    return range[0] === '<' ? version[0] <= range[1] : version[0] >= range[1];
  }
