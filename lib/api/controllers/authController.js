...
class AuthController extends BaseController {
  ...
  logout(request) {
    ...
    return this.kuzzle.repositories.token.expire(request.context.token)
      ...
      .catch(err => {
        const error = this.getError('token_force_expire');
        error.details = err;
  ...
  getCurrentUser(request) {
    ...
    return formatProcessing.formatUserForSerialization(
      ...
      .then(result => {
        ...
        if (
          this.kuzzle.repositories.user.anonymous()._id
  ...
  getMyRights(request) {
    return request.context.user.getRights(this.kuzzle)
      .then(
        rights => Object.keys(
          rights).reduce((array, item) => array.concat(rights[item]), [])
      )
