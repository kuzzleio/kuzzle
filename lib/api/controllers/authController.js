...
class AuthController extends NativeController {
  ...
  constructor() {
    ...
    this.getStrategyMethod = global.kuzzle.pluginsManager.getStrategyMethod.bind(global.kuzzle.pluginsManager);
  }
  ...
  async signup(request) {
    const content = request.getBodyObject('credentials', {});
  ...
  async _persistUser(request, profileIds, content, { humanReadableId=true } = {}) {
    ...
    const generator = humanReadableId ? () => generateRandomName('kuid') : () => 'kuid-' + uuidv4();
