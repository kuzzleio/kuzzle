...
class AuthController extends BaseController {
  ...
  logout (request) {
    ...
    return promise
      ...
      .catch(err => {
        const error = apiErrorsManager.get('token_force_expire');
        error.details = err;
  ...
  login (request) {
    const
      strategy = this.getStringParam(request, 'args.strategy'),
   ...
   */
  checkToken (request) {
    const token = this.getStringParam(request, 'body.token');
  ...
  updateSelf (request) {
    ...
    const userContent = this.getObjectParam(request, 'body');
  ...
  createMyCredentials (request) {
    ...
    const
      ...
      strategy = this.getStringParam(request, 'args.strategy'),
      credentials = this.getObjectParam(request, 'body');
  ...
  assertIsAuthenticated (request) {
    if (request.context.user._id === this.anonymousId) {
      errorsManager.throw(
        'api',
        'security',
        'must_be_authenticated_to_execute_action');
    }
  }
}
