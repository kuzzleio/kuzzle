...
module.exports = function AuthController (kuzzle) {
  ...
  /**
   * Returns the policies of the user identified by the given jwt token
   *
   * @param {RequestObject} requestObject
   * @param context
   * @returns {Promise}
   */
  this.getCurrentUserPolicies = function (requestObject, context) {
    requestObject.data._id = context.token.user._id;

    return kuzzle.pluginsManager.trigger('auth:getCurrentUserPolicies', requestObject)
      .then(newRequestObject => kuzzle.funnel.controllers.security.getUserPolicies(newRequestObject));
  };
