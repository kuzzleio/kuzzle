...
module.exports = function AuthController (kuzzle) {
  ...
  this.logout = function (requestObject, context) {
    ...
    kuzzle.pluginsManager.trigger('auth:logout', context)
      ...
      .then(() => {
        if (Boolean(context.connection.id)) {
  ...
  this.login = function (requestObject, context) {
    ...
    this.passport.authenticate(requestObject.data, strategy)
      .then(function (userObject) {
        ...
        return kuzzle.repositories.token.generateToken(userObject, options);
      })
      .then(function(token) {
        if (context.connection && Boolean(context.connection.id)) {
          kuzzle.hotelClerk.tokenizedConnectionsController.add(token, context);
        }
