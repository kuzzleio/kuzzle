...
module.exports = function AuthController (kuzzle) {
   ...
   * @returns {Promise}
   */
  this.selfUpdate = function (requestObject, context) {
    var modifiedRequestObject = null;

    return kuzzle.pluginsManager.trigger('security:beforeSelfUpdate', requestObject)
      .then(newRequestObject => {
        modifiedRequestObject = newRequestObject;
        if (!context.token._id) {
          return q.reject(new UnauthorizedError('User must be connected in order to call selfUpdate'));
        }

        if (modifiedRequestObject.data.body.profile) {
          return q.reject(new BadRequestError('profile can not be provided in selfUpdate'));
        }
      ...
      .then(serialized => kuzzle.pluginsManager.trigger('security:afterSelfUpdate', new ResponseObject(modifiedRequestObject, serialized)))
      .catch(err => q.reject(new ResponseObject(modifiedRequestObject || requestObject, err)));
  };
};
