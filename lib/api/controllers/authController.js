...
class AuthController {
  ...
  refreshToken(request) {
    ...
    return this.kuzzle.repositories.token.generateToken(request.context.user, request, options)
      .then(token => {
        // we allow a grace period before expiring the current token
        setTimeout(
          () => this.kuzzle.repositories.token.expire(request.context.token.jwt),
          this.kuzzle.config.security.jwt.gracePeriod
        );
