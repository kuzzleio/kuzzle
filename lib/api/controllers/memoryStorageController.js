...
module.exports = function MemoryStorageController (kuzzle) {
  ...
  kuzzle.services.list.memoryStorage.commands.forEach(command => {
    ...
    this[command] = (requestObject, userContext) => {
      var
        modifiedData = null;

      return kuzzle.pluginsManager.trigger('memoryStorage:before' + command.charAt(0).toUpperCase() + command.slice(1), {requestObject, userContext})
        .then(data => {
          modifiedData = data;

          return kuzzle.services.list.memoryStorage[command].apply(
            kuzzle.services.list.memoryStorage,
            extractArgumentsFromRequestObject(command, modifiedData.requestObject)
          );
        })
        .then(response => kuzzle.pluginsManager.trigger('memoryStorage:after' + command.charAt(0).toUpperCase() + command.slice(1), {
