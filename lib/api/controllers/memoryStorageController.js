...
const scanMatchProperty = {
  ...
  map: val => {
    if (typeof val !== 'string') {
      throw new BadRequestError('Invalid match parameter.');
    }
    ...
    geoadd: {
      ...
      points: {
        map: (val, request) => {
          ...
          if (val.length === 0) {
            throw new BadRequestError('Cannot add an empty points list.');
          }

          val.forEach(v => {
            if (typeof v !== 'object' || !v.lon || !v.lat || !v.name) {
              throw new BadRequestError('Invalid points parameter.');
            }
        ...
        map: (val, request) => {
          ...
          val.forEach(v => {
            if (typeof v !== 'object' || !v.field || !v.value) {
              throw new BadRequestError('Invalid entries parameter.');
            }
...
function extractArgumentsFromRequest (command, request) {
  ...
  Object.keys(mapping[command]).forEach(key => {
    ...
    if (value === undefined) {
      ...
      throw new BadRequestError(`Missing argument ${key}.`);
    }
...
function extractArgumentsFromRequestForSet (request) {
  ...
  if (['undefined', 'boolean', 'object'].indexOf(typeof request.input.body.value) !== -1) {
    throw new BadRequestError('Accepts only scalar values (number, string).');
  }

  if (request.input.body.nx && request.input.body.xx) {
    throw new BadRequestError('NX and XX options are mutually exclusive.');
  }

  if (request.input.body.ex && request.input.body.px) {
    throw new BadRequestError('EX and PX options are mutually exclusive.');
  }
  ...
  if (request.input.body) {
    ...
    if (request.input.body.direction !== undefined) {
      ...
      if (['ASC', 'DESC'].indexOf(direction) === -1) {
        throw new BadRequestError('Invalid direction argument (expected: ASC or DESC).');
      }
...
function extractArgumentsFromRequestForZAdd (request) {
  ...
  if (request.input.body.elements.length === 0) {
    throw new BadRequestError('At least 1 score/member pair must be provided.');
  }

  if (request.input.body.incr && request.input.body.elements.length > 1) {
    throw new BadRequestError('No more than 1 score/member pair can be specified when the "incr" option is set.');
  }

  request.input.body.elements.forEach(element => {
    if (typeof element !== 'object' || Array.isArray(element) || !element.member) {
      throw new BadRequestError('Invalid score/member pair argument.');
    }
...
function extractArgumentsFromRequestForZInterstore (request) {
  ...
  if (request.input.body.keys.length === 0) {
    throw new BadRequestError('At least 1 source key must be provided.');
  }
  ...
  if (request.input.body.aggregate) {
    ...
    if (['SUM', 'MIN', 'MAX'].indexOf(aggregate) === -1) {
      throw new BadRequestError('Invalid aggregate parameter.');
    }
...
function assertFloat(request, name, value) {
  ...
  if (Array.isArray(value) || Number.isNaN(Number.parseFloat(value))) {
    throw new BadRequestError(`Invalid value for parameter ${name}: float expected.`);
  }
}
...
function assertInt(request, name, value) {
  ...
  if (Array.isArray(value) || Number.isNaN(Number.parseInt(value))) {
    throw new BadRequestError(`Invalid value for parameter ${name}: integer expected.`);
  }
}
