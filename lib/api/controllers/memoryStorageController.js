...
function MemoryStorageController (kuzzle) {
  ...
  Object.keys(mapping).forEach(command => {
    this[command] = (request) => {
      const result = extractArgumentsFromRequest(command, request);
      return kuzzle.services.list.memoryStorage[command](...result);
    };
  });
    ...
    geoadd: {
      ...
      points: {
        map: (val, request) => {
          ...
          if (val.length === 0) {
            throw new BadRequestError(`ms:${request.input.action} Cannot add an empty points list`);
          }

          val.forEach(v => {
            if (typeof v !== 'object' || !v.lon || !v.lat || !v.name) {
              throw new BadRequestError(`ms:${request.input.action} Invalid points parameter`);
            }
        ...
        map: (val, request) => {
          ...
          val.forEach(v => {
            if (typeof v !== 'object' || !v.field || !v.value) {
              throw new BadRequestError(`ms:${request.input.action} Invalid entries parameter`);
            }
...
function extractArgumentsFromRequestForZAdd (request) {
  ...
  request.input.body.elements.forEach(element => {
    if (typeof element !== 'object' || Array.isArray(element) || !element.score || !element.member) {
...
function extractArgumentsFromRequestForZInterstore (request) {
  ...
  args.push(...request.input.body.keys);
  ...
  if (request.input.body.weights) {
    ...
    if (request.input.body.weights.length > 0) {
      args.push('WEIGHTS');
      args.push(...request.input.body.weights);
    }
  }
...
function assertFloat(request, name, value) {
  ...
  if (Array.isArray(value) || Number.isNaN(Number.parseFloat(value))) {
    throw new BadRequestError(`ms:${request.input.action} Invalid value for parameter ${name}: float expected`);
  }
}
...
function assertInt(request, name, value) {
  ...
  if (Array.isArray(value) || Number.isNaN(Number.parseInt(value))) {
    throw new BadRequestError(`ms:${request.input.action} Invalid value for parameter ${name}: integer expected`);
  }
}
