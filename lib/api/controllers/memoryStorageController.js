    ...
    geoadd: {
      ...
      points: {
        map: (val, request) => {
          ...
          if (val.length === 0) {
            errorsManager.throw('api', 'memory_storage', 'add_empty_points_list');
          }

          val.forEach(v => {
            if (typeof v !== 'object' || !v.lon || !v.lat || !v.name) {
              errorsManager.throw('api', 'memory_storage', 'points_parameter');
            }
...
function extractArgumentsFromRequestForZAdd (request) {
  ...
  if (request.input.body.incr && request.input.body.elements.length > 1) {
    ...
    errorsManager.throw('api', 'memory_storage', 'too_much_score_member_pair');
  }

  request.input.body.elements.forEach(element => {
    if (typeof element !== 'object' || Array.isArray(element) || !element.member) {
      errorsManager.throw('api', 'memory_storage', 'invalid_score_member_pair');
    }
