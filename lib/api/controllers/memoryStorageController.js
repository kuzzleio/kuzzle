...
function MemoryStorageController (kuzzle) {
  ...
  Object.keys(mapping).forEach(command => {
    this[command] = (request) => {
      return kuzzle.services.list.memoryStorage[command](...extractArgumentsFromRequest(command, request));
    };
  });
  ...
  mapping = {
    ...
    geoadd: {
      ...
      points: {
        map: (val, request) => {
          ...
          if (val.length === 0) {
            throw new BadRequestError(`${getRequestRoute(request)} Cannot add an empty points list.`);
          }

          val.forEach(v => {
            if (typeof v !== 'object' || !v.lon || !v.lat || !v.name) {
              throw new BadRequestError(`${getRequestRoute(request)} Invalid points parameter.`);
            }
    ...
    georadius: {
      ...
      options: {
        ...
        merge: true,
        map: val => {
          let result = typeof val === 'string' ? val.split(',') : val;
        ...
        map: (val, request) => {
          ...
          val.forEach(v => {
            if (typeof v !== 'object' || !v.field || !v.value) {
              throw new BadRequestError(`${getRequestRoute(request)} Invalid entries parameter.`);
            }
      ...
      match: {
        ...
        map: (val, request) => {
          if (typeof val !== 'string') {
            throw new BadRequestError(`${getRequestRoute(request)} Invalid match parameter.`);
          }
    ...
    zrangebyscore: {
      ...
      limit: {
        ...
        map: val => {
          ...
          // "result" should contain LIMIT offset count
          if (result.length !== 3) {
            throw new BadRequestError('ms:zrangebyscore Invalid limit parameter.');
          }
    ...
    zrevrangebylex: {
      ...
      limit: {
        ...
        map: val => {
          ...
          // "result" should contain LIMIT offset count
          if (result.length !== 3) {
            throw new BadRequestError('ms:zrevrangebylex Invalid limit parameter.');
          }
    ...
    zrevrangebyscore: {
      ...
      limit: {
        ...
        map: val => {
          ...
          // "result" should contain LIMIT offset count
          if (result.length !== 3) {
            throw new BadRequestError('ms:zrevrangebyscore Invalid limit parameter.');
          }
...
function extractArgumentsFromRequest (command, request) {
  ...
  Object.keys(mapping[command]).forEach(key => {
    ...
    let value = path.reduce((previousValue, currentValue, currentIndex, array) => {
    ...
    if (value === undefined) {
      ...
      throw new BadRequestError(`${getRequestRoute(request)} Missing argument ${key}.`);
    }
...
function extractArgumentsFromRequestForSet (request) {
  ...
  if (['undefined', 'boolean', 'object'].indexOf(typeof request.input.body.value) !== -1) {
    throw new BadRequestError(`${getRequestRoute(request)} Accepts only scalar values (number, string).`);
  }

  if (request.input.body.nx && request.input.body.xx) {
    throw new BadRequestError(`${getRequestRoute(request)} NX and XX options are mutually exclusive.`);
  }

  if (request.input.body.ex && request.input.body.px) {
    throw new BadRequestError(`${getRequestRoute(request)} EX and PX options are mutually exclusive.`);
  }
  ...
  if (request.input.body) {
    ...
    if (request.input.body.direction !== undefined) {
      ...
      if (['ASC', 'DESC'].indexOf(direction) === -1) {
        throw new BadRequestError(`${getRequestRoute(request)} Invalid direction argument (expected: ASC or DESC).`);
      }
...
function extractArgumentsFromRequestForZAdd (request) {
  ...
  if (request.input.body.elements.length === 0) {
    throw new BadRequestError(`${getRequestRoute(request)} At least 1 score/member pair must be provided.`);
  }
  else if (request.input.body.incr && request.input.body.elements.length > 1) {
    throw new BadRequestError(`${getRequestRoute(request)} No more than 1 score/member pair can be specified when the "incr" option is set.`);
  }

  request.input.body.elements.forEach(element => {
    if (typeof element !== 'object' || Array.isArray(element) || !element.member) {
      throw new BadRequestError(`${getRequestRoute(request)} Invalid score/member pair argument`);
    }
...
function extractArgumentsFromRequestForZInterstore (request) {
  ...
  if (request.input.body.keys.length === 0) {
    throw new BadRequestError(`${getRequestRoute(request)} At least 1 source key must be provided.`);
  }
  ...
  if (request.input.body.aggregate) {
    ...
    if (['SUM', 'MIN', 'MAX'].indexOf(aggregate) === -1) {
      throw new BadRequestError(`${getRequestRoute(request)} Invalid aggregate parameter.`);
    }
...
function assertFloat(request, name, value) {
  ...
  if (Array.isArray(value) || Number.isNaN(Number.parseFloat(value))) {
    throw new BadRequestError(`${getRequestRoute(request)} Invalid value for parameter ${name}: float expected.`);
  }
}
...
function assertInt(request, name, value) {
  ...
  if (Array.isArray(value) || Number.isNaN(Number.parseInt(value))) {
    throw new BadRequestError(`${getRequestRoute(request)} Invalid value for parameter ${name}: integer expected.`);
  }
}
