...
module.exports = function CliActions (kuzzle) {
  this.actions = {};
  ...
  this.do = function cliDo (actionIdentifier, data, params) {
    ...
    return kuzzle.internalEngine.init()
      ...
      .then(() => {
        ...
        kuzzle.services.list.broker.send(kuzzle.config.queues.cliQueue, request);
...
function initActions () {
  ...
  this.actions.managePlugins = new Action({
    ...
    timeOutCB: function cliTimeOutCB () {
      ...
      console.error(`No response from Kuzzle within ̀${this.maxTimeout / 1000}s. Exiting`); // eslint-disable-line no-console
      process.exit(1);
    }
  });
