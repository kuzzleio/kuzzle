...
class RateLimiter {
  constructor (kuzzle) {
    this.profiles = kuzzle.repositories.profile;
    this.loginsPerSecond = kuzzle.config.limits.loginsPerSecond;
    this.frame = {};
  ...
  async isAllowed (request) {
    ...
    if (controller === 'auth' && action === 'login') {
      ...
      const { userId, profileIds } = request.context.user;

      // By definition, auth:logout should be unrestricted
      if (userId !== '-1' && controller === 'auth' && action === 'logout') {
        return true;
      }

      for (let i = 0; i < profileIds.length; i++) {
        const { rateLimit = 0 } = await this.profiles.load(profileIds[i]);
