...
class GraphQLEndpoint {
  ...
  async init() {
    ...
    for (const indexName of Object.keys(this.config)) {
      ...
      for (const collectionName of Object.keys(collections)) {
        ...
        try {
          this.types[typeConf.typeName] = await this.generateTypeForCollection(indexName, collectionName)
  ...
  async onNewCollectionCreated(indexName, collectionName) {
  ...
  async generateTypeForCollection(indexName, collectionName) {
    ...
    if (response.status === 404) {
      throw new NotFoundError(`Collection for Grapqhl type ${indexName}:${collectionName} not found`)
    }
