...
module.exports = function HotelClerk (kuzzle) {
   ...
   *        metadata: {},               // metadata for this customer's subscription on that room
   *        state: all/pending/done,    // notify depending on the notification state
   *        scope: all/in/out,          // notify all document changes, or only documents entering/leaving filter scope
   *      }
   *   }
   * }
   */
  ...
  this.addSubscription = function (requestObject, context) {
    ...
    createRoom.call(this, requestObject.collection, requestObject.data.body)
      .then(roomId => {
        ...
        return addRoomForCustomer.call(this, connection, roomId, requestObject);
      })
      .then(() => {
        kuzzle.notifier.notify(room, {
          error: null,
          result: new RealTimeResponseObject(room, requestObject, this.rooms[room].customers.length)
        });

        deferred.resolve(new RealTimeResponseObject(room, requestObject));
      })
  ...
  /**
   * Return the subscription parameters for a customer on a given room
   *
   * @param roomId
   * @return {object} subscription informations
   */
  this.getSubscriptions = function (roomId) {
    var response = [];

    this.rooms[roomId].customers.forEach(customer => response.push(this.customers[customer][roomId]));
 ...
 */
function addRoomForCustomer (connection, roomId, requestObject) {
  var deferred = q.defer();
  ...
  if (requestObject.scope && ['all', 'in', 'out'].indexOf(requestObject.scope) === -1) {
    deferred.reject(new BadRequestError('Incorrect value for the "scope" parameter. Expected: all, in or out. Got: ' + requestObject.scope));
    return deferred.promise;
  }
  ...
  this.customers[connection.id][roomId] = {
    connection: connection,
    metadata: requestObject.metadata,
    state: requestObject.state || 'done',
    scope: requestObject.scope || 'all'
  };
    ...
    .then(() => {
      ...
      if (count > 0) {
        requestObject = new RequestObject({
          ...
          action: 'off',
          metadata: this.customers[connection.id][roomId].metadata
