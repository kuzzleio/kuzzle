...
class HotelClerk {
  ...
  countSubscription (request) {
    ...
    return {count: this.rooms[roomId].customers.size};
  }
  ...
  getRealtimeCollections (index) {
    if (this.kuzzle.koncorde.storage.filtersIndex[index]) {
      return Object.keys(this.kuzzle.koncorde.storage.filtersIndex[index]);
    }
  ...
  listSubscriptions (request) {
    const
      list = {},
      promises = [],
      realtimeIndex = this.kuzzle.koncorde.storage.filtersIndex;
  ...
  _removeRoomForCustomer (requestContext, roomId, notify = true) {
    ...
    const customer = this.customers[userId];
