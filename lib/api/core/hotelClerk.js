...
module.exports = function HotelClerk (kuzzle) {
    ...
    return createRoom.call(this, requestObject.index, requestObject.collection, requestObject.data.body)
      ...
      .then(response => {
        if (response.diff) {
          ...
          return kuzzle.pluginsManager.trigger('subscription:coreAdd', diff);
        }
        return q(response);
      })
  ...
  this.join = function (requestObject, context) {
    ...
    return subscribeToRoom.call(this, roomId, requestObject, context)
      .then(response => {
        return kuzzle.pluginsManager.trigger('subscription:coreJoin', response);
      })
      .then(modifiedResponse => {
        return modifiedResponse.data;
      });
...
function removeRoomForCustomer (connection, roomId) {
  this.kuzzle.pluginsManager.trigger('subscription:coreRemoveRoom', {connection, roomId});
  ...
  return cleanUpRooms.call(this, roomId)
    .then(() => {
      ...
      this.kuzzle.pluginsManager.trigger('subscription:')
