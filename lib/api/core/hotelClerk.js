...
module.exports = function HotelClerk (kuzzle) {
   ...
   *    'fr4fref4f8fre47fe' // -> list of subscribed rooms id
   *  ]
   * }
   */
  this.customers = {};
  ...
  this.addSubscription = function (requestObject, connection) {
    ...
    createRoom.call(this, requestObject.collection, requestObject.data.body)
      .then(roomId => {
        if (this.customers[connection.id] && this.customers[connection.id].indexOf(roomId) !== -1) {
          deferred.reject(new ForbiddenError('User ' + connection.id + ' has already subscribed to the room ' + roomId));
          return deferred.promise;
        }

        // Add the room for the customer
        addRoomForCustomer.call(this, connection, roomId);
  ...
  this.removeSubscription = function (requestObject, connection) {
    ...
    removeRoomForCustomer.call(this, connection, requestObject.data.body.roomId)
      .then(roomId => { return cleanUpRooms.call(this, roomId); })
      .then(roomId => {
  ...
  this.removeCustomerFromAllRooms = function (connection) {
    ...
    // duplicate the room list before using async on it, since removeRoomForCustomer update it each time it is called
    rooms = this.customers[connection.id].slice();

    async.each(rooms, (roomId, callback) => {
      removeRoomForCustomer.call(this, connection, roomId)
        .then(() => { return cleanUpRooms.call(this, roomId); })
        .then(() => {
          var count = this.rooms[roomId] ? this.rooms[roomId].customers.length : 0;
 ...
 * @param {String} roomId
 */
function addRoomForCustomer (connection, roomId) {
  if (!this.customers[connection.id]) {
  ...
  this.customers[connection.id].push(roomId);
}
...
function removeRoomForCustomer (connection, roomId) {
  ...
  roomIndex = this.customers[connection.id].indexOf(roomId);
  ...
  if (this.customers[connection.id].length > 1) {
    this.customers[connection.id].splice(roomIndex, 1);
  } else {
