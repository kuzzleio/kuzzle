...
class HotelClerk {
  ...
  join (request) {
    ...
    this.kuzzle.pluginsManager.trigger('core:hotelClerk:join', response.diff);
        ...
        promises.push(request.context.user.isActionAllowed(isAllowedRequest, this.kuzzle)
          .then(isAllowed => {
            ...
            for (const roomId of realtimeIndex[index][collection]) {
              ...
              // the room may be currently registered in Koncorde and not in the hotel clerk
              if (room) {
                if (!list[index]) {
                  list[index] = {};
                }
  ...
  _createRoom (index, collection, filters) {
    ...
    return this.kuzzle.realtime.normalize(index, collection, filters)
      .then(normalized => {
        ...
        const response = this.kuzzle.realtime.store(normalized);

        return this.kuzzle.pluginsManager.trigger('room:new', {index, collection, roomId: response.id})
          .then(modified => {
            // pipes are not allowed to modify the room id
   ...
   * Will remove room from Koncorde, from the rooms list and for each customers.
   *
   * @this HotelClerk
   * @param index
   * @param collection
  ...
  /**
   * Remove a room everywhere: in customers, in Koncorde and in this.rooms
  ...
  _removeRoomEverywhere (roomId) {
    ...
    this._removeRoomFromKoncorde(roomId);
  }
  ...
  _removeRoomForCustomer (requestContext, roomId, notify = true) {
    ...
    if (!room) {
      this.kuzzle.pluginsManager.trigger('log:error', `hotelClerk:removeRoom room id ${roomId} not found`);
      return;
    }
    ...
    if (room.customers.size === 1) {
      ...
      this.kuzzle.notifier.notifyUser(roomId, request, 'out', {count: room.customers.size});
    }

    if (notify) {
      this.kuzzle.pluginsManager.trigger('core:hotelClerk:removeRoomForCustomer', {requestContext, room});
    }
  ...
  /**
   * Deletes a room if no user has subscribed to it, and removes it also from Koncorde
   *
   * @this HotelClerk
   * @param {string} roomId
   */
  _removeRoomFromKoncorde (roomId) {
