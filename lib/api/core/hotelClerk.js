...
function HotelClerk (kuzzle) {
   ...
   */
  this.addSubscription = function hotelClerkAddSubscription (requestObject, userContext) {
    var diff = [];
    ...
    return createRoom.call(this, requestObject.index, requestObject.collection, requestObject.data.body)
      .then(response => {
        ...
        return subscribeToRoom.call(this, response.roomId, requestObject, userContext);
      })
      .then(response => {
        if (response.diff) {
          diff.push(response.diff);
   ...
   */
  this.removeSubscription = function hotelClerkRemoveSubscription (requestObject, userContext) {
    var
   ...
   * @returns {Promise} promise
   */
  this.countSubscription = function hotelClerkCountSubscription (requestObject) {
    if (!requestObject.data.body || !requestObject.data.body.roomId) {
   ...
   * @returns {Promise}
   */
  this.removeCustomerFromAllRooms = function hotelClerkRemoveCustormerFromAllRooms (connection) {
    var
      rooms;
   ...
   */
  this.listSubscriptions = function hotelClerkListSubscriptions (userContext) {
    return new Promise(resolve => {
      ...
      async.each(Object.keys(this.rooms), (roomId, callback) => {
        ...
        kuzzle.repositories.user.load(userContext.token.userId)
          .then(user => {
            var requestObject = _.assign(requestObjectCollection, {index: room.index, collection: room.collection});

            return user.isActionAllowed(requestObject, userContext, kuzzle);
          })
          .then(isAllowed => {
            if (!isAllowed) {
   ...
   * @returns {Promise}
   */
  this.join = function hotelClerkJoin (requestObject, userContext) {
    var
    ...
    return Promise.resolve(subscribeToRoom.call(this, roomId, requestObject, userContext))
      .then(response => {
   ...
   * @returns {Promise}
   */
  this.removeRooms = function hotelClerkRemoveRooms (requestObject) {
...
function removeRoomForCustomer (connection, roomId, notify) {
  ...
  Object.keys(this.rooms[roomId].channels).forEach(channel => {
    ...
    this.kuzzle.pluginsManager.trigger('proxy:leaveChannel', {channel, id: connection.id});
  });
 ...
 * @returns {Promise}
 */
function subscribeToRoom (roomId, requestObject, userContext) {
  var
    connection = userContext.connection;
  ...
  return createChannelConfiguration(requestObject)
    .then(channel => {
      ...
      this.kuzzle.pluginsManager.trigger('proxy:joinChannel', {channel: channelName, id: connection.id});
