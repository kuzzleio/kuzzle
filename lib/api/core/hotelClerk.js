...
function HotelClerk (kuzzle) {
  ...
  this.countSubscription = function hotelClerkCountSubscription (request) {
    ...
    if (!this.rooms[request.input.body.roomId]) {
      return Promise.reject(new NotFoundError('The room Id ' + request.input.body.roomId + ' does not exist'));
    }

    return Promise.resolve({count: this.rooms[request.input.body.roomId].customers.length});
  };
  ...
  this.removeCustomerFromAllRooms = function hotelClerkRemoveCustormerFromAllRooms (requestContext) {
    ...
    rooms = Object.keys(this.customers[requestContext.connectionId]);
  ...
  this.listSubscriptions = function hotelClerkListSubscriptions (request) {
    return new Promise(resolve => {
      ...
      async.each(Object.keys(this.rooms), (roomId, callback) => {
        ...
        kuzzle.repositories.user.load(request.context.user._id)
          .then(user => {
  ...
  this.join = function hotelClerkJoin (request) {
    ...
    return Promise.resolve(subscribeToRoom.call(this, roomId, request))
      .then(response => {
  ...
  this.removeRooms = function hotelClerkRemoveRooms (request) {
    ...
    if (!kuzzle.dsl.exists(request.input.resource.index, request.input.resource.collection)) {
      return Promise.reject(new NotFoundError(`No subscription found on index ${request.input.resource.index} and collection ${request.input.resource.collection}`));
    }
 ...
 */
function addRoomForCustomer (request, roomId, metadata) {
  ...
  this.customers[request.context.connectionId][roomId] = metadata;
}
...
function createChannelConfiguration (request) {
  ...
  if (request.input.args.state && ['all', 'done', 'pending'].indexOf(request.input.args.state) === -1) {
    return Promise.reject(new BadRequestError('Incorrect value for the "state" parameter. Expected: all, done or pending. Got: ' + request.input.args.state));
  ...
  if (request.input.args.scope && ['all', 'in', 'out', 'none'].indexOf(request.input.args.scope) === -1) {
    return Promise.reject(new BadRequestError('Incorrect value for the "scope" parameter. Expected: all, in, out or none. Got: ' + request.input.args.scope));
  ...
  if (request.input.args.users && ['all', 'in', 'out', 'none'].indexOf(request.input.args.users) === -1) {
    return Promise.reject(new BadRequestError('Incorrect value for the "users" parameter. Expected: all, in, out or none. Got: ' + request.input.args.users));
 ...
 */
function removeRoomForCustomer (requestContext, roomId, notify) {
  if (notify === undefined) {
    notify = true;
  }
  ...
  Object.keys(this.rooms[roomId].channels).forEach(channel => {
    this.kuzzle.entryPoints.proxy.leaveChannel({channel, id: requestContext.connectionId});
    this.kuzzle.pluginsManager.trigger('proxy:leaveChannel', {channel, id: requestContext.connectionId});
  });
    ...
    .then(() => {
      ...
      if (count > 0) {
        request = new Request({
          ...
          metadata: this.customers[requestContext.connectionId][roomId]
        }, requestContext);
    ...
    .then(channel => {
      var
        ...
        diff = {hcR: {
          ...
          m: request.input.metadata
        }};

      if (!this.customers[request.context.connectionId] || !this.customers[request.context.connectionId][roomId]) {
        changed = true;
        addRoomForCustomer.call(this, request, roomId, request.input.metadata);
        this.kuzzle.notifier.notify([roomId], request, {count: this.rooms[roomId].customers.length});
      }

      this.kuzzle.entryPoints.proxy.joinChannel({channel: channelName, id: request.context.connectionId});
      this.kuzzle.pluginsManager.trigger('proxy:joinChannel', {channel: channelName, id: request.context.connectionId});
