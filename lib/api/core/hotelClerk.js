var
  ...
  NotificationObject = require('../core/models/notificationObject'),
...
module.exports = function HotelClerk (kuzzle) {
  ...
  this.removeSubscription = function (requestObject, context) {
    ...
    removeRoomForCustomer.call(this, connection, requestObject.data.body.roomId)
      .then(roomId => deferred.resolve(new NotificationObject(roomId, requestObject)))
      .catch(error => {
  ...
  this.countSubscription = function (requestObject) {
    ...
    deferred.resolve(new NotificationObject(requestObject.data.body.roomId, requestObject, {count: this.rooms[requestObject.data.body.roomId].customers.length}));
  ...
  cleanUpRooms.call(this, roomId)
    .then(() => {
      ...
      if (count > 0) {
        ...
        this.kuzzle.notifier.notify(roomId, (new NotificationObject(roomId, requestObject, {count: count})).toJson());
      }
  ...
  createChannelConfiguration(requestObject)
    .then(channel => {
      ...
      if (!this.customers[connection.id] || !this.customers[connection.id][roomId]) {
        ...
        this.kuzzle.notifier.notify(roomId, (new NotificationObject(roomId, requestObject, {count: this.rooms[roomId].customers.length})).toJson());
      }
      ...
      deferred.resolve(new NotificationObject(roomId, requestObject, {channel: channelName}));
    })
    .catch(error => deferred.reject(error));
