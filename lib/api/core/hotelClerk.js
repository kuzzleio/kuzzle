...
class HotelClerk {
  ...
  countSubscription (request) {
    ...
    if (room === undefined) {
      errorsManager.throw('room_id_not_exists', request.input.body.roomId);
  ...
  removeRooms (request) {
    if (!this.kuzzle.realtime.exists(request.input.resource.index, request.input.resource.collection)) {
      errorsManager.throw('subscription_not_found', request.input.resource.index, request.input.resource.collection);
    ...
    if (request.input.body && request.input.body.rooms) {
      if (!Array.isArray(request.input.body.rooms)) {
        errorsManager.throw('rooms_attribute_type');
  ...
  _createChannelConfiguration (request) {
    ...
    if (request.input.args.state && ['all', 'done', 'pending'].indexOf(request.input.args.state) === -1) {
      errorsManager.throw('incorrect_state_value', request.input.args.state);
    ...
    if (request.input.args.scope && ['all', 'in', 'out', 'none'].indexOf(request.input.args.scope) === -1) {
      errorsManager.throw('incorrect_scope_value', request.input.args.scope);
    ...
    if (request.input.args.users && ['all', 'in', 'out', 'none'].indexOf(request.input.args.users) === -1) {
      errorsManager.throw('incorrect_state_value', request.input.args.users);
  ...
  _createRoom (index, collection, filters) {
    if (!index || !collection) {
      console.log('######')
        return Bluebird.reject(errorsManager.getError('missing_subscription_parameter'));
    ...
    return this.kuzzle.realtime.normalize(index, collection, filters)
      .then(normalized => {
        ...
        if (this.kuzzle.config.limits.subscriptionMinterms > 0
          ...
          errorsManager.throw('limit_minterms_reached', this.kuzzle.config.limits.subscriptionMinterms, normalized.normalized.length);
        ...
        if (this.kuzzle.config.limits.subscriptionRooms > 0 && this.roomsCount >= this.kuzzle.config.limits.subscriptionRooms) {
          errorsManager.throw('limit_unique_rooms_reached');
  ...
  _removeRoomForCustomer (requestContext, roomId, notify = true) {
    if (!this.customers[requestContext.connection.id]) {
      ...
      errorsManager.getError('cant_unsubscribe_unknown_user'));
    ...
    if (customer[roomId] === undefined) {
      return Bluebird.reject(
      errorsManager.getError('cant_unsubscribe_user_not_subscribed', roomId));
    ...
    if (!room) {
      ...
      return Bluebird.reject(
        errorsManager.getError('cant_unsubscribe_unknown_room', roomId));
