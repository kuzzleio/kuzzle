...
module.exports = function HotelClerk (kuzzle) {
  ...
  this.removeAllRooms = function (requestObject) {
    ...
    async.parallel([
      ...
      callbackParallel => {
        if (!kuzzle.dsl.filtersTree[requestObject.collection].rooms) {
          return callbackParallel();
        }

        async.each(kuzzle.dsl.filtersTree[requestObject.collection].rooms, (roomId, callback) => {
      ...
      callbackParallel => {
        if (!kuzzle.dsl.filtersTree[requestObject.collection].fields) {
          return callbackParallel();
        }

        _.forEach(kuzzle.dsl.filtersTree[requestObject.collection].fields, field => {
      ...
      deferred.resolve(new ResponseObject(requestObject, {body: {acknowledge: true}}));
    });
