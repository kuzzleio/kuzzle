...
module.exports = function HotelClerk (kuzzle) {
   ...
   */
  this.removeAllRooms = function (requestObject) {
    var
      responseObject,
      deferred = q.defer();
    ...
    if (!kuzzle.dsl.filtersTree[requestObject.collection]) {
    ...
    if (requestObject.data && requestObject.data.body && requestObject.data.body.rooms) {
      ...
      removeListRoomsInCollection.call(this, requestObject.collection, requestObject.data.body.rooms)
        .then((partialErrors) => {
          responseObject = new ResponseObject(requestObject, {body: {acknowledge: true}});
      ...
      removeAllRoomsInCollection.call(this, requestObject.collection)
        .then(() => {
          return deferred.resolve(new ResponseObject(requestObject, {body: {acknowledge: true}}));
        })
        .catch(error => {
          deferred.reject(error);
        });
 ...
 */
function removeAllRoomsInCollection (collection) {
  var
    deferred = q.defer();
  ...
  async.parallel([
    // remove rooms from global subscription
      callbackParallel => {
      if (!this.kuzzle.dsl.filtersTree[collection].rooms) {
        return callbackParallel();
      }

      async.each(this.kuzzle.dsl.filtersTree[collection].rooms, (roomId, callback) => {
    ...
    // remove rooms from each fields
      callbackParallel => {
      if (!this.kuzzle.dsl.filtersTree[collection].fields) {
        return callbackParallel();
      }

      _.forEach(this.kuzzle.dsl.filtersTree[collection].fields, field => {
...
function removeListRoomsInCollection (collection, rooms) {
  var
    deferred = q.defer(),
    partialErrors = [];
 ...
 * Allow to delete a room with action admin/removeAllRooms or admin/removeRooms
 *
 * @param roomId
 */
...
function subscribeToRoom (roomId, requestObject, context) {
  ...
  if (this.customers[connection.id] && this.customers[connection.id][roomId]) {
    ...
    return new RealTimeResponseObject(roomId, requestObject);
  }
