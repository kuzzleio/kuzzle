...
module.exports = function HotelClerk (kuzzle) {
  ...
  this.listSubscriptions = function (requestObject) {
    ...
    _.forEach(this.kuzzle.dsl.filtersTree, (index, indexName) => {
      list[indexName] = { totalGlobals: 0 };

      _.forEach(index, (collection, collectionName) => {
        list[indexName][collectionName] = {};

        if (collection.rooms) {
          list[indexName][collectionName] = {
            totalGlobals: collection.rooms.length
          };
          list[indexName].totalGlobals += collection.rooms.length;
        }
        _.forEach(collection.fields, field => {
          _.forEach(field, filter => {
            _.forEach(filter.rooms, roomId => {
              ...
              list[indexName][collectionName][roomId] = this.rooms[roomId].customers.length;
            });
          });
        });
      });
