...
class HotelClerk {
  ...
  getRealtimeCollections (index) {
    if (this.kuzzle.dsl.storage.filtersIndex[index]) {
      return Object.keys(this.kuzzle.dsl.storage.filtersIndex[index]);
    }
  ...
  _createRoom (index, collection, filters) {
    ...
    return this.kuzzle.dsl.normalize(index, collection, filters)
      .then(normalized => {
        ...
        const response = this.kuzzle.dsl.store(normalized);

        return this.kuzzle.pluginsManager.trigger('room:new', {index, collection, roomId: response.id})
          .then(modified => {
            // pipes are not allowed to modified the room id
            const roomId = response.id;

            /*
             In some very rare cases, the room may have been created between
             the beginning of the function executed at the end of dsl.normalize,
             and this one
            ...
            return {diff: response.diff, roomId: modified.roomId};
          });
      });
  }
  ...
  _removeAllRoomsInCollection (index, collection) {
    ...
    const dslFilters = Array.from(this.kuzzle.dsl.getFilterIds(index, collection));
  ...
  _removeRoomForCustomer (requestContext, roomId, notify = true) {
    ...
    return this._removeRoomForCustomerWithoutThrowing(requestContext, roomId, notify);
  }

  _removeRoomForCustomerWithoutThrowing (requestContext, roomId, notify = true) {
    if (!this.customers[requestContext.connectionId]) {
      return;
    }
