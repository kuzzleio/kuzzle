...
TokenizedConnectionsController = function(kuzzle) {
  ...
  this.runTimer = function() {
    var firstExpiryDate = Math.min.apply(null, tokenizedConnections.map(function (object) {
      return object.expiresAt;
    }));
    ...
    timer = setTimeout(this.checkCustomersValidity, Date.now() - firstExpiryDate);
  };

  this.checkCustomersValidity = function () {
    var now = Date.now();

    tokenizedConnections.forEach(connectionData => {
      if (connectionData.expiresAt >= now) {
        kuzzle.hotelClerk.removeCustomerFromAllRooms(connectionData.connection);
      }
    });
