...
module.exports = function HotelClerk (kuzzle) {
  ...
  this.addSubscription = function (requestObject, context) {
    ...
    if (this.customers[connection.id] && this.customers[connection.id][requestObject.requestId]) {
      deferred.reject(new ForbiddenError('User ' + connection.id + ' has already subscribed to the room ' + requestObject.requestId));
  ...
  this.countSubscription = function (requestObject) {
    ...
    if (requestObject.data.body && !requestObject.data.body.roomId) {
      deferred.reject(new BadRequestError('The room Id is mandatory for count subscription'));
      return deferred.promise;
    }

    if (!this.rooms[requestObject.data.body.roomId]) {
      deferred.reject(new NotFoundError('The room Id ' + requestObject.data.body.roomId + ' is unknown'));
      return deferred.promise;
    }
...
function removeRoomForCustomer (connection, roomName) {
  ...
  if (!this.customers[connection.id][roomName]) {
    deferred.reject(new NotFoundError('The user with connectionId ' + connection.id + ' doesn\'t listen the room ' + roomName));
    return deferred.promise;
  }
