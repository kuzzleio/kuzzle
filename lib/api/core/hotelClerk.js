  ...
  cleanUpRooms.call(this, roomId)
    .then(() => {
      ...
      if (count > 0) {
        ...
        this.kuzzle.notifier.notify(roomId, new RealTimeResponseObject(roomId, requestObject, {count: count}));
      }
...
function subscribeToRoom (roomId, requestObject, context) {
  ...
  .then(channel => {
    ...
    if (!this.customers[connection.id] || !this.customers[connection.id][roomId]) {
      ...
      this.kuzzle.notifier.notify(roomId, new RealTimeResponseObject(roomId, requestObject, {count: this.rooms[roomId].customers.length}));
    }
