...
class HotelClerk {
  ...
  _createRoom (index, collection, filters) {
    if (!index || !collection) {
      return Bluebird.reject(errorsManager.get('internal', 'hotelclerk', 'missing_subscription_parameter'));
    }
  ...
  _removeRoomForCustomer (requestContext, roomId, notify = true) {
    if (!this.customers[requestContext.connection.id]) {
      return Bluebird.reject(
        errorsManager.get('internal', 'hotelclerk', 'cant_unsubscribe_unknown_user'));
    }
    ...
    if (customer[roomId] === undefined) {
      return Bluebird.reject(
        errorsManager.get('internal', 'hotelclerk', 'cant_unsubscribe_user_not_subscribed', roomId));
    }
    ...
    if (!room) {
      ...
      return Bluebird.reject(
        errorsManager.get('internal', 'hotelclerk', 'cant_unsubscribe_unknown_room', roomId));
    }
