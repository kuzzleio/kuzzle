...
module.exports = function HotelClerk (kuzzle) {
  ...
  this.addSubscription = function (requestObject, context) {
    ...
    createRoom.call(this, requestObject.collection, requestObject.data.body)
      ...
      .then(channel => {
        var channelName = room + '-' + crypto.createHash('md5').update(JSON.stringify(channel)).digest('hex');

        if (!this.customers[connection.id] || !this.customers[connection.id][room]) {
          addRoomForCustomer.call(this, connection, room, channelName, requestObject.metadata);

          kuzzle.notifier.notify(room, {
            error: null,
            result: new RealTimeResponseObject(room, requestObject, {count: this.rooms[room].customers.length})
          });
        }

        if (connection.type === 'websocket') {
          kuzzle.io.sockets.connected[connection.id].join(channelName);
        }

        this.customers[connection.id][room].channels[channelName] = channel;
        deferred.resolve(new RealTimeResponseObject(room, requestObject, {channel: channelName}));
  ...
  this.getChannels = function (roomId, responseObject) {
    ...
    if (this.rooms[roomId]) {
      this.rooms[roomId].customers.forEach(customer => {
        Object.keys(this.customers[customer][roomId].channels).forEach(channel => {
          var
            c = this.customers[customer][roomId].channels[channel],
 ...
 */
function addRoomForCustomer (connection, roomId, channel, metadata) {
  if (!this.customers[connection.id]) {
...
function removeRoomForCustomer (connection, roomId) {
  ...
  if (connection.type === 'websocket' && this.kuzzle.io.sockets.connected[connection.id]) {
    Object.keys(this.customers[connection.id][roomId].channels).forEach(channel => {
