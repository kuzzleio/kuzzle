...
class HotelClerk {
   ...
   *
   * @param {Request} request
   * @returns {String} removed room unique identifier
   */
  ...
  _removeRoomForCustomer (requestContext, roomId, notify = true) {
    ...
    if (!room) {
      ...
      throw new KuzzleInternalError(`Unsubscribe error: room ${roomId} not found`);
    }
  ...
  _subscribeToRoom (roomId, request) {
    ...
    return {diff, data: {roomId, channel: channelName}};
  }
}
