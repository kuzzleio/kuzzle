...
class HotelClerk {
  ...
  _removeRoomForAllCustomers (roomId) {
    const it = this.rooms[roomId].customers.values();
    let customer = it.next();

    while(!customer.done) {
      delete this.customers[customer.value][roomId];
      customer = it.next();
    }
  }
