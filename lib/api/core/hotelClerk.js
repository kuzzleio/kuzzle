...
class HotelClerk {
  ...
  countSubscription (request) {
    ...
    if (room === undefined) {
      errorsManager.throw('internal', 'hotelclerk', 'room_id_not_exists', request.input.body.roomId);
    }
  ...
  removeRooms (request) {
    if (!this.kuzzle.realtime.exists(request.input.resource.index, request.input.resource.collection)) {
      errorsManager.throw('internal', 'hotelclerk', 'subscription_not_found', request.input.resource.index, request.input.resource.collection);
    }

    if (request.input.body && request.input.body.rooms) {
      if (!Array.isArray(request.input.body.rooms)) {
        errorsManager.throw('internal', 'hotelclerk', 'rooms_attribute_type');
      }
  ...
  _createChannelConfiguration (request) {
    ...
    if (request.input.args.scope && allowed.indexOf(request.input.args.scope) === -1) {
      errorsManager.throw('internal', 'hotelclerk', 'incorrect_scope_value', request.input.args.scope);
    }
    channel.scope = request.input.args.scope || 'all';

    if (request.input.args.users && allowed.indexOf(request.input.args.users) === -1) {
      errorsManager.throw('internal', 'hotelclerk', 'incorrect_users_value', request.input.args.users);
    }
  ...
  _createRoom (index, collection, filters) {
    if (!index || !collection) {
      return Bluebird.reject(errorsManager.getError('internal', 'hotelclerk', 'missing_subscription_parameter'));
    }
    ...
    return this.kuzzle.realtime.normalize(index, collection, filters)
      .then(normalized => {
        ...
        if (this.kuzzle.config.limits.subscriptionMinterms > 0
          ...
          errorsManager.throw('internal', 'hotelclerk', 'limit_minterms_reached', this.kuzzle.config.limits.subscriptionMinterms, normalized.normalized.length);
        }
        if (this.kuzzle.config.limits.subscriptionRooms > 0 && this.roomsCount >= this.kuzzle.config.limits.subscriptionRooms) {
          errorsManager.throw('internal', 'hotelclerk', 'limit_unique_rooms_reached');
        }
  ...
  _removeRoomForCustomer (requestContext, roomId, notify = true) {
    if (!this.customers[requestContext.connection.id]) {
      return Bluebird.reject(
        errorsManager.getError('internal', 'hotelclerk', 'cant_unsubscribe_unknown_user'));
    }
    ...
    if (customer[roomId] === undefined) {
      return Bluebird.reject(
        errorsManager.getError('internal', 'hotelclerk', 'cant_unsubscribe_user_not_subscribed', roomId));
    }
    ...
    if (!room) {
      ...
      return Bluebird.reject(
        errorsManager.getError('internal', 'hotelclerk', 'cant_unsubscribe_unknown_room', roomId));
    }
