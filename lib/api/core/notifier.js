...
function NotifierController (kuzzle) {
   ...
   * @returns {boolean}
   */
  this.notify = function notifierNotify (rooms, requestObject, content, connectionId) {
    var
    ...
    rooms.forEach(room => {
      channels = channels.concat(kuzzle.hotelClerk.getChannels(room, notification));
    });
   ...
   * @param requestObject
   */
  this.publish = function notifierPublish (requestObject) {
    return kuzzle.dsl.test(requestObject.index, requestObject.collection, requestObject.data.body, requestObject.data._id)
      .then(rooms => {
   ...
   */
  this.notifyDocumentCreate = function notifierNotifyDocumentCreate (requestObject, newDocument) {
   ...
   * @param {RequestObject} requestObject object describing the original user request
   */
  this.notifyDocumentReplace = function notifierNotifyDocumentReplace (requestObject) {
    var
      matchedRooms;
   ...
   * @param {RequestObject} requestObject object describing the original user request
   */
  this.notifyDocumentUpdate = function notifierNotifyDocumentUpdate (requestObject) {
    var
      matchedRooms,
      updatedDocument,
      request = {
   ...
   */
  this.notifyDocumentDelete = function notifierNotifyDocumentDelete (requestObject, ids) {
