...
class NotifierController {
  ...
  get cacheEngine () {
    return this.kuzzle.services.internalCache;
  }

  get storageEngine () {
    return this.kuzzle.services.publicStorage;
  }
  ...
  notifyDocumentReplace (request) {
    ...
    return this
      ...
      .then(() => this.cacheEngine.get(cachePrefix + request.input.resource._id))
      .then(cachedRooms => {
        if (cachedRooms !== null) {
      ...
      .then(() => {
        if (rooms.length === 0) {
          return this.cacheEngine.del(cachePrefix + request.input.resource._id);
        }

        return this._setCacheWithTTL(
          cachePrefix + request.input.resource._id,
  ...
  notifyDocumentUpdate (request) {
    ...
    return this.storageEngine.get(getRequest)
      .then(result => {
      ...
      .then(() => this.cacheEngine.get(cachePrefix + updatedDocument._id))
      .then(cachedRooms => {
        if (cachedRooms !== null) {
      ...
      .then(() => {
        ...
        return this.cacheEngine.del(cachePrefix + updatedDocument._id);
      })
      .catch(error => this.kuzzle.log.error(error));
  }
