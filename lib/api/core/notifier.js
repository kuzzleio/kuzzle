...
function workerNotification (data) {
  switch (data.action) {
    case 'create':
      notifyDocumentCreate.call(this, data)
        .catch(function (error) {
          this.log.error(error, '\nRejected document:\n', data);
        }.bind(this));
      break;
    case 'update':
      notifyDocumentUpdate.call(this, data)
        .catch(function (error) {
          this.log.error(error, '\nRejected document:\n', data);
        }.bind(this));
      break;
    case 'delete':
    case 'deleteByQuery':
      notifyDocumentDelete.call(this, data)
        .catch(function (error) {
          this.log.error(error, '\nRejected document:\n', data);
        }.bind(this));
      break;
    default:
      this.log.error('Notifier: unknown notification action "' + data.action + '"\n', 'Rejected document:\n', data);
  }
}
