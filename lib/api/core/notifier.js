...
class NotifierController {
  ...
  publish (request, scope, state) {
    ...
    if (rooms.length > 0) {
      if (request.input.controller === 'document' && ['create', 'createOrReplace', 'replace'].includes(request.input.action)) {
         ...
         */
        this.kuzzle.services.list.internalCache.set(
          this._getKeyPrefix(request) + request.id, 
  ...
  notifyDocumentCreate (request, newDocument) {
    return this.kuzzle.services.list.internalCache.get(this._getKeyPrefix(request) + request.id)
      .then(rooms => {
   ...
   */
  notifyMDocumentCreate(request, documents) {
    const prefix = this._getKeyPrefix(request);
    ...
    for (const document of documents) {
      ...
      this.kuzzle.services.list.internalCache.add(prefix + document._id, matchedRooms);
    }
  }
  ...
  notifyDocumentReplace (request) {
    ...
    return this.kuzzle.services.list.internalCache.search(this._getKeyPrefix(request) + request.id)
      .then(rooms => {
        ...
        sur qreturn this.kuzzle.services.list.internalCache.search(this._getKeyPrefix(request) + request.input.resource._id);
      })
      .then(cachedRooms => {
  ...
  _getKeyPrefix(request) {
    return '{'  // start of redis key hash tag 
