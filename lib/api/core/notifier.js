...
class NotifierController {
  ...
  notifyDocument(rooms, request, scope, state, action, content) {
    ...
    if (channels.length > 0) {
      const payload = new Notification.Document(request, scope, state, action, content);
      this.kuzzle.entryPoints.proxy.dispatch('broadcast', {channels, payload});
    }
  }
  ...
  notifyUser(room, request, scope, content) {
    ...
    if (channels.length > 0) {
      const payload = new Notification.User(request, scope, content);
      this.kuzzle.entryPoints.proxy.dispatch('broadcast', {channels, payload});
    }
  }
   ...
   * @param  {string} action - Server notification type
   * @param  {string} message - Additional information
   */
  notifyServer(rooms, connectionId, action, message) {
    const channels = [];
    ...
    if (channels.length > 0) {
      const payload = new Notification.Server(action, message);
      this.kuzzle.entryPoints.proxy.dispatch('notify', {channels, connectionId, payload});
    }
  }
      ...
      return this.kuzzle.services.list.storageEngine.get(getRequest)
        .then(result => {
          ...
          this.notifyDocument(matchedRooms, request, 'out', 'done', 'delete', {
            _meta: deletedDocument._source._kuzzle_info,
