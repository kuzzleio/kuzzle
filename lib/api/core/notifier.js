...
function notifyDocumentCreate (responseObject) {
  ...
  this.dsl.testFilters(responseObject)
    .then(function (rooms) {
      ...
      return this.services.list.cache.add(responseObject.data._id, rooms);
    }.bind(this))
    .then(function () {
      deferred.resolve({});
    })
...
function notifyDocumentUpdate (responseObject) {
  ...
  self.services.list.readEngine.get(new RequestObject(request))
    ...
    .then(function (rooms) {
      ...
      return self.services.list.cache.search(responseObject.data._id);
    })
    .then(function (cachedRooms) {
      ...
      return self.services.list.cache.remove(updateResponseObject.data._id, stopListening);
    })
    .then(function () {
      return self.services.list.cache.add(updateResponseObject.data._id, matchedRooms);
    })
    .then(function () {
      deferred.resolve({});
    })
