...
class NotifierController {
  ...
  publish (request, scope, state) {
    ...
    if (rooms.length > 0) {
      if (request.input.controller === 'document' && ['create', 'createOrReplace', 'replace'].includes(request.input.action)) {
         ...
         */
        this.cache.setex(getCachePrefix(request) + request.id, 10, JSON.stringify(rooms));
      }
  ...
  notifyDocumentCreate (request, newDocument) {
    ...
    return this.cache.get(cachePrefix + request.id)
      .then(rooms => {
        if (rooms !== null) {
          ...
          return this.cache.set(cachePrefix + newDocument._id, rooms);
        }
  ...
  /**
   * Notify rooms matching multiple documents creations
   *
   * @param {Request} request 
   * @param {Array} documents 
   */
  notifyDocumentMCreate(request, documents) {
    ...
    for (const document of documents) {
      ...
      this.notifyDocument(matchedRooms, request, 'in', 'done', 'create', {
        _source: document._source,
        _id: document._id
      });

      this.cache.set(prefix + document._id, JSON.stringify(matchedRooms));
    }
  }
  ...
  notifyDocumentReplace (request) {
    ...
    return this.cache.get(cachePrefix + request.id)
      .then(rooms => {
        ...
        return this.cache.get(cachePrefix + request.input.resource._id);
      })
      .then(cachedRooms => {
        if (cachedRooms !== null) {
        ...
        return this.cache.set(cachePrefix + request.input.resource._id, rawMatchedRooms);
      })
      .catch(error => this.kuzzle.pluginsManager.trigger('log:error', error));
  }

  /**
   * Notify rooms matching multiple documents replacements
   *
   * @param {Request} request - object describing the original user request
   * @param {Array} documents - new documents
   * @returns {Promise}
   */
  notifyDocumentMReplace (request, documents) {
    const 
    ...
    return this.cache.mget(cacheIds)
      .then(hits => {
        for (i = 0; i < hits.length; i++) {
          ...
          if (hits[i] !== null) {
            const stopListening = _.difference(hits[i], rooms);
          ...
          if (rooms.length > 0) {
            ...
            this.cache.set(cacheIds[i], JSON.stringify(rooms));
          } else if (hits[i] !== null) {
            this.cache.del(cacheIds[i]);
          }
        }
      });
  }
  ...
  notifyDocumentUpdate (request) {
    ...
    return this.kuzzle.services.list.storageEngine.get(getRequest)
      .then(result => {
        ...
        return this.cache.get(cachePrefix + updatedDocument._id);
      })
      .then(cachedRooms => {
        if (cachedRooms !== null) {
          const stopListening = _.difference(cachedRooms, matchedRooms);
        ...
        if (matchedRooms.length > 0) {
          return this.cache.set(cachePrefix + updatedDocument._id, JSON.stringify(matchedRooms));
        }
    ...
    return this.kuzzle.services.list.storageEngine.mget(getRequest)
      .then(deletedDocuments => {
        ...
        for (const deletedDocument of deletedDocuments) {
          ...
          cacheKeys.push(cachePrefix + deletedDocument._id);
        }

        return this.cache.del(cacheKeys);
      });
  }
