...
class NotifierController {
  ...
  notifyDocument(rooms, request, scope, state, action, content) {
    ...
    if (channels.length > 0) {
      this.kuzzle.pluginsManager.trigger('notify:document', content)
        .then(updatedContent => {
          const notification = new Notification.Document(request, scope, state, action, updatedContent);
          this._dispatch(channels, notification);
        });
    }
  }
  ...
  notifyUser(room, request, scope, content) {
    ...
    if (channels.length > 0) {
      this.kuzzle.pluginsManager.trigger('notify:user', content)
        .then(updatedContent => {
          const notification = new Notification.User(request, scope, updatedContent);
          this._dispatch(channels, notification);
        });
    }
  }
  ...
  notifyServer(rooms, connectionId, type, message) {
    ...
    if (channels.length > 0) {
      ...
      this.kuzzle.pluginsManager.trigger('notify:server', notification.toJSON())
  ...
  publish(request, scope, state) {
    const rooms = this.kuzzle.dsl.test(
      ...
      return this.kuzzle.services.list.storageEngine.get(getRequest)
        .then(result => {
          const 
            ...
            _meta = deletedDocument._source ? deletedDocument._source._kuzzle_info : {};
  ...
  _dispatch(channels, notification, connectionId) {
    this.kuzzle.pluginsManager.trigger('notify:dispatch', notification)
      .then(updatedNotification => {
        this.kuzzle.entryPoints.proxy.dispatch(connectionId ? 'notify' : 'broadcast', {
          channels, 
          connectionId, 
