...
class NotifierController {
  ...
  notifyDocumentUpdate (request) {
    ...
    return this.storage.get(getRequest)
      .then(result => {
        ...
        const updatedField = Object.keys(request.input.body)
          .filter(updatedField => !['_kuzzle_info'].includes(updatedField));
  ...
  notifyDocumentMDelete (request, ids) {
    ...
    return this.storage.mget(getRequest)
      .then(deleted => {
        const
          cachePrefix = getCachePrefix(request),
          cacheKeys = [],
