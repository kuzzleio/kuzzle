...
module.exports = function (kuzzle) {
  ...
  this.getLastStats = function (requestObject) {
    ...
    if (!this.lastFrame) {
      response.statistics = _.extend(this.currentStats, {timestamp: (new Date()).toISOString()});
      deferred.resolve(new ResponseObject(requestObject, response));
    } else {
      this.kuzzle.services.list.statsCache.get(this.lastFrame)
        .then(value => {
          response.statistics = _.extend(JSON.parse(value), { timestamp: (new Date(this.lastFrame)).toISOString() });
          deferred.resolve(new ResponseObject(requestObject, response));
        })
  ...
  this.getStats = function(requestObject) {
    ...
    if (!this.lastFrame) {
      if (!stopTime || stopTime >= currentDate) {
        response.statistics.push(_.extend(this.currentStats, {timestamp: (new Date(currentDate)).toISOString()}));
      }
      deferred.resolve(new ResponseObject(requestObject, response));
      return deferred.promise;
    }
      ...
      .then(values => {
        values.forEach((v, idx) => {
          ...
          if ((!startTime || startTime <= frameDateTime) && (!stopTime || stopTime >= frameDateTime)) {
            response.statistics.push(_.extend(JSON.parse(v), {timestamp: (new Date(frameDateTime)).toISOString()}));
          }
        });
        deferred.resolve(new ResponseObject(requestObject, response));
      })
