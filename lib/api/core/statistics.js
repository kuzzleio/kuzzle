...
module.exports = function (kuzzle) {
   ...
   */
  this.getLastStat = function (requestObject) {
    var
      deferred = q.defer(),
      response = {
        statistics: {}
  ...
  this.getStats = function(requestObject) {
    ...
    timestamp = requestObject.data.body.timestamp;
    if (timestamp > currentDate) {
      deferred.resolve(new ResponseObject(requestObject, response));
      return deferred.promise;
    }
    if (!this.lastFrame) {
      if (!timestamp || currentDate >= new Date(timestamp).getTime()) {
    ...
    this.kuzzle.services.list.statsCache.getAllKeys()
      ...
      .then(values => {
        values.forEach((v, idx) => {
          if (!timestamp || (new Date(timestamp).getTime()) <= (new Date(Number(frames[idx])).getTime())) {
            response.statistics[(new Date(Number(frames[idx]))).toISOString()] = JSON.parse(v);
          }
        });
        deferred.resolve(new ResponseObject(requestObject, response));
      })
