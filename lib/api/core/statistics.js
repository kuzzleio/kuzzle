...
function StatisticsController (kuzzle) {
  ...
  this.getStats = function statisticsGetStats (request) {
    ...
    if (request && request.input.body && request.input.body.startTime) {
      startTime = new Date(request.input.body.startTime).getTime();
    }

    if (request && request.input.body && request.input.body.stopTime) {
      stopTime = new Date(request.input.body.stopTime).getTime();
    }
   ...
   * @returns {Promise<Object>}
   */
  this.getAllStats = function statisticsGetAllStats () {
