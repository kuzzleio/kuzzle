...
module.exports = function (kuzzle) {
  ...
  this.getLastStats = function (requestObject) {
    ...
    if (!this.lastFrame) {
      response = _.extend(this.currentStats, {timestamp: (new Date()).toISOString()});
      deferred.resolve(new ResponseObject(requestObject, response));
    } else {
      this.kuzzle.services.list.statsCache.get(this.lastFrame)
        .then(value => {
          response = _.extend(JSON.parse(value), { timestamp: (new Date(this.lastFrame)).toISOString() });
          deferred.resolve(new ResponseObject(requestObject, response));
        })
  ...
  this.getStats = function(requestObject) {
    ...
    if (!this.lastFrame) {
      if (!stopTime || stopTime >= currentDate) {
        response.hits.push(_.extend(this.currentStats, {timestamp: (new Date(currentDate)).toISOString()}));
      }
      ...
      .then(values => {
        values.forEach((v, idx) => {
          ...
          if ((!startTime || startTime <= frameDateTime) && (!stopTime || stopTime >= frameDateTime)) {
            response.hits.push(_.extend(JSON.parse(v), {timestamp: (new Date(frameDateTime)).toISOString()}));
          }
        });
