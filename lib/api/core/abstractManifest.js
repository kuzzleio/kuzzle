...
class AbstractManifest {
  constructor(kuzzle, pluginPath) {
    this.targetVersion = kuzzle.config.version;
  ...
  load() {
    ...
    if (!semver.satisfies(this.targetVersion, this.kuzzleVersion)) {
      throw new PluginImplementationError(`[${this.path}/manifest.json] Version mismatch: current Kuzzle version ${this.targetVersion} does not match the manifest requirements (${this.kuzzleVersion})`);
    }
