...
class AbstractManifest {
  ...
  load() {
    try {
      ...
      errorsManager.throw('cannot_load', this.manifestPath, e.message);
    }

    if (_.isNil(this.raw.kuzzleVersion)) {
      errorsManager.throw('missing_version', this.manifestPath);
    }
    ...
    if (!_.isNil(this.raw.name)) {
      if (typeof this.raw.name !== 'string' || this.raw.name.length === 0) {
        errorsManager.throw('invalid_name_type', this.manifestPath);
      }

      this.name = this.raw.name;
    }
    else {
      errorsManager.throw('missing_name', this.manifestPath);
    }
  }
