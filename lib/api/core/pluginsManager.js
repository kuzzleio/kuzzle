  ...
  this.run = function () {
    _.forEach(this.plugins, (plugin, pluginName) => {
      ...
      /*
       ...
       Default value: all
       */
      if (plugin.config.loadedBy && plugin.config.loadedBy !== 'all' && (plugin.config.loadedBy === 'server') !== this.isServer) {
        return true;
      }
      ...
      if (plugin.object.hooks) {
        _.forEach(plugin.object.hooks, (fn, event) => {
          ...
          if (this.isServer) {
            ...
            console.log('Registering hook event: ', event, ' => ', fn);
          }

          if (plugin.object[fn]) {
            kuzzle.on(event, function (message) {
              console.log('emitting event ', event, ' -- message: ', message);
              plugin.object[fn].call(plugin.object, message, event);
            });
          }
        });
      }
