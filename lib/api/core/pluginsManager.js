var
  ...
  context = require('./pluginsContext'),
  fs = require('fs'),
  q = require('q'),
  async = require('async'),
  path = require('path'),
...
module.exports = function PluginsManager (kuzzle) {
  ...
  /**
   * Inject plugin controllers within funnel Controller and HTTP Router
   */
  this.injectControllers = function () {
    _.forEach(this.controllers, function (controller, name) {
      var
        controllerInstance = controller(context),
        router = new Router();

      kuzzle.funnel[name] = controllerInstance;
      kuzzle.router.pluginRouter.use('/'+name, router);
      controllerInstance.init(kuzzle, name, router);
    });
  };
