...
module.exports = function PluginsManager (kuzzle) {
  ...
  this.run = function () {
    _.forEach(this.plugins, (plugin, pluginName) => {
      ...
      // Do not start protocol plugins on worker instances
      if (plugin.object.protocolInterface && !this.isServer) {
        return true;
      }

      kuzzle.log.info('Starting plugin: ', pluginName);
      ...
      if (plugin.object.protocolInterface && plugin.object.protocolInterface.name) {
        this.protocols[plugin.object.protocolInterface.name] = {};

        console.log('PROTOCOL INTERFACE = ', plugin.object.protocolInterface);
        _.forEach(plugin.object.protocolInterface, (f, fName) => {
          console.log('considering...', fName);
          if (['broadcast', 'joinChannel', 'leaveChannel'].indexOf(fName) !== -1) {
            console.log('ADDING INTERFACE: ', fName);
            this.protocols[plugin.object.protocolInterface.name][fName] = f;
          }
        });
      }

      kuzzle.log.info('Plugin ' + pluginName + ' started');
    });
  };
...
function installPlugins(plugins, basePlugins) {
  ...
  _.forEach(plugins, (plugin, name) => {
    ...
    this.kuzzle.log.info('Installing plugin: ' + name);
    ...
    this.kuzzle.log.info('Plugin ' + name + ' installed');
  });
