...
class FunnelProtocol extends KuzzleEventEmitter {
  ...
  query (request) {
    ...
    if (kuzzleRequest.input.controller === 'realtime' && ['subscribe', 'unsubscribe'].includes(kuzzleRequest.input.action)) {
      return Bluebird.reject(new PluginImplementationError(`realtime.${kuzzleRequest.input.action} method is not available in plugins. You should use plugin hooks instead`));
    }
