...
const
  ...
  {InternalError: KuzzleInternalError, GatewayTimeoutError} = require('kuzzle-common-objects').errors;
...
class Sandbox {
  ...
  run(data) {
    if (this.child !== null && this.child.connected) {
      return Bluebird.reject(new KuzzleInternalError('A process is already running for this sandbox'));
    }
