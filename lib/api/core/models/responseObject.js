...
function construct (requestObject, response) {
  ...
  if (response.result && response.result.action && response.result.controller) {
    return constructFromResponse.call(this, response);
  }
...
/**
 * Construct the ResponseObject from a response given by WriteEngine.
 * Useful for universalize
 *
 * @param response
 * @returns {ResponseObject}
 */
var constructFromResponse = function (response) {
  Object.keys(response.result).forEach(function (attr) {
  ...
  if (response.result._source) {
    this.data = response.result._source;
  }
