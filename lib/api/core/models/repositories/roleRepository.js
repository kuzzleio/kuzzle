 ...
 */
RoleRepository.prototype.searchRole = function roleRepositorySearchRole (controllers) {
  return this.search({query: {}}, 0, 1000)
    .then(res => {
      if (controllers && Array.isArray(controllers)) {
        res.hits = res.hits.filter(role => Object.keys(role.controllers).some(key => key === '*' || controllers.includes(key)));
      }
      return res;
    });
};
