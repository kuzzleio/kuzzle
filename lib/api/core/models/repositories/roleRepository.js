...
class RoleRepository extends Repository {
  ...
  load (id) {
    if (!id) {
      return Bluebird.reject(errorsManager.get('missing_role_id'));
    }

    if (typeof id !== 'string') {
      return Bluebird.reject(errorsManager.get('expected_role_id_to_be_a_string'));
    }
  ...
  delete (role, options = {}) {
    if (['admin', 'default', 'anonymous'].indexOf(role._id) > -1) {
      return Bluebird.reject(errorsManager.get('cannot_delete_basic_role', role._id));
    }

    return this.kuzzle.repositories.profile.searchProfiles([role._id], {from: 0, size: 1})
      .then(response => {
        if (response.total > 0) {
          return Bluebird.reject(errorsManager.get('cannot_delete_role_being_used', role._id));
        }
