 ...
 * @returns {Role}
 */
RoleRepository.prototype.getRoleFromRequestObject = function roleRepositoryGetRoleFromRequestObject (requestObject) {
  var role = new Role();
 ...
 * @param {RequestObject} requestObject
 */
RoleRepository.prototype.searchRole = function roleRepositorySearchRole (requestObject) {
  var query = {query: {}};
  ...
  return this.search(query, requestObject.data.body.from, requestObject.data.body.size);
};
 ...
 * @returns Promise
 */
RoleRepository.prototype.validateAndSaveRole = function roleRepositoryValidateAndSaveRole (role, userContext, opts) {
  if (!role._id) {
    return Promise.reject(new BadRequestError('Missing role id'));
  }

  return role.validateDefinition(userContext)
    .then(() => {
      this.roles[role._id] = role;
