...
class ProfileRepository extends Repository {
  ...
  load (id) {
    if (!id) {
      return Bluebird.reject(errorsManager.get('missing_profile_id'));
    }

    if (typeof id !== 'string') {
      return Bluebird.reject(errorsManager.get('expected_profile_id_to_be_a_string', typeof id));
    }
  ...
  loadProfiles (profileIds) {
    if (!profileIds) {
      return Bluebird.reject(errorsManager.get('missing_profile_ids'));
    }

    if (!Array.isArray(profileIds) || profileIds.some(p => typeof p !== 'string')) {
      return Bluebird.reject(errorsManager.get('profile_ids_must_be_array_of_string'));
    }
  ...
  delete (profile, options = {}) {
    ...
    if (['admin', 'default', 'anonymous'].indexOf(profile._id) > -1) {
      return Bluebird.reject(errorsManager.get('cannot_delete_basic_profile'));
    }
    ...
    return this.kuzzle.repositories.user.search(query, {from: 0, size: 1})
      .then(response => {
        if (response.total > 0) {
          return Bluebird.reject(errorsManager.get('cannot_delete_profile_being_used'));
        }
  ...
  validateAndSaveProfile (profile, options) {
    ...
    return this.kuzzle.repositories.role.loadRoles(policiesRoles)
      .catch(() => errorsManager.throw('unable_to_hydrate_profile', profile._id))
