module.exports = function (kuzzle) {
  ...
  ProfileRepository.prototype.hydrate = function (profile, data) {
    ...
    Repository.prototype.hydrate.call(this, profile, data)
      ...
      .then((roles) => {
        ...
        var rolesNotFound = _.difference(extractRoleIds(o.roles), extractRoleIds(roles));
        ...
        roles.forEach(role => {
          o.roles.forEach(dataRole => {
            if (role._id === dataRole._id && !_.isEmpty(dataRole.restrictedTo)) {
  ...
  ProfileRepository.prototype.serializeToDatabase = function (profile) {
    ...
    result.roles = [];
    async.each(profile.roles, (role, callback) => {
