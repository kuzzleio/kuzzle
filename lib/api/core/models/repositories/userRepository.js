module.exports = function (kuzzle) {
  ...
  UserRepository.prototype.generateToken = function (username) {
    var
      deferred = q.defer(),
      encodedToken,
      error;
    ...
    try {
      encodedToken = jwt.sign({_id: username}, kuzzle.config.jsonWebToken.secret, {algorithm: kuzzle.config.jsonWebToken.algorithm});
  ...
  UserRepository.prototype.loadByUsernameAndPassword = function (username, password) {
    ...
    this.loadOneFromDatabase(username)
      .then(function (user) {
        if (user === null) {
