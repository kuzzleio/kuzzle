...
const
  ...
  IndexEngine = require('../../storage/indexEngine'),
 ...
 * @property {IndexEngine} indexEngine
 */
class Repository {
  /**
   * @param {Kuzzle} kuzzle
   ...
   */
  init (options) {
    // if set to null, it means that we explicitly don't want an indexEngine for this repository
    // as null is falsy, without this check, the indexEngine is always set...
    if (options.indexEngine === null) {
      this.indexEngine = options.indexEngine;
    }
    else {
      // @todo refactor storageEngine set (it's dirty to pass it with options.indexEngine)
      // + its the same index for all repository (so better to use kuzzle.internalIndex)
      // it only change for pluginRepository
      this.indexEngine = options.indexEngine
        || new IndexEngine(
          this.kuzzle,
          this.index,
  ...
  loadOneFromDatabase (id) {
    return this.indexEngine.get(this.collection, id)
      .then(response => {
        if (response._id) {
          const dto = {};
  ...
  loadMultiFromDatabase (ids) {
    ...
    return this.indexEngine.mGet(this.collection, ids)
      .then(response => {
  ...
  search (query, options = {}) {
    return this.indexEngine.search(this.collection, query, options)
   ...
   */
  scroll (scrollId, ttl) {
    return this.indexEngine.scroll(this.collection, scrollId, ttl)
      .then(response => this._formatSearchResults(response));
  }
  ...
  persistToDatabase (object, options = {}) {
    ...
    return this.indexEngine[method](this.collection, object._id, this.serializeToDatabase(object), options);
  }
  ...
  deleteFromDatabase (id, options = {}) {
    return this.indexEngine.delete(this.collection, id, options);
  }
