...
class Repository {
  ...
  delete (object, options = {}) {
    if (! object._id) {
      return Bluebird.reject(new BadRequestError(`Missing ${this.collection.slice(0, -1)} id`));
    }
   ...
   * @param {Array} objects
   * @param {object} options
   * @returns {integer} count of deleted objects
   * @private
   */
  _truncatePart(objects, options) {
    return Bluebird.map(objects.hits, object => {
      const protectedObjects = this.collection === 'users' ? [] : ['admin', 'default', 'anonymous'];
