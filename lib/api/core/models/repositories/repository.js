...
Repository.prototype.loadMultiFromDatabase = function (ids, hydrate) {
  ...
  this.readEngine.mget(requestObject)
    .then(response => {
      ...
      if (!response.data || !response.data.body.docs || response.data.body.docs.length === 0) {
      ...
      promises = response.data.body.docs
...
Repository.prototype.search = function (filter, from, size, hydrate) {
  ...
  this.readEngine.search(requestObject)
    .then(response => {
      if (!response.data || !response.data.body.hits || response.data.body.hits.length === 0) {
        return deferred.resolve({total: 0, hits: []});
      }

      if (!hydrate) {
        return deferred.resolve({total: response.data.body.total, hits: response.data.body.hits.map((document) => {
          return {
            _id: document._id,
            _source: document._source
          };
      ...
      async.each(response.data.body.hits, (document, callback) => {
        var
          object,
          data;
        ...
        return deferred.resolve({hits: hydratedObjects, total: response.data.body.total});
      });
    })
    .catch(function (error) {
      deferred.reject(error);
