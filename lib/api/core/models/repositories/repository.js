...
function Repository (kuzzle, options) {
  ...
  this.writeEngine = options.writeEngine;
  ...
  /*
    If no custom write engine is provided, fallback to the standard Kuzzle write workers
   */
  if (!this.writeEngine) {
    this.writeEngine = {
...
Repository.prototype.persistToDatabase = function (object) {
  ...
  return this.writeEngine.execute(requestObject);
};
