...
class Repository {
  ...
  loadFromCache (id, options = {}) {
    const key = options.key || this.getCacheKey(id, this.collection);
  ...
  load (id, options = {}) {
    if (!this.cacheEngine) {
      return this.loadOneFromDatabase(id, this.collection);
    }

    return this.loadFromCache(id, options, this.collection)
      .then(object => {
        if (object === null) {
          ...
          return this.loadOneFromDatabase(id, this.collection)
            .then(objectFromDatabase => {
              if (objectFromDatabase !== null) {
                this.persistToCache(objectFromDatabase, this.collection);
              }
              return objectFromDatabase;
            });
        }
        this.refreshCacheTTL(object, this.collection);
        return object;
      });
  }
  ...
  delete (id, options = {}) {
    ...
    if (this.cacheEngine) {
      promises.push(this.deleteFromCache(id, options, this.collection));
    }

    if (this.databaseEngine) {
      promises.push(this.deleteFromDatabase(id, this.collection));
    }
  ...
  persistToCache (object, options = {}) {
    ...
    let key = options.key || this.getCacheKey(object._id, this.collection);
  ...
  deleteFromCache (id, options = {}) {
    ...
    key = options.key || this.getCacheKey(id, this.collection);
  ...
  refreshCacheTTL (object, options = {}) {
    ...
    key = options.key || this.getCacheKey(object._id, this.collection);
  ...
  expireFromCache (object, options = {}) {
    const key = options.key || this.getCacheKey(object._id, this.collection);
