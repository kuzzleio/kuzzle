...
const
  ...
  errorsManager = require('../../../../config/error-codes/throw').wrap('api', 'auth'),
  { UnauthorizedError } = require('kuzzle-common-objects').errors;
...
class TokenRepository extends Repository {
  ...
  init() {
    super.init({
      ...
      cacheEngine: this.kuzzle.services.internalCache
    });
  }
  ...
  generateToken(user, request, options = {}) {
    if (!user || user._id === null) {
      return Bluebird.reject(errorsManager.getError('cannot_generate_token_with_unknown_user'));
    }

    if (!request.context.connection.id) {
      return Bluebird.reject(errorsManager.getError('cannot_generate_token_with_unknown_context'));
    }
    ...
    if (this.kuzzle.config.security.jwt.maxTTL > -1
      ...
      return Bluebird.reject(errorsManager.getError('expiresIn_limit_reached'));
    }
    ...
    try {
      ...
      const error = errorsManager.getError('generate_token_error');
    ...
    return this.persistToCache(token, {ttl: expiresIn / 1000})
      .then(() => {
        const existingToken = this.kuzzle.tokenManager.getConnectedUserToken(user._id, request.context.connection.id);
        if (existingToken) {
      ...
      .catch(err => {
        const error = errorsManager.getError('cannot_generate_token_with_unknown_user');
        error.details = err.message;
        error.stack = err.stack;
        return Bluebird.reject(error);
      });
  ...
  verifyToken(token) {
    ...
    try {
      ...
      if (err instanceof jwt.TokenExpiredError) {
        error = errorsManager.getError('token_expired');
        error.details = {
          subCode: error.subCodes.TokenExpired,
          expiredAt: err.expiredAt
        };
      } else if (err instanceof jwt.JsonWebTokenError) {
        error = errorsManager.getError('json_web_token_error');
        error.details = {
          subCode: error.subCodes.JsonWebTokenError,
          description: err.message
        };
      } else {
        error = errorsManager.getError('error_verifying_token');
        error.details = err;
      }
    ...
    return this.load(decoded._id + '#' + token)
      .then(userToken => {
        if (userToken === null) {
          errorsManager.throw('invalid_token');
        }
      ...
      .catch(err => {
        ...
        const error = errorsManager.getError('error_verifying_token');
        error.details = err;
  ...
  deleteByUserId(userId) {
    ...
    // @todo ask @scottinet or @bvidis for this
