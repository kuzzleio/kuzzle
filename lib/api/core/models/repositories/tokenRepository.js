...
class TokenRepository extends Repository {
  ...
  generateToken(user, request, options = {}) {
    if (!user || user._id === null) {
      return Bluebird.reject(errorsManager.get('cannot_generate_token_with_unknown_user'));
    }

    if (!request.context.connection.id) {
      return Bluebird.reject(errorsManager.get('cannot_generate_token_with_unknown_context'));
    }
    ...
    if (this.kuzzle.config.security.jwt.maxTTL > -1
      ...
      return Bluebird.reject(errorsManager.get('expiresIn_limit_reached'));
    }
    ...
    try {
      ...
      const error = errorsManager.get('generate_token_error');
    ...
    return this.persistToCache(token, {ttl: expiresIn / 1000})
      ...
      .catch(err => {
        const error = errorsManager.get('cannot_generate_token_with_unknown_user');
        error.details = err.message;
        error.stack = err.stack;
        return Bluebird.reject(error);
      });
  ...
  verifyToken(token) {
    ...
    try {
      ...
      if (err instanceof jwt.TokenExpiredError) {
        error = errorsManager.get('token_expired');
        error.details = {
          subCode: error.subCodes.TokenExpired,
          expiredAt: err.expiredAt
        };
      } else if (err instanceof jwt.JsonWebTokenError) {
        error = errorsManager.get('json_web_token_error');
        error.details = {
          subCode: error.subCodes.JsonWebTokenError,
          description: err.message
        };
      } else {
        error = errorsManager.get('error_verifying_token');
        error.details = err;
      }
    ...
    return this.load(decoded._id + '#' + token)
      ...
      .catch(err => {
        ...
        const error = errorsManager.get('error_verifying_token');
        error.details = err;
