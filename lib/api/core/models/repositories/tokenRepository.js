...
const
  ...
  errorsManager = require('../../../../util/errors').wrap('security', 'token'),
...
class TokenRepository extends Repository {
  ...
  generateToken(user, request, options = {}) {
    if (!user || user._id === null) {
      return errorsManager.reject('unknown_user');
    }

    if (!request.context.connection.id) {
      return errorsManager.reject('unknown_connection');
    }
    ...
    if ( this.kuzzle.config.security.jwt.maxTTL > -1
      ...
      return errorsManager.reject('ttl_exceeded');
    }
    ...
    try {
      ...
      return errorsManager.rejectFrom(err, 'generation_failed', err.message);
    }
  ...
  verifyToken(token) {
    ...
    try {
      ...
      if (err instanceof jwt.TokenExpiredError) {
        return errorsManager.reject('expired');
      }

      if (err instanceof jwt.JsonWebTokenError) {
        return errorsManager.reject('invalid');
      }

      return errorsManager.rejectFrom(err, 'verification_error', err.message);
    }

    return this.load(decoded._id + '#' + token)
      .then(userToken => {
        if (userToken === null) {
          errorsManager.throw('invalid');
        }
      ...
      .catch(err => {
        ...
        errorsManager.throwFrom(err, 'verification_error', err.message);
      });
  }
