...
class TokenRepository extends Repository {
  ...
  generateToken (
    ...
    if ( ! bypassMaxTTL
      && this.kuzzle.config.security.jwt.maxTTL > -1
    ...
    try {
      encodedToken = jwt.sign(
        ...
        { algorithm, expiresIn: parsedExpiresIn });
    }
    catch (err) {
    ...
    const
      redisTTL = parsedExpiresIn !== -1 ? parsedExpiresIn / 1000 : false,
      expiresAt = parsedExpiresIn !== -1 ? Date.now() + parsedExpiresIn : -1,
      token = new Token({
