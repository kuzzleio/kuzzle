...
TokenRepository.prototype.expire = function tokenRepositoryExpire (token) {
  var
    error;
...
TokenRepository.prototype.generateToken = function tokenRepositoryGenerateToken (user, userContext, opts) {
  var
    expiresIn,
    token = new Token(),
    options = opts || {},
  ...
  if (!userContext || userContext.connection === null) {
  ...
  try {
    ...
    return this.persistToCache(token)
      .then(() => {
        this.kuzzle.tokenManager.add(token, userContext);
