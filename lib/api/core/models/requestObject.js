...
function construct (object, additionalData, protocol) {
  ...
  // add the creation date (can be used for 'created at' or 'update at' and useful for profiling)
...
RequestObject.prototype.isValid = function () {
  ...
  if (this.data.body === undefined || _.isEmpty(this.data.body)) {
    deferred.reject(new Error('The body can\'t be empty'));
    return deferred.promise;
  }
...
RequestObject.prototype.isPersistent = function () {
  return (typeof this.persist === 'string') ? this.persist === 'true' : this.persist;
};
