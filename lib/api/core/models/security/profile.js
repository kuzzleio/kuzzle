      ...
      if (policy.restrictedTo) {
        ...
        for (const restriction of policy.restrictedTo) {
          ...
          for (const member of Object.keys(restriction)) {
            if (member !== 'index' && member !== 'collection') {
              return Bluebird.reject(new BadRequestError(`policies[${i}].restrictedTo[${j}] Unexpected attribute "${member}". Valid attributes are "index" and "collection"`));
            }
          }
