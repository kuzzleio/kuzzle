...
class Profile {
  ...
  isActionAllowed(request) {
    ...
    return this.getPolicies()
      .map(policy => policy.role.isActionAllowed(request, policy.restrictedTo))
  ...
  validateDefinition() {
    if (!Array.isArray(this.policies)) {
      return Bluebird.reject(errorsManager.getError('missing_mandatory_policies_attribute'));
    }

    if (this.policies.length === 0) {
      return Bluebird.reject(errorsManager.getError('empty_policies_attribute'));
    }
