...
Role.prototype.isActionAllowed = function (requestObject, context, indexes, kuzzle) {
  ...
  if (_.isObject(actionRights)) {
    ...
    return executeClosure.call(this, kuzzle, actionRights, requestObject, context);
  }
 ...
 * @returns {Boolean}
 */
function executeClosure (kuzzle, actionRights, requestObject, context) {
  var
    sandboxContext,
    sandboxScript,
    error,
  ...
  if (actionRights.args && !this.closures[path].argsDefinitions) {
    /* jshint evil: true */
    this.closures[path].argsDefinitions = eval('(function (requestObject) {return ' +
      JSON.stringify(actionRights.args).replace('"$currentId"', 'requestObject.data._id') +
      ';})');
    /* jshint evil: false */
  }

  if (this.closures[path].argsDefinitions) {
    argsDefinitions = this.closures[path].argsDefinitions(requestObject);
    args = buildClosureArgs(kuzzle, argsDefinitions);
  }
...
function buildClosureArgs (kuzzle, argsDefinitions) {
  ...
  // loop on argsDefinitions -> argsFunctions with factoryFunctionClosure
  // parallel
}
...
function factoryFunctionClosure (argName, argDefinition) {
  ...
  return callback => {
    ...
    if (methodName === 'mget') {
      ...
      requestObject.body = argDefinition.action[methodName];
    }
    ...
    this.services.list.readEngine[methodName](requestObject)
      .then(responseObject => {
        console.log('response', responseObject.data);

        if (responseObject.data.hits) {
          return callback(null, responseObject.data.hits);
        }

        callback(null, {content: responseObject.data.body, id: responseObject.data._id});
      });
  };
}