...
class Role {
  ...
  isActionAllowed (request, restrictedTo = []) {
    if (!this[_kuzzle]) {
      errorsManager.throw('cannot_check_permissions_on_uninitialized_role', this._id);
    }
  ...
  validateDefinition () {
    if (typeof this.controllers !== 'object' || this.controllers === null) {
      return Bluebird.reject(errorsManager.get('controllers_definition_not_an_object'));
    }
    if (Object.keys(this.controllers).length === 0) {
      return Bluebird.reject(errorsManager.get('empty_controllers_definition'));
    }
  ...
  return buildArgsForContext(kuzzle, argsDefinitions)
    .then(args => {
      ...
      if (!role.closures[pathstr].test) {
        try {
          ...
          error = errorsManager.get('parsing_closure_rights_for_role', message);
          error.details = err;
    ...
    .then(result => {
      if (typeof result !== 'boolean') {
        ...
        kuzzle.log.error(message);
        error = errorsManager.get('rights_action_closure_execution', message);
