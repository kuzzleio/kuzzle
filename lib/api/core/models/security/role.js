...
Role.prototype.isActionAllowed = function roleIsActionAllowed (request, kuzzle) {
  ...
  if (request.input.action === 'createCollection' &&
      !doesIndexExist(request.input.resource.index, kuzzle.indexCache.indexes)) {
    promises.push(canCreateIndex.call(this, request.input.resource.index, request, kuzzle));
  }
  ...
  if (_.includes(['import', 'create', 'updateMapping', 'createOrReplace'], request.input.action) &&
    ...
    promises.push(canCreateCollection.call(this, request.input.resource.index, request.input.resource.collection, request, kuzzle));
  }

  if (_.isBoolean(actionRights)) {
    promises.push(Promise.resolve(actionRights));
  } else if (_.isObject(actionRights)) {
    promises.push(executeClosure.call(this, kuzzle, path, actionRights, request));
  } else {
    return Promise.reject(new InternalError('Invalid rights given for role ' + this._id + '(' + path.join('/') + ') : ' + actionRights));
  }

  promises.push(checkRestrictions.call(this, request));
 ...
 * @returns {Promise|*}
 */
function canCreateIndex(index, request, kuzzle) {
  var
  ...
  return this.isActionAllowed(newRequest, kuzzle);
}
 ...
 * @returns {Promise|*}
 */
function canCreateCollection(index, collection, request, kuzzle) {
  var
    newRequest = new Request ({
      controller: 'admin',
 ...
 */
function checkRestrictions(request) {
...
function checkIndexRestriction(request, restriction) {
  ...
  if (!request.input.resource.collection || request.input.resource.collection === null || request.input.resource.collection === undefined) {
    return true;
  }
 ...
 * @returns {Promise}
 */
function executeClosure (kuzzle, path, actionRights, request) {
  var
    sandboxContext,
    sandboxScript,
    error,
  ...
  if (this.closures[path].getArgsDefinitions) {
    argsDefinitions = this.closures[path].getArgsDefinitions(request);
  }
  ...
  return buildArgsForContext.call(kuzzle, argsDefinitions)
    .then(args => {
      ...
      var sandboxContextObject = {
        $request: request,
        $currentUserId: request.context.token.userId,
        args: args
      };
