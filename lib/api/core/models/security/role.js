...
class Role {
  ...
  validateDefinition () {
    if (typeof this.controllers !== 'object' || this.controllers === null) {
      return Bluebird.reject(errorsManager.getError('controllers_definition_not_an_object'));
    }
    if (Object.keys(this.controllers).length === 0) {
      return Bluebird.reject(errorsManager.getError('empty_controllers_definition'));
    }
  ...
  return buildArgsForContext(kuzzle, argsDefinitions)
    .then(args => {
      ...
      if (!role.closures[pathstr].test) {
        try {
          ...
          error = errorsManager.getError('parsing_closure_rights_for_role', message);
          error.details = err;
    ...
    .then(result => {
      if (typeof result !== 'boolean') {
        ...
        kuzzle.log.error(message);
        error = errorsManager.getError('rights_action_closure_execution', message);
