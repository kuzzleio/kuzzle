...
function executeClosure (role, kuzzle, path, actionRights, request) {
  ...
  if (typeof actionRights.test !== 'string') {
    error = new BadRequestError(`Error parsing rights for role ${role._id} (${path.join('/')}) : ${actionRights}`);
  ...
  if (actionRights.args && Object.keys(actionRights.args).length > 0 && !role.closures[path].getArgsDefinitions) {
    try {
      ...
      error = new BadRequestError(`Error parsing rights for role ${role._id} (${path.join('/')}) : ${actionRights.args}`);
      error.details = err;
