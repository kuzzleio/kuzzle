...
function isIndexExist(requestObject, context) {
  return context.kuzzle.indexes[requestObject.index] !== undefined;
}

function isCollectionExist(requestObject, context) {
  return _.contains(context.kuzzle.indexes[requestObject.index], requestObject.collection);
}

/**
 * @TODO: implement multi index : check role
 * @param requestObject
 * @param context
 * @returns {*}
 */
Role.prototype.isActionAllowed = function (requestObject, context) {
  ...
  if (requestObject.persist &&
    ...
    !isIndexExist(requestObject, context)) {
    return false;
  }
  ...
  if (requestObject.persist &&
    ...
    _.contains(['import', 'create', 'createCollection', 'putMapping', 'createOrUpdate'], requestObject.action) &&
    !isCollectionExist(requestObject, context)) {
    return false;
  }
...
Role.prototype.validateDefinition = function (context) {
  ...
  Object.keys(this.indexes).every(indexKey => {
    ...
    if (_.contains(['_canCreate'], indexKey)) {
    ...
    Object.keys(indexRights.collections).every(collectionKey => {
      ...
      if (_.contains(['_canCreate'], collectionKey)) {
