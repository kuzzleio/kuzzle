...
function checkIndexRestriction(requestObject, restriction) {
  ...
  if (!restriction.collections || _.isEmpty(restriction.collections)) {
    return true;
  }
...
function checkRestrictions(requestObject, restrictions) {
  ...
  if (!restrictions || _.isEmpty(restrictions)) {
    return true;
  }
...
Role.prototype.isActionAllowed = function (requestObject, context, restrictions, kuzzle) {
  ...
  if (requestObject.index === internalIndex) {
    if (!isInternalIndexAllowed(restrictions)) {
      return q(false);
    }
  }
  ...
  if (_.isBoolean(actionRights) || _.isObject(actionRights)) {
    ...
    .then(actionRights => {
      ...
      return q(checkRestrictions(requestObject, restrictions));
    });
  }
