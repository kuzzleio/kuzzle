...
function executeClosure (kuzzle, path, actionRights, requestObject, context) {
    ...
    try {
      ...
      this.closures[path].getArgsDefinitions = global.eval('(function (requestObject) {return ' +
        JSON
          ...
          .replace(/"\$(requestObject\.data\.[a-zA-Z0-9_\-.]+)"/g, '$1')
  ...
  return buildArgsForContext.call(kuzzle, argsDefinitions)
    .then(args => {
      ...
      sandboxContext = vm.createContext({ $requestObject: requestObject, context: context, $currentUserId: context.token.user._id, args: args });
      ...
      if (!this.closures[path].test) {
        try {
          ...
          message = `Error parsing rights for role ${this._id} (${path.join('/')}): ${actionRights}`;
...
function factoryFunctionClosure (argName, argDefinition) {
  ...
  return callback => {
    ...
    this.services.list.readEngine[methodName](requestObject)
      .then(responseObject => {
        if (responseObject.data.body.hits) {
          return callback(null, responseObject.data.body.hits.map(document => {
