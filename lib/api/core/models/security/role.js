 ...
 * @param {Object} userContext - user's userContext
 * @param {Kuzzle} kuzzle
 *
 * @returns {Promise}
 */
Role.prototype.isActionAllowed = function roleIsActionAllowed (requestObject, userContext, kuzzle) {
  var
    controllerRights,
    actionRights,
    promises = [],
  ...
  if (requestObject.index === kuzzle.internalEngine.index && !this.allowInternalIndex) {
  ...
  if (requestObject.action === 'createCollection' &&
    ...
    promises.push(canCreateIndex.call(this, requestObject.index, userContext, kuzzle));
  }
  ...
  if (_.includes(['import', 'create', 'updateMapping', 'createOrReplace'], requestObject.action) &&
    ...
    promises.push(canCreateCollection.call(this, requestObject.index, requestObject.collection, userContext, kuzzle));
  }

  if (_.isBoolean(actionRights)) {
    promises.push(Promise.resolve(actionRights));
  } else if (_.isObject(actionRights)) {
    promises.push(executeClosure.call(this, kuzzle, path, actionRights, requestObject, userContext));
  } else {
 ...
 * @returns {Promise}
 */
Role.prototype.validateDefinition = function roleValidateDefinition (userContext) {
          ...
          promises.push((function roleValidateDefinitionSandbox () {
            ...
            return sandBox.run({
              sandbox: {
                ...
                userContext: {
                  connection: {type: userContext.connection.type},
                  token: userContext.token
                },
                $currentUserId: -1,
                args: {}
              },
              code: '(function (requestObject, userContext, $currentUserId, args) { ' + actionRights.test + '\nreturn false;\n})(requestObject, userContext, $currentUserId, args)'
            })
              .then(response => {
                var error;
...
function canCreateIndex(index, userContext, kuzzle) {
  var
    rq = {
      controller: 'admin',
      action: 'createIndex',
  ...
  return this.isActionAllowed(rq, userContext, kuzzle);
}


function canCreateCollection(index, collection, userContext, kuzzle) {
  var
    rq = {
      controller: 'admin',
 ...
 * @returns {Promise}
 */
function executeClosure (kuzzle, path, actionRights, requestObject, userContext) {
  var
    sandboxContext,
    sandboxScript,
    error,
  ...
  return buildArgsForContext.call(kuzzle, argsDefinitions)
    .then(args => {
      var sandboxContextObject = {
        ...
        $currentUserId: userContext.token.userId,
        args: args
      };
      ...
      if (!this.closures[path].test) {
        try {
          sandboxScript = new vm.Script('(function ($requestObject, userContext, $currentUserId, args) { ' +
            actionRights.test +
            '\nreturn false;\n })($requestObject, userContext, $currentUserId, args)');
...
/**
 * Return a function built for async.parallel with the right action on storageEngine
 *
 * @this {Kuzzle}
...
function factoryFunctionClosure (argName, argDefinition) {
  ...
  return callback => {
    ...
    this.services.list.storageEngine[methodName](requestObject)
      .then(response => {
        if (response.hits) {
      ...
      .catch((e) => {
        this.pluginsManager.trigger('log:error', `Error during storageEngine execution for ${methodName} with ${JSON.stringify(argDefinition)}: ${e.message}`);
