...
Role.prototype.isActionAllowed = function (requestObject, context, indexes, kuzzle) {
  ...
  if (!doesIndexExist(requestObject, indexes) &&
    this.indexes._canCreate !== undefined &&
    !this.indexes._canCreate &&
    _.includes(['import', 'create', 'createCollection', 'updateMapping', 'createOrReplace'], requestObject.action)) {
    return q(false);
  }
        ...
        Object.keys(controllerRights.actions).every(actionKey => {
          ...
          if (_.isObject(actionRights)) {
            if (!actionRights.test) {
              deferred.reject(new BadRequestError('Invalid definition for ' + [indexKey, collectionKey, controllerKey] + '. `test` attribute missing from closure'));
              return false;
            }
