...
class Manifest {
  constructor(kuzzle, pluginPath, fallback = null) {
    ...
    if (typeof raw.privileged !== 'undefined') {
    ...
    if (typeof raw.kuzzleVersion !== 'undefined') {
      if (typeof raw.kuzzleVersion !== 'string') {
        throw new PluginImplementationError(`[${this.path}/manifest.json] Invalid "kuzzleVersion" property: expected a string, got a ${typeof raw.kuzzleVersion}`);
      }
    ...
    if (typeof raw.name !== 'undefined') {
      if (typeof raw.name !== 'string') {
        throw new PluginImplementationError(`[${this.path}/manifest.json] Invalid "name" property: expected a string, got a ${typeof raw.name}`);
      }
    ...
    if (!semver.satisfies(kuzzle.config.version, this.kuzzleVersion)) {
      throw new PluginImplementationError(`[${this.path}/manifest.json] Requires Kuzzle version '${this.kuzzleVersion}' (current Kuzzle version: ${kuzzle.config.version})`);
    }
