...
const
  ...
  PluginBootstrap = require('../bootstrap/pluginIndexBootstrap'),
  ...
  constructor(kuzzle, pluginName) {
    ...
    if (kuzzle instanceof Kuzzle) {
      const
        ...
        pluginInternalStorage = new RestrictedStorage(
          pluginIndex,
          this.services.internalStorage);

      // Set the bootstraper class to initialize plugin index
      pluginInternalStorage.indexBootstap = new PluginIndexBootstrap(
        pluginName,
        this.kuzzle,
        pluginInternalStorage);

      Object.defineProperty(this.accessors, 'storage', {
        enumerable: true,
        get: () => {
          return {
            bootstrap: () => pluginInternalStorage.bootstrap(),
            createCollection: (...args) => pluginInternalStorage.createCollection(...args)
          };
        }
      });
      ...
      this.constructors.Repository = function PluginContextRepository (
        ...
        pluginRepository.init({
          databaseEngine: pluginInternalStorage,
          cacheEngine: null,
          ObjectConstructor
        });
