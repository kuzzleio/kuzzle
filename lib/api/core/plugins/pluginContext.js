...
class PluginContext {
  ...
  constructor(kuzzle, pluginName) {
    ...
    this.errorsManager = require('../../../config/error-codes/throw').wrap(
      'plugin',
      pluginName);
    ...
    if (kuzzle instanceof Kuzzle) {
      ...
      const throwNotAvailable = name => () => {
        runtimeError.throw('unavailable_realtime', name);
      };
        ...
        get: () => {
          ...
          pluginSdk.as = user => {
            if (!(user instanceof User)) {
              runtimeError.throw('invalid_user_object');
            }
      ...
      this.constructors.Repository = function PluginContextRepository (
        ...
        if (!collection) {
          runtimeError.throw('missing_collection');
        }
...
function execute (kuzzle, request, callback) {
  ...
  if (callback && typeof callback !== 'function') {
    error = runtimeError.get('invalid_callback', typeof callback);
  ...
  if (!request || !(request instanceof Request)) {
    error = runtimeError.get('missing_request');
  ...
  if (request.input.controller === 'realtime'
    ...
    const err = runtimeError.get('unavailable_realtime', request.input.action);
...
function curryTrigger (kuzzle, pluginName) {
  ...
  return function trigger (eventName, payload) {
    if (eventName.indexOf(':') !== -1) {
      kuzzle.log.error(runtimeError.get('missing_collection', eventName));
      return;
    }
...
function instantiateRequest(request, data, options = {}) {
  ...
  if (!_request) {
    runtimeError.throw('missing_request_data');
  }
  ...
  return function addStrategy(name, strategy) {
    return new Bluebird((resolve, reject) => {
      ...
      if (
        ...
        return reject(errorsManager.get(
          'plugin',
          'strategy',
