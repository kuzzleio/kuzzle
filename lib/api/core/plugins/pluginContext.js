...
<<<<<<< 4dcdadd1b4777a24820db27ca289fb353776e788
...
=======
      const throwNotAvailable = name => () => {
        throw new PluginImplementationError(`${name} method is not available in plugins. You should use plugin hooks instead`);
>>>>>>> fix nitpicking
      };
...
function execute (kuzzle, request, callback) {
  ...
  if (request.input.controller === 'realtime' && ['subscribe', 'unsubscribe'].includes(request.input.action)) {
    error = errorsManager.getError(
      'internal',
      'plugins',
