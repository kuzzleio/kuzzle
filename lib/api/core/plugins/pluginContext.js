    ...
    if (kuzzle instanceof Kuzzle) {
      ...
      const getPluginSdk = sdk => ({
        ...
        realtime: Object.assign({}, sdk.realtime, {
          subscribe: () => { throw new PluginImplementationError('realtime.subscribe method is not available in plugins. You should use plugin hooks instead'); },
          unsubscribe: () => { throw new PluginImplementationError('realtime.unsubscribe method is not available in plugins. You should use plugin hooks instead'); }
        }),
...
function execute (kuzzle, request, callback) {
  ...
  if (request.input.controller === 'realtime'
    ...
    error = new PluginImplementationError(`realtime.${request.input.action} method is not available in plugins. You should use plugin hooks instead`);
    if (callback) {
      return callback(error);
    }
    return Bluebird.reject(error);
