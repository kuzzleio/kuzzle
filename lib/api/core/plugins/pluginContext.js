  ...
  constructor(kuzzle, pluginName) {
    ...
    if (kuzzle instanceof Kuzzle) {
      const
        ...
        pluginInternalIndex = `%plugin:${pluginName}`.toLowerCase(),
        pluginInternalEngine = new InternalEngine(kuzzle, pluginInternalIndex),
      ...
      this.constructors.Repository = function PluginContextRepository (
        ...
        pluginRepository =
          new PluginRepository(kuzzle, pluginInternalIndex, collection);
...
function curryTrigger (kuzzle, pluginName) {
  ...
  return function trigger (eventName, payload) {
    if (eventName.indexOf(':') !== -1) {
      kuzzle.log.error(errorsManager.get(
        'plugins',
        'validation',
        'collection_not_specified',
        eventName));
