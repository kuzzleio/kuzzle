  ...
  constructor(kuzzle, pluginName) {
    ...
    if (kuzzle instanceof Kuzzle) {
      const
        ...
        sdk = new KuzzleSDK(new FunnelProtocol(kuzzle));
      ...
      // Wrapper around original sdk.query method to inject request context
      let jwt = null;
      const originalQuery = sdk.query.bind(sdk);

      sdk.query = request => {
        sdk.jwt = jwt;
        const response = originalQuery(request);
        jwt = null;
      ...
      pluginSdk.context = request => {
        if (request && request.input) {
          jwt = request.input.jwt;
          sdk.protocol.requestWithContext = true;
        }
