...
const
  ...
  PluginIndexBootstrap = require('../storage/bootstrap/pluginIndexBootstrap'),
  ...
  constructor(kuzzle, pluginName) {
    ...
    if (kuzzle instanceof Kuzzle) {
      const
        ...
        pluginIndex = `plugin:${pluginName}`.toLowerCase(),
        pluginIndexEngine = new IndexEngine(
          kuzzle,
          pluginIndex,
          kuzzle.services.internalStorage);

      // Set the bootstraper class to initialize plugin index
      pluginIndexEngine.indexBootstap = new PluginIndexBootstrap(
        kuzzle,
        pluginName,
        pluginIndexEngine);
      ...
      Object.defineProperty(this.accessors, 'storage', {
        ...
        get: () => {
          return {
            bootstrap: () => pluginIndexEngine.init(),
            createCollection: (...args) => pluginIndexEngine.createCollection(...args)
          };
        }
      });

      /**
       * @todo Use a CollectionEngine class instead repository
