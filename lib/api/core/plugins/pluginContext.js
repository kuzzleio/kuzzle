...
class PluginContext {
  ...
  constructor(kuzzle, pluginName) {
    ...
    if (kuzzle instanceof Kuzzle) {
      ...
      // TODO: Wait for the end of Node.js 6 support to use the spread operator
      Object.defineProperty(this.accessors, 'sdk', {
        enumerable: true,
        get: () => ({
          query: kuzzleSdk.query.bind(kuzzleSdk),
          auth: kuzzleSdk.auth,
          bulk: kuzzleSdk.bulk,
          collection: kuzzleSdk.collection,
          document: kuzzleSdk.document,
          index: kuzzleSdk.index,
          ms: kuzzleSdk.ms,
          realtime: kuzzleSdk.realtime,
          security: kuzzleSdk.security,
          server: kuzzleSdk.server,
          context: request => getPluginSdk(
            new KuzzleSDK(new FunnelProtocol(kuzzle.funnel, request))
          )
        })
      });
