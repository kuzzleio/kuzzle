...
function PluginsManager (kuzzle) {
   ...
   * @returns {function}
   */
  this.getStrategyMethod = function pluginGetStrategyMethod (request, strategyName, methodName) {
    if (!this.authentications[strategyName].methods[methodName]) {
      throw new PluginImplementationError(`${getRequestRoute(request)} The authentication strategy "${strategyName}" has no method ${methodName}.`);
    }
...
function injectAuthentication(kuzzle, authentications, plugin, pluginName) {
  ...
  Object.keys(plugin.strategies).forEach(strategyName => {
    ...
    if (!isObject(strategy.methods)) {
      // eslint-disable-next-line no-console
      console.error(`${errorPrefix} The plugin must provide a "methods" object in strategies['${strategy}'] property.`);
      valid = false;
    ...
    if (!isObject(strategy.config)) {
      // eslint-disable-next-line no-console
      console.error(`${errorPrefix} The plugin must provide a "config" object in strategies['${strategy}'] property.`);
      valid = false;
    ...
    if (strategy.methods.get && !isFunction(plugin[strategy.methods.get])) {
      // eslint-disable-next-line no-console
      console.error(`${errorPrefix} The plugin property "${strategy.methods.get}" must be a function.`);
      valid = false;
    }

    if (!strategy.config.constructor || !isFunction(strategy.config.constructor)) {
      // eslint-disable-next-line no-console
      console.error(`${errorPrefix} The constructor of the strategy "${strategy}" must be a function.`);
      valid = false;
    }

    if (strategy.config.strategyOptions && !isObject(strategy.config.strategyOptions)) {
      // eslint-disable-next-line no-console
      console.error(`${errorPrefix} The "strategyOptions" of the strategy "${strategy}" must be an object.`);
      valid = false;
    }

    if (strategy.config.authenticateOptions && !isObject(strategy.config.authenticateOptions)) {
      // eslint-disable-next-line no-console
      console.error(`${errorPrefix} The "authenticateOptions" of the strategy "${strategy}" must be an object.`);
      valid = false;
    }

    if (!strategy.config.fields || !Array.isArray(strategy.config.fields)) {
      // eslint-disable-next-line no-console
      console.error(`${errorPrefix} The "fields" of the strategy "${strategy}" must be an array.`);
      valid = false;
    }
    ...
    if (!isFunction(plugin[strategy.config.verify])) {
      // eslint-disable-next-line no-console
        ...
        methods[methodName] = function pluginAuthMethod (...args) {
          try {
            return plugin[strategy.methods[methodName]].bind(plugin)(...args)
              .catch(error => {
                return Promise.reject(!(error instanceof KuzzleError) ? new PluginImplementationError(error) : error);
              });
          }
          catch (error) {
            throw !(error instanceof KuzzleError) ? new PluginImplementationError(error) : error;
          }
        };
      });
...
function registerStrategy(kuzzle, plugin, strategy, name) {
  ...
  if (kuzzle.pluginsManager.registeredStrategies.indexOf(name) !== -1) {
    throw new PluginImplementationError('An identical authentication strategy has already been registered');
  }
