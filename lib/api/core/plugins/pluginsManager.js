  ...
  if (!controllerImported) {
    ...
    plugin.object.routes.forEach(route => {
      ...
      if (valid) {
        ...
        if (['get', 'head', 'post', 'put', 'delete', 'connect', 'options', 'trace', 'patch'].indexOf(route.verb.toLowerCase()) === -1) {
          // eslint-disable-next-line no-console
          console.error(`[Plugin Manager] Error initializing route ${route.url}: only valid http methods are allowed`);
          return false;
        }
