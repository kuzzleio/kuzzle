...
module.exports = function PluginsManager (kuzzle) {
  ...
  this.run = function () {
    ...
    _.forEach(this.plugins, (plugin, pluginName) => {
      ...
      if (plugin.object.pipes) {
        initPipes(this.pipes, plugin, pipeWarnTime, pipeTimeout);
      }
  ...
  this.trigger = function (event, data) {
    ...
    return triggerPipes.call(this, event, data)
      .then(modifiedData => triggerWorkers.call(this, event, modifiedData))
      .then(modifiedData => triggerHooks.call(this, event, modifiedData))
      .catch(error => {
        console.error('Error with trigger a plugin', error);
      });
  };
...
function pm2Init (workers, pluginsWorker, isDummy) {
  return q.ninvoke(pm2, 'connect')
    ...
    .then(bus => {
      bus.on('ready', packet => {
        if (!pluginsWorker[packet.process.name] || !pluginsWorker[packet.process.name].config) {
          return false;
        }
        ...
        pm2.sendDataToProcessId(packet.process.pm_id, {
          topic: 'initialize',
          data: {
            config: pluginsWorker[packet.process.name].config,
            isDummy: isDummy,
            path: pluginsWorker[packet.process.name].path
          },
    ...
    .catch(err => {
      console.error('Error with PM2', err);
    });
}
      ...
      pipes[pipe].push((data, callback) => {
        ...
        if (pipeWarnTime) {
          pipeWarnTimer = setTimeout(() => {
            console.warn('Pipe plugin', plugin.name, 'exceeded', pipeWarnTime, 'ms to execute.');
          }, pipeWarnTime);
        }
