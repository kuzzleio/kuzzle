  ...
  _initPipes (plugin, pipeWarnTime, pipeTimeout) {
    ...
    _.forEach(plugin.object.pipes, (fn, event) => {
      ...
      for (const target of list) {
        ...
        {
          let message = `Unable to configure pipe for event '${event}' with`
          + ` provided method. ${target} should be a plugin method name, or a function.`;

          if (typeof target === 'string') {
            message += didYouMean(target, methodsList);
          }

          throw new PluginImplementationError(message);
        }
    ...
    _.forEach(plugin.object.hooks, (fn, event) => {
      ...
      for (const target of list) {
        if (typeof target !== 'function'
          && typeof plugin.object[target] !== 'function')
        {
          let message = `Unable to configure hook for event '${event}' with`
          + ` provided method. ${target} should be a plugin method name, or a function.`;
  ...
  _initControllers (plugin) {
    Object.keys(plugin.object.controllers).forEach(controller => {
      ...
      const
        ...
        errorControllerPrefix = `Unable to inject controller "${controller}"`
          + ` from plugin "${plugin.manifest.name}":`;

      if (!_.isPlainObject(description)) {
        throw new PluginImplementationError(
          `${errorControllerPrefix} Incorrect controller description type`
          + ` (expected object, got: "${typeof description}")`);
      }
      ...
      Object.keys(description).forEach(action => {
        ...
        {
          let message = `${errorControllerPrefix} Action for `
            + `${controller}:${action}' is not a function.`;
    ...
    if (plugin.object.routes) {
      plugin.object.routes.forEach(route => {
        const
          ...
          errorRoutePrefix = `Unable to inject api route "${JSON.stringify(route)}"`
            + ` from plugin "${plugin.manifest.name}":`;

        Object.keys(route).forEach(key => {
          if (routeProperties.indexOf(key) === -1) {
            throw new PluginImplementationError(
              `${errorRoutePrefix} Unknown property "${key}" in route definition.`
          ...
          {
            throw new PluginImplementationError(
              `${errorRoutePrefix} "${key}" must be a non-empty string`);
          }
        });

        if (!this.controllers[controllerName]) {
          throw new PluginImplementationError(
            `${errorRoutePrefix} Undefined controller "${route.controller}".`
        ...
        if (!this.controllers[controllerName][route.action]) {
          ...
          throw new PluginImplementationError(
            `${errorRoutePrefix} Undefined action "${route.action}".`
            + didYouMean(route.action, actionNames));
        }

        if (httpVerbs.indexOf(route.verb.toLowerCase()) === -1) {
          throw new PluginImplementationError(
            `${errorRoutePrefix} Only following http verbs are allowed: "${httpVerbs.join(', ')}".`
