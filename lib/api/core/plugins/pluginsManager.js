...
function loadPlugins(config) {
  const pluginsDir = path.resolve('/var/app/plugins/enabled');
  let loadedPlugins = {};

  const pluginList = fs.readdirSync(pluginsDir)
    .filter((element) => {
      const elStat = fs.lstatSync(path.join(pluginsDir, element));
      return elStat.isSymbolicLink() || elStat.isDirectory();
    });
  ...
  pluginList.forEach((pluginDirName) => {
    ...
    if (plugin) {
      ...
      console.error(`ERROR: unable to load the plugin at ${pluginDirName}. Invalid structure.`); // eslint-disable-line no-console
    }
  });
