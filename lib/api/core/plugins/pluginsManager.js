  ...
  _initControllers (plugin) {
    ...
    if (plugin.object.routes) {
      plugin.object.routes.forEach(route => {
        const
          routeController = `${plugin.manifest.name}/${route.controller}`,
        ...
        if (!this.controllers[routeController]) {
          throw new PluginImplementationError(
            `${errorRoutePrefix} Undefined controller "${route.controller}".`
            + didYouMean(route.controller, controllerNames));
        }

        if (!this.controllers[routeController][route.action]) {
          const actionNames = Object.keys(this.controllers[routeController]);
          throw new PluginImplementationError(
