...
class PluginsManager {
    ...
    this.pipes[event].push((data, callback) => {
      ...
      if (timeoutDelay) {
        pipeTimeoutTimer = setTimeout(() => {
          const errorMsg = `Timeout error. Plugin pipe ${plugin.manifest.name}:${fn} exceeded ${timeoutDelay}ms to execute. Aborting.`;
  ...
  load() {
    ...
    for (let relativePluginPath of plugins) {
      const pluginPath = path.resolve(this.pluginsDir, relativePluginPath);

      const
        manifest = new Manifest(this.kuzzle, pluginPath, manifestFallback),
        plugin = {
          manifest,
          object: null,
...
/**
 * @deprecated - There should be no falling back to package.json in the
 * next major release
 *
 * Called by the Manifest generator class if there is no manifest.json file
 *
 * @param  {Manifest} manifest
 * @param  {string} pluginPath
 * @throws {PluginImplementationError} If there is no package.json file or no name property in it
 */
function manifestFallback(manifest) {
  let packageJson;

  try {
    packageJson = require(path.resolve(manifest.path, 'package.json'));
  }
  catch(e) {
    throw new PluginImplementationError(`[${manifest.path}] No package.json file found.`);
  }

  if (!packageJson.name || packageJson.name.length === 0) {
    throw new PluginImplementationError(`[${manifest.path}] No "name" property provided in package.json`);
  }
  ...
  // Elasticsearch index.
  manifest.name = packageJson.name.toLowerCase();
}
