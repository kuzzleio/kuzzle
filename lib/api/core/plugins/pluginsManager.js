...
class PluginsManager {
  ...
  load(additionalPlugins = []) {
    ...
    for (const relativePluginPath of plugins) {
      ...
      if (!_.has(plugin.manifest.raw, 'errors')) {
        this.kuzzle.log.warn(`[${plugin.manifest.name}] This plugin doesn't have costums errors in the manifest file.`);
      } else {
        try {
          loadPluginsError(plugin.manifest.name, plugin.manifest.raw.errors);
