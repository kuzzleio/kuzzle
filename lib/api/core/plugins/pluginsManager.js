...
function PluginsManager (kuzzle) {
  ...
  this.getPluginsConfig = function getPluginsConfig () {
...
function initWorkers (plugin, pluginName) {
  console.log('initWorkers', plugin, pluginName);
  return pm2Promise
    .then(() => {
      var pm2StartPromise = Promise.promisify(pm2.start);

      console.log('pm2StartPromise', {
...
function pm2Init (workers, pluginsWorker, kuzzleConfig) {
  console.log('pm2Init', workers, pluginsWorker, kuzzleConfig);
