...
class PluginsManager {
  ...
  registerPipe(plugin, warnDelay, timeoutDelay, event, fn) {
    ...
    this.pipes[event].push((data, callback) => {
      ...
      let errorMsg = `Timeout error. Plugin pipe ${plugin.manifest.name}:${fn} exceeded ${timeoutDelay}ms to execute. Aborting.`;
      if (timeoutDelay) {
      ...
      try {
        const cb = (err, request) => {
          ...
          if (!err && !(request instanceof KuzzleRequest)) {
            return callback(new PluginImplementationError(`Plugin pipe ${plugin.manifest.name}:${fn} must resolve to a valid Request object.`));
          }
        ...
        if (typeof pipeResponse === 'object'
          ...
          errorMsg = `Plugin pipe ${plugin.manifest.name}:${fn} must return a valid promise.`;
        }
      }
      catch (error) {
  ...
  _initPipes (plugin, pipeWarnTime, pipeTimeout) {
    ...
    _.forEach(plugin.object.pipes, (fn, event) => {
      ...
      for (const target of list) {
        if (!_.isFunction(target) && !_.isFunction(plugin.object[target])) {
          throw new PluginImplementationError(`Unable to configure pipe for event '${event}' with provided method. ${target} should be a plugin method or a function.`);
        }
  ...
  _initHooks (plugin) {
    _.forEach(plugin.object.hooks, (fn, event) => {
      ...
      for (const target of list) {
        if (!_.isFunction(target) && !_.isFunction(plugin.object[target])) {
          throw new PluginImplementationError(`Unable to configure hook for event '${event}' with provided method. ${target} should be a plugin method or a function.`);
        }
  ...
  _initControllers (plugin) {
    Object.keys(plugin.object.controllers).forEach(controller => {
      ...
      Object.keys(description).forEach(action => {
        ...
        if (!_.isFunction(description[action]) && !_.isFunction(plugin.object[description[action]])) {
        ...
        if (_.isFunction(description[action])) {
