...
class PluginsManager {
  ...
  run() {
    ...
    promises.push(...Object.keys(this.plugins).map(pluginName => {
      ...
      if (Object.keys(pluginsWorker).indexOf(pluginName) > -1) {
      ...
      return new Bluebird((resolve, reject) => {
        ...
        Bluebird.resolve(plugin.object.init(
          ...
          .then(initStatus => {
            if (initStatus === false) {
              return reject(new PluginImplementationError(`Something went wrong during initialization of "${pluginName}" plugin.`));
            }

            if (plugin.object.controllers && !initControllers(this.controllers, this.routes, plugin, pluginName)) {
              return reject(new PluginImplementationError(`Unable to initialize plugin "${pluginName}": controllers are malformed`));
            }

            if (plugin.object.strategies && !injectAuthentication(this.kuzzle, this.authentications, plugin.object, pluginName)) {
              return reject(new PluginImplementationError(`Unable to initialize plugin "${pluginName}": authentications strategies are malformed`));
            }
...
function initControllers (controllers, routes, plugin, pluginName) {
  ...
  if (plugin.object.routes) {
    return plugin.object.routes.every(route => {
      const errorRoutePrefix = `[!] [WARNING][Plugin Manager]: Unable to inject api route "${clc.bold(JSON.stringify(route))}" from plugin "${clc.bold(pluginName)}":`;
