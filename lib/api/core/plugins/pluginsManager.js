  ...
  run() {
    const pluginsWorker = _.pickBy(this.plugins, plugin => {
      if (plugin.config.threads > 0) {
        ...
        console.warn(clc.yellow(`[!] [WARNING][Plugin manager]: Loading plugin "${clc.bold(plugin.name)}" as non worker: this plugin does not support multi-threading; check "threadable" plugin property`)); // eslint-disable-line no-console
      }
      ...
      if (plugin.object.threadable && plugin.config.threads) {
        ...
        return initWorkers(plugin, pluginName, this.kuzzle.config)
          ...
          .catch(err => {
            ...
            return Bluebird.reject(new PluginImplementationError(`Unable to start worker plugin "${pluginName}"; ${err.message}`));
          });
      }
        ...
        return Bluebird.resolve(plugin.object.init(
          ...
          .then(initStatus => {
            ...
            if (plugin.object.controllers) {
              if (!initControllers(this.controllers, this.routes, plugin, pluginName)) {
            ...
            if (plugin.object.strategies) {
              if (!injectAuthentication(this.kuzzle, this.authentications, plugin.object, pluginName)) {
