...
class PluginsManager {
    ...
    return Bluebird.all(Object.keys(this.plugins).map(pluginName => {
      ...
      return Bluebird.resolve(plugin.object.init(
        ...
        .then(initStatus => {
          ...
          if (plugin.object.strategies || plugin.object.authenticators) {
  ...
  registerStrategy(pluginName, strategyName, strategy) {
    if (!this.plugins[pluginName].initialized) {
      throw new PluginImplementationError(`The plugin ${pluginName} is not initialized yet. Did you try to load a new strategy before your init() method has finished?`);
    }
  ...
  _initAuthentication (plugin) {
    ...
    if (!_.isNil(plugin.object.strategies) && (!_.isPlainObject(plugin.object.strategies) || _.isEmpty(plugin.object.strategies))) {
    ...
    // Plugins may declare authenticators without strategies just yet, if
    // those are registered completely dynamically
    if (!_.isNil(plugin.object.strategies)) {
