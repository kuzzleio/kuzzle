...
const
  formatError = errorsManager.wrap('plugins', 'format'),
  runtimeError = errorsManager.wrap('plugins', 'runtime'),
  strategyError = errorsManager.wrap('plugins', 'strategy');
...
class PluginsManager {
    ...
    return new Bluebird((resolve, reject) => {
      async.waterfall(preparedPipes, (error, result) => {
        if (error) {
          ...
          return reject(runtimeError.getErrorFrom(
            error,
            'unexpected_error',
            error.message));
        }
  ...
  validateStrategy (pluginName, strategyName, strategy) {
    ...
    // property
    if ( !_.isNil(strategy.config.constructor)
      && !_.isNative(strategy.config.constructor)
    ) {
      if (!_.isNil(strategy.config.authenticator)) {
        strategyError.throw('unexpected_constructor', errorPrefix);
      }
      ...
      if (isConstructor(strategy.config.constructor)) {
        ...
        strategyError.throw('invalid_constructor', errorPrefix);
      }
    }
    ...
    for (const opt of ['strategyOptions', 'authenticateOptions']) {
      ...
      if (!_.isNil(obj) && !_.isPlainObject(obj)) {
        strategyError.throw('expected_object', errorPrefix, opt, obj);
      }
    }
        ...
        return new Bluebird((resolve, reject) => {
          try {
            ...
            reject(error instanceof KuzzleError
              ? error
              : runtimeError.getErrorFrom(error, 'unexpected_error', error.message)
            );
          }
        });
      };
        ...
        ret
          .then(result => {
            ...
            if (!_.isPlainObject(result)) {
              strategyError.throw('invalid_result', errorPrefix);
            }
    ...
    _.forEach(plugin.object.pipes, (fn, event) => {
      ...
      for (const target of list) {
        if (typeof target !== 'function'
          ...
          formatError.throw('invalid_pipe', event, target, message);
        }
    ...
    _.forEach(plugin.object.hooks, (fn, event) => {
      ...
      for (const target of list) {
        if (
          ...
          formatError.throw('invalid_hook', event, target, message);
        }
  ...
  _initControllers (plugin) {
    const controllerError = errorsManager.wrap('plugins', 'controller');
    ...
    Object.keys(plugin.object.controllers).forEach(controller => {
      ...
      Object.keys(description).forEach(action => {
        ...
        if ( typeof description[action] !== 'function'
          ...
          let suggestion = typeof description[action] === 'string'
      ...
      plugin.object.routes.forEach(route => {
        ...
        Object.keys(route).forEach(key => {
          ...
          if (typeof route[key] !== 'string'
            ...
            controllerError.throw('cannot_be_empty', errorRoutePrefix, key);
          }
        });
  ...
  _initAuthenticators (plugin) {
    ...
    if (!_.isNil(plugin.object.authenticators)) {
      if (!_.isPlainObject(plugin.object.authenticators)) {
        strategyError.throw('invalid_authenticators', errorPrefix);
      }
  ...
  load(additionalPlugins = []) {
    ...
    try {
      ...
      formatError.throw('invalid_plugins_dir', this.pluginsDir, e.message);
    }
    ...
    for (const plugin of plugins) {
      ...
      try {
        ...
        formatError.throw('cannot_load', pluginPath, e.message);
      }
    }
    ...
    for (const relativePluginPath of plugins) {
      ...
      try {
        ...
        if (e.message.match(/not a constructor/i)) {
          formatError.throw('expected_constructor', plugin.manifest.name);
        }
      ...
      if (typeof plugin.object.init !== 'function') {
        formatError.throw('init_not_found', plugin.manifest.name);
      }
      ...
      if (plugin.config.privileged) {
        if (!plugin.manifest.privileged) {
          formatError.throw('privileged_not_supported', plugin.manifest.name);
        }
      } else if (plugin.manifest.privileged) {
        formatError.throw('privileged_not_set', plugin.manifest.name);
      }

      const key = plugin.manifest.name.toLowerCase();

      if (loadedPlugins[key]) {
        formatError.throw('name_already_exists', key);
      }
