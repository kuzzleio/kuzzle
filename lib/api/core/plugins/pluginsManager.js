    ...
    this.pipes[event].push((data, callback) => {
      ...
      if (warnDelay) {
        pipeWarnTimer = setTimeout(() => {
          this.trigger('log:warn', `Plugin ${name} pipe for event '${event}' exceeded ${warnDelay}ms to execute.`);
        }, warnDelay);
      }
      ...
      try {
        ...
        cb(new PluginImplementationError(
          `Plugin ${name} pipe for event '${event}' throwed a non-Kuzzle error: ${error}`));
      }
    });
  }
  ...
  return new Bluebird((resolve, reject) => {
    async.waterfall([callback => callback(null, data)].concat(preparedPipes), (error, result) => {
      ...
      if (!(result instanceof Request)) {
        return reject(new PluginImplementationError(`Invalid return value on pipe '${event}': expected a Request object, got '${result}'`));
      }
