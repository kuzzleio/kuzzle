...
const
  ...
  { loadPluginsErrors } = require('../../../config/error-codes/index');
  ...
  load() {
    ...
    for (const relativePluginPath of plugins) {
      ...
      if (!plugin.manifest.raw.errors) {
        this.kuzzle.log.warn(`[${plugin.manifest.name}] This plugin doesn't have custom errors in the manifest file.`);
      } else {
        try {
          const pluginCode = this.config[plugin.manifest.name] && this.config[plugin.manifest.name].code
            ? this.config[plugin.manifest.name].code
            : 0x00;
          ...
          if (e.message.match(/Error configuration file/i)
