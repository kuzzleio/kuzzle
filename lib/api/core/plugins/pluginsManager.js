...
const
  ...
  loadPluginsError = require('../../../config/error-codes/index').loadPluginsError;
...
class PluginsManager {
  ...
  load(additionalPlugins = []) {
    ...
    for (const relativePluginPath of plugins) {
      ...
      try {
        const pluginErrors = require(`${pluginPath}/${plugin.manifest.name}-errors`);
        loadPluginsError(plugin.manifest.name, pluginErrors);
        this.kuzzle.log.info(`[${plugin.manifest.name}] Errors configuration file '${plugin.manifest.name}-errors.json' successfully loaded.`);
      } catch (e) {
        if (e.message.match(/Error (codes|configuration) file/i)
          ...
          errorsManager.throw(
            ...
            `${plugin.manifest.name}-errors.json`,
            e.message);
        } else if (e.message.match(/Cannot find module/i)) {
          this.kuzzle.log.warn(`[${plugin.manifest.name}] This plugin doesn't have an error configuration file '${plugin.manifest.name}-errors.json'.`);
        } else {
          this.kuzzle.log.warn(`[${plugin.manifest.name}] ${e}.`);
        }
      }
