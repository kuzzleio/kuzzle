    ...
    return new Bluebird((resolve, reject) => {
      async.waterfall(preparedPipes, (error, result) => {
        if (error) {
          ...
          return reject(errorsManager.getFrom(error, 'plugins', 'runtime', 'plugin_error'));
        }
    ...
    this.pipes[event].push((data, ...info) => {
      ...
      if (timeoutDelay) {
        pipeTimeoutTimer = setTimeout(() => {
          ...
          timedOut = true;
          callback(errorsManager.get('plugins', 'runtime', 'register_pipe_timeout', errorMsg));
        }, timeoutDelay);
      }
          ...
          try {
            ...
            reject(
              error instanceof KuzzleError
                ? error
                : errorsManager.getFrom(error, 'plugins', 'runtime', 'plugin_error')
            );
          }
        });
      };
    ...
    const
      ...
      verifyAdapter = (...args) => {
        ...
        if (!ret || !_.isFunction(ret.then)) {
          return callback(errorsManager.get(
            'plugins',
            'runtime',
  ...
  load(additionalPlugins = []) {
    ...
    for (const relativePluginPath of plugins) {
      ...
      try {
        ...
        errorsManager.throwFrom(e, 'plugins', 'runtime', 'plugin_error');
      }
