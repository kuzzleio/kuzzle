    ...
    this.pipes[event].push((data, callback) => {
      ...
      if (warnDelay) {
        pipeWarnTimer = setTimeout(() => {
          this.trigger('log:warn', `Plugin ${plugin.manifest.name} pipe for event '${event}' exceeded ${warnDelay}ms to execute.`);
        }, warnDelay);
      }
      ...
      try {
        const cb = (err, request) => {
          ...
          if (!err && !(request instanceof KuzzleRequest)) {
            return callback(new PluginImplementationError(`Plugin ${plugin.manifest.name} pipe for event '${event}' must resolve to a valid Request object.`));
          }
        ...
        if (typeof pipeResponse === 'object'
          ...
          errorMsg = `Plugin ${plugin.manifest.name} pipe for event '${event}' must return a valid promise.`;
        }
      }
      catch (error) {
