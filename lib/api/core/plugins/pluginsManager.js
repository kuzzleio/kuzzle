    ...
    this.pipes[event].push((data, callback) => {
      ...
      try {
        ...
        if (typeof pipeResponse === 'object'
          ...
          pipeResponse
            .then(response => {
              if (!(response instanceof KuzzleRequest)) {
                throw new PluginImplementationError(`Plugin pipe ${plugin.manifest.name}:${fn} must resolve to a valid Request object.`);
              }
          ...
          throw new PluginImplementationError(`Plugin pipe ${plugin.manifest.name}:${fn} must return a valid promise.`);
        }
      }
      catch (error) {
