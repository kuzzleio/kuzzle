  ...
  run() {
    const pluginsWorker = _.pickBy(this.plugins, plugin => {
      if (plugin.config.threads > 0) {
        ...
        console.warn(`WARNING: Unable to init worker plugin "${plugin.name}": this plugin does not support multi-threading; check "threadable" plugin property; falling back to reagular plugin`); // eslint-disable-line no-console
      }
  ...
  const pluginList = fs.readdirSync(pluginsDir)
    .filter(element => {
      ...
      try {
        ...
        console.warn(`WARNING: Unable to load plugin from path "${pluginPath}"; this may occurs with dead symbolic links; ${error.message}`); // eslint-disable-line no-console
        return false;
      }
    });
  ...
  Object.keys(plugin.strategies).forEach(strategyName => {
    ...
    if (!isObject(strategy.methods)) {
      ...
      if (strategy.methods.getById && (!isString(strategy.methods.getById) || !isFunction(plugin[strategy.methods.getById]))) {
        // eslint-disable-next-line no-console
        console.error(`${errorPrefix} The plugin property "${strategy.methods.getById}" must be a function.`);
        valid = false;
      }
      ...
      if (strategy.methods.afterRegister && (!isString(strategy.methods.afterRegister) || !isFunction(plugin[strategy.methods.afterRegister]))) {
        // eslint-disable-next-line no-console
        console.error(`${errorPrefix} The plugin property "${strategy.methods.afterRegister}" must be a function.`);
        valid = false;
      }
    }
