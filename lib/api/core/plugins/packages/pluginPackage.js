...
PluginPackage.prototype.needsInstall = function pluginPackageNeedsInstall () {
  ...
  return this.dbConfiguration()
    .then(config => {
      ...
      return config.hasOwnProperty('url') || config.hasOwnProperty('path') || compareVersions(config.version, this.localVersion()) > 0;
    })
    .catch(error => {
...
PluginPackage.prototype.install = function pluginPackageInstall () {
  ...
  return Promise.promisify(exec)(`npm install ${pkg}`)
          ...
          if (r) {
            ...
            if (this.name && this.name !== r[1]) {
              ...
              if (this.kuzzle.pluginsManager.isInit) {
                // the plugin manager is init. We are most likely running action for the cli
    ...
    .catch(error => {
      this.kuzzle.pluginsManager.trigger('log:error', 'ERROR: Unable to install plugin: '.concat(pkg));
