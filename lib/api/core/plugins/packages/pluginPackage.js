...
PluginPackage.prototype.setDefinition = function (definition) {
  ['path', 'gitUrl', 'url', 'npmVersion', 'version', 'activated'].forEach(k => {
...
PluginPackage.prototype.localConfiguration = function () {
  ...
  // allow default plugin configuration to be overriden by kuzzle
...
PluginPackage.prototype.needsInstall = function () {
  ...
  return this.dbConfiguration()
    .then(config => {
      ...
      return compareVersions(config.version, this.localVersion());
    })
    .catch(error => {
  ...
  return this.dbConfiguration()
    .then(definition => {
      ...
      if (definition.config[key] === undefined) {
        return Promise.reject(`Property ${key} not found in plugin configuration`);
      }
...
PluginPackage.prototype.importConfigurationFromFile = function (path) {
  ...
  try {
    ...
    return Promise.reject(new Error(`Error opening file ${path}: ${err.message}`));
  }
  ...
  try {
    ...
    return Promise.reject(new Error(`Unable to parse ${path}: ${err}`));
  }
