...
class Janitor {
  ...
  loadFixtures (fixtures = {}) {
    return Bluebird.resolve()
      ...
      .each(request => {
        return this.kuzzle.storageEngine.public.import(request)
          .then(res => {
          ...
          .then(() => {
            ...
            return this.kuzzle.storageEngine.public.refreshIndex(indexRequest);
          });
      });
  }
  ...
  loadMappings (mappings = {}) {
    return Bluebird.resolve()
      ...
      .each(request => {
        ...
        return this.kuzzle.storageEngine.public.indexExists(indexRequest)
          .then(exist => {

            if (! exist) {
              return this.kuzzle.storageEngine.public.createIndex(indexRequest);
            }

            return null;
          })
          .then(() => this.kuzzle.storageEngine.public.createCollection(request))
          .then(() => this.kuzzle.storageEngine.public.refreshIndex(indexRequest))
          .then(() => {
