...
class Janitor {
  ...
  loadFixtures (fixtures = {}) {
    return Bluebird.resolve()
      ...
      .each(request => {
        return this.kuzzle.services.publicStorage.import(request)
          .then(res => {
            if (res.partialErrors && res.partialErrors.length > 0) {
              errorsManager.throw('internal', 'janitor', 'import_data', request.input.resource.index, request.input.resource.collection, res.partialErrors.length, res.items.length + res.partialErrors.length, res.partialErrors);
            }
          })
          .then(() => {
            const indexRequest = new Request({ index: request.input.resource.index });

            return this.kuzzle.services.publicStorage.refreshIndex(indexRequest);
          });
      });
  }
  ...
  loadMappings (mappings = {}) {
    return Bluebird.resolve()
      ...
      .each(request => {
        ...
        return this.kuzzle.services.publicStorage.indexExists(indexRequest)
          .then(exist => {

            if (! exist) {
              return this.kuzzle.services.publicStorage.createIndex(indexRequest);
            }

            return null;
          })
          .then(() => this.kuzzle.services.publicStorage.createCollection(request))
          .then(() => this.kuzzle.services.publicStorage.refreshIndex(indexRequest))
          .then(() => {
  ...
  dump (suffix) {
    if (this._dump) {
      return Bluebird.reject(errorsManager.getError('internal', 'janitor', 'dump_already_generated'));
    }
    ...
    return new Bluebird((resolve, reject) => {
      ...
      fs.readdir(logDir, (err, files) => {
        if (err) {
          return reject(errorsManager.getError('internal', 'janitor', 'reading_log_directory', logDir, err));
        }
