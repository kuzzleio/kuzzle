...
const
  ...
  errorsManager = require('../../config/error-codes/throw').wrap('internal', 'janitor'),
...
class Janitor {
          ...
          .then(res => {
            if (res.partialErrors && res.partialErrors.length > 0) {
              errorsManager.throw(
                ...
                res.partialErrors.length,
                res.items.length + res.partialErrors.length,
                res.partialErrors);
            }
          })
          .then(() => {
  ...
  dump (suffix) {
    if (this._dump) {
      return errorsManager.reject('dump_already_generated');
    }
    ...
    return new Bluebird((resolve, reject) => {
      ...
      fs.readdir(logDir, (err, files) => {
        if (err) {
          return reject(
            errorsManager.getError('reading_log_directory', logDir, err));
        }
