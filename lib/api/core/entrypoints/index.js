...
class EntryPoint {
  ...
  dispatch (event, data) {
    switch (event) {
      ...
      case 'shutdown':
        ...
        return;
      default:
        errorsManager.throw('unknown_event_received', event);
    }
  }
  ...
  init () {
    ...
    if (!Number.isInteger(this.config.port)) {
      errorsManager.throw('invalid_network_port_number', this.config.port);
    }
      ...
      return Bluebird.resolve()
        ...
        .then(() => {
          if (this.protocols[manifest.name]) {
            errorsManager.throw('conflicting_protocol_name', manifest.name);
          }
  ...
  */
  ...
  _isShuttingDownError (request, cb) {
    request.setError(errorsManager.getError('shutting_down'));
