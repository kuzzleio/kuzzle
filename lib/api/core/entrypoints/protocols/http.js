  ...
  get content () {
    if (this.json === null && this.raw !== null) {
      try {
        ...
        errorsManager.throw('unable_to_convert_http_body_to_json');
      }
    }
...
class HttpProtocol extends Protocol {
  ...
  onMessage(request, response) {
    ...
    debug('[%s] Received HTTP request: %a', message);
    ...
    if (request.headers['content-length'] > this.maxRequestSize) {
      ...
      this._replyWithError(
        message,
        response,
        errorsManager.getError('http_request_size_exceeded'));
      return;
    }
    ...
    request.on('error', err => {
      const kerr = err instanceof KuzzleError
        ? err
        : errorsManager.getError('http_request_error', err);
    ...
    this.entryPoint.kuzzle.router.http.route(message, request => {
      ...
      this._compress(data, response, message.headers, (err, deflated) => {
        if (err) {
          ...
          this._replyWithError(message, response, kuzerr);
          return;
  ...
  _replyWithError(message, response, error) {
    ...
    const kerr = error instanceof KuzzleError
      ? error
      : errorsManager.getError('http_request_error', error);
    ...
    if (request.headers['content-encoding']) {
      ...
      for (const encoding of encodings) {
        ...
        if (this.decoders[encoding]) {
          ...
          request.removeAllListeners().resume();
          errorsManager.throw('unsupported_compression_algorithm', encoding);
        }
  ...
  _createWritableStream(request, message) {
    if (request.headers['content-type'] &&
      ...
      try {
        ...
        errorsManager.throw('http_request_error', error.message);
      }
    }
    ...
    return new Writable({
      write(chunk, encoding, callback) {
        ...
        if (streamLength > maxRequestSize) {
          callback(errorsManager.getError('http_request_size_exceeded'));
          return;
        }
  ...
  _setDecoders() {
    const
      ...
      disabledfn = () => {
        errorsManager.throw('compression_support_disabled');
      };
