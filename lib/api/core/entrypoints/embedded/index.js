...
const
  bearerRegexp = /^Bearer /i,
  networkError = errorsManager.wrap('network', 'entrypoint');
...
class EmbeddedEntryPoint extends EntryPoint {
  ...
  dispatch (event, data) {
    ...
    networkError.throw('unexpected_event', event);
  }
  ...
  init () {
    ...
    if (! Number.isInteger(this.config.port)) {
      networkError.throw('invalid_port', this.config.port);
    }
