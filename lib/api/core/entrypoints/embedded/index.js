...
class EmbeddedEntryPoint extends EntryPoint {
  ...
  logAccess (request, extra = null) {
    let
      connection = this.clients[request.context.connection.id],
      verb = 'DO',
      user = null,
      url;

    // user init: prioritize the already identified user stored in the request,
    // to prevent costly token decoding
    ...
    if (request.context.user !== null) {
      if (request.context.user._id === this.anonymousUserId) {
        user = '(anonymous)';
      } else if (request.context.token !== null) {
    ...
    if (!connection) {
      connection = new ClientConnection(
        request.context.connection.protocol ? request.context.connection.protocol.toUpperCase() : '[unknown protocol]',
