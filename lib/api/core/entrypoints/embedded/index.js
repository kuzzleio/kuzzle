...
const
  ...
  Manifest = require('../../plugins/manifest');
  ...
  loadMoreProtocols () {
    ...
    return Bluebird.map(dirs, protoDir => {
      const
        ...
        manifest = new Manifest(this.kuzzle, protoDir, m => {
          // @deprecated - This fallback is obsolete and will be removed in the next
          // major release of Kuzzle
          if (!protocol.protocol) {
            throw new PluginImplementationError(`[${m.path}] Missing "protocol" property`);
          }
          m.name = protocol.protocol;
        }),
        name = manifest.name;
