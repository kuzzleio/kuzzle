...
class WebSocketProtocol extends Protocol {
   ...
   * module, to prevent recomputing the same frame over and over for each
   * broadcast.
   *
  ...
  broadcast(data) {
    ...
    payload.write(jsonRoomProp, size);
    size += jsonRoomProp.length;
    ...
    for (const channel of data.channels) {
      ...
      if (connectionIds) {
        ...
        const frame = Sender.frame(payloadSafeCopy, {
          fin: true,
          rsv1: false,
          opcode: 1,
          mask: false,
