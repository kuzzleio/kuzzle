...
class WebSocketProtocol extends Protocol {
  constructor () {
    ...
    // to have a timestamp property refreshed every second or so
   ...
   * destination sockets. This is done by this function instead of the WebSocket
   * module, to prevent recomputing the same frame over and over again for each
   * broadcast.
   *
  ...
  broadcast(data) {
    const
      ...
      // (with current channel encofding, this is less than 100 bytes)
      ...
      if (connectionIds) {
        ...
        for (const connectionId of connectionIds) {
          ...
          if (connection && connection.alive &&
            ...
            connection.lastActivity = this.activityTimestamp;
