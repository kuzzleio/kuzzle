...
class HttpProtocol extends Protocol {
  ...
  onMessage(request, response) {
    ...
    if (request.headers['content-length'] > this.maxRequestSize) {
      ...
      this._replyWithError(
        ...
        proxyRequest,
        response,
        this.getError('http_request_size_exceeded'));
      return;
    }
    ...
    request.on('error', err => {
      const kerr = err instanceof KuzzleError
        ? err
        : this.getError('http_request_error', err);
      ...
      this._compress(data, response, proxyRequest.headers, (err, deflated) => {
        if (err) {
          const kuzerr = err instanceof KuzzleError 
            ? err
            : this.getError('http_request_error', err.message);
  ...
  _replyWithError(connection, proxyRequest, response, error) {
    ...
    const kerr = error instanceof KuzzleError
      ? error
      : this.getError('http_request_error', error);
    ...
    if (request.headers['content-encoding']) {
      ...
      for (const encoding of encodings) {
        ...
        if (this.decoders[encoding]) {
          ...
          request.removeAllListeners().resume();
          this.throw('unsupported_compression_algorithm', encoding);
        }
  ...
  _createWritableStream(request, proxyRequest) {
    if (request.headers['content-type'] &&
      ...
      try {
        ...
        this.throw('http_request_error', error.message);
      }
    }
    ...
    return new Writable({
      write(chunk, encoding, callback) {
        ...
        if (streamLength > maxRequestSize) {
          callback(errorsManager.getError('protocols', 'http', 'http_request_size_exceeded'));
          return;
        }
  ...
  _setDecoders() {
    const
      ...
      disabledfn = () => {
        this.throw('compression_support_disabled');
      };
