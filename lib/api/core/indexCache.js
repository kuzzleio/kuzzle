...
class IndexCache {
  ...
  constructor (kuzzle) {
    ...
    this.kuzzle = kuzzle;
    this.commonMapping = {
      properties: this.kuzzle.config.services.db.commonMapping
    };
  }
        ...
        for (const index of indexes) {
          ...
          promises.push(this.kuzzle.internalEngine.listCollections(index)
            .then(collections => {
              ...
              return Bluebird.map(collections, collection => this.kuzzle.internalEngine.updateMapping(collection, this.commonMapping, index));
            })
          );
        }
   ...
   * @returns {Promise} resolve if index or collection exists, reject with PreconditionError otherwise
   */
