...
const
  ...
  path = require('path'),
  fs = require('fs'),
  errorsManager = require('../../util/errors').wrap('core', 'vault');

class Vault {
  constructor () {
    this.defaultEncryptedSecretsFile = path.resolve(`${__dirname}/../../../config/secrets.enc.json`);
    this.defaultSecretsFile = path.resolve(`${__dirname}/../../../config/secrets.json`);
    this.secrets = {};
  }
  ...
  init (secretsFile = null) {
    this.encryptedSecretsFile = secretsFile || process.env.KUZZLE_SECRETS_FILE || this.defaultEncryptedSecretsFile;
    ...
    return this._readJsonFile(this.encryptedSecretsFile)
      ...
      .catch(error => {
        errorsManager.throw('cannot_decrypt', error.message);
      });
  }
  ...
  decryptObject (encryptedSecrets) {
    if (!this.vaultKey) {
      errorsManager.throw('key_not_found');
    }
