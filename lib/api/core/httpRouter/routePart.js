 ...
 * @param {Array<string>} parameters - sorted array to populate the list parametric values
 * @return {RouteHandler} registered function handler
 */
function getHandlerPart (routePart, parts, routeHandler, parameters = []) {
  let part;
  ...
  if (part === undefined) {
    ...
    if (routePart.parameters !== null) {
      for (const i of Object.keys(routePart.parameters)) {
        routeHandler.addArgument(routePart.parameters[i], parameters[i]);
      }
    }
    return routeHandler;
  }
  ...
  if (routePart.subparts[part]) {
    return getHandlerPart(routePart.subparts[part], parts, routeHandler, parameters);
  }

  if (routePart.subparts['*']) {
    parameters.push(part);
    return getHandlerPart(routePart.subparts['*'], parts, routeHandler, parameters);
  }
