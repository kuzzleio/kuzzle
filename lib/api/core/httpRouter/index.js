...
const
  errorsManager = require('../../../config/error-codes/throw').wrap('entrypoints', 'http_router'),
  ...
  route(httpRequest, cb) {
    ...
    if (!this.routes[httpRequest.method]) {
      ...
      replyWithError(
        cb,
        request,
        errorsManager.getError('unrecognized_http_method', httpRequest.method));
    ...
    try {
      ...
      && !httpRequest.headers['content-type'].startsWith('application/json')) {
        errorsManager.throw('invalid_request_content_type', httpRequest.headers['content-type']);
      }
      ...
      const e = (err instanceof KuzzleError) && err || errorsManager.getError('request_error', err);
