...
Router.prototype.route = function route (request, cb) {
  ...
  routeHandler = this.routes[request.method].getHandler(parsed.pathname);
  ...
  if (routeHandler !== null) {
    ...
    ['index', 'collection'].forEach(attr => {
      if (routeHandler.args[attr]) {
        base[attr] = routeHandler.args[attr];
        delete routeHandler.args[attr];
      }
    });

    requestObject = new RequestObject(base, routeHandler.args, 'rest');
    Object.assign(requestObject.data.body, parsed.query);
    ...
    if (request.content.length > 0) {
      if (!request.headers['content-type'] || request.headers['content-type'].startsWith('application/json')) {
        ...
        try {
          ...
          routeHandler.handler(requestObject, cb);
        }
        catch (e) {
