...
Router.prototype.route = function route (request, cb) {
  let
    ...
    requestObject = new RequestObject({}, parsed.query, 'rest');
    ...
    if (request.content.length > 0) {
      if (!request.headers['content-type'] || request.headers['content-type'].startsWith('application/json')) {
        try {
          ...
          if (pojo.body) {
            Object.assign(requestObject.data, pojo);
          }
          else {
            requestObject.data.body = pojo;
          }

          handler(requestObject, cb);
        }
        catch (e) {
          replyWithError(cb, requestObject, new BadRequestError('Unable to convert incoming POST data to JSON'));
        }
      }
      else {
        replyWithError(cb, requestObject, new BadRequestError(`Invalid request content-type. Expected "application/json", got: "${requestObject.data.body.headers['content-type']}"`));
      }
    }
    else {
      handler(requestObject, cb);
