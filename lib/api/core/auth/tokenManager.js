...
class TokenManager {
  ...
  getConnectedUserToken(userId, connectionId) {
    const data = this.tokensByConnectedUser[`${userId}#${connectionId}`];
  ...
  _add(token, connectionId, roomIds = []) {
    ...
    this.tokensByConnectedUser[`${data.userId}#${data.connectionId}`] = data;
  ...
  _deleteByIndex(index) {
    ...
    delete this.tokensByConnectedUser[`${data.userId}#${data.connectionId}`];
