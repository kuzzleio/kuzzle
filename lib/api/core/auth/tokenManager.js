...
function TokenManager (kuzzle) {
   ...
   * @return {boolean}
   */
  this.add = (requestContext) => {
    if (requestContext.connectionId !== null && requestContext.token._id) {
      let idx = getTokenIndex(requestContext.token);

      this.tokens.insert({
        _id: requestContext.token._id,
        expiresAt: requestContext.token.expiresAt,
        idx,
