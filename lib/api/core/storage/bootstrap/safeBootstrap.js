...
class SafeBootstrap {
   ...
   * @param {IndexEngine} indexEngine - Index engine
   * @param {Number} timeout - Bootstrap timeout
   */
  constructor (kuzzle, indexEngine, timeout) {
    this.kuzzle = kuzzle;
    this.indexEngine = indexEngine;
    this.attemptDelay = Math.round(timeout / 10);

    // IDs for config documents
    this._LOCK_ID = `${this.indexEngine.index}.lock`;
    this._BOOTSTRAP_DONE_ID = `${this.indexEngine.index}.done`;
  }
  ...
  async startOrWait (...args) {
    const bootstrapDone =
      await this.indexEngine.exists('config', this._BOOTSTRAP_DONE_ID);
  ...
  async _waitTillUnlocked (attempts = 0) {
    const exists = await this.indexEngine.exists('config', this._LOCK_ID);
  ...
  async _isLocked () {
    ...
    try {
      lock = await this.indexEngine.get('config', this._LOCK_ID);
    } catch (error) {
      if (error.status !== 404) {
        throw error;
      }
    ...
    if (lock._source.timestamp < thirtySecondsAgo) {
      await this.indexEngine.createOrReplace(
        'config',
        this._LOCK_ID,
        { timestamp: Date.now() });
   ...
   */
  _unlock () {
    return this.indexEngine.delete('config', this._LOCK_ID);
  }
}
