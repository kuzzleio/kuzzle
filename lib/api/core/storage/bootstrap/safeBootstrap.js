...
class SafeBootstrap {
   ...
   * @param {IndexStorage} indexStorage - Index engine
   * @param {Number} timeout - Bootstrap timeout
   */
  constructor (kuzzle, indexStorage, timeout) {
    this.kuzzle = kuzzle;
    this.indexStorage = indexStorage;
    this.attemptDelay = Math.round(timeout / 10);

    // IDs for config documents
    this._LOCK_ID = `${this.indexStorage.index}.lock`;
    this._BOOTSTRAP_DONE_ID = `${this.indexStorage.index}.done`;
  }
  ...
  async startOrWait (...args) {
    const bootstrapDone =
      await this.indexStorage.exists('config', this._BOOTSTRAP_DONE_ID);
  ...
  async _waitTillUnlocked (attempts = 0) {
    const exists = await this.indexStorage.exists('config', this._LOCK_ID);
  ...
  async _isLocked () {
    ...
    try {
      lock = await this.indexStorage.get('config', this._LOCK_ID);
    } catch (error) {
      if (error.status !== 404) {
        throw error;
      }
    ...
    if (lock._source.timestamp < thirtySecondsAgo) {
      await this.indexStorage.createOrReplace(
        'config',
        this._LOCK_ID,
        { timestamp: Date.now() });
   ...
   */
  _unlock () {
    return this.indexStorage.delete('config', this._LOCK_ID);
  }
}
