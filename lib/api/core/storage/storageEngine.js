...
class StorageEngine {
  constructor(kuzzle) {
    ...
    const publicESClient = new Elasticsearch(
      this._kuzzle,
      kuzzle.config.services.db,
      'public');
    this._publicClient = new ClientAdapter(publicESClient, this.indexCache);

    // Storage client for internal indexes only
    const internalESClient = new Elasticsearch(
      this._kuzzle,
      kuzzle.config.services.db,
      'internal');
    this._internalClient = new ClientAdapter(internalESClient, this.indexCache);
  }
  ...
  async _populateIndexCache () {
    ...
    for (const publicIndex of publicIndexes) {
      if (internalIndexes.includes(publicIndex)) {
        errorsManager.throw('duplicated_index_name');
      }
    }
