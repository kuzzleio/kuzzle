...
class ClientAdapter {
  ...
  async createCollection (index, collection, mappings) {
    const response = await this._client.createCollection(index, collection, mappings);

    this._indexCache.add({ index, collection, scope: this._client.scope });
  ...
  async deleteCollection (index, collection) {
    ...
    this._indexCache.remove({ index, collection, scope: this._client.scope });
  ...
  _assertIndexAndCollectionExists (index, collection) {
    ...
    if (! exists) {
      errorsManager.throw('unknown_collection', collection);
    }
  }
