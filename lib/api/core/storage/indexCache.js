...
class IndexCache {
   ...
   */
  add ({ index, collection, scope='public' } = {}) {
    let modified = false;
   ...
   * @returns {boolean}
   */
  exists ({ index, collection=null, scope='public' } = {}) {
    if (! collection) {
   ...
   * @returns {boolean}
   */
  remove ({ index, collection, scope='public' }) {
    let modified = false;
  ...
  _indexExists (index, scope = 'public') {
    return !!this.indexes[index] && this.indexes[index].scope === scope;
  }
