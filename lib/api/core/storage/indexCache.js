...
class IndexCache {
   ...
   */
  add ({ index, collection, indexType='public' } = {}) {
    let modified = false;
   ...
   * @returns {boolean}
   */
  remove ({ index, collection, indexType='public' }) {
    let modified = false;

    if (this.indexes[index] && this.indexes[index].indexType === indexType) {
      if (collection) {
    ...
    if (modified) {
      this.kuzzle.emit('core:indexCache:remove', { index, collection, indexType });
    }
   ...
   * @param {String} indexType - Index type (public or internal)
   *
   * @returns {boolean}
   */
  ...
  _addCollections (indexes, indexType) {
    const
      storageEngine = indexType === 'public'
        ? this.publicStorage
  ...
  async _addCollectionsAliases () {
    ...
    for (const { name, index, collection } of aliases) {
      if (! this.exists({ index, collection, indexType: 'public' })) {
        errorsManager.throw(
          'internal',
          'index_cache',
