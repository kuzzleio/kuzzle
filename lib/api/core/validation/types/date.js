...
class DateType extends BaseType {
  ...
  validate(typeOptions, fieldValue, errorMessages) {
    ...
    if (_.isPlainObject(typeOptions.range)) {
  ...
  validateFieldSpecification(typeOptions) {
    if (_.has(typeOptions, 'formats')) {
      if (
        ...
        errorsManager.throw('internal', 'validation', 'invalid_formats_option');
      }
    ...
    if (_.has(typeOptions, 'range')) {
      ...
      if (_.has(typeOptions.range, 'min')) {
        min = convertRangeValue('min', typeOptions.range.min);
      }

      if (_.has(typeOptions.range, 'max')) {
...
function convertRangeValue(name, value) {
  ...
  if (typeof value === 'string') {
    ...
    errorsManager.throw('internal', 'validation', 'invalid_range_format', `range.${name}`);
  }

  if (converted && !converted.isValid()) {
    errorsManager.throw('internal', 'validation', 'unable_to_parse_range_value', value);
  }
