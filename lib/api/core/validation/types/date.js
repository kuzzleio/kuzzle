...
util.inherits(DateType, BaseConstructor);
 ...
 */
DateType.prototype.validate = function dateTypeValidate (typeOptions, fieldValue, errorMessages) {
  var
    i,
    momentDate,
    valid = false,
  ...
  for (i = 0; i < typeOptions.formats.length; i++) {
  ...
  if (typeOptions.hasOwnProperty('range')) {
    if (typeOptions.range.min) {
      min = typeof typeOptions.range.min === 'string' && typeOptions.range.min === 'NOW' ? moment.utc() : typeOptions.range.min;
    ...
    if (typeOptions.range.max) {
      max = typeof typeOptions.range.max === 'string' && typeOptions.range.max === 'NOW' ? moment.utc() : typeOptions.range.max;
 ...
 */
DateType.prototype.validateFieldSpecification = function dateTypeValidateFieldSpecification (typeOptions) {
  var
    min,
    max;
