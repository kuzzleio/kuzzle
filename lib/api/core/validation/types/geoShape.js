...
class GeoShapeType extends BaseType {
  ...
  recursiveShapeValidation(allowedShapes, shape, errorMessages) {
    ...
    switch (shape.type) {
      ...
      case 'circle':
        ...
        break;
      default:
        errorMessages.push(`Unrecognized shape type "${shape.type}"`);
        result = false;
    }
  ...
  validateFieldSpecification(typeOptions) {
    if (typeOptions.hasOwnProperty('shapeTypes')) {
      ...
      const invalid = typeOptions.shapeTypes.filter(shape => allowedShapeTypes.indexOf(shape) === -1);

      if (invalid.length > 0) {
        throw new PreconditionError(`Invalid shape${invalid.length > 1 ? 's' : ''}: ${invalid}`);
      }
    }
    else {
