  ...
  recursiveShapeValidation(allowedShapes, shape, errorMessages) {
    ...
    switch (shape.type) {
      ...
      case 'multipolygon':
        ...
        if (
          shape.orientation
          && allowedOrientations.indexOf(shape.orientation) === -1
        ) {
  ...
  validateFieldSpecification(typeOptions) {
    if (Object.prototype.hasOwnProperty.call(typeOptions, 'shapeTypes')) {
      if (
        ...
        errorsManager.throw('internal', 'validation', 'shapetypes_option_cannot_be_empty');
      }
      ...
      if (invalid.length > 0) {
        errorsManager.throw('internal', 'validation', 'invalid_shape', invalid.length > 1 ? 's' : '', invalid);
      }
    }
    else {
