...
/**
 * @param {RequestObject} requestObject
 * @param {string[]|*} structuredError
 * @returns {Promise.<{documentBody: *, errorMessages:string[], validation: boolean}>}
 */
Validation.prototype.validationPromise = function (requestObject, structuredError) {
  var
    documentCandidate = requestObject.data.body,
  ...
  return new Promise((resolve, reject) => {
    if (controller === 'write' && action === 'update') {
      this.kuzzle.services.list.storageEngine.get({index, collection, controller: 'read', action: 'get', data: {_id: id}}).then(document => {
        // Avoid side effects on the requestObject during the validation (on update)
      ...
      if (collectionSpec.fields) {
        try {
          ...
          if (error.message === 'strictness') {
            ...
            manageErrorMessage('document', errorMessages, 'The document validation is strict; it can not add unspecified sub-fields.', structuredError);
            validation = false;
        ...
        if (!strictnessError && !validation && structuredError) {
          manageErrorMessage('document', errorMessages, 'The document is not valid due to field validation.', structuredError);
        }
      }
        ...
        return this.dsl.test(index, collection, documentBody, id)
          .then(filters => {
            if (!filters.length > 0 || filters[0] !== collectionSpec.validators) {
              validation = false;
              manageErrorMessage('document', errorMessages, 'The document does not match validation filters.', structuredError);
            }
...
Validation.prototype.recurseFieldValidation = function (documentSubset, originalBodySubset, collectionSpecSubset, strictness, errorMessages, structuredError) {
  ...
  return Object.keys(collectionSpecSubset).reduce((reductionResult, fieldName) => {
    ...
    if (field.mandatory && !field.hasOwnProperty('defaultValue') && (!documentSubset.hasOwnProperty(fieldName) || typeof documentSubset[fieldName] === 'undefined' || documentSubset[fieldName] === null)) {
      manageErrorMessage(field.path, errorMessages, 'The field is mandatory.', structuredError);
      return false;
    }
    ...
    if (documentSubset.hasOwnProperty(fieldName) && typeof documentSubset[fieldName] !== 'undefined') {
      if (field.multivalued.value) {
        if (!Array.isArray(documentSubset[fieldName])) {
          manageErrorMessage(field.path, errorMessages, 'The field must be multivalued, unary value provided.', structuredError);
          return false;
        }

        if (field.multivalued.hasOwnProperty('minCount') && documentSubset[fieldName].length < field.multivalued.minCount) {
          manageErrorMessage(field.path, errorMessages, `Not enough elements. Minimum count is set to ${field.multivalued.minCount}.`, structuredError);
          return false;
        }

        if (field.multivalued.hasOwnProperty('maxCount') && documentSubset[fieldName].length > field.multivalued.maxCount) {
          manageErrorMessage(field.path, errorMessages, `Too many elements. Maximum count is set to ${field.multivalued.maxCount}.`, structuredError);
          return false;
        }
        ...
        if (Array.isArray(documentSubset[fieldName])) {
          manageErrorMessage(field.path, errorMessages, 'The field is not a multivalued field; Multiple values provided.', structuredError);
          return false;
        }
      ...
      for (i = 0; i < fieldValues.length; i++) {
        ...
        if (!this.types[field.type].validate(field.typeOptions, fieldValues[i], fieldErrors)) {
          fieldErrors.forEach(message => manageErrorMessage(field.path, errorMessages, message, structuredError));
          return false;
        }
        ...
        if (this.types[field.type].allowChildren && typeof fieldValues[i] === 'object' && field.children) {
          try {
            ...
            if (error.message === 'strictness') {
              manageErrorMessage(field.path, errorMessages, 'The field is set to "strict"; cannot add unspecified "${subFields}".', structuredError);
            } else {
              manageErrorMessage(field.path, errorMessages, error.message, structuredError);
            }
