...
class Validation {
      ...
      if (collectionSpec.fields) {
        try {
          ...
          if (result.isValid === false) {
            ...
            const errorObject = errorsManager.get(
              'validation',
              'format',
  ...
  addType(validationType) {
    if (!validationType.typeName) {
      errorsManager.throw('validation', 'extension', 'missing_typename');
    }
    ...
    if ( !validationType.validateFieldSpecification
      ...
      errorsManager.throw(
        'validation',
        'extension',
        'missing_validateFieldSpecification',
        validationType.typeName);
    }
...
function curateStructuredFields(typeAllowsChildren, fields, maxDepth) {
  ...
  for (i = 1; i <= maxDepth; i++) {
    if (!_.has(fields, i)) {
      errorsManager.throw('validation', 'format', 'missing_nested_spec');
    }
