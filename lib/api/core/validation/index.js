...
class Validation {
  ...
  addType(validationType) {
    if (!validationType.typeName) {
      errorsManager.throw('internal', 'validation', 'missing_typename_property');
    }
  ...
  for (i = 1; i <= maxDepth; i++) {
    ...
    fields[i].forEach(field => {
      ...
      if (!parent.root && typeAllowsChildren.indexOf(parent.type) === -1) {
        errorsManager.throw('internal', 'validation', 'type_not_allowed', parent.type);
      }
...
function manageErrorMessage(errorContext, errorHolder, message, structured) {
  if (structured) {
    ...
    errorsManager.throw('internal', 'validation', 'errorcontext_document', message);
  } else {
