      ...
      .then(documentBody => {
        ...
        if (collectionSpec) {
          if (collectionSpec.fields && collectionSpec.fields.children) {
            try {
              isValid = this.recurseFieldValidation(documentBody, collectionSpec.fields.children, collectionSpec.strict, errorMessages, verbose);
            }
            catch (error) {
      ...
      .then(validation => {
          ...
          for(const collectionName of Object.keys(this.rawConfiguration[indexName])) {
            const promise = this.curateCollectionSpecification(indexName, collectionName, this.rawConfiguration[indexName][collectionName])
              .then(curatedSpec => {
                ...
                this.kuzzle.emit('log:info', `Validation specification for ${indexName} / ${collectionName} has been loaded.`);
                return null;
              })
              .catch(error => {
                this.kuzzle.emit('log:error', `Specification for the collection ${collectionName} triggered an error`);
                this.kuzzle.emit('log:error', `Error: ${error.message}`);
                return null;
              });
        ...
        return Bluebird
          ...
          .then(() => {
            ...
            return this.kuzzle.emit('log:info', 'Validators initialized');
          });
      });
  }
    ...
    for (const fieldName of Object.keys(collectionSpec.fields)) {
      ...
      try {
        ...
        if (result.isValid === false) {
          ...
          this.kuzzle.emit('log:error', result.errors.join('\n'));
        } else {
