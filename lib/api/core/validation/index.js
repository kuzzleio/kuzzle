...
class Validation {
      ...
      .then(documentBody => {
        ...
        if (collectionSpec) {
          ...
          if (collectionSpec.validators) {
            const filters = this.dsl.test(request.input.resource.index, request.input.resource.collection, documentBody, id);
    ...
    Object.keys(collectionSpecSubset).forEach(fieldName => {
      if (
        ...
        if (Array.isArray(documentSubset[fieldName])) {
          for (let i = 0; i < documentSubset[fieldName].length; i++) {
      ...
      else if (collectionSpecSubset[fieldName].defaultValue && isUpdate && documentSubset[fieldName] === null) {
        documentSubset[fieldName] = collectionSpecSubset[fieldName].defaultValue;
      }
    });
   ...
   */
  isValidSpecification(indexName, collectionName, collectionSpec, verboseErrors) {
   ...
   */
  curateCollectionSpecification(indexName, collectionName, collectionSpec, dryRun, verboseErrors) {
    var
      errorMessage = '',
      result,
      errorObject;
    ...
    return new Bluebird((resolve, reject) => {
      ...
      if (!checkAllowedProperties(collectionSpec, ['strict', 'fields', 'validators'])) {
        ...
        return reject(new KuzzleInternalError(errorMessage));
      }
      ...
      if (collectionSpec.validators && Array.isArray(collectionSpec.validators)) {
        this
          ...
          .catch(error => {
            ...
            return reject(new KuzzleInternalError(`Validator specification of the collection ${indexName}.${collectionName} triggered an error`));
          });
      }
      else {
        return resolve(treatedSpecification);
  ...
  structureCollectionValidation(collectionSpec, indexName, collectionName, verboseErrors) {
    var
      result,
      errors = [],
      fields = {},
   ...
   */
  curateFieldSpecification(fieldSpec, indexName, collectionName, fieldName, verboseErrors) {
    var
      returnedTypeOptions,
      result,
      errors = [];
   ...
   * @throws InternalError
   */
  curateFieldSpecificationFormat(fieldSpec, indexName, collectionName, fieldName, verboseErrors) {
    var errors = [];
...
function curateStructuredFields(typeAllowsChildren, fields, maxDepth) {
  ...
  for (i = 1; i <= maxDepth; i++) {
    if (!fields.hasOwnProperty(i)) {
      throw new KuzzleInternalError('All levels of an object have to be defined in the specification.');
    }
    ...
    fields[i].forEach(field => {
      ...
      if (!parent.root && typeAllowsChildren.indexOf(parent.type) === -1) {
        throw new KuzzleInternalError(`The field type ${parent.type} is not allowed to have children fields.`);
      }
...
function getParent(structuredFields, fieldPath) {
  ...
  for (i = 0; i < fieldPath.length - 1; i++) {
    if (!pointer.children.hasOwnProperty(fieldPath[i])) {
      throw new KuzzleInternalError(`The parent field of the field "${fieldPath.join('.')}" is not defined.`);
    }
