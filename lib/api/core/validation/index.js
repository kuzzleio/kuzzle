 ...
 */
Validation.prototype.validate = function validationValidate (request) {
  if (!request.input.body) {
    throw new BadRequestError('The request must provide a document body');
  }
  if (!request.input.resource.index) {
    throw new BadRequestError('The request must provide an index');
  }
  if (!request.input.resource.collection) {
    throw new BadRequestError('The request must provide a collection');
  }
  ...
  return this.validationPromise(request, false);
};
    ...
    if (collectionSpec.fields) {
      try {
        ...
        if (verboseErrors && result.isValid === false) {
          let error = new BadRequestError(result.errors[0]);
