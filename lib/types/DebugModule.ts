...
export abstract class DebugModule extends EventEmitter {
  ...
  constructor(name: string, options: DebugModuleOptions = {}) {
    ...
    if (this.name.charAt(0) !== this.name.charAt(0).toUpperCase()) {
      throw new Error(
        `Debug Module name "${name}" should start with an uppercase letter`
      );
    }
    ...
    for (const event of this.events) {
      ...
      if (event.charAt(0) !== event.charAt(0).toLowerCase()) {
        throw new Error(
          `Event name "${event}" should start with a lowercase letter for module "${name}"`
        );
      }
    }
    ...
    for (const method of this.methods) {
      if (method.length === 0) {
        throw new Error(
          `Method name should not be empty for Debug Module "${name}"`
        );
      }
      if (method.charAt(0) !== method.charAt(0).toLowerCase()) {
        throw new Error(
          `Method name "${method}" should start with a lowercase letter for module "${name}"`
        );
      }
    }
  }
