...
/**
 * Construct and return a promise who rejects with the corresponding error
 *
 * @param  {string} domain - Domain (eg: 'external')
 * @param  {string} subdomain - Subdomain (eg: 'elasticsearch')
 * @param  {string} error - Error name: (eg: 'index_not_found')
...
function rejectError (domain, subdomain, error, ...placeholders) {
  return Bluebird.reject(getError(domain, subdomain, error, ...placeholders));
}
 ...
 * @param  {...any} placeholders - Placeholders value to inject in error message
 */
function throwErrorFrom (errorSource, domain, subdomain, error, ...placeholders) {
  throw getErrorFrom(errorSource, domain, subdomain, error, ...placeholders);
}

/**
 * Wrap getError, getErrorFrom, throw and throwErrorFrom
 ...
 *   - throwErrorFrom: (errorSource, error, ...placeholders)
...
function wrap (domain, subdomain) {
  return {
    ...
    getFrom: (errorSource, error, ...placeholders) => getErrorFrom(errorSource, domain, subdomain, error, ...placeholders),
    throw: (error, ...placeholders) => throwError(domain, subdomain, error, ...placeholders),
    throwFrom: (errorSource, error, ...placeholders) => throwErrorFrom(errorSource, domain, subdomain, error, ...placeholders),
    reject: (error, ...placeholders) => rejectError(domain, subdomain, error, ...placeholders)
  };
}

module.exports = {
  get:        getError,
  getFrom:    getErrorFrom,
  throw:      throwError,
  throwFrom:  throwErrorFrom,
  reject:     rejectError,
  wrap
};
