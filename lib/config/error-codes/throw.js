...
function getError (domain, subdomain, error, ...placeholders) {
  ...
  /* It permits to throw errors that are not KuzzleError by retrieving
   * the message and the stacktrace becauce of the KuzzleError constructor 
   */ 
  if (error.match(/derived/)) {
    const derivedError = placeholders.slice(-1)[0];
    return new errors[kuzzleError.class](derivedError, errorName, code);
  }
