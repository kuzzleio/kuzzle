...
function checkErrorCodes(errorCodes) {
  ...
  for (const domain in errorCodes) {
    assert(!! errorCodes[domain]);
    
    const domainName = errorCodes[domain];
    
    assert(!! domainName.subdomains, `Error configuration file : Missing required 'subdomains' field in '${domain}'.`);
    assert(_.isPlainObject(domainName.subdomains), `Error configuration file : Field 'subdomains' must be an object in '${domain}'.`);
    assert(!! domainName.code, `Error configuration file : Missing required 'code' field in '${domain}'.`);
    ...
    for (const subdomain in domainName.subdomains) {
      assert(!!domainName.subdomains[subdomain]);

      const subdomainName = domainName.subdomains[subdomain];
      
      assert(!! subdomainName.errors, `Error configuration file : Missing required 'errors' field in '${domain}-${subdomain}'.`);
      ...
      assert(!! subdomainName.code, `Error configuration file : Missing required 'code' field in '${domain}-${subdomain}'.`);
      ...
      for (const error in subdomainName.errors) {
        assert(!! subdomainName.errors[error]);
        
        const errorName = subdomainName.errors[error];
        
        assert(!! errorName.message, `Error configuration file : Missing required 'message' field in '${domain}-${subdomain}-${error}'.`);
        assert(typeof errorName.message === 'string', `Error configuration file : Field 'message' must be a string in '${domain}-${subdomain}-${error}'.`);
        assert(!! errorName.class, `Error configuration file : Missing required 'class' field in '${domain}-${subdomain}-${error}'.`);
        assert(typeof errorName.class === 'string', `Error configuration file : Field 'class' must be a string in '${domain}-${subdomain}-${error}'.`);
        assert(!! errors[errorName.class], `Error configuration file : Invalid field 'class' in '${domain}-${subdomain}-${error}', '${errorName.class}' does not exist.`);
        assert(!! errorName.code, `Error configuration file : Missing required 'code' field in '${domain}-${subdomain}-${error}'.`);
