...
const OfficialsPluginsCode = {
  ...
  'kuzzle-plugin-auth-passport-oauth': 26,
  'computed-fields-plugin': 27,
  'kuzzle-plugin-probe-listener': 28,
  'kuzzle-plugin-multi-tenancy': 29
};

function loadPluginsError(pluginManifest) {
 
  plugins.subdomains[pluginManifest.name] = {
    code: 
      pluginManifest.pluginCode
      || OfficialsPluginsCode[pluginManifest.name]
      || _.max(_.map(plugins.subdomains, 'code')) + 1,
