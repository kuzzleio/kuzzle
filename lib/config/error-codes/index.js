...
function checkErrorCodes(errorCodes) {
  ...
  for (const domain of Object.keys(errorCodes)) {
    ...
    assert(domainName.hasOwnProperty('subdomains'), `Error configuration file : Missing required 'subdomains' field in '${domain}'.`);
    assert(typeof domainName.subdomains === 'object', `Error configuration file : Field 'subdomains' must be an object in '${domain}'.`);
    ...
    for (const subdomain of Object.keys(domainName.subdomains)) {
      ...
      assert(subdomainName.hasOwnProperty('errors'), `Error configuration file : Missing required 'errors' field in '${domain}-${subdomain}'.`);
      assert(typeof subdomainName.errors === 'object', `Error configuration file : Field 'subdomains' must be an object in '${domain}-${subdomain}'.`);
      ...
      for (const error of Object.keys(subdomainName.errors)) {
        ...
        assert(errorName.hasOwnProperty('message'), `Error configuration file : Missing required 'message' field in '${domain}-${subdomain}-${error}'.`);
        assert(typeof errorName.message === 'string', `Error configuration file : Field 'message' must be a string in '${domain}-${subdomain}-${error}'.`);
        assert(errorName.hasOwnProperty('class'), `Error configuration file : Missing required 'class' field in '${domain}-${subdomain}-${error}'.`);
        assert(typeof errorName.class === 'string', `Error configuration file : Field 'class' must be a string in '${domain}-${subdomain}-${error}'.`);
        assert(errors.hasOwnProperty(errorName.class), `Error configuration file : Invalid field 'class' in '${domain}-${subdomain}-${error}', '${errorName.class}' does not exist.`);
        assert(errorName.hasOwnProperty('code'), `Error configuration file : Missing required 'code' field in '${domain}-${subdomain}-${error}'.`);
