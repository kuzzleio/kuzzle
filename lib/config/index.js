...
function load () {
  ...
  checkCompatibilitiesConfig(config);
...
/**
 * Check if there is some incompatibilities in the config
 * @param {*} cfg 
 */
function checkCompatibilitiesConfig(cfg) {
  // When the support of cookie as an Authentication Token is enabled
  // we need to perform some checks before starting Kuzzle
  if (cfg.http.supportCookieAuthentication) {
    if (cfg.http.accessControlAllowOrigin === '*') {
      throw kerror.get('incompatible', 'Option [http.supportCookieAuthentication] is incompatible with [http.accessControlAllowOrigin] set to \'*\'');
    }
    // Mandatory to enable cookie transmission
    cfg.http.accessControlAllowCredentials = true;
  }
}
