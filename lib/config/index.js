...
function checkClusterOptions (config) {
  ...
  assert(typeof cfg.ip === 'string', `[FATAL] kuzzlerc.cluster.ip: value must be a string ("public", "private" or a valid IP)`);
  if (cfg.ip !== 'public' && cfg.ip !== 'private') {
    if (cfg.ipv6) {
      assert(ip.isV6Format(cfg.ip), `[FATAL] kuzzlerc.cluster.ip: value is not a valid IPV6 ("public", "private" or a valid IPV6)`);
    }
    else {
      assert(ip.isV4Format(cfg.ip), `[FATAL] kuzzlerc.cluster.ip: value is not a valid IPV4 ("public", "private" or a valid IPV4)`);
    }
  }
}
