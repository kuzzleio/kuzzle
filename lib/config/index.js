...
function preprocessHttpOptions (config) {
  ...
  config.internal.allowAllOrigins = cfg.accessControlAllowOrigin.includes('*'); // Stored to avoid doing includes multiple times later
  config.internal.cookieAuthentication = !cfg.accessControlAllowOrigin.includes('*'); // If there is no wild card, cookieAuthentication is enabled
}
