...
function checkLimitsConfig (cfg) {
  ...
  if (!isPlainObject(cfg.limits.requestsRate)) {
    errorsManager.throw('invalid_type', 'limits.requestsRate', 'object');
  }

  for (const opt of Object.keys(cfg.limits.requestsRate)) {
    if (typeof cfg.limits.requestsRate[opt] !== 'number') {
      errorsManager.throw('invalid_type', `limits.requestsRate.${opt}`, 'number');
    }

    if (cfg.limits.requestsRate[opt] < 0) {
      errorsManager.throw('out_of_range', `limits.requestsRate.${opt}`, '>= 0');
    }
  }
