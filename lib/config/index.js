...
function preprocessHttpOptions (config) {
  ...
  if (typeof cfg.accessControlAllowOrigin === 'string') {
  ...
  config.internal.allowAllOrigins = cfg.accessControlAllowOrigin.indexOf('*') > -1; // Stored to avoid doing indexOf multiple times later
  config.internal.cookieAuthentication = cfg.accessControlAllowOrigin.indexOf('*') === -1; // If there is no wild card, cookieAuthentication is enabled
}
