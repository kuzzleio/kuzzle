...
function checkErrorCodes() {
  ...
  for (const domain of Object.keys(errorCodes)) {
    ...
    if (!(domainName.hasOwnProperty('subdomains')) 
      || typeof domainName['subdomains'] !== 'object' 
      || !(domainName.hasOwnProperty('code'))
      || typeof domainName['code'] !== 'number'
      || !(domainName['code'] > 0 && domainName['code'] < 99)) {
      throw new KuzzleInternalError(`Bad formatted ${domain} error-codes config file.`, domainName, 0);
    }
    for (const subdomain of Object.keys(errorCodes[domain]['subdomains'])) {
      const subdomainName = errorCodes[domain]['subdomains'][subdomain];
      if (!(subdomainName.hasOwnProperty('errors'))
        || typeof subdomainName['errors'] !== 'object' 
        || !(subdomainName.hasOwnProperty('code')) 
        || typeof subdomainName['code'] !== 'number'
        || !(subdomainName['code'] > 0 && subdomainName['code'] < 99)) {
        throw new KuzzleInternalError(`Bad formatted ${domain} error-codes config file in field ${subdomain}.`, subdomainName, 0);
      }
      for (const error of Object.keys(errorCodes[domain]['subdomains'][subdomain]['errors'])) {
        const errorName = errorCodes[domain]['subdomains'][subdomain]['errors'][error];
        if (!(errorName.hasOwnProperty('code')) || typeof errorName['code'] !== 'number'
          ...
          || !(errorName['code'] > 0 && errorName['code'] < 99)) {
          throw new KuzzleInternalError(`Bad formatted ${domain} error-codes config file in field ${subdomain}-${error}.`, errorName, 0);
        }
      }
    }
  }
