...
function preprocessRedisOptions (redisConfig) {
  ...
  redisConfig.clusterOptions.dnsLookup = Redis.DNS_LOOKUP_CALLBACK_STRING;
  if (redisConfig.nodes) {

    // Callback can't be serialized when passed to the worker thread for ID card
    // renewal.
    // Also see https://github.com/luin/ioredis#special-note-aws-elasticache-clusters-with-tls
    if (redisConfig.overrideDnsLookup) {
      redisConfig.clusterOptions.dnsLookup = Redis.DNS_LOOKUP_CALLBACK_STRING;
    }
  }
}
