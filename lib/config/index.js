...
function loadConfig () {
  ...
  if (config.services.proxyBroker) {
    ...
    config.server.proxy = Object.assign({
      host: 'localhost',
      port: 7331,
      retryInterval: 1000,
      resendClientListDelay: 1000
    }, config.services.proxyBroker);
...
function checkLimitsConfig (cfg) {
  ...
  ['documentsWriteCount', 'documentsFetchCount'].forEach(prop => {
    if (cfg.limits[prop] > 9999 || cfg.limits[prop] > .8 * cfg.limits.requestsBufferSize) {
      throw new KuzzleInternalError(`Invalid limits.${prop} configuration: cannot exceed 80% of limits.requestsBufferSize and cannot be higher than 9999`);
    }
  });
}
