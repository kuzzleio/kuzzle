  ...
  get content () {
    if (this.json === null && this.raw !== null) {
      try {
        ...
        throw errorsManager.get('body_parse_failed');
      }
    }
...
class HttpProtocol extends Protocol {
  ...
  _uncompress(request) {
    ...
    if (request.headers['content-encoding']) {
      ...
      if (encodings.length > this.maxEncodingLayers) {
        throw errorsManager.get('too_many_encodings');
      }
      ...
      for (const encoding of encodings) {
        ...
        if (decoder) {
          ...
          request.removeAllListeners().resume();
          throw errorsManager.get('unsupported_compression', encoding);
        }
  ...
  _createWritableStream(request, message) {
    if (request.headers['content-type'] &&
      ...
      try {
        ...
        throw errorsManager.getFrom(error, 'unexpected_error', error.message);
      }
    }
  ...
  _setDecoders() {
    const
      ...
      disabledfn = () => {
        throw errorsManager.get('compression_disabled');
      };
