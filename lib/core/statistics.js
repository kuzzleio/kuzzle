...
class StatisticsController {
  ...
  getLastStats() {
    ...
    return this.kuzzle.cacheEngine.internal.get(this.cacheKeyPrefix + this.lastFrame)
  ...
  getStats(request) {
    ...
    if (startTime !== undefined && isNaN(startTime)) {
      return errorsManager.reject('invalid_argument', 'startTime', 'number');
    }

    if (stopTime !== undefined && isNaN(stopTime)) {
      return errorsManager.reject('invalid_argument', 'stopTime', 'number');
    }
    ...
    return this.kuzzle.cacheEngine.internal.searchKeys(this.cacheKeyPrefix + '*')
      .then(keys => {
        frames = keys;
        // Statistics keys are timestamp.
        // Ordering them guarantees stats frames to be returned in the right order
        return this.kuzzle.cacheEngine.internal.mget(keys.sort());
      })
      .then(values => {
