...
class Janitor {
   ...
   * @returns {Promise}
   */
  async loadSecurities (securities = {}, { force=false, user=null } = {}) {
    ...
    await this._deleteUsers(securities.users, { force, user });
    await this._createSecurity('createUser', securities.users, user, 'users');
  ...
  async _createSecurity (action, objects, user, collection) {
    ...
    return this.kuzzle.storageEngine.internal.refreshCollection(
      this.kuzzle.storageEngine.config.internalIndex.name,
      collection);
  }

  async _deleteUsers (users, { force } = {}) {
    if (_.isEmpty(users)) {
      return;
    }
