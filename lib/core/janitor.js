...
class Janitor {
   ...
   * @returns {Promise}
   */
  async loadSecurities (securities = {}, { onExistingUsers='reject', user=null } = {}) {
    assertIsObject(securities);

    await this._createSecurity('createOrReplaceRole', securities.roles, user, 'roles');
    await this._createSecurity('createOrReplaceProfile', securities.profiles, user, 'profiles');

    const usersToCreate = await this._deleteUsers(securities.users, { onExistingUsers, user });
    await this._createSecurity('createUser', usersToCreate, user, 'users');
  }
  ...
  async _createSecurity (action, objects, user, collection) {
    if (! objects) {
      return;
    }
  ...
  async _deleteUsers (users, { onExistingUsers } = {}) {
    if (_.isEmpty(users)) {
      return users;
    }
    ...
    if (onExistingUsers === 'reject') {
      throw errorsManager.get('security', 'user', 'prevent_override');
    }

    if (onExistingUsers === 'skip') {
