...
class Janitor {
   ...
   * @returns {Promise}
   */
  async loadSecurities (securities = {}, userId) {
    assertIsObject(securities);
    await this._createSecurity('createOrReplaceRole', securities.roles, userId);
    await this._createSecurity('createOrReplaceProfile', securities.profiles, userId);
    await this._deleteUsers(securities.users, userId);
    await this._createSecurity('createUser', securities.users, userId);
  }
