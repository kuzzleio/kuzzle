...
class SecurityLoader {
  ...
  async init () {
    this.kuzzle.onAsk('core:security:load', this.load.bind(this));
  }
  ...
  async load(securities = {}, {force, onExistingUsers='fail', user=null} = {}) {
    ...
    const usersToLoad = await this.getUsersToLoad(securities.users, {
      onExistingUsers,
      user,
    });
  ...
  async _getUsersToLoad (kuzzle, users, { onExistingUsers } = {}) {
    if (_.isEmpty(users)) {
      return users;
    }
