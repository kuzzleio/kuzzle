...
class TokenRepository extends Repository {
  static AUTH_PREFIX = 'kauth-';
  static APIKEY_PREFIX = 'kapikey-';
  ...
  async verifyToken (rawToken) {
    ...
    const token = this.removeTokenPrefix(rawToken);
    let decoded = null;
  ...
  async _loadApiKeys () {
    ...
    try {
      ...
      const promises = [];
      const createToken = async apiKey => {
        return await this.persistForUser(
          apiKey._source.token,
          apiKey._source.userId,
          apiKey._source.ttl);
      };
