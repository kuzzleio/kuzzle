...
class TokenRepository extends Repository {
  constructor (opts = {}) {
    ...
    this.tokenGracePeriod = Math.floor(this.kuzzle.config.security.jwt.gracePeriod);
  ...
  async expire (token) {
    ...
    await this.kuzzle.tokenManager.expire(token);
  }
  ...
  async verifyToken(token) {
    ...
    try {
      decoded = jwt.verify(token, this.kuzzle.secret);
  ...
  async deleteByKuid (kuid) {
    ...
    const keys = await this.kuzzle.ask('core:cache:internal:searchKeys', userKey);
