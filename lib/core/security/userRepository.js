...
class UserRepository extends Repository {
   ...
   * @returns {Promise}
   */
  async create (request, profileIds, content, { humanReadableId = true } = {}) {
    ...
    do {
      ...
      for (const strategy of strategies) {
        ...
        const exists = global.kuzzle.pluginsManager.getStrategyMethod(strategy, 'exists');
    ...
    try {
      user = await this.persist(user, {
        database: { method: 'create', refresh: request.getRefresh('wait_for') }
      });
    }
    catch (error) {
    ...
    for (const strategy of strategies) {
      try {
        const validate = global.kuzzle.pluginsManager.getStrategyMethod(strategy, 'validate');
      ...
      try {
        const create = global.kuzzle.pluginsManager.getStrategyMethod(strategy, 'create');
    ...
    for (const strategy of createdStrategies) {
      try {
        const del = global.kuzzle.pluginsManager.getStrategyMethod(strategy, 'delete');
    ...
    try {
      global.kuzzle.ask('core:security:user:delete', id, { refresh: 'false' });
    }
    catch (e) {
