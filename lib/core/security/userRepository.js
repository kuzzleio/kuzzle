...
class UserRepository extends Repository {
  ...
  async init (options = {}) {
    ...
    onAsk('core:security:users:delete', (id, opts) => this.delete(id, opts));
    onAsk('core:security:users:get', id => this.load(id));
    onAsk('core:security:users:mGet', ids => this.loadMultiFromDatabase(ids));
    onAsk(
      'core:security:users:replace',
      (id, content, opts) => this.replace(id, content, opts));
    onAsk('core:security:users:scroll', (id, scroll) => this.scroll(id, scroll));
    onAsk('core:security:users:search', (query, opts) => this.search(query, opts));
  ...
  async create (id, profileIds, content, {userId, refresh = null} = {}) {
    try {
      await this.load(id);
      throw errorsManager.get('security', 'user', 'already_exists', id);
    }
    catch (error) {
