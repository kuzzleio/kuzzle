...
const Profile = require('./document/profile');
...
class ProfileRepository extends Repository {
  ...
  init (options = {}) {
    ...
    onAsk('core:security:profiles:delete', (id, opts) => this.delete(id, opts));
    onAsk('core:security:profiles:get', id => this.load(id));
    onAsk('core:security:profiles:mGet', ids => this.loadMultiFromDatabase(ids));
    onAsk(
      'core:security:profiles:search',
      (query, opts) => this.searchProfiles(query, opts));
    onAsk('core:security:profiles:truncate', opts => this.truncate(opts));
  ...
  async loadProfiles (profileIds) {
    if (!profileIds) {
      throw assertionError.get('missing_argument', 'profileIds');
    }

    if ( !Array.isArray(profileIds)
      || profileIds.some(p => typeof p !== 'string')
    ) {
      throw assertionError.get('invalid_type', 'profileIds', 'string[]');
    }
  ...
  async validateAndSaveProfile (profile, options) {
    if (!profile._id) {
      throw assertionError.get('missing_argument', 'profileId');
    }
  ...
  async fromDTO (dto) {
    ...
    const
      policiesRoles = profile.policies.map(p => p.roleId),
      roles = await this.kuzzle.repositories.role.loadRoles(policiesRoles);
