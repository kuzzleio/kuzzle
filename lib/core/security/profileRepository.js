...
class ProfileRepository extends Repository {
  ...
  async _createOrReplace (
    id,
    content,
    { force, method, refresh = 'false', userId = null } = {}
  ) {
    ...
    return this.validateAndSaveProfile(profile, {force, method, refresh});
  }
   ...
   **/
  async validateAndSaveProfile (profile, { force, method, refresh, retryOnConflict } = {}) {
    ...
    await profile.validateDefinition({ force });
  ...
  async fromDTO (dto) {
    ...
    if (profile.constructor._hash('') === false) {
      profile.constructor._hash = obj => this.kuzzle.hash(obj);
    }
