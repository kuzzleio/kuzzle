...
class RoleRepository extends Repository {
  ...
  load(id) {
    ...
    return this.loadOneFromDatabase(id).then(role => {
  ...
  searchRole(controllers, from, size) {
    return this.search({ query: {} }, { from: 0, size: 1000 }).then(res => {
      const result = {
        hits: res.hits,
        total: res.total
      };
  ...
  validateAndSaveRole(role, options) {
    return role
      ...
      .then(() => {
        ...
        this.checkRoleControllersAndActions(role)
    ...
    Object.keys(role.controllers).forEach(roleController => {
      if (roleController === '*') {
        this.kuzzle.funnel.controllers.forEach(controller => {
          Object.keys(role.controllers['*'].actions).forEach(
            action => {
              if ( action !== '*'
        ...
        Object.keys(role.controllers[roleController].actions).forEach(
          action => {
  ...
  delete(role, options = {}) {
    ...
    return this.kuzzle.repositories.profile
      ...
      .then(response => {
        ...
        return this.deleteFromDatabase(role._id, options).then(
          deleteResponse => {
