  ...
  validateAndSaveRole (role, options) {
    ...
    return role
      ...
      .then(updatedRole => {
        ...
        if (errors.length) {
          errors = errorsManager.get('security', 'role', 'invalid_plugin_rights', role._id, errors);
          errors.role = updatedRole;
          throw errors;
        }
        return updatedRole;
      });
  ...
  checkRolePluginsRights(role) {
    ...
    Object.keys(role.controllers).forEach(roleController => {
      ...
      if (!availablePlugins.includes(pluginName)) {
        ...
        errors.push(roleRightsError.get('unavailable_plugin', role._id, pluginName));
        return;
      }
      ...
      if (pluginController !== '*' && !pluginControllers.includes(pluginController)) {
        ...
        errors.push(roleRightsError.get('unavailable_controller_plugin', role._id, pluginController, pluginName));
        return;
      }
      const pluginActions = [];
      ...
      roleActions.forEach(action => {
        if (action !== '*' && !pluginActions.includes(action)) {
          ...
          errors.push(roleRightsError.get('unavailable_action_plugin', role._id, action, pluginController, pluginName));
          return;
        }
      });
    });
