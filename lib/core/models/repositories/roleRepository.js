...
class RoleRepository extends Repository {
  ...
  validateAndSaveRole (role, options) {
    return role
      ...
      .then(() => this.checkNativeRoleControllersAndActions(role))
      .then(() => this.checkPluginsRoleControllersAndActions(role))
   ...
   * @returns Promise
   */
  checkNativeRoleControllersAndActions(role) {
    Object.keys(role.controllers).forEach(roleController => {
      if (roleController !== '*' && !this.kuzzle.funnel.controllers.has(roleController)) {
        if (roleController.split('/').length > 1) {
          // Meaning this is a plugin controller
          return;
        }
        errorsManager.throw(
          'security',
  ...
  * @returns Promise
  */
  checkPluginsRoleControllersAndActions(role) {
    Object.keys(role.controllers).forEach(roleController => {
      if (roleController.split('/').length === 1) {
        // Meaning this is not a plugin controller
        return;
      }
      ...
      if (!availablePlugins.includes(pluginName)) {
        console.log(`Emettre un warning: Le plugin ${pluginName} n'existe pas`);
        return;
      }
      ...
      console.log(`Controllers du plugin ${pluginName}: ${pluginControllers}`);
      if (pluginController !== '*' && !pluginControllers.includes(pluginController)) {
        console.log(`Emettre un warning: le controller ${pluginController} n'existe pas dans le plugin ${pluginName}`);
        return;
      }
      const pluginActions = [];
      ...
      roleActions.forEach(action => {
        if (action !== '*' && !pluginActions.includes(action)) {
          console.log(`Emettre un warning : l'action ${action} pour le controller ${pluginController} n'existe pas dans le plugin ${pluginName}`);
        }
      });
    });
  }
