...
class Profile {
  ...
  async validateDefinition() {
    ...
    if (!this.policies) {
      assertionError.throw('missing_argument', 'policies');
    }

    if (!Array.isArray(this.policies)) {
      assertionError.throw('invalid_type', 'policies', 'object[]');
    }

    if (this.policies.length === 0) {
      assertionError.throw('empty_argument', 'policies');
    }

    let i = 0;
    for (const policy of this.policies) {
      if (!policy.roleId) {
        assertionError.throw('missing_argument', `policies[${i}].roleId`);
      }
  ...
  validateRateLimit () {
    ...
    if ( typeof this.rateLimit !== 'number'
      ...
      assertionError.throw('invalid_type', 'rateLimit', 'integer');
    }

    if (this.rateLimit < 0) {
      assertionError.throw('invalid_argument', 'rateLimit', 'positive integer, or zero');
    }
  }
}
