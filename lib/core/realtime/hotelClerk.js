...
class Channel {
  constructor(
    kuzzle,
    roomId,
    { scope = 'all', users = 'none', propagate = true } = {}
  ) {
    this.scope = scope;
    this.users = users;
    ...
    this.name = `${roomId}-${kuzzle.constructor.hash(this)}`;
  }
}
...
class HotelClerk {
  constructor(kuzzle) {
    ...
    this.kuzzle.onAsk(
      'core:realtime:hotelClerk:registerSubscription',
      (connection, index, collection, filters, options) => {
   ...
   * @param {*} options
   */
  async registerSubscription(connectionId, index, collection, filters, options) {
    ...
    const res = await this.addSubscription(customRequest);
      ...
      for (const collection of this.kuzzle.koncorde.getCollections(index)) {
        ...
        promises.push(
          request.context.user
            .isActionAllowed(isAllowedRequest)
            .then((isAllowed) => {
              if (!isAllowed) {
                return;
              }

              for (const roomId of this.kuzzle.koncorde.getFilterIds(
                index,
                collection
              )) {
                const room = this.rooms.get(roomId);
  ...
  removeCustomerFromAllRooms(requestContext) {
    ...
    for (const roomId of customer.keys()) {
      promises.push(
        this._removeRoomForCustomer(requestContext, roomId).catch((err) =>
  ...
  async _createRoom(index, collection, filters) {
    ...
    if (
      this.kuzzle.config.limits.subscriptionMinterms > 0 &&
    ...
    if (
      this.kuzzle.config.limits.subscriptionRooms > 0 &&
  ...
  _removeAllRoomsInCollection(index, collection) {
    ...
    const realtimeFilters = Array.from(
  ...
  _removeListRoomsInCollection(index, collection, rooms) {
    ...
    for (const roomId of rooms) {
      ...
      if (!room) {
        ...
        partialErrors.push(
          `The room ${roomId} does not match collection ${collection}`
        );
      } else {
        this._removeRoomEverywhere(roomId);
      }
  ...
  _removeRoomEverywhere(roomId) {
    ...
    if (!room) {
      ...
      this.kuzzle.log.warn(
        `hotelClerk._removeRoomEveryWhere could not find room ${roomId}`
      );
      return;
    }
  ...
  async _removeRoomForCustomer(requestContext, roomId, notify = true) {
    ...
    if (!room) {
      this.kuzzle.log.error(
        `[hotelClerk] Cannot remove room "${roomId}": room not found`
      );
      throw realtimeError.get('room_not_found', roomId);
    }
    ...
    await this.kuzzle.notifier.notifyUser(roomId, request, 'out', {
    ...
    if (notify && Object.keys(room.channels).length > 0) {
      ...
      if (Object.values(room.channels).some((channel) => !channel.cluster)) {
        roomPayload.channels = Object.entries(room.channels).reduce(
  ...
  async _subscribeToRoom(roomId, request) {
    ...
    if (!customer || !customer.has(roomId)) {
      ...
      notifyPromise = this.kuzzle.notifier.notifyUser(roomId, request, 'in', {
        count: room.customers.size,
      });
    } else {
