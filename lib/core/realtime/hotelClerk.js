...
class Channel {
  static ALLOWED_VALUES = ['all', 'in', 'out', 'none'];

  constructor (kuzzle, roomId, { scope='all', users='none', cluster=true } = {}) {
    this.scope = 'all';
    this.users = 'none';
    this.cluster = cluster;

    if (! Channel.ALLOWED_VALUES.includes(this.scope)) {
      throw realtimeError.get('invalid_scope');
    }

    if (! Channel.ALLOWED_VALUES.includes(this.users)) {
...
class HotelClerk {
  ...
  async join (request) {
    ...
    if (response.cluster) {
      // @todo should be an emit since we don't use the result?
  ...
  _createChannelConfiguration (request) {
    const channel = {
      scope: request.input.args.scope || 'all',
      users: request.input.args.users || 'none',
      cluster: request.input.args.cluster
    };

    const allowed = ['all', 'in', 'out', 'none'];
  ...
  async _removeRoomForCustomer (requestContext, roomId, notify = true) {
    const userId = requestContext.connection.id;
    const customer = this.customers.get(userId);

    if (! customer) {
      throw realtimeError.get('not_subscribed', userId, roomId);
    }
    ...
    for (const channel of Object.keys(room.channels)) {
      this.kuzzle.entryPoint.leaveChannel(channel, requestContext.connection.id);
    }
    ...
    if (notify) {
      // Do not propagate local channels, only cluster ones
      room.channels = Object.entries(room.channels)
