...
class HotelClerk {
  ...
  async init () {
    ...
    this.kuzzle.onAsk(
      'core:realtime:list',
      requestContext => this.list(requestContext));
  ...
  async subscribe (request) {
    ...
    if (subResponse.diff && subResponse.cluster) {
      ...
      await this.kuzzle.pipe('core:realtime:user:subscribed', diff);
    }
  ...
  async join (request) {
    ...
    if (response.cluster) {
      await this.kuzzle.pipe('core:realtime:user:joined', response.diff);
    }
  ...
  async list (requestContext) {
    ...
    for (const index of this.kuzzle.koncorde.getIndexes()) {
      for (const collection of this.kuzzle.koncorde.getCollections(index)) {
        ...
        promises.push(requestContext.user.isActionAllowed(isAllowedRequest)
          .then(isAllowed => {
            if (!isAllowed) {
              return;
            }
  ...
  async unsubscribe (connectionId, roomId, notify = true) {
    ...
    if (notify && Object.keys(room.channels).length > 0) {
      ...
      // @todo - will be removed with the native cluster implementation
      await this.kuzzle.pipe('core:realtime:user:unsubscribed', {
        requestContext,
        room: roomPayload,
      });
    }
  ...
  async _removeRoomFromRealtimeEngine (roomId) {
    try {
      const id = await this.kuzzle.pipe('core:realtime:room:remove', roomId);
