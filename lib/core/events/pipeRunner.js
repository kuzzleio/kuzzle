...
const errorsManager = require('../../util/errors').wrap('plugin', 'runtime');
 ...
 */
class PipeChain {
  constructor (chain, payload, callback, callbackContext) {
    this.chain = chain;
...
class PipeRunner {
   ...
   */
  run (chain, payload, callback, callbackContext) {
    if (this.running >= this.maxConcurrent) {
      if (this.buffer.length >= this.maxBufferSize) {
        callback.call(callbackContext, errorsManager.get('too_many_pipes'));
      }
      else {
        this.buffer.push(new PipeChain(chain, payload, callback, callbackContext));
      }
    ...
    waterfall(chain, payload, waterfallCallback, waterfallCallbackCtx);
  }
  ...
  _runNext () {
    ...
    this.run(pipe.chain, pipe.payload, pipe.callback, pipe.callbackContext);
  }
}
