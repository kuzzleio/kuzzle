...
class NotifierController {
  ...
  notifyServer (rooms, connectionId, type, message) {
    ...
    if (channels.length > 0) {
      ...
      return this.kuzzle
        .pipe('notify:server', notification)
  ...
  notifyDocumentCreate (request, newDocument) {
    ...
    return this.notifyDocument(rooms, request, 'in', 'create', {
      _source: newDocument._source,
      _id: newDocument._id
    }).then(() =>
      this._setCacheWithTTL(
  ...
  notifyDocumentReplace (request) {
    ...
    return this.notifyDocument(rooms, request, 'in', 'replace', {
      _source: request.input.body,
      _id: request.input.resource._id
    })
  ...
  notifyDocumentMChanges (request, documents, cached) {
    ...
    return (cached
      ? this.cacheEngine.mget(cacheIds)
      : Bluebird.resolve([])
    ).then(hits => {
      const idsToDelete = [],
        promises = [];
      ...
      for (let i = 0; i < documents.length; i++) {
        const documentAction = documents[i].created
            ? 'create'
            : controllerAction,
   ...
   * @return {Promise}
   */
  notifyDocumentUpdate (request, updatedDocument) {

    const matchedRooms = this._test(
      request,
    ...
    const updatedFields = Object.keys(request.input.body).filter(
      _updatedFields => _updatedFields !== '_kuzzle_info'
    );
  ...
  _dispatch (channels, notification, connectionId) {
    return this.kuzzle
      .pipe('notify:dispatch', notification)
      .then(updatedNotification => {
  ...
  _notifyDocument (rooms, request, scope, action, content) {
    ...
    if (channels.length > 0) {
      ...
      return this.kuzzle
        .pipe('notify:document', notification)
        .then(updatedNotification =>
