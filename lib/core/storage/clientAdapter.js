...
class ClientAdapter {
  ...
  async init () {
     ...
     *
     * @param options.from Provenance of the command ("cluster" or "myself")
     */
    global.kuzzle.onAsk(
      `core:storage:${this.scope}:cache:refresh`,
      options => this.populateCache(options));
   ...
   * @returns {Promise}
   */
  async populateCache ({ from='cluster' } = {}) {
    ...
    // Don't propagate the event if it comes from the cluster
    if (from !== 'cluster') {
      global.kuzzle.emit(`core:storage:${this.scope}:cache:refresh:after`);
    }
  }
