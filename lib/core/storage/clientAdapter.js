  ...
  async loadFixtures (fixtures = {}) {
    ...
    for (const index of Object.keys(fixtures)) {
      ...
      for (const [collection, payload] of Object.entries(fixtures[index])) {
        const { errors } = await this.import(
          index,
          collection,
          payload,
          { refresh: 'wait_for' });
        ...
        if (errors.length > 0) {
          throw errorsManager.get('import_failed', errors);
        }
      }
    }
  }
  ...
  async loadMappings (fixtures = {}) {
    ...
    for (const index of Object.keys(fixtures)) {
      ...
      for (const [collection, mappings] of Object.entries(fixtures[index])) {
        if (await this.indexExists(index) === false) {
          await this.createIndex(index);
        }

        await this.kuzzle.storageEngine.public.createCollection(
          index,
          collection,
          { mappings });
      }
