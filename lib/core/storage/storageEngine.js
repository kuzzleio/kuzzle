...
class StorageEngine {
  constructor (kuzzle) {
    ...
    this.public = new ClientAdapter(kuzzle, this.indexCache, scopeEnum.PUBLIC);

    // Storage client for private indexes only
    this.private = new ClientAdapter(kuzzle, this.indexCache, scopeEnum.PRIVATE);
  }
  ...
  async init () {
    ...
    const privateIndexes = await this.private.listIndexes();

    for (const publicIndex of await this.public.listIndexes()) {
