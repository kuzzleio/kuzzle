...
const DocumentPipes = require('./DocumentPipes');
...
class PluginsManager {
  constructor(kuzzle) {
    ...
    this.__documentPipes = null;
  }

  // Lazy loading because DocumentPipes embeds a Koncorde engine
  get _documentPipes () {
    if (! this.__documentPipes) {
      this.__documentPipes = new DocumentPipes(this.kuzzle);
    }

    return this.__documentPipes;
  }
  ...
  run () {
    ...
    return Bluebird.all(Object.keys(this.plugins).map(pluginName => {
      ...
      const initTimeout = this.config.common.initTimeout;
  ...
  registerPipe (plugin, event, fn) {
    ...
    this.pipes[event].push((...data) => {
      ...
      const warnDelay = plugin.config.pipeWarnTime
        || this.config.common.pipeWarnTime;
      const timeoutDelay = plugin.config.pipeTimeout
        || this.config.common.pipeTimeout;
  ...
  async beforeDocumentWrite (pluginName, index, collection, filters, callback) {
    debug('[%s] Register beforeDocumentWrite callback for "%s:%s" with %o', pluginName, index, collection, filters);

    const count = await this._documentPipes.registerOnWrite(
      pluginName,
      index,
      collection,
      filters,
    ...
    if (count === 1) {
      ...
      this.registerPipe(
        ...
        this._documentWriteHandler.bind(this));
    }
  }

  async _documentWriteHandler (documents, request) {
    for (let i = 0; i < documents.length; i++) {
      const document = documents[i];

      const documentCallbacks = this._documentPipes.triggerOnWrite(
        request.input.resource.index,
        request.input.resource.collection,
        document);

      if (documentCallbacks.length > 0) {
        for (let j = 0; j < documentCallbacks.length; j++) {
          const documentCallback = documentCallbacks[j];

          debug('[%s] Trigger callback "%s" for "%s:%s" with %o', documentCallback.pluginName, documentCallback.filterId, documentCallback.index, documentCallback.collection, documentCallback.filters);

          const promise = documentCallback.callback(document, request);

          if ( typeof promise.then !== 'function'
            || typeof promise.catch !== 'function'
          ) {
            throw new Error(`Document callback ${documentCallback} should return a promise`);
          }

          documents[i] = await promise;
        }
      }
    }
  ...
  _initPipes (plugin) {
    ...
    _.forEach(plugin.object.pipes, (fn, event) => {
      ...
      for (const target of list) {
        ...
        this.registerPipe(plugin, event, target);
      }
    });
  }
