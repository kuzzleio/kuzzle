...
class PluginsManager {
  ...
  async run () {
    ...
    return Bluebird.all(Object.keys(this.plugins).map(async pluginName => {
   ...
   */
  registerPipe (plugin, warnDelay, timeout, event, fn) {
    ...
    const wrapper = (...data) => {
      ...
      if (timeout) {
        pipeTimeoutTimer = setTimeout(() => {
          const error = runtimeError.get('pipe_timeout', name, event, timeout);
