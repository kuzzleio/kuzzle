...
const
  ...
  waterfall = require('../../util/waterfall'),
 ...
 */
class PipeChain {
  constructor (chain, callback, callbackContext) {
    this.chain = chain;
...
class PipeRunner {
   ...
   */
  run (chain, callback, callbackContext) {
    if (this.running >= this.maxConcurrent) {
      if (this.buffer.length >= this.maxBufferSize) {
        callback.call(callbackContext, errorsManager.get('too_many_pipes'));
      }
      else {
        this.buffer.push(new PipeChain(chain, callback, callbackContext));
      }
    ...
    waterfall(chain, waterfallCallback, waterfallCallbackCtx);
  }
  ...
  _runNext () {
    ...
    this.run(pipe.chain, pipe.callback, pipe.callbackContext);
  }
}
