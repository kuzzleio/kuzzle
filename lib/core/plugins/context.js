  ...
  constructor(kuzzle, pluginName) {
    ...
    if (kuzzle instanceof Kuzzle) {
      ...
      Reflect.defineProperty(this, 'log', {
        ...
        get: () => {
          return {
            debug: (...args) => kuzzle.log.debug(...addPluginName(pluginName, args)),
            error: (...args) => kuzzle.log.error(...addPluginName(pluginName, args)),
            info: (...args) => kuzzle.log.info(...addPluginName(pluginName, args)),
            silly: (...args) => kuzzle.log.silly(...addPluginName(pluginName, args)),
            verbose: (...args) => kuzzle.log.verbose(...addPluginName(pluginName, args)),
            warn: (...args) => kuzzle.log.warn(...addPluginName(pluginName, args))
          };
        }
      });
      ...
      const throwNotAvailable = name => () => {
        throw contextError.get('unavailable_realtime', name);
      };
...
/**
 * Adds the plugin name to log lines
 *
 * @param {String} pluginName
 * @param {String[]} lines
 ...
 * @returns {String[]}
 */
function addPluginName (pluginName, lines = []) {
  return lines.map(line => `[plugin-${pluginName}] ${line}`);
}
