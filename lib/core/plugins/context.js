...
function execute (kuzzle, request, callback) {
  ...
  if (!request || !(request instanceof Request)) {
    const error = contextError.get('missing_request');
  ...
  if ( request.input.controller === 'realtime'
    ...
    const error = contextError.get('unavailable_realtime', request.input.action);
    return callback ? callback(error) : reject(error);
  }
  ...
  kuzzle.funnel.executePluginRequest(request)
    .then(result => {
      ...
      return callback ? callback(null, request) : resolve(request);
    })
    .catch(err => {
