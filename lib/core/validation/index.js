...
const
  debug = require('../../util/debug')('kuzzle:validation'),
  _ = require('lodash'),
  Bluebird = require('bluebird'),
  errorsManager = require('../../util/errors'),
  Koncorde = require('koncorde'),
  { errors: { KuzzleError } } = require('kuzzle-common-objects'),
  { has, isPlainObject, get } = require('../../util/safeObject');

const assertionError = errorsManager.wrap('validation', 'assert');
/**
 * @class Validation
 * @param {Kuzzle} kuzzle
 */
class Validation {
  ...
  async validate (request, verbose = false) {
    ...
    if ( request.input.controller === 'document'
      ...
      const document = await this.kuzzle.storageEngine.public.get(
        index,
        collection,
        _id);
    ...
    if (collectionSpec) {
      ...
      if (collectionSpec.validators) {
        const filters = this.koncorde.test(index, collection, body, _id);
  ...
  async curateCollectionSpecification(
    ...
    const
      ...
      allowed = ['strict', 'fields', 'validators'];
    ...
    if (spec.validators && Array.isArray(spec.validators)) {
      try {
        const filter = await this.curateValidatorFilter(
          index,
          collection,
          spec.validators,
          dryRun);

        processed.validators = filter.id;
      }
      catch(e) {
  ...
  addType(validationType) {
    if (!validationType.typeName) {
      throw errorsManager.get('validation', 'types', 'missing_type_name');
    }
...
function manageErrorMessage(errorContext, errorHolder, message, structured) {
  if (structured) {
    ...
    throw errorsManager.get('validation', 'check', 'failed_document', message);
  }
  else {
    throw errorsManager.get(
      'validation',
