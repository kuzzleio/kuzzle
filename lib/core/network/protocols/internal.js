...
class InternalProtocol extends Protocol {
  ...
  async init (entryPoint) {
    ...
    this.kuzzle.onAsk('core:network:internal:connectionId', () => (
      this.connection.id
    ));

    this.kuzzle.onAsk('core:network:internal:channels', () => (
      this.channels
    ));
  }

  joinChannel (channel, connectionId) {
    debug('joinChannel: %s', channel);

    this.channels.add(channel);
  }

  leaveChannel (channel, connectionId) {
    debug('leaveChannel: %s', channel);
  ...
  _send (data) {
    for (let i = 0; i < data.channels.length; i++) {
      ...
      this.kuzzle.emit('core:network:internal:message', message);
    }
  }
}
