...
class HttpWsProtocol extends Protocol {
  ...
  async wsOnMessageHandler(socket, data) {
    ...
    ({ payload: message } = await global.kuzzle.pipe("protocol:websocket:beforeParsingPayload", { payload: message }));

    try {
      ({ payload: parsed } = await global.kuzzle.pipe("protocol:websocket:afterParsingPayload", { payload: JSON.parse(message) }));
    } catch (e) {
      /*
