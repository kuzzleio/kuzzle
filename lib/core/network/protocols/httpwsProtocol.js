...
class HttpWsProtocol extends Protocol {
  ...
  httpProcessRequest (response, message) {
    ...
    global.kuzzle.router.http.route(message, request => {
      ...
      this.httpCompress(message, data, result => {
        ...
        response.cork(() => {
          this.httpWriteRequestHeader(request, response, message);
   ...
   * @param {HttpMessage} message 
   */
  httpWriteRequestHeader(request, response, message) {
  ...
  httpSendStream(request, response, httpStream, message) {
    ...
    if (httpStream.stream._readableState.errored || httpStream.stream._readableState.closed) {
      return;
    }
    ...
    httpStream.stream.on('data', chunk => {
      ...
      let arrayBufferOffset = response.getWriteOffset();
      ...
      if (backpressure) {
        ...
        response.onWritable(offset => {
          ...
          let backpressure = false;
          const remainingChunkData = res.arrayBuffer.slice(offset - response.arrayBufferOffset);
