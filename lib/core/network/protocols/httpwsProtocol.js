...
class HttpWsProtocol extends Protocol {
  ...
  httpSendStream (request, response, httpStream, message) {
    ...
    if (httpStream.stream._readableState.errored) {
      this.httpSendError(
        message,
        response,
        kerror.get('network', 'http', 'stream_errored', httpStream.stream._readableState.errored)
      );
      return;
    }

    if ( httpStream.stream._readableState.closed
      || httpStream.stream._readableState.destroyed
      || httpStream.stream._readableState.ended
    ) {
      this.httpSendError(
        message,
        response,
