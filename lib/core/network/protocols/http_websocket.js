...
class HTTPWS extends Protocol {
  ...
  disconnect (connectionId, message = null) {
    ...
    socket.end(1011, message ? Buffer.from(message) : WS_GENERIC_CLOSE_MESSAGE);
  }
  ...
  wsOnMessageHandler (socket, data) {
    ...
    if (!data || data.byteLength === 0 || !connection) {
      return;
    }
  ...
  wsSend (socket, payload) {
    ...
    if (socket.getBufferedAmount() < WS_MAX_BACKPRESSURE) {
      ...
      if (buffer.length > WS_BACKPRESSURE_BUFFER_MAX_LENGTH) {
        socket.end(1011, WS_BACKPRESSURE_MESSAGE);
      }
    }
  }
  ...
  httpReadData (message, response, cb) {
    ...
    response.onData((data, isLast) => {
      ...
      this.httpUncompress(message, payload, (err, inflated) => {
        ...
        message.content = this.httpParseContent(message, inflated, cb);
      });
    });
  ...
  httpParseContent (message, content, cb) {
    ...
    if (type.includes('multipart/form-data')) {
      ...
      try {
        ...
        cb(kerrorHTTP.get('body_parse_failed', content.slice(0, 50)));
        return;
      }
    }
      ...
      this.httpCompress(message, data, (err, result) => {
        ...
        if (err) {
          const kuzerr = err instanceof KuzzleError
            ? err
            : kerrorHTTP.getFrom(err, 'unexpected_error', err.message);
