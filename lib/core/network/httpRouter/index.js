...
const errorsManager = require('../../../util/errors').wrap('network', 'http');
const { has } = require('../../../util/safeObject');
const debug = require('../../../util/debug')('kuzzle:http:router');
const RoutePart = require('./routePart');
const { Request, errors: { KuzzleError } } = require('kuzzle-common-objects');
...
class Router {
  ...
  route(message, cb) {
    ...
    try {
      ...
      if (routeHandler.handler === null) {
        throw errorsManager.get('url_not_found', routeHandler.url);
      }
  ...
  routeUnhandledHttpMethod(message, cb) {
    ...
    replyWithError(
      cb,
      request,
      errorsManager.get('unsupported_verb', message.method));
  }
  ...
  validateMessage (message) {
    ...
    if (contentType && contentType.indexOf('application/json') !== 0) {
      throw errorsManager.get('unsupported_content', contentType);
    }

    const encoding = CharsetRegex.exec(contentType);

    if (encoding !== null && encoding[1].toLowerCase() !== 'utf-8') {
      throw errorsManager.get('unsupported_charset', encoding[1].toLowerCase());
    }
  }
}
...
function attach(url, handler, target) {
  ...
  if (!attachParts(sanitized.split('/'), handler, target)) {
    throw errorsManager.get('duplicate_url', sanitized);
  }
}
