...
class EntryPoint {
  ...
  */
  ...
  execute (connection, request, cb) {
    ...
    global.kuzzle.pipe('request:beforeExecution', request)
      .then((modifiedRequest) => {
        global.kuzzle.funnel.execute(modifiedRequest, (error, result) => {
          const _res = result || modifiedRequest;
        ...
        const response = request.response.toJSON();
