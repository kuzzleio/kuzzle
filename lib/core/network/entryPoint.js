...
class EntryPoint {
  constructor (kuzzle) {
    ...
    for (const ProtocolClass of DEFAULT_PROTOCOLS) {
      ...
      this.protocols[ProtocolClass.name] = protocol;
    }
  }

  dispatch (event, data) {
    switch (event) {
      case DISPATCH_ACTIONS.NOTIFY:
        this._notify(data);
        return;
      case DISPATCH_ACTIONS.BROADCAST:
        this._broadcast(data);
        return;
      case DISPATCH_ACTIONS.SHUTDOWN:
  ...
  init () {
    ...
    for (const protocol of Object.values(this.protocols)) {
      ...
      initPromises.push(protocol.init(this)
        .then(enabled => {
          if (! enabled) {
            delete this.protocols[protocol.name];
          }
        })
      );
    }
  ...
  _broadcast (data) {
    ...
    for (const [name, protocol] of Object.entries(this.protocols)) {
      try {
        ...
        this.kuzzle.log.error(`[broadcast] protocol ${name} failed: ${e.message}\n${e.stack}`);
      }
    }
  }
