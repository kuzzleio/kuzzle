...
class CacheEngine {
  constructor (kuzzle) {
    ...
    this.public = new Redis(kuzzle, config.memoryStorage);
    this.internal = new Redis(kuzzle, config.internalCache);
  }
  ...
  registerInternalEvents () {
    ...
    /**
     * Executes a setnx command. Returns a boolean indicating if the key
     * was set or not.
     *
     * @param {string} key
     * @param {string} value
    ...
    this.kuzzle.onAsk(
      ...
      (key, value) => this.internal.commands.setnx(key, value));

    /**
     * Convenience method: sets a single key value: uses "set" if no timeout is
     * provided (or if it is 0), otherwise uses "setex"
     *
     * @param {string} key
     * @param {string} value
     * @param {number} [timeout] in seconds. if unset (or 0), stores the
     *                           key permanently
     */
    this.kuzzle.onAsk(
      'core:cache:internal:store',
      (key, value, timeout) => this.internal.store(key, value, timeout));
  }
  ...
  registerPublicEvents () {
    ...
    this.kuzzle.onAsk(
      ...
      (key, value, timeout) => this.public.store(key, value, timeout));
  }
}
