...
class FunnelProtocol extends KuzzleEventEmitter {
  ...
  async query (request) {
    ...
    // Use the provided request.kuid, otherwise use the impersonatedId previously given at funnel construction
    if (request.__kuid__) {
    ...
    try {
      if (this.requestOptions.user
        && (request.__kuid__ && request.__checkRights__)
        && ! await this.requestOptions.user.isActionAllowed(kuzzleRequest)
      ) {
        throw kerror.get(
          'security',
          'rights',
