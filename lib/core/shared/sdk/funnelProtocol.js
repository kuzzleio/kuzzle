...
class FunnelProtocol extends KuzzleEventEmitter {
  ...
  async _impersonate(requestedUserId) {
    if (typeof requestedUserId !== 'string') {
      throw kerror.get('plugin', 'context', 'invalid_user');
    }

    // Make sure to request the user only if needed
    if (!this.requestOptions.user || this.requestOptions.user._id !== requestedUserId) {
      this.requestOptions.user = await global.kuzzle.ask('core:security:user:get', requestedUserId);
    }
  }
  ...
  async query (request) {
    ...
    if (Object.prototype.hasOwnProperty.call(request, '__userId__')) {
      await this._impersonate(request.__userId__);
    }
    else if (this._impersonateId) { // Funnel can also be initialized without an impersonatedId
      await this._impersonate(this._impersonateId);
    }
