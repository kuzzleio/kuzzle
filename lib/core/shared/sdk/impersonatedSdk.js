      ...
      Reflect.defineProperty(this, controllerName, {
        get: () => {
          ...
          if (! this[subProxyName]) {
            // Create a local wrap function (proxy) that injects our local 'query' behaviour in the underlying base 'sdk',
            // So we can send impersonation configuration (kuid & checkRights) to the funnel
              ...
              get: (target, property) => {
                return target[property].bind({
                  query: (request, opts) => {
                    ...
                    return this.query(request, opts); // Call the impersonatedSdk.query method instead
                  }
                });
              }
            });
