...
const debug = require('../../../util/debug')('kuzzle:store:bootstrap');
...
class StoreBootstrap {
  /**
   * @param {Store} store - Index storage wrapper
  ...
  constructor (store, timeout) {
    ...
    this._LOCK_ID = `${this._store.index}.lock`;
    this._BOOTSTRAP_DONE_ID = `${this._store.index}.done`;
  }
  ...
  async startOrWait () {
    const bootstrapDone =
      await this._store.exists('config', this._BOOTSTRAP_DONE_ID);
  ...
  async _waitTillUnlocked (attempts = 0) {
    const exists = await this._store.exists('config', this._LOCK_ID);
  ...
  async _isLocked () {
    ...
    try {
      ...
      await this._store.create('config', this._LOCK_ID, {
        refresh: 'wait_for',
        timestamp: Date.now(),
      });
    ...
    try {
      lock = await this._store.get('config', this._LOCK_ID);
    }
    catch (error) {
    ...
    if (lock._source.timestamp < thirtySecondsAgo) {
      ...
      await this._store.createOrReplace('config', this._LOCK_ID, {
        timestamp: Date.now(),
      });
  ...
  _unlock () {
    ...
    return this._store.delete('config', this._LOCK_ID);
  }
}
