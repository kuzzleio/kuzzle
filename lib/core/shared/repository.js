 ...
 * @property {cacheDbEnum|null} cacheDb
...
class Repository {
  ...
  constructor(kuzzle, index = kuzzle.internalIndex.index) {
    ...
    this.cacheDb = cacheDbEnum.INTERNAL;
  }

  /**
   * Initialize the repository
   *
   * @param {{cache: cacheDbEnum}} options
   */
  init (options) {
  ...
  async scroll (scrollId, ttl) {
    const response = await this.indexStorage.scroll(scrollId, ttl);
  ...
  async persistToCache (object, options = {}) {
    ...
    await this.kuzzle.ask(`core:cache:${this.cacheDb}:store`, key, value, ttl);
