...
class Store {
  ...
  async init (collections = {}) {
    ...
    if (creatingLocked) {
      ...
      const cachingLocked = await creatingMutex.lock();
      try {
        await this.createCollections(collections, {
          indexCacheOnly: true
        });
    ...
    return Bluebird.map(
      ...
      async ([collection, config]) => {
        ...
        if (config.mappings !== undefined &&
          ...
          let isConfigDeprecated = false;

          // @deprecated
          if (config.settings.number_of_shards === undefined &&
            config.settings.number_of_replicas === undefined
          ) {
            isConfigDeprecated = true;
          }
          ...
          if (exist) {
            ...
            const dynamicSettings =
              (isConfigDeprecated) ? null : getESIndexDynamicSettings(config.settings);
