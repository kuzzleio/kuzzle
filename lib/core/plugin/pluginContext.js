...
class PluginContext {
  ...
  constructor(kuzzle, pluginName) {
    ...
    if (kuzzle instanceof Kuzzle) {
      ...
      Reflect.defineProperty(this.accessors, 'realtime', {
        enumerable: true,
        value: {
          registerSubscription: (request) =>
            kuzzle.ask(
              'core:realtime:hotelClerk:subscribe',
              request
            ),
          unregisterSubscription: (connectionId, roomId, notify) => 
            kuzzle.ask(
              'core:realtime:hotelClerk:unsubscribe',
