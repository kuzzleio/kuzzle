...
class PluginsManager {
  ...
  set primaryPlugin (plugin) {
    assert(this._plugins.size === 0, 'The primary plugin can only be set before every other plugins are loaded');
  ...
  get plugins () {
    return Array.from(this._plugins.values())
      .filter(plugin => ! plugin.primary)
      .map(plugin => plugin.name);
  }

  get application () {
    return Array.from(this._plugins.values()).find(plugin => plugin.primary);
  }
  ...
  init (additionalPlugins = []) {
    this._plugins.push(...this.loadPlugins(additionalPlugins));
