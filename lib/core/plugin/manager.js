  ...
  registerStrategy (pluginName, strategyName, strategy) {
    ...
    for (const methodName of Object.keys(strategy.methods).filter(
      ...
      methods[methodName] = async (...args) => {
        try {
          const boundFunction = plugin.object[strategy.methods[methodName]]
            .bind(plugin.object);
    ...
    const verifyAdapter = this.wrapStrategyVerify(
      pluginName,
      strategyName,
      plugin.object[strategy.methods.verify].bind(plugin.object));

    try {
      const Ctor = this.authenticators[pluginName][strategy.config.authenticator];
      const instance = new Ctor(opts, verifyAdapter);
