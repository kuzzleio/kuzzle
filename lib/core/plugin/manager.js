...
class PluginsManager {
  ...
  _initApi (plugin) {
    for (const [controller, definition] of Object.entries(plugin.instance.api)) {
      ...
      for (const [action, { http, handler} ] of Object.entries(definition)) {
  ...
  _initControllers (plugin) {
    ...
    if (plugin.instance.routes) {
      plugin.instance.routes.forEach(route => {
        ...
        route.url = `/_plugin/${plugin.name}${url}`;
  ...
  loadPlugins (plugins = {}) {
    ...
    for (const [name, instance] of Object.entries(plugins)) {
      const plugin = new Plugin(instance, { name });

      plugin.init(this.kuzzle, name);
