...
class FunnelProtocol extends KuzzleEventEmitter {
  constructor(kuzzle, user = null) {
    ...
    /**
     * Responses are sended by the InternalProtocol through the internal
     * event system.
     */
    this.kuzzle.on('core:network:internal:message', payload => {
      assert(Boolean(payload.room), 'Emitted payload must contain a "room" property.');

      this.emit(payload.room, payload);
    });
  }
  ...
  async query (request) {
    if (! this.requestOptions) {
      this.requestOptions = {
        connection: {
          protocol: this.id,
          id: this.kuzzle.ask('core:network:internal:connectionId')
        },
        user: this.user
      }
    }
