      ...
      const cb = (error, result) => {
        if (warnDelay) {
          ...
          if (elapsed > warnDelay) {
            global.kuzzle.log.warn(`${plugin.logPrefix} pipe for event '${event}' is slow (${elapsed}ms)`);
          }
        }
  ...
  async _initApi (plugin) {
    for (const [controller, definition] of Object.entries(plugin.instance.api)) {
      ...
      for (const [action, actionDefinition ] of Object.entries(definition.actions)) {
        ...
        // Retreive openAPI route custom specification, if any
        let openAPIRoute;
        if (actionDefinition.openapi) {
          openAPIRoute = await convertJsonToOpenAPI(actionDefinition.openapi);
          openAPIRoute = await new Enforcer(openAPIRoute).value;
        }
        ...
        for (const httpRoute of httpRoutes) {
          ...
          // Retreive openAPI HTTP route custom specification, if any
          let openAPIHTTPRoute;
          if (httpRoute.openapi) {
            openAPIHTTPRoute = await convertJsonToOpenAPI(httpRoute.openapi);
            openAPIHTTPRoute = await new Enforcer(openAPIHTTPRoute).value;
          }

          this.routes.push({
            action,
            controller,
            // OpenAPI definition: http level > action level > undefined (auto-generated by Kuzzle)
            openapi: openAPIHTTPRoute ? openAPIHTTPRoute : openAPIRoute,
            path: routePath,
            verb: httpRoute.verb,
          });
        }
