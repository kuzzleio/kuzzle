...
class PluginsManager {
  ...
  registerPipe (plugin, warnDelay, event, fn) {
    ...
    const _fn = typeof fn === 'function'
      ? fn
      : plugin.instance[fn].bind(plugin.instance);
      ...
      for (const [action, { http, handler} ] of Object.entries(definition.actions)) {
        ...
        if (http) {
          for (const { url, verb } of http) {
            ...
            this.routes.push({ action, controller, url: `/_/${url}`, verb });
          }
        }
      }
    }
  ...
  _initControllers (plugin) {
    ...
    for (const route of (plugin.instance.routes || [])) {
      ...
      this.routes.push({
        action: route.action,
        controller,
        url: `/_plugin/${plugin.name}${route.url}`,
        verb: route.verb
      });

      const url = route.url.charAt(0) === '/'
        ? `/_${route.url}`
        : `/_/${plugin.name}/${route.url}`;
