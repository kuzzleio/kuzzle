...
class PluginsManager {
  ...
  set application (plugin) {
    ...
    assert(plugin.application, 'The application plugin must have the "application" property equal to true');
   ...
   * and holds the Application instance
   *
   * @param {Application} - Application plugin instance
   * @param {Object} - Plugin additional options
   * @returns {Promise}
   * @this {PluginsManager}
   * @throws PluginImplementationError - Throws when an error occurs while registering a plugin
   */
  init (application, plugins = {}) {
    ...
    /**
     * Gets all the registered plugin controllers
     * @returns {Map.<controller, BaseController instance>} Controllers
     */
    this.kuzzle.onAsk('core:plugin:controllers:get', () => this.controllers);

    /**
     * Gets all the existing plugin routes
     * @returns {Array.<Object>} Routes
     */
    this.kuzzle.onAsk('core:plugin:routes:get', () => this.routes);

    /**
     * Gets all the existing plugin descriptions
     * @returns {Array.<Object>} Plugin Descriptions
     */
    this.kuzzle.onAsk('core:plugin:descriptions:get', () => this.getPluginsDescription());
    ...
    return Promise.all(loadPlugins).then(() => {
      this.kuzzle.log.info(`[✔] Successfully loaded ${this.plugins.length} plugins: ${this.plugins.map(p => p.name).join(', ')}`);
    });
  }
  ...
  /**
   * Loads detected plugins into memory
   *
