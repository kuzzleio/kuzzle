...
const CORE_PLUGINS = ['kuzzle-plugin-logger', 'kuzzle-plugin-auth-passport-local'];
...
class PluginsManager {
  ...
  init (plugins = {}) {
    ...
    for (const plugin of this._plugins.values()) {
      if (this.config.common.failsafeMode && ! CORE_PLUGINS.includes(plugin.name)) {
        global.kuzzle.log.info(`Failsafe mode activated, skipping plugin "${plugin.name}"`);
        continue;
      }
