...
const EmbeddedSDK = require('./sdk/embeddedSdk');
...
class PluginContext {
  ...
  constructor(kuzzle, pluginName) {
    ...
    if (kuzzle instanceof Kuzzle) {
      const pluginIndex = `plugin-${pluginName}`.toLowerCase(), // uppercase are forbidden by ES
      ...
      this.constructors.Repository = function PluginContextRepository(
        ...
        return {
          ...
          createOrReplace: (...args) =>
            pluginRepository.createOrReplace(...args),
      ...
      Reflect.defineProperty(this, 'log', {
        ...
        get: () => {
          return {
            debug: (msg) => kuzzle.log.debug(`[plugin-${pluginName}] ${msg}`),
            error: (msg) => kuzzle.log.error(`[plugin-${pluginName}] ${msg}`),
            info: (msg) => kuzzle.log.info(`[plugin-${pluginName}] ${msg}`),
            silly: (msg) => kuzzle.log.silly(`[plugin-${pluginName}] ${msg}`),
            verbose: (msg) =>
              kuzzle.log.verbose(`[plugin-${pluginName}] ${msg}`),
            warn: (msg) => kuzzle.log.warn(`[plugin-${pluginName}] ${msg}`),
          };
        },
      });
      ...
      Reflect.defineProperty(this.accessors, 'registerSubscription', {
        enumerable: true,
        get: () => (connectionId, index, collection, filters, options) =>
          kuzzle.ask(
...
function curryAddStrategy(kuzzle, pluginName) {
  return async function addStrategy(name, strategy) {
    ...
    return kuzzle.pipe('core:auth:strategyAdded', {
      name,
      pluginName,
      strategy,
    });
