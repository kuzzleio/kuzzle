...
const convertJsonToOpenAPI = require('@openapi-contrib/json-schema-to-openapi-schema');
    ...
    for (const [action, actionDefinition] of Object.entries(definition.actions)) {
      ...
      if (actionDefinition.http) {
        ...
        for (const route of actionDefinition.http) {
          ...
          if (route.openapi) {
            if (! isPlainObject(actionDefinition.openapi)) {
              throw assertionError.get(
                'invalid_controller_definition',
                name,
                'If defined in a http route, "openapi" property must be an object');
            }
            await checkOpenAPISpecification(name, action, actionDefinition.openapi, route);
          }
        }
      }

      if (actionDefinition.openapi) {
        if (! isPlainObject(actionDefinition.openapi)) {
          throw assertionError.get(
            'invalid_controller_definition',
            name,
            'If defined in an action, "openapi" property must be an object');
        }
        await checkOpenAPISpecification(name, action, actionDefinition.openapi);
      }
    }
  }
}
...
async function checkOpenAPISpecification (controller, action, openapi, route = null) {
  const routeMessage = isPlainObject(route) ? ` in ${route} =>` : '';

  let openAPISpec = await convertJsonToOpenAPI(openapi);
  const { error, warning } = await new Enforcer(openAPISpec);

  if (warning) {
    global.kuzzle.log.warn(`Open API warning for "${controller}:${action}":${routeMessage} ${warning}`);
  }
  if (error) {
    if (isPlainObject(route)) {
      throw controllerError.get('invalid_openapi_schema', controller, action, routeMessage, error);
    }
    throw controllerError.get('invalid_openapi_schema', controller, action, error);
  }
}
