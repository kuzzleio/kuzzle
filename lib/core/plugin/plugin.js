    ...
    for (const [action, actionDefinition] of Object.entries(definition.actions)) {
      ...
      if (actionDefinition.http) {
        ...
        for (const route of actionDefinition.http) {
          ...
          if (route.openapi) {
            checkOpenAPISpecification(name, action, actionDefinition.openapi);
          }
        }
      }
...
function checkOpenAPISpecification (controller, action, openapi) {
  if (! isPlainObject(openapi)) {
    throw assertionError.get(
      'invalid_controller_definition',
      controller,
      'When defined, "openapi" property must be an object');
  }
  ...
  if (warning) {
    global.kuzzle.log.warn(`Warning for OpenAPI specification in "${controller}:${action}", ${openapi} : ${warning}`);
  }
  if (error) {
    throw controllerError.get('invalid_openapi_schema', controller, action, openapi, error);
  }
}
