...
class Plugin {
  ...
  info () {
    ...
    if (has(this.instance, 'hooks')) {
      description.hooks = _.uniq(Object.keys(this.instance.hooks));
    }

    if (has(this.instance, 'pipes')) {
      description.pipes = _.uniq(Object.keys(this.instance.pipes));
    }

    if (has(this.instance, 'controllers')) {
      description.controllers = _
        .uniq(Object.keys(this.instance.controllers))
        .map(controller => `${this.name}/${controller}`);
    }

    if (has(this.instance, 'routes')) {
      description.routes = _.uniq(this.instance.routes);
    }
  ...
  set name (name) {
    if (! this.constructor.checkName(name)) {
      ...
      this.kuzzle.log.warn(`/!\\ Plugin name "${name}" converted to "${this._name}. Plugin names should be in kebab-case."`)
    }
    else {
      this._name = name;
    }
  ...
  static loadFromDirectory (kuzzle, pluginPath) {
    ...
    const plugin = this._instantiatesPlugin(kuzzle, pluginPath);
  ...
  static checkControllerDefinition (name, definition) {
    ...
    for (const [action, { handler, http }] of Object.entries(definition.actions)) {
   ...
   */
  static _instantiatesPlugin (kuzzle, PluginClassOrPath) {
    try {
      ...
      return new Plugin(kuzzle, pluginInstance);
    }
    catch (error) {
