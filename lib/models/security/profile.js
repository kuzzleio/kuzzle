...
class Profile {
  ...
  async getPolicies() {
    ...
    return Bluebird.all(this.policies.map(async policy => {
      const role = await this[_kuzzle].repositories.role.load(policy.roleId);
  ...
  async getRights() {
    ...
    for (const policy of policies) {
      ...
      for (const [controller, actions] of Object.entries(role.controllers)) {
        for (const [action, actionRights] of Object.entries(actions)) {
