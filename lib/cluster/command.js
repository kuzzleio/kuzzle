...
class ClusterCommand {
  ...
  async getFullState (nodes) {
    ...
    for (let retries = 0; fullState === null && retries < nodes.length; retries++) {
      ...
      try {
        ...
        // with another one
        this.kuzzle.log.warn(`Unable to fetch a full state from node ${nodes[idx][0]} (no response received)`);
    ...
    return decoder.decode(fullState).toJSON();
  }
  ...
  async broadcastHandshake (nodes) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      if (responses[i] === null) {
        result[nodes[i][0]] = null;
      }
      else {
        result[nodes[i][0]] = decoder.decode(responses[i]).toJSON();
      }
    }
  ...
  async _sendSingleHandshake (id, ip, payload) {
    ...
    try {
      ...
      this.kuzzle.log.warn(`Couldn't complete handshake with node ${id}: no response received`);
    }
    finally {
      req.close();
    }
