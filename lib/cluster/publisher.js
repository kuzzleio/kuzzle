...
class ClusterPublisher {
  ...
  /**
   * Publishes an event about a new virtual index being added
   *
   * @param  {string} physical
   * @param  {string} virtual
   * @returns {Long} ID of the message sent
   */
  sendAddVirtualIndex(physical, virtual) {
    return this.send("AddVirtualIndex", { physical, virtual });
  }

  /**
   * Publishes an event about a deletion of a virtual index
   *
   * @param  {string} virtual
   * @returns {Long} ID of the message sent
   */
  sendRemoveVirtualIndex(virtual) {
    return this.send("RemoveVirtualIndex", { virtual });
  }
