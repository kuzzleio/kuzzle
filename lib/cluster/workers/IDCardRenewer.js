...
class IDCardRenewer {
  ...
  async renewIDCard() {
    ...
    try {
      let keyTTL = this.refreshDelay * this.refreshMultiplier;
...
if (!isMainThread) {
  ...
  parentPort.on("message", async (message) => {
    ...
    } else if (message.action === "preventEviction") {
      idCardRenewer.evictionPrevented = message.state;
