...
class ClusterNode {
  ...
  registerEvents() {
    ...
    global.kuzzle.on(
      VirtualIndex.createVirtualIndexEvent,
      ({ physical, virtual }) => this.onVirtualIndexAdded(physical, virtual)
    );

    global.kuzzle.on(VirtualIndex.deleteVirtualIndexEvent, ({ virtual }) => {
      this.onVirtualIndexRemoved(virtual);
    });
  ...
  onVirtualIndexAdded(physical, virtual) {
    this.publisher.sendAddVirtualIndex(physical, virtual);
  }

  onVirtualIndexRemoved(virtual) {
    this.publisher.sendRemoveVirtualIndex(virtual);
  }
