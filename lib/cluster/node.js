...
class ClusterNode {
  ...
  async handshake() {
    ...
    try {
      ...
      // The cluster id and the id everything else logs under are generated
      // independently — `idCardHandler.createIdCard()` and `backend.ts`'s
      // `global.nodeId` — from the same generator, with the same `knode`
      // prefix. Nothing ever printed both, so a line naming a peer could not be
      // matched to a container, and six occurrences of TD-33 (#2715) were read
      // off logs where no node could be identified. This line is the binding.
      this.logger.info(
