...
class ClusterNode {
  ...
  async generateId () {
    ...
    this.heartbeatTimer = setInterval(
      async () => {
        ...
        await this.publisher.send('Heartbeat', {
  ...
  async addNode (id, ip, lastMessageId) {
    ...
    const subscriber = new ClusterSubscriber(id, `tcp://${ip}:${this.config.ports.sync}`);
  ...
  async handshake () {
    ...
    try {
      const nodes = await this.listRemoteNodes();

      // Create this node ID until AFTER: 1/ we get the list of redis keys for
      // remote nodes, and 2/ the handshake mutex is actually locked, to prevent
      // race conditions (other nodes attempting to connect to this node
      ...
      const fullState = await this.command.getFullState(nodes);
      ...
      // means we're facing a network split, and we must then shut down.
      if (fullState === null) {
      }
    }
    finally {
  ...
  async listRemoteNodes () {
    ...
    const keys = await this.kuzzle.ask(
