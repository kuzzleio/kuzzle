...
class ClusterSubscriber {
  ...
  constructor(localNode, remoteNodeId, remoteNodeIP) {
    ...
    this.handlers = Object.freeze({
      ...
      AddVirtualIndex: this.handleVirtualIndexAddition,
      ...
      RemoveVirtualIndex: this.handleVirtualIndexRemoval,
  ...
  /**
   * Adds a new virtual index in the map of virtual index
   *
   * @param {Object} message
   * @return {void}
   */
  async handleVirtualIndexAddition(message) {
    const { real, virtual } = message;

    debug(
      "New virtual index added by node %s (index: %s)",
      this.remoteNodeId,
      virtual
    );

    await global.kuzzle.ask(VirtualIndex.createEvent, real, virtual);
  }

  /**
   * Removes a virtual index in the map of virtual index
   *
   * @param {Object} message
   * @return {void}
   */
  async handleVirtualIndexRemoval(message) {
    const { virtual } = message;

    debug(
      "New virtual index removed by node %s ( index: %s)",
      this.remoteNodeId,
      virtual
    );

    await global.kuzzle.ask(VirtualIndex.deleteEvent, virtual);
  }
