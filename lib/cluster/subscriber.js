...
class ClusterSubscriber {
  ...
  constructor (localNode, remoteNodeId, remoteNodeAddress) {
    ...
    this.handlers = Object.freeze({
      ...
      IndexCacheAdd: this.handleIndexCacheAddition,
      IndexCacheRemove: this.handleIndexCacheRemoval,
  ...
  async processData (topic, data) {
    ...
    try {
      ...
      global.kuzzle.log.error(`[FATAL] Unable to process sync message (topic: ${topic}, message: ${JSON.stringify(message)}`);
      global.kuzzle.log.error(e.stack);
  ...
  handleNewRealtimeRoom (message) {
    const { roomId, index, collection, filters, messageId } = message;
   ...
   * @return {void}
   */
  async handleClusterWideEvent (message) {
  ...
  async handleDocumentNotification (message) {
    ...
    notification.node = this.remoteNodeId;
    notification.status = message.status;
    notification.requestId = message.requestId;
    notification.timestamp = message.timestamp.toNumber();
    notification.index = message.index;
    notification.collection = message.collection;
    notification.controller = message.controller;
    notification.action = message.action;
    notification.protocol = message.protocol;
    notification.scope = message.scope;
    notification.volatile = JSON.parse(message.volatile);
    notification.result = JSON.parse(message.result);
  ...
  async handleUserNotification (message) {
    ...
    notification.action = message.action;
    notification.protocol = message.protocol;
    notification.user = message.user;
    notification.volatile = JSON.parse(message.volatile);
  ...
  handleDumpRequest (message) {
    ...
    global.kuzzle.janitor.dump(message.suffix);
  }
  ...
  /**
   * Adds a new index or a new index/collection pair to the index cache
   *
   * @param {Object} message - decoded IndexCacheAdd protobuf message
   * @return {void}
   */
  async handleIndexCacheAddition (message) {
    const { collection, index, scope } = message;

    debug(
      'New index cache entry added by node %s (scope: %s, index: %s, collection: %s)',
      this.remoteNodeId,
      scope,
      index,
      collection);
   ...
   * @return {void}
   */
  async handleIndexCacheRemoval (message) {
    const { collection, index, scope } = message;

    debug(
      'Index cache entry removed by node %s (scope: %s, index: %s, collection: %s)',
      this.remoteNodeId,
      scope,
      index,
      collection);
    ...
    await global.kuzzle.ask(
      `core:storage:${scope}:cache:remove`,
      index,
      collection);
  }
  ...
  async validateMessage (message) {
    ...
    if (this.lastMessageId.notEquals(message.messageId)) {
      global.kuzzle.log.error(`[FATAL] Node out-of-sync: ${message.messageId - this.lastMessageId - 1} messages lost from node ${this.remoteNodeId}`);
      global.kuzzle.shutdown();
      return false;
    }
