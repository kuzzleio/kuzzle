...
const { binarySearch } = require('../../util/array');
...
class Role {
  ...
  /**
   * @param {Request} request
   * @param {{[key: string]: string[]}} restrictedTo Restricted indexes
   * @returns {Boolean} resolves to a Boolean value
   */
  checkRestrictions(request, restrictedTo = {}) {
    ...
    if (! has(restrictedTo, index)) {
      return false;
    }

    const collections = restrictedTo[index];
    ...
    const requestedCollection = request.input.args.collection;

    // Find collection index in array
    const indexOfCollection = binarySearch(collections, (collection) => {
      if (requestedCollection > collection) {
        return 1;
      }
      return requestedCollection < collection
        ? -1
        : 0;
    });
 ...
 * @returns {Boolean}
 */
function checkCollectionRestriction(request, collections) {
