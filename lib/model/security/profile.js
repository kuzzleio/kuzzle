...
class Profile {
  ...
  async getPolicies() {
    ...
    return Bluebird.map(this.policies, async ({restrictedTo, roleId}) => {
      const role = await this[_kuzzle].ask('core:security:role:get', roleId);
   ...
   * @param {boolean} [options.force] - Forces the validation even if the index
   *                                    or the collection doesn't exist
   * @returns {Promise}
   */
  async validateDefinition({ force = false } = {}) {
    ...
    for (const policy of this.policies) {
      ...
      if (policy.restrictedTo) {
        ...
        for (const restriction of policy.restrictedTo) {
          ...
          const indexExists = await this[_kuzzle].storageEngine.public
            .indexExists(restriction.index);
          ...
          if (!indexExists) {
            ...
            this[_kuzzle].log.warn(`The profile "${this._id}" gives access to the non-existing index "${restriction.index}".`);
          }
          ...
          if ( restriction.collections !== undefined
            ...
            if (indexExists) {
              ...
              for (const collection of restriction.collections) {
                const isValid = await this[_kuzzle].storageEngine.public
                  .collectionExists(restriction.index, collection);
              ...
              if (invalidCollections.length > 0) {
                ...
                this[_kuzzle].log.warn(`The profile "${this._id}" gives access to non-existing collections "${invalidCollections}".`);
              }
            }
          }
