...
class BaseModel {
  ...
  static init (kuzzle) {
    this.kuzzle = kuzzle;
    this.store = new Store(kuzzle, kuzzle.internalIndex, scopeEnum.PRIVATE);
  }
  ...
  async save ({ userId=null, refresh } = {}) {
    if (! this.__persisted) {
      const { _id, _source } = await BaseModel.store.create(
  ...
  static async load (id) {
    const result = await BaseModel.store.get(
      this.collection,
      id);
  ...
  static async deleteByQuery (query, { refresh } = {}) {
    const { documents } = await BaseModel.store.deleteByQuery(
      this.collection,
      query);
    ...
    if (refresh) {
      await BaseModel.store.refreshCollection(this.collection);
    }
  }
  ...
  static async search (searchBody, options) {
    const resp = await BaseModel.store.search(
      this.collection,
      searchBody,
      options);
  ...
  static batchExecute (query, callback) {
    return BaseModel.store.mExecute(this.collection, query, callback);
  }
