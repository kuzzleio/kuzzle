...
class BaseModel {
  ...
  async save ({ userId=null, refresh } = {}) {
    if (! this.__persisted) {
      const { _id, _source } = await Kuzzle.getInstance().internalIndex.create(
      ...
      await Kuzzle.getInstance().internalIndex.update(
        this.constructor.collection,
        this._id,
        this._source,
        { refresh, userId });
  ...
  async delete ({ refresh } = {}) {
    ...
    await Kuzzle.getInstance().internalIndex.delete(
      this.constructor.collection,
      this._id,
      { refresh });
  ...
  static async load (id) {
    const result = await Kuzzle.getInstance().internalIndex.get(
      this.collection,
      id);
  ...
  static async deleteByQuery (query, { refresh } = {}) {
    const { documents } = await Kuzzle.getInstance().internalIndex.deleteByQuery(
      this.collection,
      query);
    ...
    if (refresh) {
      await Kuzzle.getInstance().internalIndex.refreshCollection(this.collection);
    }
  }
  ...
  static async search (searchBody, options) {
    const resp = await Kuzzle.getInstance().internalIndex.search(
      this.collection,
      searchBody,
      options);
  ...
  static batchExecute (query, callback) {
    return Kuzzle.getInstance().internalIndex.mExecute(
      this.collection,
      query,
      callback);
  }
