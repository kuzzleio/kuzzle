...
class BaseModel {
  ...
  async save ({ userId=null, refresh } = {}) {
    if (! this.__persisted) {
      const { _id, _source } = await BaseModel.kuzzle.internalIndex.create(
      ...
      await BaseModel.kuzzle.internalIndex.update(
        this.constructor.collection,
        this._id,
        this._source,
        { refresh, userId });
  ...
  async delete ({ refresh } = {}) {
    ...
    await BaseModel.kuzzle.internalIndex.delete(
      this.constructor.collection,
      this._id,
      { refresh });
  ...
  static async load (id) {
    const result = await BaseModel.kuzzle.internalIndex.get(
      this.collection,
      id);
  ...
  static async deleteByQuery (query, { refresh } = {}) {
    const { documents } = await BaseModel.kuzzle.internalIndex.deleteByQuery(
      this.collection,
      query);
    ...
    if (refresh) {
      await BaseModel.kuzzle.internalIndex.refreshCollection(this.collection);
    }
  }
  ...
  static async search (searchBody, options) {
    const resp = await BaseModel.kuzzle.internalIndex.search(
      this.collection,
      searchBody,
      options);
  ...
  static batchExecute (query, callback) {
    return BaseModel.kuzzle.internalIndex.mExecute(
      this.collection,
      query,
      callback);
  }
