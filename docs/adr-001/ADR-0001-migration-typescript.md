...
**Counters** (baselines in `.migration/`, re-verified on the H3 branch 2026-09-11): **js = 20**, **mocha = 150**, **any = 205**, **implicit-any = 457**; **strict adopted = 125** *(`--candidates` empty)*. H1 took js 49 → 35 and strict 102 → 117; H2 took js 35 → 23, strict 117 → 123, and implicit-any 461 → 457 by declaring `global.nodeId`, which five files read and nothing declared; H3 took js 23 → 20, strict 123 → 125, and **moved the mocha counter for the first time** (151 → 150) by replacing `pluginManifest`'s Mocha spec rather than duplicating it. The `any` metric includes `as unknown as`; it dropped 208 → 207 when TD-21 removed the cast its guard needed, then 207 → 205 when TD-34 typed `profileRepository`'s `_hash` patch site. **Coverage is measured again on `.ts`** — `.ci/scripts/prepare-coverage.ts` normalises both reports and gives each file a single owning runner. Unit tests: **3027 Mocha + 189 vitest** (was 7). A **fifth ratchet**, `cpd-exclusions`, was added 2026-09-11 ([TD-23](type-debt-register.md#td-23)); its baseline is the 4 files currently excluded from CPD.
**Remaining JS by layer** (20 = 17 `lib/` + 3 `bin/`): `core` **5** (validation 1 — `validation.js`, H4 · plugin 2 — `plugin` + `pluginsManager`, H5 · network 2 — `entryPoint` + `httpwsProtocol`, H6) · `api` **0** · `kuzzle` 6 · `cluster` 6 · `bin` **3**, all plugin **fixtures** under `bin/plugins/available/**`, i.e. the floor — reached by [#2713](https://github.com/kuzzleio/kuzzle/pull/2713).
