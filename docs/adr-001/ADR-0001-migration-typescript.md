...
1. **Sprint 4 — `lib/api` ([step 05](steps/05-sprint-4-api.md)): converted, closing.** PRs A–E1 (#2679, #2680, #2681, #2682, #2685) are **merged**; js 79→67. **Open: PR E2** = `funnel.js`, the last JS file of the layer ([#2686](https://github.com/kuzzleio/kuzzle/pull/2686), `chore/ts-migration-sprint4-funnel`) — js 67→66, `lib/api` now **100% TS**. Locally green (tsc, lint, ratchets, strict, Mocha 3025 + vitest 7/7 in Docker); **still to do on E2:** the SonarCloud gate (the 1143-line rename re-scores as new code), then merge and flip step 05 to ✅. *(Sprint 2 — real `bin/` entrypoints — stays deprioritized, see [step 03](steps/03-sprint-2-bin.md).)*
