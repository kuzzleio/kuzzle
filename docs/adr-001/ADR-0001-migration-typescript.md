...
**Counters** (baselines in `.migration/`): **js = 51**, **mocha = 151**, **any = 208** *(metric broadened to include `as unknown as`; was 200 on the narrower grep)*, **implicit-any = 518**; **strict adopted = 101** *(`--candidates` empty)*. **Coverage is measured again on `.ts`: the 30 converted files sit at 85.9%**, `lib/**/*.ts` at 88.5%. Unit tests: 3025 Mocha + **88 vitest** (was 7).
**Remaining JS by layer** (51 = 47 `lib/` + 4 `bin/`): `core` 35 · `api` **0** · `kuzzle` 6 · `cluster` 6 · `bin` 4 *(all four are plugin **fixtures** under `bin/plugins/available/**` — out of the Definition of Done, see the register)*.
