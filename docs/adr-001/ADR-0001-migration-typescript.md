...
**Counters** (baselines in `.migration/`, re-verified on the H2 branch 2026-09-11): **js = 23**, **mocha = 151**, **any = 205**, **implicit-any = 457**; **strict adopted = 123** *(`--candidates` empty)*. H1 took js 49 → 35 and strict 102 → 117; H2 took js 35 → 23, strict 117 → 123, and implicit-any 461 → 457 by declaring `global.nodeId`, which five files read and nothing declared. The `any` metric includes `as unknown as`; it dropped 208 → 207 when TD-21 removed the cast its guard needed, then 207 → 205 when TD-34 typed `profileRepository`'s `_hash` patch site. implicit-any reached **461** with TD-28's third `Reflect.set` site (`core/shared/store.ts`). **Coverage is measured again on `.ts`** — `.ci/scripts/prepare-coverage.ts` normalises both reports and gives each file a single owning runner. Unit tests: 3030 Mocha + **183 vitest** (was 7). A **fifth ratchet**, `cpd-exclusions`, was added 2026-09-11 ([TD-23](type-debt-register.md#td-23)); its baseline is the 4 files currently excluded from CPD.
**Remaining JS by layer** (23 = 20 `lib/` + 3 `bin/`): `core` **8** (validation 1 — `validation.js`, H4 · network 2 — `entryPoint` + `httpwsProtocol`, H6 · plugin 5, H3 + H5) · `api` **0** · `kuzzle` 6 · `cluster` 6 · `bin` **3**, all plugin **fixtures** under `bin/plugins/available/**`, i.e. the floor — reached by [#2713](https://github.com/kuzzleio/kuzzle/pull/2713).
...
| 09 | Sprint 6 — core II (validation, plugin, network) | 🟦 In progress | [#2722](https://github.com/kuzzleio/kuzzle/pull/2722) H1 · [#2723](https://github.com/kuzzleio/kuzzle/pull/2723) H2 | [detail](steps/09-sprint-6-core-ii.md) |
