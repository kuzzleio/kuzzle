...
- **2026-09-09** — **The 4 remaining `bin/` `.js` are out of the Definition of Done**: all live under `bin/plugins/available/**` (`functional-test-plugin`, `kuzzle-plugin-cluster`) and are plugin **fixtures**, not product code. "0 `.js` in `bin/`" targets `bin/` proper. → [03](steps/03-sprint-2-bin.md) ⚠️ **Partly wrong** — only 3 of the 4 are fixtures; `bin/copy-binaries.js` is build tooling, so the floor is 3 ([#2705](https://github.com/kuzzleio/kuzzle/issues/2705)).
