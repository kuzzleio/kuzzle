...
- [ ] `0` `.js` files in `lib/` and `bin/` (excluding generated files **and the 3 plugin fixtures under `bin/plugins/available/**`** — see the register, 2026-09-09: the `js` ratchet's floor is therefore `3`, not `0`). ⚠️ The 4th `bin/` `.js`, `bin/copy-binaries.js`, is **build tooling, not a fixture** — it is in scope, and whether it is converted or deliberately exempted is still open ([#2705](https://github.com/kuzzleio/kuzzle/issues/2705)).
...
0. **Four review follow-ups are in review** (opened 2026-09-10, all off `2-dev`, none stacked): [#2709](https://github.com/kuzzleio/kuzzle/pull/2709) TD-27 · [#2710](https://github.com/kuzzleio/kuzzle/pull/2710) TD-28 · [#2711](https://github.com/kuzzleio/kuzzle/pull/2711) TD-31 · [#2712](https://github.com/kuzzleio/kuzzle/pull/2712) TD-29. **[#2705](https://github.com/kuzzleio/kuzzle/issues/2705) (TD-30) needs a decision**, not a patch: convert `bin/copy-binaries.js` — `npm run build` would then run it from `dist/`, the directory the script itself populates — or exempt it with a stated reason. The wording and the floor are fixed here; the decision is not.
1. **Sprint 6 — `lib/core` II** (validation, plugin, network/protocols): the next conversion step, and the hardest so far. 34 `.js` files, including `httpwsProtocol` (1 254 LOC) and `pluginsManager` (1 244). Open `steps/09-sprint-6-core-ii.md` when it starts. Sequence it by **measured coverage first** (step 07's rule: a rename makes the whole file new code, so today's coverage is tomorrow's `new_coverage`), and expect the `prepare-coverage` ownership pass to carry the vitest-tested files.
2. **[Step 08 — type-debt backlog](steps/08-type-debt-backlog.md), in parallel.** Next up, in priority order: [#2703](https://github.com/kuzzleio/kuzzle/issues/2703) (re-scope TD-21 so it stays a `fix`, not a breaking change), [#2704](https://github.com/kuzzleio/kuzzle/issues/2704) (drop the class-wide index signature), then [#2705](https://github.com/kuzzleio/kuzzle/issues/2705) / [#2706](https://github.com/kuzzleio/kuzzle/issues/2706) / [#2707](https://github.com/kuzzleio/kuzzle/issues/2707). Still open from earlier: [#2688](https://github.com/kuzzleio/kuzzle/issues/2688) (TD-20, needs a product decision — the migration off the deprecated request APIs is breaking) and [#2691](https://github.com/kuzzleio/kuzzle/issues/2691) (TD-23, 4 CPD exclusions left, `memoryStorageController` the only real target). TD-25 (`@types/debug` is wrong) has no issue — it is a note against repeating an experiment.
...
npm run test:strict                 # strict on adopted files (101)
...
| 08 | Type-debt backlog, worked in parallel with the sprints | 🟦 In progress | [#2699](https://github.com/kuzzleio/kuzzle/pull/2699) TD-26 · [#2700](https://github.com/kuzzleio/kuzzle/pull/2700) TD-21 · [#2701](https://github.com/kuzzleio/kuzzle/pull/2701) TD-23 · [#2702](https://github.com/kuzzleio/kuzzle/pull/2702) TD-22 | [detail](steps/08-type-debt-backlog.md) |
| 09 | Sprint 6 — core II (validation, plugin, network) | ⬜ To do | — | — |
