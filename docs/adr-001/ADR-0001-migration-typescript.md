...
**Where we are (2026-07-20):** the foundation is in place (ADR, register, 3 CI ratchets, progressive strict, ESLint). **Sprint 1 done** (`lib/util` 100% TS) and **Sprint 3 done** (`lib/model` + `lib/service` 100% TS, PR #2676), both merged into `2-dev`. **Sprint 4 in progress:** PR A (6 clean `lib/api` controllers + `rateLimiter`, [#2679](https://github.com/kuzzleio/kuzzle/pull/2679)) and PR B (`documentController`, [#2680](https://github.com/kuzzleio/kuzzle/pull/2680)) are both **merged into `2-dev`** (2026-07-17); js 79→72. **PR C** (`memoryStorageController`, [#2681](https://github.com/kuzzleio/kuzzle/pull/2681)) is up for review — js 72→71. The migration advances one layer-PR at a time off `2-dev`.

**Counters** (baselines in `.migration/`, incl. PR C [#2681](https://github.com/kuzzleio/kuzzle/pull/2681)): **js = 71**, **mocha = 151**, **any = 200**; **strict adopted = 46**.
**Remaining JS by layer** (71 = 67 `lib/` + 4 `bin/`): `core` 50 · `api` 5 · `kuzzle` 6 · `cluster` 6 · `bin` 4.
...
1. **Sprint 4 — `lib/api` (in progress, [step 05](steps/05-sprint-4-api.md)):** PR A ([#2679](https://github.com/kuzzleio/kuzzle/pull/2679), 6 files) and PR B ([#2680](https://github.com/kuzzleio/kuzzle/pull/2680), `documentController`) **merged** — js 79→72. **In review:** PR C = `memoryStorageController` ([#2681](https://github.com/kuzzleio/kuzzle/pull/2681), clean, own PR — dynamic Redis-command registration + a `rewire`-driven spec); then `serverController` + `documentExtractor`, which need the deprecated request-API migration (`setResult(result, options)` → `response.configure`, `getArrayLegacy`) **plus** `kuzzle.statistics` made non-private + `config.version` modelled (TD-18); then `funnel`, `httpRoutes`, `controllers/index` barrel (+ drop the `new AdminController.default()` workaround). *(Sprint 2 — real `bin/` entrypoints — stays deprioritized, see [step 03](steps/03-sprint-2-bin.md).)*
...
| 05 | Sprint 4 — `lib/api` (controllers, `funnel`) | 🟦 In progress | #2679 A✅ · #2680 B✅ · #2681 C | [detail](steps/05-sprint-4-api.md) |
