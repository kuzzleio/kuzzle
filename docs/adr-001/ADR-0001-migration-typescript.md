...
**Where we are (2026-07-21):** the foundation is in place (ADR, register, 3 CI ratchets, progressive strict, ESLint). **Sprint 1 done** (`lib/util` 100% TS) and **Sprint 3 done** (`lib/model` + `lib/service` 100% TS, PR #2676), both merged into `2-dev`. **Sprint 4 in progress:** PR A (6 clean `lib/api` controllers + `rateLimiter`, [#2679](https://github.com/kuzzleio/kuzzle/pull/2679)), PR B (`documentController`, [#2680](https://github.com/kuzzleio/kuzzle/pull/2680)) and PR C (`memoryStorageController`, [#2681](https://github.com/kuzzleio/kuzzle/pull/2681)) are all **merged into `2-dev`** (A/B 2026-07-17, C 2026-07-21); js 79→71. **PR D** (`serverController` + `documentExtractor`, #2682, branch `chore/ts-migration-sprint4-api-server`) is **open** — js 71→69. Remaining in `lib/api`: PR E (`funnel`, `httpRoutes`, barrel). The migration advances one layer-PR at a time off `2-dev`.

**Counters** (baselines in `.migration/`, incl. PR D #2682): **js = 69**, **mocha = 151**, **any = 200**; **strict adopted = 46**.
**Remaining JS by layer** (69 = 65 `lib/` + 4 `bin/`): `core` 50 · `api` 3 · `kuzzle` 6 · `cluster` 6 · `bin` 4.
...
1. **Sprint 4 — `lib/api` (in progress, [step 05](steps/05-sprint-4-api.md)):** PR A ([#2679](https://github.com/kuzzleio/kuzzle/pull/2679)), PR B ([#2680](https://github.com/kuzzleio/kuzzle/pull/2680), `documentController`) and PR C ([#2681](https://github.com/kuzzleio/kuzzle/pull/2681), `memoryStorageController`) **merged** — js 79→71. **Open:** PR D = `serverController` + `documentExtractor` (#2682, `chore/ts-migration-sprint4-api-server`) — `export =`; `kuzzle.statistics` made `public`; `config.version` modelled (TD-18 partial); the planned deprecated-API migration proved **behaviour-changing**, so `setResult`/`getArrayLegacy` are **kept with `// NOSONAR`** and deferred as [TD-20](type-debt-register.md). **Next:** PR E = `funnel`, `httpRoutes`, `controllers/index` barrel (+ drop the `new AdminController.default()` workaround). *(Sprint 2 — real `bin/` entrypoints — stays deprioritized, see [step 03](steps/03-sprint-2-bin.md).)*
...
| 05 | Sprint 4 — `lib/api` (controllers, `funnel`) | 🟦 In progress | #2679 A✅ · #2680 B✅ · #2681 C✅ · #2682 D | [detail](steps/05-sprint-4-api.md) |
