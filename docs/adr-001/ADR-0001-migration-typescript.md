...
1. **Sprint 6 — `lib/core` II is open: [step 09](steps/09-sprint-6-core-ii.md).** The step file carries the measured, gate-facing coverage of all 34 files and the six-PR plan built from it. **Start with H1** (`validation/types/*` + `baseType`, 14 files, 99.3% — pure leaves) and **H2** (`network` minus `entryPoint`/`httpwsProtocol`, 12 files, 88.3%); they are independent and can go in parallel. H4–H6 are spec efforts and should not be started as conversions.
2. **[Step 08 — type-debt backlog](steps/08-type-debt-backlog.md), in parallel.** [#2691](https://github.com/kuzzleio/kuzzle/issues/2691) (TD-23) is **closed** — the CPD exclusion list is now a ratchet, which was the part that prose could not do. Remaining: [#2688](https://github.com/kuzzleio/kuzzle/issues/2688) (TD-20), which a 2026-09-11 scoping pass established is **two issues wearing one number** — the `getArrayLegacy` half is a breaking HTTP API change belonging to a major, and the `setResult` half is blocked on the fact that its documented replacement (`response.configure`) cannot set a result at all; the recommendation to split is recorded on the issue and awaits a product call. [#2715](https://github.com/kuzzleio/kuzzle/issues/2715) (TD-33) is **mostly fixed** — readiness gate and `fail-fast: false` done; the `admin:resetDatabase` cross-node visibility race is the piece still open. TD-25 (`@types/debug` is wrong) has no issue — it is a note against repeating an experiment.
...
| 09 | Sprint 6 — core II (validation, plugin, network) | 🟦 In progress | — | [detail](steps/09-sprint-6-core-ii.md) |
