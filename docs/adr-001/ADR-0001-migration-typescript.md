...
**Counters** (baselines in `.migration/`, re-verified on `2-dev` 2026-09-10, after the review's follow-ups): **js = 49**, **mocha = 151**, **any = 205**, **implicit-any = 461**; **strict adopted = 102** *(`--candidates` empty)*. The `any` metric includes `as unknown as`; it dropped 208 → 207 when TD-21 removed the cast its guard needed, then 207 → 205 when TD-34 typed `profileRepository`'s `_hash` patch site. implicit-any reached **461** with TD-28's third `Reflect.set` site (`core/shared/store.ts`). **Coverage is measured again on `.ts`** — `.ci/scripts/prepare-coverage.ts` normalises both reports and gives each file a single owning runner. Unit tests: 3030 Mocha + **183 vitest** (was 7).
