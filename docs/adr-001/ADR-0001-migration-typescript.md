...
**Counters** (baselines in `.migration/`): **js = 66**, **mocha = 151**, **any = 208** *(metric broadened to include `as unknown as`; was 200 on the narrower grep)*, **implicit-any = 520** *(new)*; **strict adopted = 94** *(was 46; `--candidates` empty)*. **Coverage is measured again on `.ts`: the 30 converted files sit at 85.9%**, `lib/**/*.ts` at 88.5%. Unit tests: 3025 Mocha + **64 vitest** (was 7).
...
1. **Step 06 — mid-course hardening ([detail](steps/06-hardening-mid-course.md)): all three PRs written, stacked, awaiting review.** **F1** ([#2689](https://github.com/kuzzleio/kuzzle/pull/2689) — strict foundation: the `never[]` fix, 94 adopted files, the `implicit-any` ratchet, the anchored `strict-check.sh` match, 5 preflight checks, the amended conversion standard) — **45/45 CI checks green**, review pending. **F2** ([#2693](https://github.com/kuzzleio/kuzzle/pull/2693) — coverage restored, converted files measured at 85.9%; closes [#2692](https://github.com/kuzzleio/kuzzle/issues/2692)) — stacked on F1. **F3** (`chore/ts-migration-hardening-tests` — vitest spec location in `CONTRIBUTING.md`, 5 specs / 64 tests, `promback` fixed and adopted, dead `wildcard.ts` deleted) — stacked on F2. **Merge order F1 → F2 → F3, retargeting each onto `2-dev` as the one below it lands.** Deferred findings tracked as [#2690](https://github.com/kuzzleio/kuzzle/issues/2690) (TD-22), [#2691](https://github.com/kuzzleio/kuzzle/issues/2691) (TD-23).
2. **Then Sprint 5 — `lib/core` I** (storage, security, realtime; 50 JS files, the first layer where the cucumber net is the primary guarantee). ⚠️ **The coverage gate now applies to conversions**: a converted file below the new-code threshold fails CI where it used to pass silently, so budget a spec per converted file (the conversion standard now requires one).
...
| 06 | Mid-course hardening (enforcement before `lib/core`) | 🟦 In progress | [#2689](https://github.com/kuzzleio/kuzzle/pull/2689) F1 · [#2693](https://github.com/kuzzleio/kuzzle/pull/2693) F2 · F3 | [detail](steps/06-hardening-mid-course.md) |
