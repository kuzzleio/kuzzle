...
1. **Step 06 — mid-course hardening ([detail](steps/06-hardening-mid-course.md)): in progress.** PR F1 ([#2689](https://github.com/kuzzleio/kuzzle/pull/2689) — strict foundation: the `never[]` fix, 94 adopted files, the `implicit-any` ratchet, the anchored `strict-check.sh` match, 5 preflight checks, the amended conversion standard) is **open**, all local gates green. The review's three deferred findings have issues: [#2690](https://github.com/kuzzleio/kuzzle/issues/2690) (TD-22), [#2691](https://github.com/kuzzleio/kuzzle/issues/2691) (TD-23), [#2692](https://github.com/kuzzleio/kuzzle/issues/2692) (TD-24). **Next: PR F2 — restore `.ts` coverage** (drop `**/*.ts` from `sonar.coverage.exclusions`, remap the c8 lcov off `dist/`, feed vitest's report to the scanner, and record the measured coverage of the 30 converted files). Then **PR F3 — test debt** (vitest spec location decided in `CONTRIBUTING.md`, specs for the 6 untested converted utils, `promback`'s typing hole fixed under test).
...
| 06 | Mid-course hardening (enforcement before `lib/core`) | 🟦 In progress | [#2689](https://github.com/kuzzleio/kuzzle/pull/2689) F1 · F2 · F3 | [detail](steps/06-hardening-mid-course.md) |
