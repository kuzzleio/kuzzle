...
**Counters** (baselines in `.migration/`): **js = 56**, **mocha = 151**, **any = 208** *(metric broadened to include `as unknown as`; was 200 on the narrower grep)*, **implicit-any = 520** *(new)*; **strict adopted = 98** *(`--candidates` empty)*. **Coverage is measured again on `.ts`: the 30 converted files sit at 85.9%**, `lib/**/*.ts` at 88.5%. Unit tests: 3025 Mocha + **64 vitest** (was 7).
**Remaining JS by layer** (56 = 52 `lib/` + 4 `bin/`): `core` 40 · `api` **0** · `kuzzle` 6 · `cluster` 6 · `bin` 4 *(all four are plugin **fixtures** under `bin/plugins/available/**` — out of the Definition of Done, see the register)*.
...
| 07 | Sprint 5 — core I (storage, security, realtime, cache, shared) | 🟦 In progress | G1 · G2 · G3 | [detail](steps/07-sprint-5-core-i.md) |
