...
**[Step 07 — Sprint 5 (`lib/core` I)](steps/07-sprint-5-core-i.md) is open.** 16 files, 3 801 LOC (storage, security, realtime + the `cache` and `shared` leaves the sequencing had left unassigned). ⚠️ **The coverage gate is armed now** (`new_coverage ≥ 80%`), so the sprint is sequenced by measured coverage as well as by layer: **G1** = the 10 files already ≥ 93% covered, **G2** = the 5 small ones needing a spec first, **G3** = `clientAdapter.js` alone (1 045 LOC at **24%**).

**Counters** (baselines in `.migration/`): **js = 50**, **mocha = 151**, **any = 208** *(metric broadened to include `as unknown as`; was 200 on the narrower grep)*, **implicit-any = 518**; **strict adopted = 101** *(`--candidates` empty)*. **Coverage is measured again on `.ts`: the 30 converted files sit at 85.9%**, `lib/**/*.ts` at 88.5%. Unit tests: 3025 Mocha + **166 vitest** (was 7).
**Remaining JS by layer** (50 = 46 `lib/` + 4 `bin/`): `core` 34 · `api` **0** · `kuzzle` 6 · `cluster` 6 · `bin` 4 *(all four are plugin **fixtures** under `bin/plugins/available/**` — out of the Definition of Done, see the register)*.
...
1. **Sprint 5 — `lib/core` I ([step 07](steps/07-sprint-5-core-i.md)): in progress.** **PR G1 ([#2695](https://github.com/kuzzleio/kuzzle/pull/2695), 51/51 green) and PR G2 are done** — G1 the 10 gate-safe files (`realtime/*`, `security/{roleRepository,userRepository,securityLoader}`, `cache/cacheEngine`) converted, js 66 → 56. ⚠️ **The `implicit-any` ratchet caught 55 inferred `any` the compiler and the `any` ratchet both missed** (32 in `notifier.ts` alone) — without step 06 this PR would have shipped as a rename. G1 also **measures an unknown**: whether a `.js` → `.ts` rename makes *all* of a file's lines count as new for **coverage** (it does for issues — the sprint-4 gotcha). Sprint 4 gives no evidence (`new_lines_to_cover = 0`, `.ts` was excluded then), and the answer sizes G2/G3. G2 ([#2696](https://github.com/kuzzleio/kuzzle/pull/2696), 51/51 green, gate `OK`) the 5 module-wiring files (js 56 → 51, 5 new vitest specs, and a **`kerror` fix without which no vitest spec could reach an error path at all** — `module.filename` is a CommonJS global and vitest loads `lib/` as ESM). ⚠️ **G2 cleared the coverage gate by only 3.5 points** (`new_coverage` 83.5% over 424 new lines, vs G1's 98.5%) — the threshold is real now. **G3 is open** — `clientAdapter` converted with a 70-test table-driven spec (211/211 instrumented source lines); js 51 → 50, so **`lib/core/storage` is 100% TS**. With G3 merged, **Sprint 5 is done** and the next step is Sprint 6 (`lib/core` II — validation, plugin, network/protocols), which holds the 34 remaining `lib/core` `.js` files including `httpwsProtocol` (1 254 LOC) and `pluginsManager` (1 244).
2. **Deferred findings** with issues: [#2690](https://github.com/kuzzleio/kuzzle/issues/2690) (TD-22, `memoryStorageController` typing), [#2691](https://github.com/kuzzleio/kuzzle/issues/2691) (TD-23, CPD exclusions), [#2687](https://github.com/kuzzleio/kuzzle/issues/2687) (TD-21), [#2688](https://github.com/kuzzleio/kuzzle/issues/2688) (TD-20). TD-25 (`@types/debug` is wrong) has no issue — it is a note against repeating an experiment.
...
| 07 | Sprint 5 — core I (storage, security, realtime, cache, shared) | 🟦 In progress | [#2695](https://github.com/kuzzleio/kuzzle/pull/2695) G1 · G2 · G3 | [detail](steps/07-sprint-5-core-i.md) |
