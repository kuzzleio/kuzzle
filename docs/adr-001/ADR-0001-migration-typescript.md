...
**Counters** (baselines in `.migration/`, re-verified 2026-09-11): **js = 22** *(was 20; [TD-45](type-debt-register.md#td-45) fixed the predicate, not the code — the two extensionless `bin/` executables had never been counted)*, **mocha = 150**, **any = 205**, **implicit-any = 456**; **strict adopted = 127** *(`--candidates` empty)*. H1 took js 49 → 35 and strict 102 → 117; H2 took js 35 → 23, strict 117 → 123, and implicit-any 461 → 457 by declaring `global.nodeId`, which five files read and nothing declared; H3 took js 23 → 20, strict 123 → 125, and **moved the mocha counter for the first time** (151 → 150) by replacing `pluginManifest`'s Mocha spec rather than duplicating it. The `any` metric includes `as unknown as`; it dropped 208 → 207 when TD-21 removed the cast its guard needed, then 207 → 205 when TD-34 typed `profileRepository`'s `_hash` patch site. **Coverage is measured again on `.ts`** — `.ci/scripts/prepare-coverage.ts` normalises both reports and gives each file a single owning runner. Unit tests: **3027 Mocha + 189 vitest** (was 7). A **fifth ratchet**, `cpd-exclusions`, was added 2026-09-11 ([TD-23](type-debt-register.md#td-23)); its baseline is the 4 files currently excluded from CPD.
**Remaining JS by layer** (22 = 17 `lib/` + 5 `bin/`): `core` **5** (validation 1 — `validation.js`, H4 · plugin 2 — `plugin` + `pluginsManager`, H5 · network 2 — `entryPoint` + `httpwsProtocol`, H6) · `api` **0** · `kuzzle` 6 · `cluster` 6 · `bin` **5** — 3 plugin **fixtures** under `bin/plugins/available/**` (the floor, reached by [#2713](https://github.com/kuzzleio/kuzzle/pull/2713)) plus `start-kuzzle-server` and `wait-kuzzle`, two extensionless Node executables the ratchet's predicate had never seen ([TD-45](type-debt-register.md#td-45)).
...
1. **Sprint 6 — `lib/core` II is open: [step 09](steps/09-sprint-6-core-ii.md).** The step file carries the measured, gate-facing coverage of all 34 files and the six-PR plan built from it. **H1 is merged** ([#2722](https://github.com/kuzzleio/kuzzle/pull/2722), `validation/types/*` + `baseType`, 14 files) and **H2 is open** ([#2723](https://github.com/kuzzleio/kuzzle/pull/2723), `network` minus `entryPoint`/`httpwsProtocol`, 12 files). `lib/core` is down to **8** `.js`: `validation.js` (H4), the three `plugin` leaves plus `plugin.js`/`pluginsManager.js` (H3, H5) and `entryPoint`/`httpwsProtocol` (H6). **Next is H3** (the three `plugin` leaves, 76.1% — a conversion plus four lines of test). H4–H6 are spec efforts and should not be started as conversions.
...
| 09 | Sprint 6 — core II (validation, plugin, network) | 🟦 In progress | [#2722](https://github.com/kuzzleio/kuzzle/pull/2722) H1 · [#2723](https://github.com/kuzzleio/kuzzle/pull/2723) H2 · [#2724](https://github.com/kuzzleio/kuzzle/pull/2724) H3 | [detail](steps/09-sprint-6-core-ii.md) |
