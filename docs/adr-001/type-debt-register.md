...
- **2026-07-17** — Sprint 4 (`lib/api`) started, PR A (#2679): 7 files → TS, js baseline 79 → 72. Surfaced two items to fix in PR B alongside `serverController`: **TD-18** (`config.version` unmodelled) and a **`kuzzle.statistics` private-but-accessed-cross-class** visibility bug. Fixed in passing: `validation.js` `validate()` JSDoc pointed at the DOM `Request` instead of `KuzzleRequest`.
