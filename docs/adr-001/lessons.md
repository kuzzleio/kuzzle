...
| *A resume point is only valid if the transport it resumes was already listening when the point was taken.* | [TD-65](type-debt-register.md#td-65) | 📝 open ([#2773](https://github.com/kuzzleio/kuzzle/issues/2773)) — two sockets, one counter, and no ordering between them is a gap by construction |
| *A broadcast is not a way to tell yourself something.* | [TD-67](type-debt-register.md#td-67) | 📝 open ([#2776](https://github.com/kuzzleio/kuzzle/issues/2776)) — `evictSelf` publishes its own eviction on a `PUB` socket that does not loop back, so the node keeps serving traffic |
