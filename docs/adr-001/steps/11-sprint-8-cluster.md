...
- **TD-33 will interleave with this sprint's own CI.** Its seventh occurrence (2026-09-16) is the first attributable one and it names `kuzzle_node_prod`: every out-of-sync report in the run came from the production-mode node, which differs from the other three in exactly one way in `.ci/test-cluster-{7,8}.yml` (`NODE_ENV: "production"`). **Look at the publisher, not at the membership logic.** Both diagnostics that made it readable — [TD-58](../type-debt-register.md#td-58)'s counter and [TD-59](../type-debt-register.md#td-59)'s single node id — landed before this sprint opened, on purpose.
