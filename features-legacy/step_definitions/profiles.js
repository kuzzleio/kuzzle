...
When(
  ...
  function (profile, id) {
    ...
    return this.api
      .createOrReplaceProfile(id, this.profiles[profile])
      .then((body) => {
        if (body.error) {
          throw new Error(body.error.message);
        }
  ...
  function (callback) {
    this.api
      ...
      .then(() => {
        callback(
          new Error(
            "Creating profile with unexisting role succeeded. Expected to throw."
          )
        );
      })
      .catch(() => callback());
  ...
  function (callback) {
    this.api
      ...
      .then(() => {
        callback(
          new Error(
            "Creating profile without roles succeeded. Expected to throw."
          )
        );
      })
      .catch(() => callback());
...
Then(/^I cannot get a profile without ID$/, function (callback) {
  this.api
    ...
    .then(() => {
      callback(
        new Error("Getting profile without id succeeded. Expected to throw.")
      );
    })
    .catch(() => callback());
});
          ...
          .then((body) => {
            ...
            if (profile) {
              const compare = (a, b) => {
                ...
                policies = stringify(
                  body.result._source.policies.sort(compare)
                ),
                expected = stringify(
                  this.profiles[profile].policies.sort(compare)
                );
  ...
  function (not, id) {
    return this.api
      ...
      .then((body) => {
        ...
        const policies = stringify(body.result.hits),
          expected = stringify(this.policies[id]);

        if (policies !== expected) {
          throw new Error(
            `Bad profileRights for ${id}.\nExpected: ${expected}\nGot: ${policies}`
          );
        }
  ...
  function (id, callback) {
    ...
    this.api
      ...
      .then(() => {
        callback(
          new Error(
            "Trying to delete a profile still used by a user. Expected to throw."
          )
        );
      })
      .catch(() => callback());
          ...
          .then((response) => {
            ...
            if (!response.result) {
              return callbackAsync(
                new Error("Malformed response (no error, no result)")
              );
            }

            if (!Array.isArray(response.result.hits)) {
              return callbackAsync(
                new Error("Malformed response (hits is not an array)")
              );
            }
            ...
            if (!response.result.hits) {
              ...
              if (response.result.hits.length !== parseInt(profilesCount)) {
                return callbackAsync(
                  `Expected ${profilesCount} profiles. Got ${response.result.hits.length}`
                );
              }
            }
...
Given(
  ...
  function (_default, profileId, roleId) {
    ...
    return this.api
      .createOrReplaceProfile(profileId, { policies })
      .then((response) => {
        if (response.error) {
          throw new Error(response.error.message);
        }
