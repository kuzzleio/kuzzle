  ...
  function (not, type, collection, callback) {
    ...
    if (!collection) {
      if (this.result.collections.length === 0) {
        ...
        return callback(
          "Collection list is empty, expected collections to be listed",
        );
      }
    }
    ...
    if (
      ...
      if (not) {
        return callback(
          "Expected collection " +
            collection +
            " not to appear in the collection list",
        );
      }
    ...
    callback(
      "Expected to find the collection <" +
        collection +
        "> in this collections list: " +
        JSON.stringify(this.result.collections),
    );
  },
);
  ...
  function (index, collection, countRaw) {
    return this.api.createCollection(index, collection).then(() => {
      ...
      for (let i = 0; i < count; ++i) {
        promises.push(
          this.api.create({ number: `doc-${i}` }, index, collection),
        );
      }
...
Then(
  ...
  function (index, collection, dynamicValue) {
    return this.api
      ...
      .then(({ result }) => {
        const expectedValue =
          dynamicValue === "the default value"
            ? "true" // we set the default value to true in the environment for tests
            : dynamicValue;

        should(result.dynamic).not.be.undefined().be.eql(expectedValue);
      });
  },
);
...
Then(
  ...
  function (index, collection, rawMapping) {
    ...
    return this.api
      .getCollectionMapping(index, collection, includeKuzzleMeta)
      .then(({ result }) => {
        const expectedValue =
          rawMapping === "the default value"
            ? this.kuzzleConfig.services.storageEngine.commonMapping.properties
            : JSON.parse(rawMapping);

        should(result.properties).be.eql(expectedValue);
      });
  },
);
...
Then(
  ...
  function (index, collection, rawMapping) {
    ...
    return this.api
      .getCollectionMapping(index, collection)
      .then(({ result }) => {
        should(result._meta).not.be.undefined().be.eql(mapping._meta);
      });
  },
);
