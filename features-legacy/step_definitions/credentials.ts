...
When(
  ...
  function (strategy, user, id) {
    ...
    return this.api
      .createCredentials(strategy, id, this.credentials[user])
      .then((response) => {
...
When(
  ...
  function (strategy, user, id) {
    ...
    return this.api
      .validateCredentials(strategy, id, this.credentials[user])
      .then((response) => {
        if (response.error !== null) {
          throw new Error(response.error.message);
        }
  ...
  function (strategy, user, id) {
    ...
    return this.api.getCredentials(strategy, id).then((response) => {
      ...
      if (response.result.username !== this.credentials[user].username) {
        throw new Error(
          `username missmatch: got ${response.result.username} instead of ${this.credentials[user].username}`,
        );
      }
    });
  },
...
When(/^I get my ([^ ]+) credentials$/, function (strategy) {
  return this.api.getMyCredentials(strategy).then((response) => {
    ...
    if (response.result.username !== this.credentials[defaultUser].username) {
      throw new Error(
        `username mismatch: got ${response.result.username} instead of ${this.credentials[defaultUser].username}`,
      );
    }
  });
});
