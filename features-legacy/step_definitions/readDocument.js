        ...
        if (body.result && body.result.hits && body.result.total !== 0) {
          if (dont) {
            return Bluebird.reject(
              new Error("A document exists for the query")
            );
          }
          return Bluebird.resolve();
        }
...
Then(
  ...
  function (id, doesnt, callback) {
    this.api
      ...
      .then((response) => {
        if (response.result) {
          return doesnt === undefined
            ? callback()
            : callback(new Error("The document exists"));
        }

        return doesnt === undefined
          ? callback(new Error("The document doesn't exists"))
          : callback();
      })
      .catch(function (error) {
        callback(error);
