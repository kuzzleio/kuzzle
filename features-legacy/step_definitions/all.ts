    ...
    if (type === "json") {
      ...
      try {
        ...
        if (err instanceof should.AssertionError) {
          return callback(
            new Error(JSON.stringify(val) + " does not match " + pattern),
          );
        }
...
Then(
  ...
  function (message, callback) {
    ...
    try {
      ...
      if (err instanceof should.AssertionError) {
        return callback(
          new Error(`"${val.message}" does not match "${message}"`),
        );
      }
...
Then(
  ...
  function (field, prop, type, callback) {
    ...
    if (!this.result[field].properties[prop].type) {
      return callback(
        new Error("No type found for field " + field + "." + prop),
      );
    }
