...
When(
  ...
  function (id, reset, callback) {
    var userObject = this.users.useradmin;

    delete userObject.content.profileIds;
    id = this.idPrefix + id;

    this.api
      .createFirstAdmin(userObject, id, reset)
      .then((body) => {
        if (body.error) {
          return callback(new Error(body.error.message));
        }
...
Then(/^I am able to perform a scrollUsers request$/, function () {
  ...
  return this.api.scrollUsers(this.scrollId).then((response) => {
    if (response.error) {
      throw new Error(response.error.message);
    }
