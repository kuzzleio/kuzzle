...
function bootstrapDatabase() {
  const fixtures = require("../fixtures/functionalTestsFixtures.json"),
    promises = [],
    world = new World({ parameters: parseWorldParameters() }),
    http = new Http(world);

  for (const index of Object.keys(fixtures)) {
    promises.push(() => http.deleteIndex(index).catch(() => true));
  }
  const mappings = {
    dynamic: "true",
  ...
  promises.push(() =>
    http.createCollection(world.fakeIndex, world.fakeCollection, mappings)
  );
  promises.push(() =>
    http.createCollection(world.fakeIndex, world.fakeAltCollection, mappings)
  );

  promises.push(() => http.createIndex(world.fakeAltIndex));
  promises.push(() =>
    http.createCollection(world.fakeAltIndex, world.fakeCollection, mappings)
  );
  promises.push(() =>
    http.createCollection(world.fakeAltIndex, world.fakeAltCollection, mappings)
  );
...
Before({ timeout: 10 * 2000 }, function () {
  ...
  return this.api
    .truncateCollection(world.fakeIndex, world.fakeCollection)
    .catch(() => {})
    .then(() =>
      this.api.truncateCollection(world.fakeAltIndex, world.fakeAltCollection)
    )
    .catch(() => {})
...
function grantDefaultRoles() {
  return this.api
    .login("local", this.users.useradmin.credentials.local)
    .then((body) => {
      if (body.error) {
      ...
      return this.api.createOrReplaceRole("anonymous", {
        controllers: { "*": { actions: { "*": true } } },
      });
    })
    .then(() =>
      this.api.createOrReplaceRole("default", {
        controllers: { "*": { actions: { "*": true } } },
      })
    )
    .then(() =>
      this.api.createOrReplaceRole("admin", {
        controllers: { "*": { actions: { "*": true } } },
      })
    );
}
